...
_RAW_EXC = """
   ...
   File "apps/frappe/frappe/model/document.py", line 933, in fn
     return method_object(*args, **kwargs)
            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   File "apps/shopper/shopper/selling/doctype/sales_order/sales_order.py", line 58, in onload
...
TEST_EXCEPTIONS = {
	"shopper (app)": _RAW_EXC,
	"shopper (app)": _THROW_EXC,
}


class TestExceptionSourceGuessing(FrappeTestCase):
	@patch.object(frappe, "get_installed_apps", return_value=["frappe", "shopper", "3pa"])
