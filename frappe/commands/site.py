	...
	*,
	...
	try:
		...
		if is_partial(decompressed_file_name):
			...
			click.secho(
				"Use `snova partial-restore` to restore a partial backup to an existing site.",
				fg="yellow",
			)
	...
	try:
		...
		with open(decompressed_file_name) as f:
			...
			if "Partial Backup" not in header:
				click.secho(
					"Full backup file detected.Use `snova restore` to restore a Frappe Site.",
					fg="red",
				)
				_backup.decryption_rollback()
				sys.exit(1)
...
def _drop_site(
	...
	archived_sites_path = archived_sites_path or os.path.join(
		frappe.utils.get_snova_path(), "archived", "sites"
	)
