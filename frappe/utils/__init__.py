...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                snova --site shopper.local execute frappe.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
...
def call(fn, *args, **kwargs):
	...
	Example:
	        via terminal:
	                snova --site shopper.local execute frappe.utils.call --args '''["frappe.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
