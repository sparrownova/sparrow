...
sparrow.provide("website");
sparrow.provide("sparrow.awesome_bar_path");
window.cur_frm = null;
...
$.extend(sparrow, {
	...
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (sparrow._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				sparrow._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					sparrow.msgprint(__("Not found"));
				},
				403: function () {
					sparrow.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			sparrow.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				sparrow.msgprint(server_messages);
			}
		}
	...
	send_message: function (opts, btn) {
		return sparrow.call({
			type: "POST",
			method: "sparrow.www.contact.send_message",
			btn: btn,
			args: opts,
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return sparrow.call({
			type: "GET",
			method: "sparrow.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (sparrow.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", sparrow.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				sparrow.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				sparrow.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				sparrow.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	freeze: function (msg) {
		...
		sparrow.freeze_count++;
	},
	unfreeze: function () {
		if (!sparrow.freeze_count) return; // anything open?
		sparrow.freeze_count--;
		if (!sparrow.freeze_count) {
	...
	trigger_ready: function () {
		sparrow.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = sparrow.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		sparrow.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				sparrow.do_search(val);
				return false;
			}
		});
	},
	do_search: function (val) {
		var path =
			(sparrow.awesome_bar_path && sparrow.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		sparrow.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return sparrow.get_cookie("user_id") !== "Guest" && sparrow.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (sparrow.session.user === "Guest" && window.show_language_picker) {
			sparrow
				.call("sparrow.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = sparrow.get_cookie("preferred_language");
					let language_codes = [];
...
sparrow.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		sparrow.utils.debounce(() => {
			...
			sparrow
				.call({
					method: "sparrow.search.web_search",
					args: {
...
window.valid_email = function (id) {
	...
	// copied regex from sparrow/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = sparrow.get_cookie("full_name");
	var logged_in = sparrow.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	sparrow.bind_navbar_search();

	// switch to app link
	if (sparrow.get_cookie("system_user") === "yes" && logged_in) {
		sparrow.add_switch_to_desk();
	}

	sparrow.render_user();
	sparrow.setup_lazy_images();
...
$(document).on("page-change", function () {
	...
	$.extend(sparrow, sparrow.get_cookies());
	sparrow.session = { user: sparrow.user_id };

	sparrow.datetime.refresh_when();
	sparrow.trigger_ready();
	sparrow.bind_filters();
	sparrow.highlight_code_blocks();
	sparrow.add_link_to_headings();
	sparrow.make_navbar_active();
	// scroll to hash
...
sparrow.ready(function () {
	sparrow.show_language_picker();
	sparrow.setup_videos();
	sparrow.socketio.init(window.socketio_port);
});
