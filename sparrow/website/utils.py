...
from sparrow.utils import cint, get_system_timezone, md_to_html
...
def can_cache(no_cache=False):
	if sparrow.flags.force_website_cache:
		return True
	if sparrow.conf.disable_website_cache or sparrow.conf.developer_mode:
		return False
	if getattr(sparrow.local, "no_cache", False):
		return False
	return not no_cache


def get_comment_list(doctype, name):
	comments = sparrow.get_all(
		"Comment",
		...
		or_filters=[["owner", "=", sparrow.session.user], ["published", "=", 1]],
	)

	communications = sparrow.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{sparrow.session.user}%"],
			["cc", "like", f"%{sparrow.session.user}%"],
			["bcc", "like", f"%{sparrow.session.user}%"],
		],
	)
...
def get_home_page():
	if sparrow.local.flags.home_page and not sparrow.flags.in_test:
		return sparrow.local.flags.home_page
	...
	def _get_home_page():
		...
		if sparrow.session.user != "Guest":
			# by role
			for role in sparrow.get_roles():
				home_page = sparrow.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = sparrow.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = sparrow.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if sparrow.session.user == "Guest" else "me"
	...
	return sparrow.cache().hget("home_page", sparrow.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = sparrow.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = sparrow.get_attr(home_page_method[-1])(sparrow.session.user)
	elif sparrow.get_hooks("website_user_home_page"):
		home_page = sparrow.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = sparrow.get_hooks("role_home_page")
		if role_home_page:
			for role in sparrow.get_roles():
	...
	if not home_page:
		home_page = sparrow.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	return {
		"sysdefaults": {
			"float_precision": cint(sparrow.get_system_settings("float_precision")) or 3,
			"date_format": sparrow.get_system_settings("date_format") or "yyyy-mm-dd",
			"time_format": sparrow.get_system_settings("time_format") or "HH:mm:ss",
		},
		"time_zone": {
			"system": get_system_timezone(),
			"user": sparrow.db.get_value("User", sparrow.session.user, "time_zone") or get_system_timezone(),
		},
	}


def is_signup_disabled():
	return sparrow.db.get_single_value("Website Settings", "disable_signup", True)
...
def get_shade(color, percent=None):
	sparrow.msgprint(_("get_shade method has been deprecated."))
	return color
...
def get_toc(route, url_prefix=None, app=None):
	...
	return sparrow.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	if not sparrow.local.flags.children_map:
		...
		children_map = sparrow.cache().get_value("website_full_index", _build)

		sparrow.local.flags.children_map = children_map

	return sparrow.local.flags.children_map
...
def clear_cache(path=None):
	...
	for key in (
		...
		sparrow.cache().delete_value(key)

	sparrow.cache().delete_value("website_404")
	if path:
		sparrow.cache().hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		sparrow.clear_cache("Guest")
		for key in (
			"portal_menu_items",
			"home_page",
			"website_route_rules",
	...
	for method in sparrow.get_hooks("website_clear_cache"):
		sparrow.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = sparrow.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else sparrow.flags.look_for_sidebar
...
def get_portal_sidebar_items():
	sidebar_items = sparrow.cache().hget("portal_menu_items", sparrow.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = sparrow.get_roles()
		portal_settings = sparrow.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = sparrow.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		sparrow.cache().hset("portal_menu_items", sparrow.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			sparrow.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		if can_cache():
			html = None
			page_cache = sparrow.cache().hget("website_page", args[0].path)
			if page_cache and sparrow.local.lang in page_cache:
				html = page_cache[sparrow.local.lang]
			if html:
				sparrow.local.response.from_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = sparrow.cache().hget("website_page", args[0].path) or {}
			page_cache[sparrow.local.lang] = html
			sparrow.cache().hset("website_page", args[0].path, page_cache)
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = sparrow.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	...
	for css in sparrow.local.preload_assets["style"]:
		links.append(f"<{css}>; rel=preload; as=style")

	for js in sparrow.local.preload_assets["script"]:
