...
from sparrow.website.page_renderers.base_renderer import BaseRenderer
from sparrow.website.website_components.metatags import MetaTags
...
class BaseTemplatePage(BaseRenderer):
	...
	def init_context(self):
		self.context = sparrow._dict()
		self.context.update(get_website_settings())
		self.context.update(sparrow.local.conf.get("website_context") or {})

	def add_csrf_token(self, html):
		if sparrow.local.session:
			csrf_token = sparrow.local.session.data.csrf_token
			return html.replace(
				"<!-- csrf_token -->", f'<script>sparrow.csrf_token = "{csrf_token}";</script>'
			)
	...
	def set_base_template_if_missing(self):
		if not self.context.base_template_path:
			app_base = sparrow.get_hooks("base_template")
	...
	def set_missing_values(self):
		# set using sparrow.respond_as_web_page
		if hasattr(sparrow.local, "response") and sparrow.local.response.get("context"):
			self.context.update(sparrow.local.response.context)

		# to be able to inspect the context dict
		# Use the macro "inspect" from macros.html
		self.context.canonical = sparrow.utils.get_url(sparrow.utils.escape_html(self.path))
		...
		self.context.pathname = (
			getattr(sparrow.local, "path", None) if hasattr(sparrow, "local") else self.path
		)

	def update_website_context(self):
		# apply context from hooks
		update_website_context = sparrow.get_hooks("update_website_context")
		for method in update_website_context:
			values = sparrow.get_attr(method)(self.context)
			if values:
