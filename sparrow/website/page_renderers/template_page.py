...
import sparrow
from sparrow.website.page_renderers.base_template_page import BaseTemplatePage
from sparrow.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def get_index_path_options(search_path):
		return (
			sparrow.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = sparrow.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = sparrow.get_module(self.pymodule_name)
		...
		self.context.developer_mode = sparrow.conf.developer_mode
	...
	def render_template(self):
		if self.template_path.endswith("min.js"):
			...
			html = sparrow.render_template(self.source, self.context, safe_render=safe_render)
	...
	def get_raw_template(self):
		return sparrow.get_jloader().get_source(sparrow.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = sparrow.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "sparrow"
		self.app_path = sparrow.get_app_path("sparrow")
		self.path = path
	...
	def set_user_info(self):
		from sparrow.utils.user import get_fullname_and_avatar

		info = get_fullname_and_avatar(sparrow.session.user)
