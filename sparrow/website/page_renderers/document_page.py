...
from sparrow.website.router import (
	get_doctypes_with_web_view,
	get_page_info_from_web_page_with_dynamic_routes,
)
from sparrow.website.utils import cache_html
...
class DocumentPage(BaseTemplatePage):
	...
	def search_in_doctypes_with_web_view(self):
		for doctype in get_doctypes_with_web_view():
			...
			try:
				self.docname = sparrow.db.get_value(doctype, filters, "name")
				if self.docname:
	...
	@cache_html
	def get_html(self):
		self.doc = sparrow.get_doc(self.doctype, self.docname)
		self.init_context()
		self.update_context()
		self.post_process_context()
		html = sparrow.get_template(self.template_path).render(self.context)
		return html
