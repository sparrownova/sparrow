# Copyright (c) 2013, Sparrow Technologies and contributors
# License: MIT. See LICENSE

from datetime import datetime

import sparrow
from sparrow.query_builder.functions import Coalesce, Count
...
class WebsiteAnalytics:
	def __init__(self, filters=None):
		self.filters = sparrow._dict(filters or {})

		if not self.filters.to_date:
			self.filters.to_date = datetime.now()

		if not self.filters.from_date:
			self.filters.from_date = sparrow.utils.add_days(self.filters.to_date, -7)

		if not self.filters.range:
			self.filters.range = "Daily"

		self.filters.to_date = sparrow.utils.add_days(self.filters.to_date, 1)
	...
	def get_data(self):
		WebPageView = sparrow.qb.DocType("Web Page View")
		count_all = Count("*").as_("count")
		case = sparrow.qb.terms.Case().when(WebPageView.is_unique == "1", "1")
		count_is_unique = Count(case).as_("unique_count")

		return (
			sparrow.qb.from_(WebPageView)
			...
			.orderby("count", Order=sparrow.qb.desc)
		).run()
	...
	def get_chart_data(self):
		current_dialect = sparrow.db.db_type or "mariadb"
		...
		self.chart_data = sparrow.db.sql(query, values=values, as_dict=1)
	...
	def prepare_chart_data(self, data):
		...
		if self.filters.range == "Monthly":
			date_range = [sparrow.utils.add_days(dd, 1) for dd in date_range]
