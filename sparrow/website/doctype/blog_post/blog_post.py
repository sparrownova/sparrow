...
from sparrow.website.utils import (
	clear_cache,
	find_first_image,
	get_comment_list,
...
class BlogPost(WebsiteGenerator):
	...
	def validate(self):
		...
		if self.featured:
			if not self.meta_image:
				sparrow.throw(_("A featured post must have a cover image"))
			self.reset_featured_for_other_blogs()

		self.set_read_time()

	def reset_featured_for_other_blogs(self):
		all_posts = sparrow.get_all("Blog Post", {"featured": 1})
		for post in all_posts:
			sparrow.db.set_value("Blog Post", post.name, "featured", 0)
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and sparrow.db.get_single_value(
			"Blog Settings", "show_cta_in_blog", cache=True
		)

		if self.blogger:
			context.blogger_info = sparrow.get_doc("Blogger", self.blogger).as_dict()
		...
		context.category = sparrow.db.get_value(
		...
		context.guest_allowed = sparrow.db.get_single_value("Blog Settings", "allow_guest_to_comment")

	def fetch_cta(self):
		if sparrow.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
			blog_settings = sparrow.get_cached_doc("Blog Settings")
	...
	def fetch_social_links_info(self):
		if not sparrow.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
			return []

		url = sparrow.local.site + "/" + self.route
	...
	def load_likes(self, context):
		...
		context.like_count = sparrow.db.count("Comment", filters) or 0
		...
		context.like = sparrow.db.count("Comment", filters) or 0
...
def get_list_context(context=None):
	...
	elif sparrow.local.form_dict.blogger:
		blogger = sparrow.db.get_value("Blogger", {"name": sparrow.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif sparrow.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(sparrow.local.form_dict.txt))
	...
	blog_settings = sparrow.get_doc("Blog Settings").as_dict(no_default_fields=True)
...
def get_blog_categories():
	...
	post, category = sparrow.qb.DocType("Blog Post"), sparrow.qb.DocType("Blog Category")
	return (
		sparrow.qb.from_(category)
		.select(category.name, category.route, category.title)
		.where(
			(category.published == 1)
			& ExistsCriterion(
				sparrow.qb.from_(post)
				.select("name")
...
def get_blog_category(route):
	return sparrow.db.get_value("Blog Category", {"name": route}, "title") or route
...
def get_blog_list(
	...
	if filters and filters.get("blogger"):
		conditions.append("t1.blogger=%s" % sparrow.db.escape(filters.get("blogger")))

	if category:
		conditions.append("t1.blog_category=%s" % sparrow.db.escape(category))

	if txt:
		conditions.append(
			'(t1.content like {0} or t1.title like {0}")'.format(sparrow.db.escape("%" + txt + "%"))
		)

	if conditions:
		sparrow.local.no_cache = 1
	...
	posts = sparrow.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = sparrow.db.get_value(
