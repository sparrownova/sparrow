...
class TestWebTemplate(SparrowTestCase):
	...
	def test_custom_stylesheet(self):
		...
		sparrow.get_doc("Website Theme", "Standard").set_as_default()

	def create_web_page(self):
		if not sparrow.db.exists("Web Page", "test-web-template"):
			sparrow.get_doc(
				{
					"doctype": "Web Page",
	...
	def create_website_theme(self):
		if not sparrow.db.exists("Website Theme", "Custom"):
			theme = sparrow.get_doc({"doctype": "Website Theme", "theme": "Custom"}).insert()
		else:
			theme = sparrow.get_doc("Website Theme", "Custom")
		return theme
