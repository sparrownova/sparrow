...
from sparrow.modules.export_file import get_module_path, scrub_dt_dn, write_document_file
...
class WebTemplate(Document):
	def validate(self):
		if self.standard and not (sparrow.conf.developer_mode or sparrow.flags.in_patch):
			sparrow.throw(_("Enable developer mode to create a standard Web Template"))

		for field in self.fields:
			if not field.fieldname:
				field.fieldname = sparrow.scrub(field.label)
	...
	def on_trash(self):
		if sparrow.conf.developer_mode and self.standard:
	...
	def get_template_path(self):
		...
		file_name = sparrow.scrub(self.name) + ".html"
	...
	def render(self, values=None):
		if not values:
			values = {}
		values = sparrow.parse_json(values)
		values.update({"values": values})
		template = self.get_template(self.standard)

		return sparrow.render_template(template, values)
