...
from sparrow.utils import get_datetime, get_fullname, time_diff_in_hours
...
class PersonalDataDeletionRequest(Document):
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = sparrow.get_hooks("user_data_fields")
	...
	def autoname(self):
		from sparrow.model.naming import set_name_from_naming_options
		...
		domain = sparrow.local.site.replace("_", "-")
		...
		sparrow.utils.validate_email_address(self.email, throw=True)

	def after_insert(self):
		self.send_verification_mail()

	def generate_url_for_confirmation(self):
		params = {"email": self.email, "name": self.name, "host_name": sparrow.local.site}
		api = sparrow.utils.get_url(
			"/api/method/sparrow.website.doctype.personal_data_deletion_request"
	...
	def disable_user(self):
		user = sparrow.get_doc("User", self.email)
		user.enabled = False
		user.save()
	...
	def send_verification_mail(self):
		...
		sparrow.sendmail(
			...
			args={
				"email": self.email,
				"name": self.name,
				"host_name": sparrow.utils.get_url(),
				"link": url,
			},
	...
	def notify_system_managers(self):
		...
		sparrow.sendmail(
			...
			args={"user": self.email, "url": sparrow.utils.get_url(self.get_url())},
	...
	def validate_data_anonymization(self):
		...
		if self.status != "Pending Approval":
			sparrow.throw(_("This request has not yet been approved by the user."))
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = sparrow.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			sparrow.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		sparrow.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in sparrow.get_meta(doctype["doctype"]).fields:
...
def process_data_deletion_request():
	auto_account_deletion = sparrow.db.get_single_value("Website Settings", "auto_account_deletion")
	if auto_account_deletion < 1:
		return

	requests = sparrow.get_all(
		"Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="name"
	)

	for request in requests:
		doc = sparrow.get_doc("Personal Data Deletion Request", request)
...
def confirm_deletion(email, name, host_name):
	...
	doc = sparrow.get_doc("Personal Data Deletion Request", name)
	host_name = sparrow.utils.get_url()
