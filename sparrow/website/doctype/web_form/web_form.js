sparrow.ui.form.on("Web Form", {
	setup: function () {
		sparrow.meta.docfield_map["Web Form Field"].fieldtype.formatter = (value) => {
			const prefix = {
				"Page Break": "--red-600",
		...
		sparrow.meta.docfield_map["Web Form Field"].fieldname.formatter = (value) => {
			if (!value) return;
			return sparrow.unscrub(value);
		};

		sparrow.meta.docfield_map["Web Form List Column"].fieldname.formatter = (value) => {
			if (!value) return;
			return sparrow.unscrub(value);
		};
	},
	...
	refresh: function (frm) {
		...
		frm.get_field("is_standard").toggle(sparrow.boot.developer_mode);

		if (frm.doc.is_standard && !sparrow.boot.developer_mode) {
			frm.set_read_only();
			frm.disable_save();
		}
	...
	validate: function (frm) {
		...
		if (!frm.doc.web_form_fields) {
			...
			sparrow.throw(__("Atleast one field is required in Web Form Fields Table"));
		}
		...
		if (page_break_count >= 10) {
			sparrow.throw(__("There can be only 9 Page Break fields in a Web Form"));
		}
	},
	...
	add_get_fields_button(frm) {
		frm.add_custom_button(__("Get Fields"), () => {
			let webform_fieldtypes = sparrow.meta
...
sparrow.ui.form.on("Web Form List Column", {
	fieldname: function (frm, doctype, name) {
		let doc = sparrow.get_doc(doctype, name);
		let df = sparrow.meta.get_docfield(frm.doc.doc_type, doc.fieldname);
		if (!df) return;
...
sparrow.ui.form.on("Web Form Field", {
...
function get_fields_for_doctype(doctype) {
	return new Promise((resolve) => sparrow.model.with_doctype(doctype, resolve)).then(() => {
		return sparrow.meta.get_docfields(doctype).filter((df) => {
			return (
