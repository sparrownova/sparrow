...
def website_theme_fixture(**theme):
	...
	sparrow.delete_doc_if_exists("Website Theme", test_theme)
	theme = sparrow.get_doc(doctype="Website Theme", theme=test_theme, **theme)
	theme.insert()
	yield theme
	sparrow.db.set_single_value("Website Settings", "website_theme", "Standard")
	theme.delete()
...
class TestWebsiteTheme(FrappeTestCase):
	def test_website_theme(self):
		with website_theme_fixture(
			...
			theme_path = sparrow.get_site_path("public", theme.theme_url[1:])
	...
	def test_get_scss_paths(self):
		self.assertIn("sparrow/public/scss/website.bundle", get_scss_paths())

	def test_imports_to_ignore(self):
		with website_theme_fixture(ignored_apps=[{"app": "sparrow"}]) as theme:
			self.assertTrue('@import "sparrow/public/scss/website"' not in theme.theme_scss)
