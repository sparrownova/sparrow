...
import sparrow
import sparrow.database
import sparrow.utils
import sparrow.utils.user
from sparrow import _
from sparrow.core.doctype.activity_log.activity_log import add_authentication_log
from sparrow.modules.patch_handler import check_session_stopped
from sparrow.sessions import Session, clear_sessions, delete_session
from sparrow.translate import get_language
...
from sparrow.utils import cint, date_diff, datetime, get_datetime, today
...
class HTTPRequest:
	def __init__(self):
		...
		sparrow.local.cookie_manager.init_cookies()

	@property
	def domain(self):
		if not getattr(self, "_domain", None):
			self._domain = sparrow.request.host
	...
	def set_request_ip(self):
		if sparrow.get_request_header("X-Forwarded-For"):
			sparrow.local.request_ip = (
				sparrow.get_request_header("X-Forwarded-For").split(",", 1)[0]
			).strip()

		elif sparrow.get_request_header("REMOTE_ADDR"):
			sparrow.local.request_ip = sparrow.get_request_header("REMOTE_ADDR")
	...
	def validate_csrf_token(self):
		if sparrow.local.request and sparrow.local.request.method in ("POST", "PUT", "DELETE"):
			if not sparrow.local.session:
				return
			if (
				not sparrow.local.session.data.csrf_token
				or sparrow.local.session.data.device == "mobile"
				or sparrow.conf.get("ignore_csrf", None)
			):
				# not via boot
				return

			csrf_token = sparrow.get_request_header("X-Sparrow-CSRF-Token")
			if not csrf_token and "csrf_token" in sparrow.local.form_dict:
				csrf_token = sparrow.local.form_dict.csrf_token
				del sparrow.local.form_dict["csrf_token"]

			if sparrow.local.session.data.csrf_token != csrf_token:
				sparrow.local.flags.disable_traceback = True
				sparrow.throw(_("Invalid Request"), sparrow.CSRFTokenError)

	def set_lang(self):
		sparrow.local.lang = get_language()
...
class LoginManager:
	...
	def __init__(self):
		...
		if (
			sparrow.local.form_dict.get("cmd") == "login" or sparrow.local.request.path == "/api/method/login"
		):
	...
	def login(self):
		if sparrow.get_system_settings("disable_user_pass_login"):
			sparrow.throw(_("Login with username and password is not allowed."), sparrow.AuthenticationError)

		# clear cache
		sparrow.clear_cache(user=sparrow.form_dict.get("usr"))
		user, pwd = get_cached_user_pass()
		self.authenticate(user=user, pwd=pwd)
		if self.force_user_to_reset_password():
			doc = sparrow.get_doc("User", self.user)
			sparrow.local.response["redirect_to"] = doc.reset_password(
				send_email=False, password_expired=True
			)
			sparrow.local.response["message"] = "Password Reset"
			return False
		...
		if should_run_2fa(self.user):
			...
			if not confirm_otp_token(self):
				return False
		sparrow.form_dict.pop("pwd", None)
		self.post_login()
	...
	def get_user_info(self):
		self.info = sparrow.get_cached_value(
	...
	def setup_boot_cache(self):
		sparrow.cache_manager.build_table_count_cache()
		sparrow.cache_manager.build_domain_restriced_doctype_cache()
		sparrow.cache_manager.build_domain_restriced_page_cache()
	...
	def set_user_info(self, resume=False):
		...
		if self.info.user_type == "Website User":
			sparrow.local.cookie_manager.set_cookie("system_user", "no")
			if not resume:
				sparrow.local.response["message"] = "No App"
				sparrow.local.response["home_page"] = "/" + get_home_page()
		else:
			sparrow.local.cookie_manager.set_cookie("system_user", "yes")
			if not resume:
				sparrow.local.response["message"] = "Logged In"
				sparrow.local.response["home_page"] = "/app"

		if not resume:
			sparrow.response["full_name"] = self.full_name

		# redirect information
		redirect_to = sparrow.cache().hget("redirect_after_login", self.user)
		if redirect_to:
			sparrow.local.response["redirect_to"] = redirect_to
			sparrow.cache().hdel("redirect_after_login", self.user)

		sparrow.local.cookie_manager.set_cookie("full_name", self.full_name)
		sparrow.local.cookie_manager.set_cookie("user_id", self.user)
		sparrow.local.cookie_manager.set_cookie("user_image", self.info.user_image or "")

	def clear_preferred_language(self):
		sparrow.local.cookie_manager.delete_cookie("preferred_language")

	def make_session(self, resume=False):
		# start session
		sparrow.local.session_obj = Session(
			user=self.user, resume=resume, full_name=self.full_name, user_type=self.user_type
		)

		# reset user if changed to Guest
		self.user = sparrow.local.session_obj.user
		sparrow.local.session = sparrow.local.session_obj.data
	...
	def clear_active_sessions(self):
		...
		if not (
			cint(sparrow.conf.get("deny_multiple_sessions"))
			or cint(sparrow.db.get_system_setting("deny_multiple_sessions"))
		):
			return

		clear_sessions(sparrow.session.user, keep_current=True)

	def authenticate(self, user: str = None, pwd: str = None):
		from sparrow.core.doctype.user.user import User

		if not (user and pwd):
			user, pwd = sparrow.form_dict.get("usr"), sparrow.form_dict.get("pwd")
		if not (user and pwd):
		...
		ignore_tracker = should_run_2fa(user.name) and ("otp" in sparrow.form_dict)
	...
	def force_user_to_reset_password(self):
		...
		if self.user in sparrow.STANDARD_USERS:
			return False
	...
	def fail(self, message, user=None):
		if not user:
			user = _("Unknown User")
		sparrow.local.response["message"] = message
		add_authentication_log(message, user, status="Failed")
		sparrow.db.commit()
		raise sparrow.AuthenticationError

	def run_trigger(self, event="on_login"):
		for method in sparrow.get_hooks().get(event, []):
			sparrow.call(sparrow.get_attr(method), login_manager=self)

	def validate_hour(self):
		"""check if user is logging in during restricted hours"""
		login_before = int(sparrow.db.get_value("User", self.user, "login_before", ignore=True) or 0)
		login_after = int(sparrow.db.get_value("User", self.user, "login_after", ignore=True) or 0)
		...
		if login_before and current_hour >= login_before:
			sparrow.throw(_("Login not allowed at this time"), sparrow.AuthenticationError)
	...
	def logout(self, arg="", user=None):
		...
		if user == sparrow.session.user:
			delete_session(sparrow.session.sid, user=user, reason="User Manually Logged Out")
			self.clear_cookies()
		else:
			clear_sessions(user)
...
class CookieManager:
	...
	def init_cookies(self):
		if not sparrow.local.session.get("sid"):
			return

		# sid expires in 3 days
		expires = datetime.datetime.now() + datetime.timedelta(days=3)
		if sparrow.session.sid:
			self.set_cookie("sid", sparrow.session.sid, expires=expires, httponly=True)
		if sparrow.session.session_country:
			self.set_cookie("country", sparrow.session.session_country)

	def set_cookie(self, key, value, expires=None, secure=False, httponly=False, samesite="Lax"):
		if not secure and hasattr(sparrow.local, "request"):
			secure = sparrow.local.request.scheme == "https"

		# Cordova does not work with Lax
		if sparrow.local.session.data.device == "mobile":
			samesite = None
...
def clear_cookies():
	if hasattr(sparrow.local, "session"):
		sparrow.session.sid = ""
	sparrow.local.cookie_manager.delete_cookie(
...
def validate_ip_address(user):
	...
	from sparrow.core.doctype.user.user import get_restricted_ip_list

	# Only fetch required fields - for perf
	user_fields = ["restrict_ip", "bypass_restrict_ip_check_if_2fa_enabled"]
	user_info = (
		sparrow.get_cached_value("User", user, user_fields, as_dict=True)
		if not sparrow.flags.in_test
		else sparrow.db.get_value("User", user, user_fields, as_dict=True)
	)
	...
	system_settings = (
		sparrow.get_cached_doc("System Settings")
		if not sparrow.flags.in_test
		else sparrow.get_single("System Settings")
	)
	...
	for ip in ip_list:
		if sparrow.local.request_ip.startswith(ip) or bypass_restrict_ip_check:
			return

	sparrow.throw(_("Access not allowed from this IP Address"), sparrow.AuthenticationError)
...
def get_login_attempt_tracker(user_name: str, raise_locked_exception: bool = True):
	...
	"""
	sys_settings = sparrow.get_doc("System Settings")
...
class LoginAttemptTracker:
	...
	def login_failed_count(self):
		return sparrow.cache().hget("login_failed_count", self.user_name)

	@login_failed_count.setter
	def login_failed_count(self, count):
		sparrow.cache().hset("login_failed_count", self.user_name, count)

	@login_failed_count.deleter
	def login_failed_count(self):
		sparrow.cache().hdel("login_failed_count", self.user_name)
	...
	def login_failed_time(self):
		...
		"""
		return sparrow.cache().hget("login_failed_time", self.user_name)

	@login_failed_time.setter
	def login_failed_time(self, timestamp):
		sparrow.cache().hset("login_failed_time", self.user_name, timestamp)

	@login_failed_time.deleter
	def login_failed_time(self):
		sparrow.cache().hdel("login_failed_time", self.user_name)
