sparrow.provide("sparrow.core");

sparrow.ui.form.on("Workflow", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		if (frm.doc.document_type) {
			frm.add_custom_button(__("Go to {0} List", [frm.doc.document_type]), () => {
				sparrow.set_route("List", frm.doc.document_type);
			});
		}
	...
	update_field_options: function (frm) {
		...
		if (doc.document_type) {
			const get_field_method =
				"sparrow.workflow.doctype.workflow.workflow.get_fieldnames_for";
			sparrow.xcall(get_field_method, { doctype: doc.document_type }).then((resp) => {
	...
	set_table_html: function (frm) {
		const promises = frm.states.map((r) => {
			...
			return sparrow.utils.get_indicator_color(state).then((color) => {
				return `<tr>
				<td>
	...
	get_orphaned_states_and_count: function (frm) {
		...
		return sparrow
			.xcall("sparrow.workflow.doctype.workflow.workflow.get_workflow_state_count", {
		...
		if (frm.states && frm.states.length) {
			...
			$(frm.state_table)
				...
				.on("click", (e) => {
					...
					sparrow.set_route("List", frm.doc.document_type, filters);
				});
		}
	},
});

sparrow.ui.form.on("Workflow Document State", {
