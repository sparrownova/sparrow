...
class TestWorkflow(SparrowTestCase):
	...
	def setUp(self):
		...
		if self._testMethodName == "test_if_workflow_actions_were_processed_using_user":
			if not sparrow.db.has_column("Workflow Action", "user"):
				...
				sparrow.cache().delete_value("table_columns")

	def tearDown(self):
		sparrow.delete_doc("Workflow", "Test ToDo")
		if self._testMethodName == "test_if_workflow_actions_were_processed_using_user":
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		sparrow.db.delete("Workflow Action")
		...
		workflow_actions = sparrow.get_all("Workflow Action", fields=["*"])
		...
		workflow_actions = sparrow.get_all("Workflow Action", fields=["status"])
		self.assertEqual(len(workflow_actions), 1)
	...
	def test_if_workflow_actions_were_processed_using_user(self):
		...
		sparrow.qb.update(WorkflowAction).set(WorkflowAction.user, "test2@example.com").run()
		sparrow.qb.update(WorkflowActionPermittedRole).set(WorkflowActionPermittedRole.role, "").run()
...
def create_todo_workflow():
	from sparrow.tests.ui_test_helpers import UI_TEST_USER

	if sparrow.db.exists("Workflow", "Test ToDo"):
		sparrow.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not sparrow.db.exists("Role", TEST_ROLE):
		sparrow.get_doc(dict(doctype="Role", role_name=TEST_ROLE)).insert(ignore_if_duplicate=True)
		if sparrow.db.exists("User", UI_TEST_USER):
			sparrow.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = sparrow.new_doc("Workflow")
...
def create_new_todo():
	return sparrow.get_doc(dict(doctype="ToDo", description="workflow " + random_string(10))).insert()
