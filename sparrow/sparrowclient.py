...
class SparrowClient:
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": sparrow.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return sparrow._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "sparrow.client.insert_many", "docs": sparrow.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": sparrow.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return sparrow._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "sparrow.client.bulk_update", "docs": sparrow.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "sparrow.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "sparrow.client.submit", "doc": sparrow.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "sparrow.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": sparrow.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "sparrow.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(
		...
		for doc in docs:
			...
			if doctype != "User" and not sparrow.db.exists("User", doc.get("owner")):
				sparrow.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = sparrow.db.get_single_value(doctype, "modified")
