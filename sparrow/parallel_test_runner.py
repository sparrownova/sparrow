...
class ParallelTestRunner:
	def __init__(self, app, site, build_number=1, total_builds=1, dry_run=False):
		self.app = app
		self.site = site
		self.build_number = sparrow.utils.cint(build_number) or 1
		self.total_builds = sparrow.utils.cint(total_builds)
		self.dry_run = dry_run
		self.setup_test_site()
		self.run_tests()

	def setup_test_site(self):
		sparrow.init(site=self.site)
		if not sparrow.db:
			sparrow.connect()
	...
	def before_test_setup(self):
		...
		for fn in sparrow.get_hooks("before_tests", app_name=self.app):
			sparrow.get_attr(fn)()

		test_module = sparrow.get_module(f"{self.app}.tests")
	...
	def get_module(self, path, filename):
		app_path = sparrow.get_pymodule_path(self.app)
...
class ParallelTestWithOrchestrator(ParallelTestRunner):
	...
	def __init__(self, app, site):
		...
		if not self.orchestrator_url:
			...
			click.echo("Pass public URL after hosting https://github.com/sparrownova/test-orchestrator")
			sys.exit(1)

		self.ci_build_id = os.environ.get("CI_BUILD_ID")
		self.ci_instance_id = os.environ.get("CI_INSTANCE_ID") or sparrow.generate_hash(length=10)
