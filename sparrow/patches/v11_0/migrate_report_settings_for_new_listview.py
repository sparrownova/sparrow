...
def execute():
	...
	reports = sparrow.get_all("Report", {"report_type": "Report Builder"})

	for report_name in reports:
		settings = sparrow.db.get_value("Report", report_name, "json")
		if not settings:
			continue

		settings = sparrow._dict(json.loads(settings))
		...
		sparrow.db.set_value("Report", report_name, "json", json.dumps(settings))
