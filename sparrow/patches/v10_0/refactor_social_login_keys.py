...
def execute():
	...
	sparrow.reload_doc("integrations", "doctype", "social_login_key", force=True)

	if not sparrow.db.exists("DocType", "Social Login Keys"):
		return

	social_login_keys = sparrow.get_doc("Social Login Keys", "Social Login Keys")
	if social_login_keys.get("facebook_client_id") or social_login_keys.get("facebook_client_secret"):
		facebook_login_key = sparrow.new_doc("Social Login Key")
	...
	if social_login_keys.get("google_client_id") or social_login_keys.get("google_client_secret"):
		google_login_key = sparrow.new_doc("Social Login Key")
	...
	sparrow.delete_doc("DocType", "Social Login Keys")


def run_patch():
	sparrow.reload_doc("core", "doctype", "user", force=True)
	sparrow.reload_doc("core", "doctype", "user_social_login", force=True)
...
def insert_user_social_login(user, modified_by, provider, idx, userid=None, username=None):
	...
	creation_time = sparrow.utils.get_datetime_str(sparrow.utils.get_datetime())
	values = [
	...
	query = """INSERT INTO `tabUser Social Login` (`{source_cols}`)
		VALUES ({values})
	""".format(
		source_cols="`, `".join(source_cols), values=", ".join([sparrow.db.escape(d) for d in values])
	)
