...
def execute():
	...
	db_tables = sparrow.db.get_tables(cached=False)
	...
	for doctype, index_fields in UNUSED_INDEXES:
		table = f"tab{doctype}"
		index_name = sparrow.db.get_index_name(index_fields)
		if table not in db_tables:
			continue
...
def _drop_index_if_exists(table: str, index: str):
	if not sparrow.db.has_index(table, index):
		click.echo(f"- Skipped {index} index for {table} because it doesn't exist")
		return

	try:
		sparrow.db.sql_ddl(f"ALTER TABLE `{table}` DROP INDEX `{index}`")
	except Exception as e:
		sparrow.log_error("Failed to drop index")
