...
def execute():
	...
	all_tables = sparrow.db.get_tables()
	final_deletion_map = sparrow._dict()

	for table in all_tables:
		indexes_to_keep_map = sparrow._dict()
		indexes_to_delete = []
		index_info = sparrow.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				sparrow.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
