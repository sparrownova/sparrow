...
def execute():
	...
	"""
	sparrow.reload_doc("email", "doctype", "email_domain", force=True)
	sparrow.reload_doc("email", "doctype", "email_account", force=True)
...
def setup_incoming_email_port_in_email_domains():
	email_domains = sparrow.get_all("Email Domain", ["incoming_port", "use_imap", "use_ssl", "name"])
...
def setup_incoming_email_port_in_email_accounts():
	email_accounts = sparrow.get_all(
