...
def execute():
	...
	sparrow.reload_doc("automation", "doctype", "Auto Repeat")
	sparrow.db.sql("update `tabDocPerm` set submit=0, cancel=0, amend=0 where parent='Auto Repeat'")
	sparrow.db.sql("update `tabAuto Repeat` set docstatus=0 where docstatus=1 or docstatus=2")

	for entry in sparrow.get_all("Auto Repeat"):
		doc = sparrow.get_doc("Auto Repeat", entry.name)

		# create custom field for allow auto repeat
		fields = sparrow.get_meta(doc.reference_doctype).fields
