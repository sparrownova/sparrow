...
def execute():
	if not sparrow.db.table_exists("List View Setting"):
		return
	if not sparrow.db.exists("DocType", "List View Setting"):
		return

	sparrow.reload_doc("desk", "doctype", "List View Settings")

	existing_list_view_settings = sparrow.get_all(
		"List View Settings", as_list=True, order_by="modified"
	)
	for list_view_setting in sparrow.get_all(
		"List View Setting",
