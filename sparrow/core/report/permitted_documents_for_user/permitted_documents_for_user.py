...
from sparrow.permissions import rights
...
def execute(filters=None):
	...
	data = sparrow.get_list(doctype, fields=fields, as_list=True, user=user)

	if show_permissions:
		columns = columns + [sparrow.unscrub(right) + ":Check:80" for right in rights]
		data = list(data)
		for i, doc in enumerate(data):
			permission = sparrow.permissions.get_doc_permissions(sparrow.get_doc(doctype, doc[0]), user)
...
def get_columns_and_fields(doctype):
	...
	fields = ["`name`"]
	for df in sparrow.get_meta(doctype).fields:
...
def query_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	user_perms = sparrow.utils.user.UserPermissions(user)
	...
	single_doctypes = [d[0] for d in sparrow.db.get_values("DocType", {"issingle": 1})]
