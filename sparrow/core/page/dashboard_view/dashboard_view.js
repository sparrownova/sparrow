...
sparrow.pages["dashboard-view"].on_page_load = function (wrapper) {
	...
	sparrow.dashboard = new Dashboard(wrapper);
	$(wrapper).bind("show", function () {
		sparrow.dashboard.show();
	});
};
...
class Dashboard {
	...
	show() {
		...
		if (this.route.length > 1) {
			...
			// last opened
			if (sparrow.last_dashboard) {
				sparrow.set_re_route("dashboard-view", sparrow.last_dashboard);
			} else {
				// default dashboard
				sparrow.db.get_list("Dashboard", { filters: { is_default: 1 } }).then((data) => {
					if (data && data.length) {
						sparrow.set_re_route("dashboard-view", data[0].name);
					} else {
						// no default, get the latest one
						sparrow.db.get_list("Dashboard", { limit: 1 }).then((data) => {
							if (data && data.length) {
								sparrow.set_re_route("dashboard-view", data[0].name);
							} else {
								// create a new dashboard!
								sparrow.new_doc("Dashboard");
							}
						});
					}
				});
	...
	show_dashboard(current_dashboard_name) {
		...
		this.charts = {};
		sparrow.last_dashboard = current_dashboard_name;
	}

	set_breadcrumbs() {
		sparrow.breadcrumbs.add("Desk", "Dashboard");
	}

	refresh() {
		sparrow.run_serially([() => this.render_cards(), () => this.render_charts()]);
	}

	render_charts() {
		return this.get_permitted_items(
			"sparrow.desk.doctype.dashboard.dashboard.get_permitted_charts"
		).then((charts) => {
			if (!charts.length) {
				sparrow.msgprint(
			...
			sparrow.dashboard_utils.get_dashboard_settings().then((settings) => {
				...
				this.chart_group = new sparrow.widget.WidgetGroup({
					title: null,
	...
	render_cards() {
		return this.get_permitted_items(
			"sparrow.desk.doctype.dashboard.dashboard.get_permitted_cards"
		).then((cards) => {
			if (!cards.length) {
				return;
			}
			...
			this.number_card_group = new sparrow.widget.WidgetGroup({
				container: this.container,
				type: "number_card",
				columns: 3,
				options: {
	...
	set_dropdown() {
		...
		this.page.add_menu_item(__("Edit"), () => {
			sparrow.set_route("Form", "Dashboard", sparrow.dashboard.dashboard_name);
		});
		...
		sparrow.db.get_list("Dashboard").then((dashboards) => {
			dashboards.map((dashboard) => {
				let name = dashboard.name;
				if (name != this.dashboard_name) {
					this.page.add_menu_item(
						name,
						() => sparrow.set_route("dashboard-view", name),
						1
					);
				}
			});
