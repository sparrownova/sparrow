sparrow.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = sparrow.ui.make_app_page({
		parent: wrapper,
	...
	$(sparrow.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new sparrow.PermissionEngine(wrapper);
};

sparrow.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

sparrow.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			options: "DocType",
			change: function () {
				sparrow.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return sparrow.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (sparrow.get_route()[1]) {
			this.doctype_select.set_value(sparrow.get_route()[1]);
		} else if (sparrow.route_options) {
			if (sparrow.route_options.doctype) {
				this.doctype_select.set_value(sparrow.route_options.doctype);
			}
			if (sparrow.route_options.role) {
				this.role_select.set_value(sparrow.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = sparrow.confirm(__("Reset Permissions for {0}?", [doctype]), () => {
			return sparrow
				.call({
					module: "sparrow.core",
					page: "permission_manager",
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(sparrow.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${sparrow.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return sparrow.call({
					...
					callback: (r) => {
						if (r.exc) {
							sparrow.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			sparrow.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					sparrow.call({
						...
						callback: (r) => {
							if (r.exc) {
								sparrow.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						},
	...
	get_link_fields(doctype) {
		return sparrow.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
