...
sparrow.ui.form.on("Data Import", {
	setup(frm) {
		sparrow.realtime.on("data_import_refresh", ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			sparrow.model.clear_doc("Data Import", frm.doc.name);
			sparrow.model.with_doc("Data Import", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					name: ["in", sparrow.boot.user.can_import],
				},
			};
		});
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes("Success")) {
			frm.add_custom_button(__("Go to {0} List", [__(frm.doc.reference_doctype)]), () =>
				sparrow.set_route("List", frm.doc.reference_doctype)
			);
		}
	},
	...
	update_indicators(frm) {
		const indicator = sparrow.get_indicator(frm.doc);
		if (indicator) {
	...
	show_import_status(frm) {
		sparrow.call({
			method: "sparrow.core.doctype.data_import.data_import.get_import_status",
			args: {
				...
				.then((result) => {
					if (result.length > 0) {
						frm.add_custom_button("Report Error", () => {
							...
							sparrow.request.report_error(fake_xhr, {});
						});
					}
				});
		}
	...
	download_template(frm) {
		sparrow.require("data_import_tools.bundle.js", () => {
			frm.data_exporter = new sparrow.data_import.DataExporter(
	...
	show_import_preview(frm, preview_data) {
		...
		sparrow.require("data_import_tools.bundle.js", () => {
			frm.import_preview = new sparrow.data_import.ImportPreview({
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/sparrow.core.doctype.data_import.data_import.download_errored_template",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/sparrow.core.doctype.data_import.data_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
						...
						if (log.success) {
							if (frm.doc.import_type === "Insert New Records") {
								html = __("Successfully imported {0}", [
									`<span class="underline">${sparrow.utils.get_form_link(
										frm.doc.reference_doctype,
										log.docname,
										true
									)}<span>`,
							...
							let messages = JSON.parse(log.messages || "[]")
								...
								.join("");
							let id = sparrow.dom.get_unique_id();
							html = `${messages}
	...
	show_import_log(frm) {
		...
		sparrow.call({
			method: "sparrow.client.get_count",
			args: {
				doctype: "Data Import Log",
				filters: {
					data_import: frm.doc.name,
