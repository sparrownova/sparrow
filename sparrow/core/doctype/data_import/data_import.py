...
from sparrow.utils.csvutils import validate_google_sheets_url
...
class DataImport(Document):
	...
	def start_import(self):
		from sparrow.core.page.background_jobs.background_jobs import get_info
		from sparrow.utils.scheduler import is_scheduler_inactive

		if is_scheduler_inactive() and not sparrow.flags.in_test:
			sparrow.throw(_("Scheduler is inactive. Cannot import data."), title=_("Scheduler Inactive"))
		...
		if self.name not in enqueued_jobs:
			enqueue(
				...
				now=sparrow.conf.developer_mode or sparrow.flags.in_test,
			)
			return True
...
def get_preview_from_template(data_import, import_file=None, google_sheets_url=None):
	return sparrow.get_doc("Data Import", data_import).get_preview_from_template(
...
def form_start_import(data_import):
	return sparrow.get_doc("Data Import", data_import).start_import()


def start_import(data_import):
	"""This method runs in background job"""
	data_import = sparrow.get_doc("Data Import", data_import)
	try:
	...
	sparrow.publish_realtime("data_import_refresh", {"data_import": data_import.name})
...
def download_template(
	...
	export_fields = sparrow.parse_json(export_fields)
	export_filters = sparrow.parse_json(export_filters)
...
def download_errored_template(data_import_name):
	data_import = sparrow.get_doc("Data Import", data_import_name)
...
def get_import_status(data_import_name):
	...
	total_payload_count = sparrow.db.get_value("Data Import", data_import_name, "payload_count")
...
def import_file(doctype, file_path, import_type, submit_after_import=False, console=False):
	...
	data_import = sparrow.new_doc("Data Import")
...
def import_doc(path, pre_process=None):
	...
	for f in files:
		if f.endswith(".json"):
			sparrow.flags.mute_emails = True
			import_file_by_path(
...
def export_json(doctype, path, filters=None, or_filters=None, name=None, order_by="creation asc"):
	...
	out = []
	if name:
		out.append(sparrow.get_doc(doctype, name).as_dict())
	elif sparrow.db.get_value("DocType", doctype, "issingle"):
		out.append(sparrow.get_doc(doctype).as_dict())
	else:
		for doc in sparrow.get_all(
			doctype,
			fields=["name"],
			filters=filters,
			...
			out.append(sparrow.get_doc(doctype, doc.name).as_dict())
	post_process(out)
	...
	with open(path, "w") as outfile:
		outfile.write(sparrow.as_json(out))


def export_csv(doctype, path):
	from sparrow.core.doctype.data_export.exporter import export_data

	with open(path, "wb") as csvfile:
		export_data(doctype=doctype, all_doctypes=True, template=True, with_data=True)
		csvfile.write(sparrow.response.result.encode("utf-8"))
