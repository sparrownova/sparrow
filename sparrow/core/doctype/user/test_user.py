...
from sparrow.core.doctype.user.user import (
...
class TestUser(SparrowTestCase):
	def tearDown(self):
		...
		sparrow.db.set_single_value("System Settings", "minimum_password_score", "")
	...
	def test_user_type(self):
		...
		# social login userid for sparrow
		self.assertTrue(new_user.social_logins[0].userid)
		self.assertEqual(new_user.social_logins[0].provider, "sparrow")
		...
		sparrow.delete_doc("User", new_user.name)

	def test_delete(self):
		sparrow.get_doc("User", "test@example.com").add_roles("_Test Role 2")
		self.assertRaises(sparrow.LinkExistsError, delete_doc, "Role", "_Test Role 2")
		sparrow.db.delete("Has Role", {"role": "_Test Role 2"})
		delete_doc("Role", "_Test Role 2")

		if sparrow.db.exists("User", "_test@example.com"):
			delete_contact("_test@example.com")
			delete_doc("User", "_test@example.com")

		user = sparrow.copy_doc(test_records[1])
		user.email = "_test@example.com"
		user.insert()

		sparrow.get_doc({"doctype": "ToDo", "description": "_Test"}).insert()

		delete_contact("_test@example.com")
		delete_doc("User", "_test@example.com")

		self.assertTrue(
			not sparrow.db.sql("""select * from `tabToDo` where allocated_to=%s""", ("_test@example.com",))
		)

		from sparrow.core.doctype.role.test_role import test_records as role_records

		sparrow.copy_doc(role_records[1]).insert()

	def test_get_value(self):
		self.assertEqual(sparrow.db.get_value("User", "test@example.com"), "test@example.com")
		self.assertEqual(sparrow.db.get_value("User", {"email": "test@example.com"}), "test@example.com")
		self.assertEqual(
			sparrow.db.get_value("User", {"email": "test@example.com"}, "email"), "test@example.com"
		)
		self.assertEqual(
			sparrow.db.get_value("User", {"email": "test@example.com"}, ["first_name", "email"]),
		...
		test_user = sparrow.db.sql("select * from tabUser where name='test@example.com'", as_dict=True)[0]
		self.assertEqual(
			sparrow.db.get_value("User", {"email": "test@example.com"}, "*", as_dict=True), test_user
		)

		self.assertEqual(sparrow.db.get_value("User", "xxxtest@example.com"), None)

		sparrow.db.set_single_value("Website Settings", "_test", "_test_val")
		self.assertEqual(sparrow.db.get_value("Website Settings", None, "_test"), "_test_val")
		self.assertEqual(
			sparrow.db.get_value("Website Settings", "Website Settings", "_test"), "_test_val"
		)

	def test_high_permlevel_validations(self):
		user = sparrow.get_meta("User")
		self.assertTrue("roles" in [d.fieldname for d in user.get_high_permlevel_fields()])

		me = sparrow.get_doc("User", "testperm@example.com")
	...
	def test_delete_user(self):
		...
		delete_contact(new_user.name)
		sparrow.delete_doc("User", new_user.name)
		self.assertFalse(sparrow.db.exists("User", new_user.name))
	...
	def test_password_strength(self):
		...
		sparrow.db.set_single_value("System Settings", "enable_password_policy", 1)
		sparrow.db.set_single_value("System Settings", "minimum_password_score", 2)
		...
		self.assertRaises(
			sparrow.exceptions.ValidationError, handle_password_test_fail, result["feedback"]
		)
		self.assertRaises(
			sparrow.exceptions.ValidationError, handle_password_test_fail, result
		...
		sparrow.flags.in_test = False
		user.new_password = "password"
		self.assertRaises(sparrow.exceptions.ValidationError, user.save)
		user.reload()
		user.new_password = "Eastern_43A1W"
		user.save()
		sparrow.flags.in_test = True
	...
	def test_comment_mentions(self):
		...
		sparrow.delete_doc("User Group", "Team")
		doc = sparrow.get_doc(
			{
				"doctype": "User Group",
				"name": "Team",
	...
	def test_rate_limiting_for_reset_password(self):
		...
		sparrow.db.set_single_value("System Settings", "password_reset_limit", 1)
		sparrow.db.commit()

		url = get_url()
		data = {"cmd": "sparrow.core.doctype.user.user.reset_password", "user": "test@test.com"}
	...
	def test_user_rename(self):
		...
		sparrow.rename_doc("User", user.name, new_name)
		self.assertTrue(sparrow.db.exists("Notification Settings", new_name))

		sparrow.delete_doc("User", new_name)

	def test_signup(self):
		import sparrow.website.utils
		...
		with patch.object(user_module, "is_signup_disabled", return_value=True):
			self.assertRaisesRegex(
				sparrow.exceptions.ValidationError,
				"Sign Up is disabled",
				sign_up,
				random_user,
				random_user_name,
		...
		self.assertEqual(sparrow.cache().hget("redirect_after_login", random_user), "/welcome")
		...
		# disabled user
		user = sparrow.get_doc("User", random_user)
		user.enabled = 0
		user.save()
		...
		with patch.object(user_module.sparrow.db, "get_creation_count", return_value=301):
			self.assertRaisesRegex(
				sparrow.exceptions.ValidationError,
				"Throttled",
				sign_up,
	...
	def test_reset_password(self):
		...
		sparrow.local.request_ip = "127.0.0.69"
		sparrow.db.set_single_value("System Settings", "password_reset_limit", 6)

		sparrow.set_user("testpassword@example.com")
		test_user = sparrow.get_doc("User", "testpassword@example.com")
		...
		self.assertRaises(sparrow.exceptions.AuthenticationError, verify_password, old_password)
		verify_password(new_password)

		# reset password
		update_password(old_password, old_password=new_password)
		self.assertRaisesRegex(
			sparrow.exceptions.ValidationError, "Invalid key type", update_password, "test", 1, ["like", "%"]
		)
		...
		# test API endpoint
		with patch.object(user_module.sparrow, "sendmail") as sendmail:
			sparrow.clear_messages()
			test_user = sparrow.get_doc("User", "test2@example.com")
	...
	def test_reset_password_link_expiry(self):
		...
		sparrow.db.set_single_value("System Settings", "reset_password_link_expiry_duration", 1)
		sparrow.set_user("testpassword@example.com")
		test_user = sparrow.get_doc("User", "testpassword@example.com")
		test_user.reset_password()
...
def delete_contact(user):
	sparrow.db.delete("Contact", {"email_id": user})
	sparrow.db.delete("Contact Email", {"email_id": user})
