...
# this is a separate file since it is imported in sparrow.model.document
...
def run_server_script_for_doc_event(doc, event):
	...
	if scripts:
		...
		for script_name in scripts:
			sparrow.get_doc("Server Script", script_name).execute_doc(doc)
...
def get_server_script_map():
	...
	# 	}
	# }
	if sparrow.flags.in_patch and not sparrow.db.table_exists("Server Script"):
		return {}

	script_map = sparrow.cache().get_value("server_script_map")
	if script_map is None:
		script_map = {"permission_query": {}}
		enabled_server_scripts = sparrow.get_all(
			"Server Script",
		...
		sparrow.cache().set_value("server_script_map", script_map)
