...
from sparrow.utils.safe_exec import NamespaceDict, get_safe_globals, safe_exec
...
class ServerScript(Document):
	...
	def on_trash(self):
		if self.script_type == "Scheduler Event":
			for job in self.scheduled_jobs:
				sparrow.delete_doc("Scheduled Job Type", job.name)
	...
	def sync_scheduled_jobs(self):
		...
		for scheduled_job in self.scheduled_jobs:
			if bool(scheduled_job.stopped) != bool(self.disabled):
				job = sparrow.get_doc("Scheduled Job Type", scheduled_job.name)
	...
	def clear_scheduled_events(self):
		...
		if self.script_type == "Scheduler Event" and (
			...
			for scheduled_job in self.scheduled_jobs:
				sparrow.delete_doc("Scheduled Job Type", scheduled_job.name)
	...
	def check_if_compilable_in_restricted_context(self):
		...
		try:
			...
			sparrow.msgprint(str(e), title=_("Compilation warning"))

	def execute_method(self) -> dict:
		"""Specific to API endpoint Server Scripts

		Raises:
		        sparrow.DoesNotExistError: If self.script_type is not API
		        sparrow.PermissionError: If self.allow_guest is unset for API accessed by Guest user

		Returns:
		        dict: Evaluates self.script with sparrow.utils.safe_exec.safe_exec and returns the flags set in it's safe globals
		"""
		# wrong report type!
	...
	def execute_scheduled_method(self):
		...
		Raises:
		        sparrow.DoesNotExistError: If script type is not a scheduler event
		"""
		if self.script_type != "Scheduler Event":
			raise sparrow.DoesNotExistError
	...
	def get_autocompletion_items(self):
		...
		Returns:
		        ...
		        For e.g., ["sparrow.utils.cint", "sparrow.get_all", ...]
		"""
		...
		items = sparrow.cache().get_value("server_script_autocompletion_items")
		if not items:
			items = get_keys(get_safe_globals())
			items = [{"value": d[0], "score": d[1]} for d in items]
			sparrow.cache().set_value("server_script_autocompletion_items", items)
		return items
...
def setup_scheduler_events(script_name: str, frequency: str, cron_format: str | None = None):
	...
	Args:
	        ...
	        frequency (str): Event label compatible with the Sparrow scheduler
	"""
	method = sparrow.scrub(f"{script_name}-{frequency}")
	scheduled_script = sparrow.db.get_value("Scheduled Job Type", {"method": method})
	...
	if not scheduled_script:
		...
		sparrow.msgprint(_("Enabled scheduled execution for script {0}").format(script_name))

	else:
		doc = sparrow.get_doc("Scheduled Job Type", scheduled_script)
		...
		sparrow.msgprint(_("Scheduled execution for script {0} has updated").format(script_name))
