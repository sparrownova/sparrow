sparrow.ui.form.on("System Settings", {
	refresh: function (frm) {
		sparrow.call({
			method: "sparrow.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				sparrow.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", sparrow.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					sparrow.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					sparrow.app.setup_moment();
	...
	on_update: function (frm) {
		if (sparrow.boot.time_zone && sparrow.boot.time_zone.system !== frm.doc.time_zone) {
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == sparrow.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		sparrow.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", sparrow.boot.sysdefaults.rounding_method);
			}
		);
	},
});
