...
from sparrow.database.schema import SPECIAL_CHAR_PATTERN
from sparrow.model.document import Document
from sparrow.permissions import get_doctypes_with_read
from sparrow.utils import call_hook_method, cint, get_files_path, get_hook_method, get_url
from sparrow.utils.file_manager import is_safe_path
from sparrow.utils.image import optimize_image, strip_exif_data
...
class File(Document):
	...
	def autoname(self):
		...
		if self.is_folder:
			...
			self.name = sparrow.generate_hash(length=10)
	...
	def before_insert(self):
		...
		if self.is_remote_file:
			...
			sparrow.local.rollback_observers.append(self)
	...
	def validate(self):
		...
		self.file_size = sparrow.form_dict.file_size or self.file_size
	...
	def validate_attachment_references(self):
		...
		if not self.attached_to_name or not isinstance(self.attached_to_name, (str, int)):
			sparrow.throw(_("Attached To Name must be a string or an integer"), sparrow.ValidationError)

		if self.attached_to_field and SPECIAL_CHAR_PATTERN.search(self.attached_to_field):
			sparrow.throw(_("The fieldname you've specified in Attached To Field is invalid"))
	...
	def on_trash(self):
		if self.is_home_folder or self.is_attachments_folder:
			sparrow.throw(_("Cannot delete Home and Attachments folders"))
	...
	def get_successors(self):
		return sparrow.get_all("File", filters={"folder": self.name}, pluck="name")
	...
	def handle_is_private_changed(self):
		...
		private_file_path = Path(sparrow.get_site_path("private", "files", file_name))
		public_file_path = Path(sparrow.get_site_path("public", "files", file_name))
	...
	def fetch_attached_to_field(self, old_file_url):
		...
		reference_dict = sparrow.get_doc(self.attached_to_doctype, self.attached_to_name).as_dict()
	...
	def validate_attachment_limit(self):
		...
		if self.attached_to_doctype and self.attached_to_name:
			attachment_limit = cint(sparrow.get_meta(self.attached_to_doctype).max_attachments)
			...
			if current_attachment_count >= attachment_limit:
				sparrow.throw(
					_("Maximum Attachment Limit of {0} has been reached for {1} {2}.").format(
						sparrow.bold(attachment_limit), self.attached_to_doctype, self.attached_to_name
					),
	...
	def set_folder_name(self):
		...
		if self.attached_to_doctype:
			self.folder = sparrow.db.get_value("File", {"is_attachments_folder": 1})
	...
	def validate_file_on_disk(self):
		...
		if not os.path.exists(full_path):
			sparrow.throw(_("File {0} does not exist").format(self.file_url), IOError)
	...
	def validate_duplicate_entry(self):
		if not self.flags.ignore_duplicate_entry_error and not self.is_folder:
			...
			duplicate_file = sparrow.db.get_value("File", filters, ["name", "file_url"], as_dict=1)

			if duplicate_file:
				duplicate_file_doc = sparrow.get_cached_doc("File", duplicate_file.name)
	...
	def set_file_name(self):
		if not self.file_name and not self.file_url:
			sparrow.throw(
				_("Fields `file_name` or `file_url` must be set for File"), exc=sparrow.MandatoryError
			)
	...
	def generate_content_hash(self):
		...
		try:
			...
			sparrow.throw(_("File {0} does not exist").format(file_path))
	...
	def make_thumbnail(
		...
		path = os.path.abspath(sparrow.get_site_path("public", thumbnail_url.lstrip("/")))
		...
		try:
			...
			sparrow.msgprint(_("Unable to write file format for {0}").format(path))
			return
	...
	def validate_empty_folder(self):
		...
		if self.is_folder and sparrow.get_all("File", filters={"folder": self.name}, limit=1):
			sparrow.throw(_("Folder {0} is not empty").format(self.name), FolderNotEmpty)

	def _delete_file_on_disk(self):
		"""If file not attached to any other record, delete it"""
		on_disk_file_not_shared = self.content_hash and not sparrow.get_all(
			"File",
	...
	def unzip(self) -> list["File"]:
		...
		if not self.file_url.endswith(".zip"):
			sparrow.throw(_("{0} is not a zip file").format(self.file_name))
		...
		with zipfile.ZipFile(zip_path) as z:
			for file in z.filelist:
				...
				file_doc = sparrow.new_doc("File")
				try:
					file_doc.content = z.read(file.filename)
				except zipfile.BadZipFile:
					sparrow.throw(_("{0} is a not a valid zip file").format(self.file_name))
		...
		sparrow.delete_doc("File", self.name)
		return files
	...
	def get_content(self) -> bytes:
		if self.is_folder:
			sparrow.throw(_("Cannot get file contents of a Folder"))
	...
	def get_full_path(self):
		...
		if not is_safe_path(file_path):
			sparrow.throw(_("Cannot access file path {0}").format(file_path))

		if os.path.sep in self.file_name:
			sparrow.throw(_("File name cannot have {0}").format(os.path.sep))
	...
	def save_file(
		...
		if (
			...
			and sparrow.get_system_settings("strip_exif_metadata_from_uploaded_images")
		):
		...
		if not ignore_existing_file_check:
			duplicate_file = sparrow.get_value(
				"File",
		...
		if duplicate_file:
			file_doc: "File" = sparrow.get_cached_doc("File", duplicate_file.name)
	...
	def check_max_file_size(self):
		...
		if file_size > max_file_size:
			...
			if sparrow.has_permission("System Settings", "write"):
				msg += ".<br>" + _("You can increase the limit from System Settings.")
			sparrow.throw(msg, exc=MaxFileSizeReachedError)
	...
	def create_attachment_record(self):
		...
		file_url = (
			quote(sparrow.safe_encode(self.file_url), safe="/:") if self.file_url else self.file_name
		)
	...
	def add_comment_in_reference_doc(self, comment_type, text):
		if self.attached_to_doctype and self.attached_to_name:
			try:
				doc = sparrow.get_doc(self.attached_to_doctype, self.attached_to_name)
	...
	def zip_files(files):
		...
		for _file in files:
			if isinstance(_file, str):
				_file = sparrow.get_doc("File", _file)
...
def on_doctype_update():
	sparrow.db.add_index("File", ["attached_to_doctype", "attached_to_name"])
...
def has_permission(doc, ptype=None, user=None):
	...
	if ptype == "create":
		return sparrow.has_permission("File", "create", user=user)
	...
	if doc.attached_to_doctype and doc.attached_to_name:
		...
		try:
			ref_doc = sparrow.get_doc(attached_to_doctype, attached_to_name)
		except sparrow.DoesNotExistError:
