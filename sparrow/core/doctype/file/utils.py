...
import sparrow
from sparrow import _, safe_decode
from sparrow.utils import cstr, encode, get_files_path, random_string, strip
from sparrow.utils.file_manager import safe_b64decode
...
def setup_folder_path(filename: str, new_parent: str) -> None:
	file: "File" = sparrow.get_doc("File", filename)
...
def get_local_image(file_url: str) -> tuple["ImageFile", str, str]:
	...
	file_path = sparrow.get_site_path(*file_url_path)

	try:
		image = Image.open(file_path)
	except OSError:
		sparrow.throw(_("Unable to read file format for {0}").format(file_url))
...
def get_web_image(file_url: str) -> tuple["ImageFile", str, str]:
	# download
	file_url = sparrow.utils.get_url(file_url)
	r = requests.get(file_url, stream=True)
	try:
		r.raise_for_status()
	except requests.exceptions.HTTPError as e:
		if "404" in e.args[0]:
			sparrow.msgprint(_("File '{0}' not found").format(file_url))
		else:
			sparrow.msgprint(_("Unable to read file format for {0}").format(file_url))
		raise

	try:
		image = Image.open(BytesIO(r.content))
	except Exception as e:
		sparrow.msgprint(_("Image link '{0}' is not valid").format(file_url), raise_exception=e)
...
def remove_file_by_url(file_url: str, doctype: str = None, name: str = None) -> "Document":
	if doctype and name:
		fid = sparrow.db.get_value(
			"File", {"file_url": file_url, "attached_to_doctype": doctype, "attached_to_name": name}
		)
	else:
		fid = sparrow.db.get_value("File", {"file_url": file_url})

	if fid:
		from sparrow.utils.file_manager import remove_file
...
def get_file_name(fname: str, optional_suffix: str | None = None) -> str:
	...
	suffix = optional_suffix or sparrow.generate_hash(length=6)
...
def extract_images_from_html(doc: "Document", content: str, is_private: bool = False):
	...
	def _save_file(match):
		...
		sparrow.flags.has_dataurl = True
...
def update_existing_file_docs(doc: "File") -> None:
	...
	file_doctype = sparrow.qb.DocType("File")
	(
		sparrow.qb.update(file_doctype)
...
def attach_files_to_document(doc: "Document", event) -> None:
	...
	for df in attach_fields:
		...
		unattached_file = sparrow.db.exists(
			"File",
			{
				"file_url": value,
