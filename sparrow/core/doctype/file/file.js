sparrow.ui.form.on("File", {
	...
	preview_file: function (frm) {
		...
		if (sparrow.utils.is_image_file(frm.doc.file_url)) {
		...
		} else if (sparrow.utils.is_video_file(frm.doc.file_url)) {
			$preview = $(`<div class="img_preview">
	...
	optimize: function (frm) {
		sparrow.show_alert(__("Optimizing image..."));
		frm.call("optimize_file").then(() => {
			sparrow.show_alert(__("Image optimized"));
		});
	},

	unzip: function (frm) {
		sparrow.call({
			method: "sparrow.core.api.file.unzip_file",
			args: {
				name: frm.doc.name,
			},
			callback: function () {
