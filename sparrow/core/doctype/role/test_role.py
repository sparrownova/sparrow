...
class TestUser(sparrowTestCase):
	def test_disable_role(self):
		...
		role = sparrow.get_doc("Role", "_Test Role 3")
		role.disabled = 1
		role.save()

		self.assertTrue("_Test Role 3" not in sparrow.get_roles("test@example.com"))
		...
		sparrow.get_doc("User", "test@example.com").add_roles("_Test Role 3")
		self.assertTrue("_Test Role 3" in sparrow.get_roles("test@example.com"))

	def test_change_desk_access(self):
		"""if we change desk acecss from role, remove from user"""
		sparrow.delete_doc_if_exists("User", "test-user-for-desk-access@example.com")
		sparrow.delete_doc_if_exists("Role", "desk-access-test")
		user = sparrow.get_doc(
			dict(doctype="User", email="test-user-for-desk-access@example.com", first_name="test")
		).insert()
		role = sparrow.get_doc(dict(doctype="Role", role_name="desk-access-test", desk_access=0)).insert()
	...
	def test_get_users_by_role(self):
		...
		for user in sys_managers:
			self.assertIn(role, sparrow.get_roles(user))
