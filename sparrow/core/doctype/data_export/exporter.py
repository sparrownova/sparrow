...
from sparrow.utils import cint, cstr, format_datetime, format_duration, formatdate, parse_json
from sparrow.utils.csvutils import UnicodeWriter
...
class DataExporter:
	...
	def prepare_args(self):
		...
		if self.all_doctypes:
			self.child_doctypes = []
			for df in sparrow.get_meta(self.doctype).get_table_fields():
	...
	def build_response(self):
		...
		if self.file_type == "Excel":
			...
			sparrow.response["result"] = cstr(self.writer.getvalue())
			sparrow.response["type"] = "csv"
			sparrow.response["doctype"] = self.doctype
	...
	def build_field_columns(self, dt, parentfield=None):
		...
		tablecolumns = []
		table_name = "tab" + dt
		for f in sparrow.db.get_table_columns_description(table_name):
		...
		_column_start_end = sparrow._dict(start=0)
		...
		if dt == self.doctype:
			...
			_column_start_end = sparrow._dict(start=len(self.columns))
	...
	def getinforow(docfield):
		...
		if docfield.fieldtype == "Select":
			...
			return cstr(sparrow.defaults.get_defaults().date_format)
	...
	def add_data(self):
		...
		sparrow.permissions.can_export(self.parent_doctype, raise_exception=True)

		# sort nested set doctypes by `lft asc`
		order_by = None
		table_columns = sparrow.db.get_table_columns(self.parent_doctype)
		if "lft" in table_columns and "rgt" in table_columns:
			order_by = f"`tab{self.parent_doctype}`.`lft` asc"
		# get permitted data only
		self.data = sparrow.get_list(
		...
		for doc in self.data:
			...
			if self.all_doctypes:
				...
				for c in self.child_doctypes:
					...
					data_row = (
						sparrow.qb.from_(child_doctype_table)
						.select("*")
	...
	def build_response_as_excel(self):
		filename = sparrow.generate_hash(length=10)
		...
		sparrow.response["filename"] = _(self.doctype) + ".xlsx"
