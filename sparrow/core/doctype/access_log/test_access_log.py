...
class TestAccessLog(SparrowTestCase):
	def setUp(self):
		...
		generated_secret = sparrow.utils.password.get_decrypted_password(
			"User", sparrow.session.user, fieldname="api_secret"
		)
		...
		self.file_name = sparrow.utils.random_string(10) + ".txt"
		self.test_content = sparrow.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = sparrow.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = sparrow.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(sparrow.local.site) + new_private_file.file_url
