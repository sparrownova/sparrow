...
from sparrow.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			sparrow.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> sparrow._dict:
	...
	return sparrow._dict(
		...
		arguments=sparrow.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == sparrow.local.site
...
@sparrow.whitelist()
def stop_job(job_id):
	sparrow.get_doc("RQ Job", job_id).stop_job()
