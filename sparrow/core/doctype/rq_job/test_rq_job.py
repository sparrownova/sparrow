...
class TestRQJob(SparrowTestCase):
	...
	def check_status(self, job: Job, status, wait=True):
		...
		self.assertEqual(sparrow.get_doc("RQ Job", job.id).status, status)

	def test_serialization(self):

		job = sparrow.enqueue(method=self.BG_JOB, queue="short")
		rq_job = sparrow.get_doc("RQ Job", job.id)
	...
	def test_configurable_ttl(self):
		sparrow.conf.rq_job_failure_ttl = 600
		job = sparrow.enqueue(method=self.BG_JOB, queue="short")

		self.assertEqual(job.failure_ttl, 600)

	def test_func_obj_serialization(self):
		job = sparrow.enqueue(method=test_func, queue="short")
		rq_job = sparrow.get_doc("RQ Job", job.id)
	...
	def test_get_list_filtering(self):
		...
		# Fail a job
		job = sparrow.enqueue(method=self.BG_JOB, queue="short", fail=True)
		...
		job = sparrow.enqueue(method=self.BG_JOB, queue="short", sleep=10)
		time.sleep(3)
	...
	def test_delete_doc(self):
		job = sparrow.enqueue(method=self.BG_JOB, queue="short")
		sparrow.get_doc("RQ Job", job.id).delete()
	...
	def test_multi_queue_burst_consumption(self):
		for _ in range(3):
			for q in ["default", "short"]:
				sparrow.enqueue(self.BG_JOB, sleep=1, queue=q)
	...
	def test_job_id_dedup(self):
		job_id = "test_dedup"
		job = sparrow.enqueue(self.BG_JOB, sleep=10, job_id=job_id)
	...
	def test_clear_failed_jobs(self):
		...
		jobs = [sparrow.enqueue(method=self.BG_JOB, queue="short", fail=True) for _ in range(limit * 2)]
