...
from sparrow.cache_manager import clear_controller_cache, clear_user_cache
...
from sparrow.database.schema import validate_column_length, validate_column_name
from sparrow.desk.notifications import delete_notification_count_for, get_filters_for
...
from sparrow.model.docfield import supports_translation
from sparrow.model.document import Document
from sparrow.model.meta import Meta
from sparrow.modules import get_doc_path, make_boilerplate
from sparrow.modules.import_file import get_file_path
from sparrow.query_builder.functions import Concat
from sparrow.utils import cint, is_a_property, random_string
from sparrow.website.utils import clear_cache

if TYPE_CHECKING:
	from sparrow.custom.doctype.customize_form.customize_form import CustomizeForm
...
class InvalidFieldNameError(sparrow.ValidationError):
	pass


class UniqueFieldnameError(sparrow.ValidationError):
	pass


class IllegalMandatoryError(sparrow.ValidationError):
	pass


class DoctypeLinkError(sparrow.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(sparrow.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(sparrow.ValidationError):
	pass


class NonUniqueError(sparrow.ValidationError):
	pass


class CannotIndexedError(sparrow.ValidationError):
	pass


class CannotCreateStandardDoctypeError(sparrow.ValidationError):
	pass
...
class DocType(Document):
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = sparrow.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			sparrow.throw(_("Standard DocType cannot have default print format, use Customize Form"))
	...
	def check_developer_mode(self):
		...
		if sparrow.flags.in_patch or sparrow.flags.in_test:
			return

		if not sparrow.conf.get("developer_mode") and not self.custom:
			sparrow.throw(
		...
		if sparrow.conf.get("developer_mode"):
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = sparrow.get_meta(sparrow.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except sparrow.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = sparrow.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if not "route" in [d.fieldname for d in self.fields]:
				sparrow.throw(_('Field "route" is mandatory for Web Views'), title="Missing Field")
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from sparrow.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = sparrow.bold(self.meta.get_field("max_attachments").label)
			sparrow.msgprint(
	...
	def change_modified_of_parent(self):
		...
		if sparrow.flags.in_import:
			return
		parent_list = sparrow.get_all(
			"DocField", "parent", dict(fieldtype=["in", sparrow.model.table_fields], options=self.name)
		)
		for p in parent_list:
			sparrow.db.update("DocType", p.parent, {}, for_update=False)
			...
			if d.fieldtype:
				if not getattr(d, "fieldname", None):
					if d.label:
						...
						sparrow.throw(_("Row #{}: Fieldname is required").format(d.idx), title="Missing Fieldname")
				else:
					if d.fieldname in restricted:
						sparrow.throw(_("Fieldname {0} is restricted").format(d.fieldname), InvalidFieldNameError)
	...
	def on_update(self):
		...
		try:
			sparrow.db.updatedb(self.name, Meta(self))
		...
		allow_doctype_export = (
			not self.custom
			and not sparrow.flags.in_import
			and (sparrow.conf.developer_mode or sparrow.flags.allow_doctype_export)
		)
		...
		sparrow.clear_cache(doctype=self.name)

		# clear user cache so that on the next reload this doctype is included in boot
		clear_user_cache(sparrow.session.user)

		if not sparrow.flags.in_install and hasattr(self, "before_update"):
	...
	def setup_autoincrement_and_sequence(self):
		...
		name_type = f"varchar({sparrow.db.VARCHAR_LEN})"

		if self.autoname == "autoincrement":
			name_type = "bigint"
			sparrow.db.create_sequence(self.name, check_not_exists=True, cache=sparrow.db.SEQUENCE_CACHE)
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not sparrow.request) or sparrow.flags.in_test or sparrow.flags.in_install
			sparrow.enqueue("sparrow.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = sparrow.scrub(self.name)
		scrubbed_module = sparrow.scrub(self.module)
		controller_path = sparrow.get_module_path(
			scrubbed_module, "doctype", scrubbed_name, f"{scrubbed_name}.py"
		)

		document_cls_tag = f"class {despaced_name}(Document)"
		document_import_tag = "from sparrow.model.document import Document"
		website_generator_cls_tag = f"class {despaced_name}(WebsiteGenerator)"
		website_generator_import_tag = "from sparrow.website.website_generator import WebsiteGenerator"
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and sparrow.session.user != "Administrator":
			sparrow.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			sparrow.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			sparrow.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			sparrow.db.sql(
			...
			sparrow.db.rename_table(old, new)
			sparrow.db.commit()

		# Do not rename and move files and folders for custom doctype
		if not self.custom:
			if not sparrow.flags.in_patch:
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if sparrow.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(sparrow.scrub(old), sparrow.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		sparrow.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(sparrow.scrub(new)))
				...
				with open(fname, "w") as f:
					if fname.endswith(".js"):
						...
						file_content = code.replace(
							sparrow.scrub(old), sparrow.scrub(new)
		...
		doctype_json_path = os.path.join(new_path, f"{sparrow.scrub(new)}.json")
		current_data = sparrow.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		existing_property_setter = sparrow.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = sparrow.get_module_path(
				sparrow.scrub(self.module), "doctype", sparrow.scrub(self.name), "templates"
			)
	...
	def make_amendable(self):
		...
		if self.is_submittable:
			docfield_exists = sparrow.get_all(
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = sparrow.bold(_("Parent Field (Tree)"))
			sparrow.throw(_("{0} is a mandatory field").format(field_label), sparrow.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			sparrow.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = sparrow.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = sparrow.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = sparrow.db.MAX_COLUMN_LENGTH - 3
		if len(name) > max_length:
			# length(tab + <Doctype Name>) should be equal to 64 characters hence doctype should be 61 characters
			sparrow.throw(
				_("Doctype name is limited to {0} characters ({1})").format(max_length, name), sparrow.NameError
			)

		# a DocType name should not start or end with an empty space
		if WHITESPACE_PADDING_PATTERN.search(name):
			sparrow.throw(_("DocType's name should not start or end with whitespace"), sparrow.NameError)
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		if not dt.get("fields", {"fieldname": fieldname}):
			sparrow.throw(
				_("Fieldname called {0} must exist to enable autonaming").format(sparrow.bold(fieldname)),
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			sparrow.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if (
		...
		if used_in:
			sparrow.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
...
def validate_autoincrement_autoname(dt: Union[DocType, "CustomizeForm"]) -> bool:
	...
	def get_autoname_before_save(dt: Union[DocType, "CustomizeForm"]) -> str:
		if dt.doctype == "Customize Form":
			property_value = sparrow.db.get_value(
			...
			if not property_value:
				return sparrow.db.get_value("DocType", dt.doc_type, "autoname") or ""
	...
	if not dt.is_new():
		...
		if (
			...
			if dt.doctype == "Customize Form":
				sparrow.throw(_("Cannot change to/from autoincrement autoname in Customize Form"))

			if sparrow.get_meta(dt.name).issingle:
				return False

			if not sparrow.get_all(dt.name, limit=1):
...
def change_name_column_type(doctype_name: str, type: str) -> None:
	...
	sparrow.db.change_column_type(*args)


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or sparrow.flags.in_patch or sparrow.flags.in_fixtures or sparrow.flags.in_migrate:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			sparrow.throw(message, sparrow.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			sparrow.throw(message, sparrow.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = sparrow.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, sparrow.bold(link.table_fieldname), sparrow.bold(meta.name)
			)
			sparrow.throw(message, sparrow.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		sparrow.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		sparrow.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		sparrow.throw(msg, InvalidFieldNameError)


def validate_fields_for_doctype(doctype):
	meta = sparrow.get_meta(doctype, cached=False)
...
def validate_fields(meta):
	...
	:param meta: `sparrow.model.meta.Meta` object to check."""
		...
		if fieldname in Document._reserved_keywords:
			sparrow.throw(
				_("{0}: fieldname cannot be set to reserved keyword {1}").format(
					sparrow.bold(docname),
					sparrow.bold(fieldname),
				),
	...
	def check_link_table_options(docname, d):
		if sparrow.flags.in_patch or sparrow.flags.in_fixtures:
			return
		...
		if d.fieldtype in ("Link",) + table_fields:
			...
			if d.options != d.parent:
				options = sparrow.db.get_value("DocType", d.options, "name")
				if not options:
					sparrow.throw(
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default and not sparrow.flags.in_migrate:
			sparrow.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			sparrow.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_illegal_default(d):
		...
		if d.fieldtype == "Select" and d.default:
			if not d.options:
				...
				sparrow.throw(
					_("Default value for {0} must be in the list of options.").format(sparrow.bold(d.fieldname))
				)
	...
	def check_precision(d):
		if (
			...
			sparrow.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and sparrow.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = sparrow.db.sql(
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					sparrow.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						sparrow.throw(_("Fold must come before a Section Break"))
				else:
					sparrow.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				sparrow.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			sparrow.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			sparrow.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			sparrow.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			sparrow.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			sparrow.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			sparrow.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			sparrow.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and DEPENDS_ON_PATTERN.match(depends_on):
				sparrow.throw(_("Invalid {0} condition").format(sparrow.unscrub(field)), sparrow.ValidationError)
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (
			...
			if docfield.options and (docfield.options not in data_field_options):
				df_str = sparrow.bold(_(docfield.label))
				text_str = (
				...
				sparrow.msgprint(text_str + df_options_str, title="Invalid Data Field", alert=True)

	def check_child_table_option(docfield):
		if sparrow.flags.in_fixtures:
			return
		...
		if not meta.istable:
			sparrow.throw(
				_("Option {0} for field {1} is not a child table").format(
					sparrow.bold(doctype), sparrow.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			sparrow.throw(f"Max for {sparrow.bold(docfield.fieldname)} height must be in px, em, rem")
	...
	def check_no_of_ratings(docfield):
		if docfield.fieldtype == "Rating":
			if docfield.options and (int(docfield.options) > 10 or int(docfield.options) < 3):
				sparrow.throw(_("Options for Rating field can range from 3 to 10"))
	...
	for d in fields:
		...
		if not sparrow.flags.in_migrate:
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = sparrow.get_doc("DocType", doctype)
...
def clear_permissions_cache(doctype):
	...
	for user in sparrow.db.sql_list(
		"""
		SELECT
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		sparrow.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_atleast_one_set(d):
		if (
			...
			sparrow.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			sparrow.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			sparrow.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			sparrow.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			sparrow.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			sparrow.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
		...
		for ptype, label in [["set_user_permissions", _("Set User Permissions")]]:
			if d.get(ptype):
				d.set(ptype, 0)
				sparrow.msgprint(_("{0} cannot be set for Single types").format(label))

	def check_if_submittable(d):
		if d.submit and not issubmittable:
			sparrow.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			sparrow.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			sparrow.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
			...
			if d.role == "All":
				sparrow.throw(
					_("Row # {0}: Non administrator user can not set the role {1} to the custom doctype").format(
						d.idx, sparrow.bold(_("All"))
					),
					title=_("Permissions Error"),
				)

			roles = [row.name for row in sparrow.get_all("Role", filters={"is_custom": 1})]

			if d.role in roles:
				sparrow.throw(
					_("Row # {0}: Non administrator user can not set the role {1} to the custom doctype").format(
						d.idx, sparrow.bold(_(d.role))
					),
					title=_("Permissions Error"),
				)
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not sparrow.db.exists("Domain", doc.restrict_to_domain)
		):
			sparrow.get_doc(dict(doctype="Domain", domain=doc.restrict_to_domain)).insert()

		if "tabModule Def" in sparrow.db.get_tables() and not sparrow.db.exists("Module Def", doc.module):
			m = sparrow.get_doc({"doctype": "Module Def", "module_name": doc.module})
			if sparrow.scrub(doc.module) in sparrow.local.module_app:
				m.app_name = sparrow.local.module_app[sparrow.scrub(doc.module)]
			else:
				m.app_name = "sparrow"
			m.flags.ignore_mandatory = m.flags.ignore_permissions = True
			if sparrow.flags.package:
				m.package = sparrow.flags.package.name
				m.custom = 1
			m.insert()
		...
		for role in list(set(roles)):
			if sparrow.db.table_exists("Role", cached=False) and not sparrow.db.exists("Role", role):
				r = sparrow.new_doc("Role")
				r.role_name = role
				r.desk_access = 1
				r.flags.ignore_mandatory = r.flags.ignore_permissions = True
				r.insert()
	except sparrow.DoesNotExistError as e:
		pass
	except sparrow.db.ProgrammingError as e:
...
def check_fieldname_conflicts(docfield):
	...
	doc = sparrow.get_doc({"doctype": docfield.dt})
	...
	if docfield.fieldname in method_list + property_list:
		sparrow.msgprint(msg, raise_exception=not docfield.is_virtual)


def clear_linked_doctype_cache():
	sparrow.cache().delete_value("linked_doctypes_without_ignore_user_permissions_enabled")
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		sparrow.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		sparrow.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		sparrow.throw(_("Select a valid Sender Field for creating documents from Email"))

	if not sender_field.options == "Email":
		sparrow.throw(_("Sender Field should have Email in options"))
