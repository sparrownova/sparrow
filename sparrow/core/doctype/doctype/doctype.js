...
sparrow.ui.form.on("DocType", {
	refresh: function (frm) {
		frm.set_query("role", "permissions", function (doc) {
			if (doc.custom && sparrow.session.user != "Administrator") {
				return {
					query: "sparrow.core.doctype.role.role.role_query",
					filters: [["Role", "name", "!=", "All"]],
				};
			}
		});

		if (sparrow.session.user !== "Administrator" || !sparrow.boot.developer_mode) {
			if (frm.is_new()) {
		...
		if (!frm.is_new() && !frm.doc.istable) {
			if (frm.doc.issingle) {
				frm.add_custom_button(__("Go to {0}", [__(frm.doc.name)]), () => {
					window.open(`/app/${sparrow.router.slug(frm.doc.name)}`);
				});
			} else {
		...
		if (!sparrow.boot.developer_mode && !frm.doc.custom) {
		...
		} else if (sparrow.boot.developer_mode) {
			let msg = __(
	...
	setup_default_views: (frm) => {
		sparrow.model.set_default_views_for_doctype(frm.doc.name, frm);
	},
});

sparrow.ui.form.on("DocField", {
	form_render(frm, doctype, docname) {
		...
		let row = sparrow.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			sparrow.model.with_doctype(link_doctype, () => {
				let fields = sparrow.meta
					.get_docfields(link_doctype, null, {
						fieldtype: ["not in", sparrow.model.no_value_type],
					})
