...
from sparrow.core.doctype.user_permission.user_permission import (
...
class TestUserPermission(SparrowTestCase):
	def setUp(self):
		...
		sparrow.db.delete("User Permission", {"user": ("in", test_users)})
		sparrow.delete_doc_if_exists("DocType", "Person")
		sparrow.db.sql_ddl("DROP TABLE IF EXISTS `tabPerson`")
		sparrow.delete_doc_if_exists("DocType", "Doc A")
		sparrow.db.sql_ddl("DROP TABLE IF EXISTS `tabDoc A`")
	...
	def test_default_user_permission_validation(self):
		...
		self.assertRaises(sparrow.ValidationError, add_user_permissions, param)
	...
	def test_default_user_permission_corectness(self):
		...
		add_user_permissions(param)
		sparrow.db.delete("User Permission", filters={"for_value": test_blog.name})
		sparrow.delete_doc("Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not sparrow.db.exists("Blog Category", category):
				sparrow.get_doc({"doctype": "Blog Category", "title": category}).insert()
		...
		sparrow.set_user("test_user_perm1@example.com")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = sparrow.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = sparrow.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		is_created_applicable_second = sparrow.db.exists(
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = sparrow.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = sparrow.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		removed_applicable_second = sparrow.db.exists(
			"User Permission", get_exists_param(user, applicable="Contact")
		)
	...
	def test_user_perm_for_nested_doctype(self):
		...
		if not sparrow.db.exists("DocType", "Person"):
			doc = new_doctype(
				"Person",
		...
		parent_record = sparrow.get_doc(
			{"doctype": "Person", "person_name": "Parent", "is_group": 1}
		).insert()

		child_record = sparrow.get_doc(
			{
				"doctype": "Person",
				"person_name": "Child",
				"is_group": 0,
		...
		self.assertTrue(has_user_permission(sparrow.get_doc("Person", parent_record.name), user.name))
		self.assertTrue(has_user_permission(sparrow.get_doc("Person", child_record.name), user.name))

		sparrow.db.set_value(
			"User Permission", {"allow": "Person", "for_value": parent_record.name}, "hide_descendants", 1
		)
		sparrow.cache().delete_value("user_permissions")

		# check if adding perm on a group record with hide_descendants enabled,
		# hides child records
		self.assertTrue(has_user_permission(sparrow.get_doc("Person", parent_record.name), user.name))
		self.assertFalse(has_user_permission(sparrow.get_doc("Person", child_record.name), user.name))

	def test_user_perm_on_new_doc_with_field_default(self):
		"""Test User Perm impact on sparrow.new_doc. with *field* default value"""
		sparrow.set_user("Administrator")
		user = create_user("new_doc_test@example.com", "Blogger")

		# make a doctype "Doc A" with 'doctype' link field and default value ToDo
		if not sparrow.db.exists("DocType", "Doc A"):
			doc = new_doctype(
				"Doc A",
				fields=[
					{
		...
		sparrow.set_user("new_doc_test@example.com")

		new_doc = sparrow.new_doc("Doc A")
	...
	def test_user_perm_on_new_doc_with_user_default(self):
		"""Test User Perm impact on sparrow.new_doc. with *user* default value"""
		...
		if not sparrow.db.exists("Session Default", {"ref_doctype": "DocType"}):
...
def create_user(email, *roles):
	...
	if sparrow.db.exists("User", email):
		return sparrow.get_doc("User", email)
