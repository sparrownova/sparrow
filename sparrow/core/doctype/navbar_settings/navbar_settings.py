...
class NavbarSettings(Document):
	...
	def validate_standard_navbar_items(self):
		...
		if not sparrow.flags.in_patch and (len(before_save_items) > len(after_save_items)):
			sparrow.throw(_("Please hide the standard navbar items instead of deleting them"))


def get_app_logo():
	app_logo = sparrow.db.get_single_value("Navbar Settings", "app_logo", cache=True)
	if not app_logo:
		app_logo = sparrow.get_hooks("app_logo_url")[-1]
