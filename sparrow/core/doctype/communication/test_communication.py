...
class TestCommunication(sparrowTestCase):
	def test_email(self):
		...
		for i, x in enumerate(valid_email_list):
			with self.subTest(i=i, x=x):
				self.assertTrue(sparrow.utils.parse_addr(x)[1])

		for i, x in enumerate(invalid_email_list):
			with self.subTest(i=i, x=x):
				self.assertFalse(sparrow.utils.parse_addr(x)[0])
	...
	def test_name(self):
		...
		for x in valid_email_list:
			self.assertTrue(sparrow.utils.parse_addr(x)[0])
	...
	def test_circular_linking(self):
		...
		a = sparrow.get_doc("Communication", a.name)
		a.reference_doctype = "Communication"
		a.reference_name = c.name

		self.assertRaises(sparrow.CircularLinkingError, a.save)

	def test_deduplication_timeline_links(self):
		sparrow.delete_doc_if_exists("Note", "deduplication timeline links")
	...
	def test_contacts_attached(self):
		contact_sender: "Contact" = sparrow.get_doc(
			{
				"doctype": "Contact",
		...
		contact_recipient: "Contact" = sparrow.get_doc(
			{
				"doctype": "Contact",
		...
		contact_cc: "Contact" = sparrow.get_doc(
			{
				"doctype": "Contact",
		...
		comm: Communication = sparrow.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
	...
	def test_get_communication_data(self):
		from sparrow.desk.form.load import get_communication_data

		sparrow.delete_doc_if_exists("Note", "get communication data")

		note = sparrow.get_doc(
			{"doctype": "Note", "title": "get communication data", "content": "get communication data"}
		).insert(ignore_permissions=True)

		comm_note_1 = sparrow.get_doc(
			{
				"doctype": "Communication",
				"communication_type": "Communication",
	...
	def test_link_in_email(self):
		...
		notes = {}
		for i in range(2):
			sparrow.delete_doc_if_exists("Note", f"test document link in email {i}")
			notes[i] = sparrow.get_doc(
				{
					"doctype": "Note",
	...
	def test_signature_in_email_content(self):
		...
		comm_with_signature = sparrow.get_doc(
			base_communication
			| {
		...
		comm_without_signature = sparrow.get_doc(
			base_communication
			| {
...
class TestCommunicationEmailMixin(sparrowTestCase):
	...
	def test_add_attachments_by_filename(self):
		...
		attached_file_name, attached_content_hash = sparrow.db.get_value(
			"File",
	...
	def test_add_attachments_by_file_content(self):
		...
		attached_file_name = sparrow.db.get_value(
			"File",
			{"attached_to_name": comm.name, "attached_to_doctype": comm.doctype},
		)
		attached_file = sparrow.get_doc("File", attached_file_name)
...
def create_email_account() -> "EmailAccount":
	sparrow.delete_doc_if_exists("Email Account", "_Test Comm Account 1")
