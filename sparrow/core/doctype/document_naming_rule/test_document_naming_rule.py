...
class TestDocumentNamingRule(FrappeTestCase):
	def test_naming_rule_by_series(self):
		...
		todo = sparrow.get_doc(
			dict(doctype="ToDo", description="Is this my name " + sparrow.generate_hash())
		).insert()
	...
	def test_naming_rule_by_condition(self):
		...
		# another rule
		naming_rule_1 = sparrow.copy_doc(naming_rule)
		...
		naming_rule_2 = sparrow.copy_doc(naming_rule)
		...
		todo = sparrow.get_doc(
			dict(doctype="ToDo", priority="High", description="Is this my name " + sparrow.generate_hash())
		).insert()

		todo_1 = sparrow.get_doc(
			dict(doctype="ToDo", priority="Medium", description="Is this my name " + sparrow.generate_hash())
		).insert()

		todo_2 = sparrow.get_doc(
			dict(doctype="ToDo", priority="Low", description="Is this my name " + sparrow.generate_hash())
		).insert()
