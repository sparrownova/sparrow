...
from sparrow.desk.notifications import notify_mentions
from sparrow.exceptions import ImplicitCommitError
...
class Comment(Document):
	...
	def validate(self):
		if not self.comment_email:
			self.comment_email = sparrow.session.user
		self.content = sparrow.utils.sanitize_html(self.content)
...
def on_doctype_update():
	sparrow.db.add_index("Comment", ["reference_doctype", "reference_name"])
...
def get_comments_from_parent(doc):
	...
	try:
		if is_virtual_doctype(doc.reference_doctype):
			...
			_comments = sparrow.db.get_value(doc.reference_doctype, doc.reference_name, "_comments") or "[]"

	except Exception as e:
		if sparrow.db.is_missing_table_or_column(e):
			_comments = "[]"
...
def update_comments_in_parent(reference_doctype, reference_name, _comments):
	...
	if (
		...
		or sparrow.db.get_value("DocType", reference_doctype, "issingle")
	...
	try:
		...
		if sparrow.db.is_column_missing(e) and getattr(sparrow.local, "request", None):
			# missing column and in request, add column and update after commit
			sparrow.local._comments = getattr(sparrow.local, "_comments", []) + [
				(reference_doctype, reference_name, _comments)
			]

		elif sparrow.db.is_data_too_long(e):
			raise sparrow.DataTooLongException
		...
		# Clear route cache
		if route := sparrow.get_cached_value(reference_doctype, reference_name, "route"):
			clear_cache(route)


def update_comments_in_parent_after_request():
	"""update _comments in parent if _comments column is missing"""
	if hasattr(sparrow.local, "_comments"):
		for (reference_doctype, reference_name, _comments) in sparrow.local._comments:
