...
import sparrow
import sparrow.desk.query_report
from sparrow import _, scrub
from sparrow.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from sparrow.core.doctype.page.page import delete_custom_role
from sparrow.desk.reportview import append_totals_row
from sparrow.model.document import Document
from sparrow.modules import make_boilerplate
from sparrow.modules.export_file import export_to_files
from sparrow.utils import cint, cstr
from sparrow.utils.safe_exec import check_safe_sql_query, safe_exec
...
class Report(Document):
	def validate(self):
		...
		if not self.module:
			self.module = sparrow.db.get_value("DocType", self.ref_doctype, "module")

		if not self.is_standard:
			self.is_standard = "No"
			if (
				sparrow.session.user == "Administrator" and getattr(sparrow.local.conf, "developer_mode", 0) == 1
			):
		...
		if self.is_standard == "No":
			...
			if self.report_type != "Report Builder":
				sparrow.only_for("Script Manager", True)

			if sparrow.db.get_value("Report", self.name, "is_standard") == "Yes":
				sparrow.throw(_("Cannot edit a standard report. Please duplicate and create a new report"))

		if self.is_standard == "Yes" and sparrow.session.user != "Administrator":
			sparrow.throw(_("Only Administrator can save a standard report. Please rename and save."))
	...
	def on_trash(self):
		if (
			self.is_standard == "Yes"
			and not cint(getattr(sparrow.local.conf, "developer_mode", 0))
			and not sparrow.flags.in_patch
		):
			sparrow.throw(_("You are not allowed to delete Standard Report"))
	...
	def set_doctype_roles(self):
		if not self.get("roles") and self.is_standard == "No":
			meta = sparrow.get_meta(self.ref_doctype)
			if not meta.istable:
	...
	def is_permitted(self):
		...
		if has_common(sparrow.get_roles(), allowed):
			return True
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and (sparrow.local.conf.get("developer_mode") or 0) == 1:
			export_to_files(
	...
	def execute_query_report(self, filters):
		if not self.query:
			sparrow.throw(_("Must specify a Query to run"), title=_("Report Document Error"))

		check_safe_sql_query(self.query)

		result = [list(t) for t in sparrow.db.sql(self.query, filters)]
		columns = self.get_columns() or [cstr(c[0]) for c in sparrow.db.get_description()]
	...
	def execute_script_report(self, filters):
		...
		if execution_time > threshold and not self.prepared_report:
			sparrow.enqueue(enable_prepared_report, report=self.name)

		sparrow.cache().hset("report_execution_time", self.name, execution_time)

		return res

	def execute_module(self, filters):
		# report in python module
		module = self.module or sparrow.db.get_value("DocType", self.ref_doctype, "module")
		method_name = get_report_module_dotted_path(module, self.name) + ".execute"
		return sparrow.get_attr(method_name)(sparrow._dict(filters))

	def execute_script(self, filters):
		# server script
		loc = {"filters": sparrow._dict(filters), "data": None, "result": None}
	...
	def run_query_report(
		...
		columns, result = [], []
		data = sparrow.desk.query_report.run(
			self.name,
			filters=filters,
			user=user,
		...
		for d in data.get("columns"):
			if isinstance(d, dict):
				...
				columns.append(
					sparrow._dict(label=parts[0], fieldtype=fieldtype, fieldname=parts[0], options=options)
				)
	...
	def get_standard_report_columns(self, params):
		if params.get("fields"):
			...
			for df in sparrow.get_meta(self.ref_doctype).fields:
				if df.in_list_view:
	...
	def get_standard_report_order_by(self, params):
		...
		if params.get("group_by"):
			group_by_args = sparrow._dict(params["group_by"])
	...
	def build_standard_report_columns(self, columns, group_by_args):
		...
		for (fieldname, doctype) in columns:
			...
			if meta.get_field(fieldname):
				...
				field = sparrow._dict(fieldname=fieldname, label=label)
	...
	def toggle_disable(self, disable):
		if not self.has_permission("write"):
			sparrow.throw(_("You are not allowed to edit the report."))

		self.db_set("disabled", cint(disable))


def is_prepared_report_disabled(report):
	return sparrow.db.get_value("Report", report, "disable_prepared_report") or 0
...
def enable_prepared_report(report: str):
	sparrow.db.set_value("Report", report, "prepared_report", 1)
