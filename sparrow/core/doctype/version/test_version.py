...
class TestVersion(SparrowTestCase):
	def test_get_diff(self):
		...
		old_doc = sparrow.get_doc("Event", test_records[0])
	...
	def test_no_version_on_new_doc(self):
		from sparrow.desk.form.load import get_versions

		t = sparrow.get_doc(doctype="ToDo", description="something")
		t.save(ignore_version=False)

		self.assertFalse(get_versions(t))

		t = sparrow.get_doc(t.doctype, t.name)
