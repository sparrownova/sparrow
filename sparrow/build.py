...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "sparrow", "dist", type)
	...
	assets_json = sparrow.read_file("assets/assets.json")
	if assets_json:
		assets_json = sparrow.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/sparrow"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["sparrow/" + name for name in missing_assets]
...
def get_assets_link(sparrow_head) -> str:
	...
	if tag:
		...
		url = f"http://assets.sparrowframework.com/{sparrow_head}.tar.gz"
...
def fetch_assets(url, sparrow_head):
	...
	click.echo(click.style("✔", fg="green") + f" Downloaded Sparrow assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./sparrow-snova/sites", "")
...
def setup():
	...
	for app in sparrow.get_all_apps(True):
		try:
			pymodules.append(sparrow.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(sparrow.local.sites_path, "assets")
...
def watch(apps=None):
	...
	live_reload = sparrow.utils.cint(os.environ.get("LIVE_RELOAD", sparrow.conf.live_reload))
...
def generate_assets_map():
	...
	for app_name in sparrow.get_all_apps():
		app_doc_path = None

		pymodule = sparrow.get_module(app_name)
...
def html_to_js_template(path, content):
	"""returns HTML template content as Javascript code, adding it to `sparrow.templates`"""
	return """sparrow.templates["{key}"] = '{content}';\n""".format(
