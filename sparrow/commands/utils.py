...
from sparrow.coverage import CodeCoverage
from sparrow.exceptions import SiteNotSpecifiedError
from sparrow.utils import cint, update_progress_bar
...
def build(
	...
	if make_copy or restore:
		...
		click.secho(
			"snova build: --make-copy and --restore options are deprecated in favour of --hard-link",
			fg="yellow",
		)
...
def watch(apps=None):
	...
	from sparrow.build import watch
...
def destroy_all_sessions(context, reason=None):
	...
	for site in context.sites:
		try:
			sparrow.init(site=site)
			sparrow.connect()
			sparrow.sessions.clear_all_sessions(reason)
			sparrow.db.commit()
		finally:
			sparrow.destroy()
	if not context.sites:
...
def show_config(context, format):
	...
	from sparrow.utils.commands import render_table
	...
	for site in context.sites:
		...
		configuration = sparrow.get_site_config(sites_path=sites_path, site_path=site)
	...
	if format == "json":
		click.echo(sparrow.as_json(sites_config))
...
def reset_perms(context):
	...
	from sparrow.permissions import reset_perms

	for site in context.sites:
		try:
			sparrow.init(site=site)
			sparrow.connect()
			for d in sparrow.db.sql_list(
				"""select name from `tabDocType`
				where istable=0 and custom=0"""
			):
				sparrow.clear_cache(doctype=d)
				reset_perms(d)
		finally:
			sparrow.destroy()
	if not context.sites:
...
def execute(context, method, args=None, kwargs=None, profile=False):
	...
	for site in context.sites:
		...
		try:
			...
			try:
				ret = sparrow.get_attr(method)(*args, **kwargs)
			except Exception:
...
def export_doc(context, doctype, docname):
	...
	for site in context.sites:
		try:
			sparrow.init(site=site)
			sparrow.connect()
			sparrow.modules.export_doc(doctype, docname)
		finally:
			sparrow.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def export_json(context, doctype, path, name=None):
	...
	from sparrow.core.doctype.data_import.data_import import export_json
...
def export_fixtures(context, app=None):
	"Export fixtures"
	from sparrow.utils.fixtures import export_fixtures
...
def data_import(
	...
	from sparrow.core.doctype.data_import.data_import import import_file
...
def bulk_rename(context, doctype, path):
	...
	from sparrow.model.rename_doc import bulk_rename
	from sparrow.utils.csvutils import read_csv_content
...
def database(context):
	...
	if not site:
		raise SiteNotSpecifiedError
	sparrow.init(site=site)
	if not sparrow.conf.db_type or sparrow.conf.db_type == "mariadb":
		_mariadb()
...
def _mariadb():
	...
	command = [
		mysql,
		"--port",
		str(sparrow.conf.db_port or MariaDBDatabase.default_port),
		"-u",
		sparrow.conf.db_name,
		f"-p{sparrow.conf.db_password}",
		sparrow.conf.db_name,
		"-h",
        sparrow.conf.db_host or "localhost",
		"--pager=less -SFX",
		"--safe-updates",
		"-A",
	]
...
def _psql():
	...
	host = sparrow.conf.db_host or "127.0.0.1"
	port = sparrow.conf.db_port or "5432"
	env = os.environ.copy()
	env["PGPASSWORD"] = sparrow.conf.db_password
	conn_string = f"postgresql://{sparrow.conf.db_name}@{host}:{port}/{sparrow.conf.db_name}"
...
def jupyter(context):
	...
	jupyter_notebooks_path = os.path.abspath(sparrow.get_site_path("jupyter_notebooks"))
	sites_path = os.path.abspath(sparrow.get_site_path(".."))
...
Run the following in your first cell to connect notebook to sparrow
```
import sparrow
sparrow.init(site='{site}', sites_path='{sites_path}')
sparrow.connect()
sparrow.local.lang = sparrow.db.get_default('lang')
sparrow.db.connect()
```
	""".format(
...
def console(context, autoreload=False):
	...
	all_apps = sparrow.get_installed_apps()
	failed_to_import = []
...
def transform_database(context, table, engine, row_format, failfast):
	...
	if sparrow.conf.db_type and sparrow.conf.db_type != "mariadb":
	...
	if table == "all":
		...
		queried_tables = (
			sparrow.qb.from_(information_schema.tables)
			.select("table_name")
			.where(
				(information_schema.tables.row_format != row_format)
				& (information_schema.tables.table_schema == sparrow.conf.db_name)
			)
			.run()
		)
	...
	for current, table in enumerate(tables):
		...
		try:
			sparrow.db.sql(f"ALTER TABLE `{table}`{values_to_set}")
...
@click.option(
	"--doctype-list-path",
	help="Path to .txt file for list of doctypes. Example shopper/tests/server/agriculture.txt",
)
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		import sparrow
		import sparrow.test_runner

		tests = test
		site = get_site(context)

		allow_tests = sparrow.get_conf(site).allow_tests
		...
		sparrow.flags.skip_before_tests = skip_before_tests
		sparrow.flags.skip_test_records = skip_test_records

		ret = sparrow.test_runner.main(
			app,
			module,
			doctype,
			module_def,
...
@click.option("--app", help="For App", default="sparrow")
...
def run_parallel_tests(
	...
	with CodeCoverage(with_coverage, app):
		site = get_site(context)
		if use_orchestrator:
			from sparrow.parallel_test_runner import ParallelTestWithOrchestrator

			ParallelTestWithOrchestrator(app, site=site)
		else:
			from sparrow.parallel_test_runner import ParallelTestRunner
...
def run_ui_tests(
	...
	"Run UI tests"
	site = get_site(context)
	app_base_path = os.path.abspath(os.path.join(sparrow.get_app_path(app), ".."))
	site_url = sparrow.utils.get_site_url(site)
	admin_password = sparrow.get_conf(site).admin_password
	...
	if not (
		...
		sparrow.commands.popen(f"yarn add {packages} --no-lockfile")
	...
	sparrow.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)
...
def serve(
	...
	with CodeCoverage(with_coverage, "sparrow"):
		if with_coverage:
...
def request(context, args=None, path=None):
	...
	for site in context.sites:
		try:
			...
			if args:
				if "?" in args:
					sparrow.local.form_dict = sparrow._dict([a.split("=") for a in args.split("?")[-1].split("&")])
				else:
					sparrow.local.form_dict = sparrow._dict()

				if args.startswith("/api/method"):
					sparrow.local.form_dict.cmd = args.split("?", 1)[0].split("/")[-1]
			elif path:
			...
			sparrow.handler.execute_cmd(sparrow.form_dict.cmd)

			print(sparrow.response)
		finally:
			sparrow.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def make_app(destination, app_name, no_git=False):
	...
	from sparrow.utils.boilerplate import make_boilerplate
...
def create_patch():
	...
	from sparrow.utils.boilerplate import PatchCreator
...
def get_version(output):
	...
	from sparrow.utils.change_log import get_app_branch
	from sparrow.utils.commands import render_table

	sparrow.init("")
	data = []

	for app in sorted(sparrow.get_all_apps()):
		module = sparrow.get_module(app)
		app_hooks = sparrow.get_module(app + ".hooks")

		app_info = sparrow._dict()

		try:
			app_info.commit = Repo(sparrow.get_app_path(app, "..")).head.object.hexsha[:7]
...
def rebuild_global_search(context, static_pages=False):
	...
	from sparrow.utils.global_search import (
