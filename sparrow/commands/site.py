...
def new_site(
	...
	"Create a new site"
	from sparrow.installer import _new_site, extract_sql_from_archive

	sparrow.init(site=site, new_site=True)
...
def restore(
	...
	from sparrow.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	try:
		...
		if is_partial(decompressed_file_name):
			click.secho(
				"Partial Backup file detected. You cannot use a partial file to restore a Sparrow site.",
				fg="red",
			)
			click.secho(
				"Use `snova partial-restore` to restore a partial backup to an existing site.", fg="yellow"
			)
	...
	# dont allow downgrading to older versions of sparrow without force
...
def partial_restore(context, sql_file_path, verbose, encryption_key=None):
	from sparrow.installer import extract_sql_from_archive, partial_restore
	from sparrow.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	try:
		...
		with open(decompressed_file_name) as f:
			...
			if "Partial Backup" not in header:
				click.secho(
					"Full backup file detected.Use `snova restore` to restore a Sparrow Site.", fg="red"
				)
				_backup.decryption_rollback()
				sys.exit(1)
...
def _reinstall(
	...
	from sparrow.installer import _new_site
	...
	try:
		...
		sparrow.connect()
		sparrow.clear_cache()
		installed = sparrow.get_installed_apps()
		sparrow.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context, apps, force=False):
	...
	from sparrow.installer import install_app as _install_app
	...
	for site in context.sites:
		...
		for app in apps:
			...
			except sparrow.IncompatibleApp as err:
				err_msg = f":\n{err}" if str(err) else ""
				print(f"App {app} is Incompatible with Site {site}{err_msg}")
				exit_code = 1
			except Exception as err:
				err_msg = f": {str(err)}\n{sparrow.get_traceback()}"
...
def list_apps(context, format):
	...
	for site in context.sites:
		...
		apps = sparrow.get_single("Installed Applications").installed_applications
		...
		if apps:
			...
			installed_applications = sparrow.get_installed_apps()
			applications_summary = "\n".join(installed_applications)
			summary = f"{site_title}\n{applications_summary}\n"
	...
	if format == "json":
		click.echo(sparrow.as_json(summary_dict))
...
def add_system_manager(context, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		sparrow.connect(site=site)
		try:
			sparrow.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			sparrow.db.commit()
		finally:
			sparrow.destroy()
	if not context.sites:
...
def disable_user(context, email):
	...
	with sparrow.init_site(site):
		sparrow.connect()
		user = sparrow.get_doc("User", email)
		user.enabled = 0
...
def migrate(context, skip_failing=False, skip_search_index=False):
	...
	from sparrow.migrate import SiteMigration
...
def run_patch(context, module, force):
	...
	for site in context.sites:
		...
		try:
			sparrow.connect()
			sparrow.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			sparrow.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context, module, doctype, docname):
	...
	for site in context.sites:
		try:
			sparrow.init(site=site)
			sparrow.connect()
			sparrow.reload_doc(module, doctype, docname, force=context.force)
			sparrow.db.commit()
		finally:
			sparrow.destroy()
	if not context.sites:
...
def reload_doctype(context, doctype):
	...
	for site in context.sites:
		try:
			sparrow.init(site=site)
			sparrow.connect()
			sparrow.reload_doctype(doctype, force=context.force)
			sparrow.db.commit()
		finally:
			sparrow.destroy()
	if not context.sites:
...
def add_to_hosts(context):
	"Add site to hosts"
	for site in context.sites:
		sparrow.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
...
def backup(
	...
	for site in context.sites:
		try:
			...
			exit_code = 1
			continue
		if sparrow.get_system_settings("encrypt_backup") and sparrow.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context, app):
	...
	from sparrow.installer import remove_from_installed_apps
...
def uninstall(context, app, dry_run, yes, no_backup, force):
	...
	from sparrow.installer import remove_app
...
def _drop_site(
	...
	from sparrow.database import drop_user_and_database
	from sparrow.utils.backups import scheduled_backup
	...
	try:
		...
		if force:
			...
			messages = [
				...
				"Hint: Use 'snova drop-site {0} --force' to force the removal of {0}".format(site),
			]
			click.echo("\n".join(messages))
			sys.exit(1)

	click.secho("Dropping site database and user", fg="green")
	drop_user_and_database(sparrow.conf.db_name, db_root_username, db_root_password)

	archived_sites_path = archived_sites_path or os.path.join(
		sparrow.get_app_path("sparrow"), "..", "..", "..", "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	sparrow.init(site)
	old_path = sparrow.utils.get_site_path()
...
def set_last_active_for_user(context, user=None):
	...
	with sparrow.init_site(site):
		...
		sparrow.db.set_value("User", user, "last_active", now_datetime())
		sparrow.db.commit()
...
def publish_realtime(context, event, message, room, user, doctype, docname, after_commit):
	"Publish realtime event from snova"
	from sparrow import publish_realtime
...
def browse(context, site, user=None):
	...
	if site not in sparrow.utils.get_sites():
	...
	sid = ""
	if user:
		if sparrow.conf.developer_mode or user == "Administrator":
			...
			sid = f"/app?sid={sparrow.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{sparrow.utils.get_site_url(site)}{sid}"
...
def clear_log_table(context, doctype, days, no_backup):
	...
	"""
	from sparrow.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from sparrow.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from sparrow.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise sparrow.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		queried_result = (
			...
			.select(table_name)
			.where(information_schema.tables.table_schema == sparrow.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run()
		)

		database_tables = [x[0] for x in queried_result]
		doctype_tables = sparrow.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			if format == "text":
				click.secho(f"No ghost tables found in {sparrow.local.site}...Great!", fg="green")
		else:
			if not yes:
			...
			for table in TABLES_TO_DROP:
				...
				if not dry_run:
					sparrow.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[sparrow.local.site] = TABLES_TO_DROP
		sparrow.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"sparrow",
		"database",
		sparrow.conf.db_type,
		f"framework_{sparrow.conf.db_type}.sql",
	)
...
def trim_tables(context, dry_run, format, no_backup):
	...
	from sparrow.model.meta import trim_tables
	from sparrow.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {sparrow.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {sparrow.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({sparrow.local.site: data}, indent=1, sort_keys=True))
	else:
