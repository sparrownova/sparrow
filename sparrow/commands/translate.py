...
def build_message_files(context):
	...
	for site in context.sites:
		try:
			...
			sparrow.connect()
			sparrow.translate.rebuild_all_translation_files()
		finally:
			sparrow.destroy()
...
@click.argument("app")  # , help="App name eg. sparrow")
def new_language(context, lang_code, app):
	...
	# init site
	sparrow.connect(site=context["sites"][0])
	sparrow.translate.write_translations_file(app, lang_code)
	...
	print(
		"You will need to add the language in sparrow/geo/languages.json, if you haven't done it already."
	)
...
def get_untranslated(context, lang, untranslated_file, app="_ALL_APPS", all=None):
	...
	try:
		sparrow.init(site=site)
		sparrow.connect()
		sparrow.translate.get_untranslated(lang, untranslated_file, get_all=all, app=app)
	finally:
		sparrow.destroy()
...
def update_translations(context, lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	try:
		sparrow.init(site=site)
		sparrow.connect()
		sparrow.translate.update_translations(lang, untranslated_file, translated_file, app=app)
	finally:
		sparrow.destroy()
...
def import_translations(context, lang, path):
	...
	try:
		sparrow.init(site=site)
		sparrow.connect()
		sparrow.translate.import_translations(lang, path)
	finally:
		sparrow.destroy()
