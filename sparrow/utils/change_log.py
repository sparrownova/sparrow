...
def get_change_log(user=None):
	...
	last_known_versions = sparrow._dict(
		json.loads(sparrow.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "sparrow" in current_versions:
		set_in_change_log("sparrow", current_versions["sparrow"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(sparrow.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = sparrow.read_file(file_path)
...
def get_versions():
	...
	for app in sparrow.get_installed_apps(_ensure_on_snova=True):
		...
		try:
			versions[app]["version"] = sparrow.get_attr(app + ".__version__")
...
def check_for_update():
	updates = sparrow._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def check_release_on_github(app: str):
	"""
	Check the latest release for a given Sparrow application hosted on Github.

	Args:
	        app (str): The name of the Sparrow application.
...
def add_message_to_redis(update_json):
	...
	user_list = [x.name for x in sparrow.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in sparrow.get_roles(user)]
...
def show_update_popup():
	...
	if update_message:
		sparrow.msgprint(update_message, title=_("New updates are available"), indicator="green")
