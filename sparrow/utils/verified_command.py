...
def get_secret():
	from sparrow.utils.password import get_encryption_key

	return sparrow.local.conf.get("secret") or get_encryption_key()
...
def verify_request():
	...
	if signature_string in query_string:
		...
		valid_method = sparrow.request.method == "GET"
		valid_request_data = not (sparrow.request.form or sparrow.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return sparrow.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(sparrow.utils.cstr(p) for p in params.values())
	if not secret:
		secret = sparrow.local.conf.get("secret") or "secret"
