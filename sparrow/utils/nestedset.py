...
from sparrow.query_builder.functions import Coalesce, Max
from sparrow.query_builder.terms import SubQuery
from sparrow.query_builder.utils import DocType


class NestedSetRecursionError(sparrow.ValidationError):
	pass


class NestedSetMultipleRootsError(sparrow.ValidationError):
	pass


class NestedSetChildExistsError(sparrow.ValidationError):
	pass


class NestedSetInvalidMergeError(sparrow.ValidationError):
	pass
...
def update_nsm(doc):
	...
	parent_field = "parent_" + sparrow.scrub(doc.doctype)
	...
	sparrow.db.set_value(doc.doctype, doc.name, old_parent_field, parent or "", update_modified=False)
...
def update_add_node(doc, parent, parent_field):
	...
	if parent:
		left, right = sparrow.db.get_value(doctype, {"name": parent}, ["lft", "rgt"], for_update=True)
	...
	sparrow.qb.update(Table).set(Table.rgt, Table.rgt + 2).where(Table.rgt >= right).run()
	sparrow.qb.update(Table).set(Table.lft, Table.lft + 2).where(Table.lft >= right).run()

	if (
		sparrow.qb.from_(Table).select("*").where((Table.lft == right) | (Table.rgt == right + 1)).run()
	):
		sparrow.throw(_("Nested set error. Please contact the Administrator."))

	# update index of new node
	sparrow.qb.update(Table).set(Table.lft, right).set(Table.rgt, right + 1).where(
		Table.name == name
	).run()
	return right
...
def update_move_node(doc: Document, parent_field: str):
	...
	# move to dark side
	sparrow.qb.update(Table).set(Table.lft, -Table.lft).set(Table.rgt, -Table.rgt).where(
		(Table.lft >= doc.lft) & (Table.rgt <= doc.rgt)
	).run()

	# shift left
	diff = doc.rgt - doc.lft + 1
	sparrow.qb.update(Table).set(Table.lft, Table.lft - diff).set(Table.rgt, Table.rgt - diff).where(
		Table.lft > doc.rgt
	).run()

	# shift left rgts of ancestors whose only rgts must shift
	sparrow.qb.update(Table).set(Table.rgt, Table.rgt - diff).where(
		(Table.lft < doc.lft) & (Table.rgt > doc.rgt)
	).run()
	...
	if parent:
		...
		# set parent lft, rgt
		sparrow.qb.update(Table).set(Table.rgt, Table.rgt + diff).where(Table.name == parent).run()
		...
		# new root
		max_rgt = sparrow.qb.from_(Table).select(Max(Table.rgt)).run(pluck=True)[0]
		new_diff = max_rgt + 1 - doc.lft

	# bring back from dark side
	sparrow.qb.update(Table).set(Table.lft, -Table.lft + new_diff).set(
...
def rebuild_tree(doctype, parent_field):
	...
	if sparrow.request and sparrow.local.form_dict.cmd == "rebuild_tree":
		sparrow.only_for("System Manager")

	meta = sparrow.get_meta(doctype)
	if not meta.has_field("lft") or not meta.has_field("rgt"):
		sparrow.throw(
			_("Rebuilding of tree is not supported for {}").format(sparrow.bold(doctype)),
	...
	sparrow.db.auto_commit_on_many_writes = 1

	for r in result:
		right = rebuild_node(doctype, r[0], right, parent_field)

	sparrow.db.auto_commit_on_many_writes = 0
...
def rebuild_node(doctype, parent, left, parent_field):
	...
	result = (sparrow.qb.from_(table).where(column == parent).select(table.name)).run()
...
def remove_subtree(doctype: str, name: str, throw=True):
	...
	sparrow.qb.update(table).set(table.lft, table.lft - width).where(table.lft > rgt).run()
	sparrow.qb.update(table).set(table.rgt, table.rgt - width).where(table.rgt > rgt).run()
...
class NestedSet(Document):
	...
	def on_trash(self, allow_root_deletion=False):
		if not getattr(self, "nsm_parent_field", None):
			self.nsm_parent_field = sparrow.scrub(self.doctype) + "_parent"

		parent = self.get(self.nsm_parent_field)
		if not parent and not allow_root_deletion:
			sparrow.throw(_("Root {0} cannot be deleted").format(_(self.doctype)))
	...
	def validate_if_child_exists(self):
		has_children = sparrow.db.count(self.doctype, filters={self.nsm_parent_field: self.name})
		if has_children:
			sparrow.throw(
	...
	def before_rename(self, olddn, newdn, merge=False, group_fname="is_group"):
		if merge and hasattr(self, group_fname):
			is_group = sparrow.db.get_value(self.doctype, newdn, group_fname)
	...
	def validate_one_root(self):
		if not self.get(self.nsm_parent_field):
			if self.get_root_node_count() > 1:
				sparrow.throw(_("""Multiple root nodes not allowed."""), NestedSetMultipleRootsError)

	def get_root_node_count(self):
		return sparrow.db.count(self.doctype, {self.nsm_parent_field: ""})
	...
	def validate_ledger(self, group_identifier="is_group"):
		if hasattr(self, group_identifier) and not bool(self.get(group_identifier)):
			if sparrow.get_all(self.doctype, {self.nsm_parent_field: self.name, "docstatus": ("!=", 2)}):
				sparrow.throw(
...
def get_root_of(doctype):
	...
	node_query = SubQuery(
		sparrow.qb.from_(t2).select(Count("*")).where((t2.lft < t1.lft) & (t2.rgt > t1.rgt))
	)
	result = sparrow.qb.from_(t1).select(t1.name).where((node_query == 0) & (t1.rgt > t1.lft)).run()
