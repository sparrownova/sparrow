...
from sparrow.utils import cint, strip_html_tags
from sparrow.utils.data import cstr
...
def reset():
	...
	:return:
	"""
	sparrow.db.delete("__global_search")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in sparrow.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = sparrow.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = sparrow.get_installed_apps()
		module_app = sparrow.local.module_app
	...
	return sparrow.cache().get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if sparrow.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = sparrow._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = sparrow.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", sparrow.model.table_fields], "options": doctype},
		)
	...
	all_records = sparrow.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = sparrow.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": sparrow.db.escape(doctype),
					"name": sparrow.db.escape(doc.name),
					"content": sparrow.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": sparrow.db.escape((title or "")[: int(sparrow.db.VARCHAR_LEN)]),
					"route": sparrow.db.escape((route or "")[: int(sparrow.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def delete_global_search_records_for_doctype(doctype):
	sparrow.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = sparrow._dict()
	child_search_fields = sparrow._dict()

	for child in meta.get_table_fields():
		child_meta = sparrow.get_meta(child.options)
		...
		if search_fields:
			...
			child_records = sparrow.get_all(
				child.options, fields=child_fieldnames, filters={"docstatus": ["!=", 1], "parenttype": doctype}
			)

			for record in child_records:
				all_children.setdefault(record.parent, sparrow._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	for field in doc.meta.get_global_search_fields():
		if doc.get(field.fieldname) and field.fieldtype not in sparrow.model.table_fields:
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(sparrow.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if sparrow.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	apps = sparrow.get_installed_apps()

	routes_to_index = []
	for app in apps:
		base = sparrow.get_app_path(app, "www")
		path_to_index = sparrow.get_app_path(app, "www")
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(sparrow.safe_decode(value))
...
def sync_global_search():
	...
	:return:
	"""
	while sparrow.cache().llen("global_search_queue") > 0:
		# rpop to follow FIFO
		# Last one should override all previous contents of same document
		value = json.loads(sparrow.cache().rpop("global_search_queue").decode("utf-8"))
		sync_value(value)
...
def sync_value_in_queue(value):
	try:
		...
		sparrow.cache().lpush("global_search_queue", json.dumps(value))
...
def delete_for_document(doc):
	...
	"""
	sparrow.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	"""
	from sparrow.desk.doctype.global_search_settings.global_search_settings import (
		get_doctypes_for_global_search,
	)
	from sparrow.query_builder.functions import Match
	...
	for doctype in allowed_doctypes:
		for index, r in enumerate(results):
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = sparrow.get_meta(r.doctype)
					if meta.image_field:
						r.image = sparrow.db.get_value(r.doctype, r.name, meta.image_field)
				except Exception:
