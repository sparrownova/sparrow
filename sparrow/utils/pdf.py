...
from sparrow.utils.jinja_globals import bundled_asset, is_rtl
...
def get_pdf(html, options=None, output: PdfWriter | None = None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				print(html, options)
				sparrow.throw(_("PDF generation failed because of broken image links"))
...
def prepare_options(html, options):
	...
	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or sparrow.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or sparrow.db.get_single_value(
...
def get_cookie_options():
	options = {}
	if sparrow.session and sparrow.session.sid and hasattr(sparrow.local, "request"):
		# Use wkhtmltopdf's cookie-jar feature to set cookies and restrict them to host domain
		cookiejar = f"/tmp/{sparrow.generate_hash()}.jar"

		# Remove port from request.host
		# https://werkzeug.palletsprojects.com/en/0.16.x/wrappers/#werkzeug.wrappers.BaseRequest.host
		domain = sparrow.utils.get_host_name().split(":", 1)[0]
		with open(cookiejar, "w") as f:
			f.write(f"sid={sparrow.session.sid}; Domain={domain};\n")
...
def prepare_header_footer(soup):
	...
	css = sparrow.read_file(os.path.join(sparrow.local.sites_path, print_css))
	...
	for html_id in ("header-html", "footer-html"):
		...
		if content:
			...
			# create temp file
			fname = os.path.join("/tmp", f"sparrow-pdf-{sparrow.generate_hash()}.html")
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/sparrow-pdf-random.html"}
...
def get_wkhtmltopdf_version():
	wkhtmltopdf_version = sparrow.cache().hget("wkhtmltopdf_version", None)

	if not wkhtmltopdf_version:
		try:
			res = subprocess.check_output(["wkhtmltopdf", "--version"])
			wkhtmltopdf_version = res.decode("utf-8").split(" ")[1]
			sparrow.cache().hset("wkhtmltopdf_version", None, wkhtmltopdf_version)
		except Exception:
			pass
