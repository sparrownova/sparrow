...
def request_cache(func: Callable) -> Callable:
	...
	sparrow.local.request_cache. The cache only persists for the current request
	...
	def wrapper(*args, **kwargs):
		if not getattr(sparrow.local, "initialised", None):
			return func(*args, **kwargs)
		if not hasattr(sparrow.local, "request_cache"):
			sparrow.local.request_cache = defaultdict(dict)
		...
		try:
			return sparrow.local.request_cache[func][args_key]
		except KeyError:
			return_val = func(*args, **kwargs)
			sparrow.local.request_cache[func][args_key] = return_val
			return return_val
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	sparrow.utils.caching._SITE_CACHE. The cache persists on the parent process.
	...
	workers, use redis (sparrow.cache API) instead.
	...
	def time_cache_wrapper(func: Callable = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			if getattr(sparrow.local, "initialised", None):
				...
				if hasattr(func, "maxsize") and len(_SITE_CACHE[func_key][sparrow.local.site]) >= func.maxsize:
					_SITE_CACHE[func_key][sparrow.local.site].pop(
						next(iter(_SITE_CACHE[func_key][sparrow.local.site])), None
					)

				if func_call_key not in _SITE_CACHE[func_key][sparrow.local.site]:
					_SITE_CACHE[func_key][sparrow.local.site][func_call_key] = func(*args, **kwargs)

				return _SITE_CACHE[func_key][sparrow.local.site][func_call_key]
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None) -> Callable:
	...
	def wrapper(func: Callable = None) -> Callable:
		...
		def clear_cache():
			sparrow.cache().delete_keys(func_key)
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			if sparrow.cache().exists(func_call_key):
				return sparrow.cache().get_value(func_call_key, user=user)
			else:
				val = func(*args, **kwargs)
				ttl = getattr(func, "ttl", 3600)
				sparrow.cache().set_value(func_call_key, val, expires_in_sec=ttl, user=user)
				return val
