...
from sparrow.utils.data import *
from sparrow.utils.html_utils import sanitize_html
...
def get_fullname(user=None):
	...
	if not hasattr(sparrow.local, "fullnames"):
		sparrow.local.fullnames = {}

	if not sparrow.local.fullnames.get(user):
		p = sparrow.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			sparrow.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			sparrow.local.fullnames[user] = user

	return sparrow.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return sparrow.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number, fieldname):
	...
	try:
		...
		if not valid_number:
			sparrow.throw(
				error_message.format(sparrow.bold(phone_number), sparrow.bold(fieldname)),
				title=error_title,
				exc=sparrow.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		sparrow.throw(
			sparrow._("{0} is not a valid Phone Number").format(phone_number), sparrow.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	if not match and throw:
		sparrow.throw(sparrow._("{0} is not a valid Name").format(name), sparrow.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = sparrow.utils.escape_html(e)
				sparrow.throw(
					sparrow._("{0} is not a valid Email Address").format(invalid_email),
					sparrow.InvalidEmailAddressError,
				)
			return None
		else:
			return matched
...
def validate_url(txt, throw=False, valid_schemes=None):
	...
	if not is_valid and throw:
		sparrow.throw(sparrow._("'{0}' is not a valid URL").format(sparrow.bold(txt)))
...
def has_gravatar(email):
	...
	if sparrow.flags.in_import or sparrow.flags.in_install or sparrow.flags.in_test:
		# no gravatar if via upload
		# since querying gravatar for every item will be slow
		return ""

	hexdigest = hashlib.md5(sparrow.as_unicode(email).encode("utf-8")).hexdigest()
...
def get_gravatar(email):
	from sparrow.utils.identicon import Identicon
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", sparrow._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	sparrow.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return sparrow.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return sparrow.db.set_default(key, val)
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = sparrow.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return sparrow.local.site_path
...
def get_site_url(site):
	return f"http://{site}:{sparrow.get_conf(site).webserver_port}"
...
def get_test_client() -> Client:
	"""Returns an test instance of the Sparrow WSGI"""
	from sparrow.app import application

	return Client(application)


def get_hook_method(hook_name, fallback=None):
	method = sparrow.get_hooks().get(hook_name)
	if method:
		method = sparrow.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in sparrow.get_hooks(hook):
		out = out or sparrow.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(sparrow.local, "request", None) or is_cli():
		lt = len(txt)
		try:
...
def get_html_format(print_path):
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in sparrow.get_installed_apps():
				include_path = sparrow.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(sparrow.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else sparrow.utils.md_to_html(text)
...
def get_site_info():
	from sparrow.email.queue import get_emails_sent_this_month
	from sparrow.utils.user import get_system_managers

	# only get system users
	users = sparrow.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", sparrow.STANDARD_USERS)},
	...
	system_settings = sparrow.db.get_singles_dict("System Settings")
	space_usage = sparrow._dict((sparrow.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": sparrow.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in sparrow.get_hooks("get_site_info"):
		site_info.update(sparrow.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(sparrow.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                snova --site shopper.local execute sparrow.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = sparrow.db.count(doctype)

	return json.loads(sparrow.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: sparrow function to be called
	...
	Example:
	        via terminal:
	                snova --site shopper.local execute sparrow.utils.call --args '''["sparrow.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return json.loads(sparrow.as_json(sparrow.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, (int, float)):
			filters = sparrow.as_unicode(filters)
...
def set_request(**kwargs):
	...
	sparrow.local.request = Request(builder.get_environ())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(sparrow.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return sparrow.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = sparrow.parse_json(sparrow.read_file("assets/assets.json"))

		if assets_rtl := sparrow.read_file("assets/assets-rtl.json"):
			assets.update(sparrow.parse_json(assets_rtl))

		return assets

	if not hasattr(sparrow.local, "assets_json"):
		if not sparrow.conf.developer_mode:
			sparrow.local.assets_json = sparrow.cache().get_value(
				"assets_json",
				_get_assets,
				shared=True,
			)
			...
			sparrow.local.assets_json = _get_assets()
