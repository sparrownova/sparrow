...
def sync_fixtures(app=None):
	...
	sparrow.flags.in_fixtures = True

	for app in apps:
		import_fixtures(app)
		import_custom_scripts(app)

	sparrow.flags.in_fixtures = False


def import_fixtures(app):
	fixtures_path = sparrow.get_app_path(app, "fixtures")
	...
	for fname in fixture_files:
		...
		file_path = sparrow.get_app_path(app, "fixtures", fname)
		try:
			import_doc(file_path)
		except (ImportError, sparrow.DoesNotExistError) as e:
...
def import_custom_scripts(app):
	...
	scripts_folder = sparrow.get_app_path(app, "fixtures", "custom_scripts")
	...
	for fname in os.listdir(scripts_folder):
		...
		with open(file_path) as f:
			script = f.read()
			if sparrow.db.exists("Client Script", {"dt": doctype}):
				client_script = sparrow.get_doc("Client Script", {"dt": doctype})
				client_script.script = script
				client_script.save()
			else:
				client_script = sparrow.new_doc("Client Script")
...
def export_fixtures(app=None):
	...
	if app:
		...
		apps = sparrow.get_installed_apps()
	for app in apps:
		for fixture in sparrow.get_hooks("fixtures", app_name=app):
			filters = None
			or_filters = None
			...
			if not os.path.exists(sparrow.get_app_path(app, "fixtures")):
				os.mkdir(sparrow.get_app_path(app, "fixtures"))

			export_json(
				fixture,
				sparrow.get_app_path(app, "fixtures", sparrow.scrub(fixture) + ".json"),
				filters=filters,
