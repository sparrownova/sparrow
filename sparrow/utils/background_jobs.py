...
def get_queues_timeout():
	common_site_config = sparrow.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	:param now: if now=True, the method is executed via sparrow.call
	...
	if not is_async and not sparrow.flags.in_test:
		print(
			_(
	...
	call_directly = now or (not is_async and not sparrow.flags.in_test)
	if call_directly:
		return sparrow.call(method, **kwargs)

	try:
		q = get_queue(queue, is_async=is_async)
	except ConnectionError:
		if sparrow.local.flags.in_migrate:
	...
	if enqueue_after_commit:
		if not sparrow.flags.enqueue_after_commit:
			sparrow.flags.enqueue_after_commit = []

		sparrow.flags.enqueue_after_commit.append(
			{
				"queue": queue,
				"is_async": is_async,
				"timeout": timeout,
...
def enqueue_doc(
	...
	return enqueue(
		"sparrow.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(sparrow.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	for before_job_task in sparrow.get_hooks("before_job"):
		sparrow.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (sparrow.db.InternalError, sparrow.RetryBackgroundJobError) as e:
		sparrow.db.rollback()

		if retry < 5 and (
			isinstance(e, sparrow.RetryBackgroundJobError)
			or (sparrow.db.is_deadlocked(e) or sparrow.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			...
			sparrow.log_error(title=method_name)
			raise
		...
		for after_job_task in sparrow.get_hooks("after_job"):
			sparrow.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
...
def validate_queue(queue, default_queue_list=None):
	...
	if queue not in default_queue_list:
		sparrow.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(sparrow.local, "conf"):
		raise Exception("You need to call sparrow.init")

	elif not sparrow.local.conf.redis_queue:
		raise Exception("redis_queue missing in common_site_config.json")

	global _redis_queue_conn

	cred = sparrow._dict()
	if sparrow.conf.get("use_rq_auth"):
		if username:
	...
	try:
		...
		log(
			...
			"You can reset credentials using `snova create-rq-users` CLI and restart the server",
			colour="red",
		)
		raise
	except Exception:
		log(f"Please make sure that Redis Queue runs @ {sparrow.get_conf().redis_queue}", colour="red")
		raise


def get_queues(connection=None) -> list[Queue]:
	"""Get all the queues linked to the current snova."""
...
def generate_qname(qtype: str) -> str:
	"""Generate qname by combining snova ID and queue type.
...
def create_job_id(job_id: str) -> str:
	...
	return f"{sparrow.local.site}::{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from sparrow.utils import create_batch

	conf = sparrow.get_conf(site=job.kwargs.get("site"))
