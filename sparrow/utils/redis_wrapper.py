...
class RedisWrapper(redis.Redis):
	...
	def make_key(self, key, user=None, shared=False):
		...
		return f"{sparrow.conf.db_name}|{key}".encode()
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in sparrow.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		if key in sparrow.local.cache:
			val = sparrow.local.cache[key]
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(sparrow.local.cache) if regex.match(cstr(k))]
	...
	def hset(self, name: str, key: str, value, shared: bool = False, cache_locally: bool = True):
		...
		# set in local
		if cache_locally:
			sparrow.local.cache.setdefault(_name, {})[key] = value
	...
	def hget(self, name, key, generator=None, shared=False):
		_name = self.make_key(name, shared=shared)
		if _name not in sparrow.local.cache:
			sparrow.local.cache[_name] = {}

		if not key:
			return None

		if key in sparrow.local.cache[_name]:
			return sparrow.local.cache[_name][key]
		...
		if value is not None:
			...
			sparrow.local.cache[_name][key] = value
		elif generator:
			value = generator()
	...
	def hdel(self, name, key, shared=False):
		...
		if _name in sparrow.local.cache:
			if key in sparrow.local.cache[_name]:
				del sparrow.local.cache[_name][key]
		try:
	...
	def hdel_keys(self, name_starts_with, key):
		...
		for name in sparrow.cache().get_keys(name_starts_with):
			name = name.split("|", 1)[1]
			self.hdel(name, key)
