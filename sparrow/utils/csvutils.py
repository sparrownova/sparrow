...
from sparrow.utils import cint, comma_or, cstr, flt


def read_csv_content_from_attached_file(doc):
	fileid = sparrow.get_all(
		"File",
		fields=["name"],
	...
	try:
		_file = sparrow.get_doc("File", fileid)
...
def read_csv_content(fcontent):
	...
	for line in fcontent.splitlines(True):
		content.append(sparrow.safe_decode(line))
	...
	try:
		...
		sparrow.msgprint(_("Not a valid Comma Separated Value (CSV File)"))
		raise
...
def send_csv_to_client(args):
	...
	sparrow.response["result"] = cstr(to_csv(args.data))
	sparrow.response["doctype"] = args.filename
...
def build_csv_response(data, filename):
	sparrow.response["result"] = cstr(to_csv(data))
	sparrow.response["doctype"] = filename
	sparrow.response["type"] = "csv"
...
def check_record(d):
	...
	from sparrow.utils.dateutils import parse_date

	doc = sparrow.get_doc(d)
	...
	for key in d:
		...
		if docfield:
			if docfield.reqd and (val == "" or val is None):
				sparrow.msgprint(_("{0} is required").format(docfield.label), raise_exception=1)
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
	...
	if d.get("name") and sparrow.db.exists(doctype, d["name"]):
		if overwrite:
			doc = sparrow.get_doc(doctype, d["name"])
