...
def get_jenv():
	...
	if not getattr(sparrow.local, "jenv", None):
		from jinja2 import DebugUndefined
		from jinja2.sandbox import SandboxedEnvironment

		from sparrow.utils.safe_exec import UNSAFE_ATTRIBUTES, get_safe_globals
		...
		sparrow.local.jenv = jenv

	return sparrow.local.jenv
...
def validate_template(html):
	...
	try:
		...
		sparrow.msgprint(f"Line {e.lineno}: {e.message}")
		sparrow.throw(sparrow._("Syntax error in template"))
...
def render_template(template, context, is_path=None, safe_render=True):
	...
	from sparrow import _, get_traceback, throw
...
def get_jloader():
	...
	if not getattr(sparrow.local, "jloader", None):
		...
		sparrow.local.jloader = ChoiceLoader(
...
def get_jinja_hooks():
	...
	if not getattr(sparrow.local, "site", None):
		return (None, None)
	...
	def get_obj_dict_from_paths(object_paths):
		...
		for obj_path in object_paths:
			try:
				obj = sparrow.get_module(obj_path)
			except ModuleNotFoundError:
				obj = sparrow.get_attr(obj_path)
	...
	values = sparrow.get_hooks("jinja")
