...
class SignupDisabledError(sparrow.PermissionError):
	...


def get_oauth2_providers() -> dict[str, dict]:
	out = {}
	providers = sparrow.get_all("Social Login Key", fields=["*"])
...
def get_oauth_keys(provider: str) -> dict[str, str]:
	...
	if keys := sparrow.conf.get(f"{provider}_login"):
		return {"client_id": keys["client_id"], "client_secret": keys["client_secret"]}

	return {
		"client_id": sparrow.db.get_value("Social Login Key", provider, "client_id"),
...
def get_oauth2_authorize_url(provider: str, redirect_to: str) -> str:
	...
	state = {
		"site": sparrow.utils.get_url(),
		"token": sparrow.generate_hash(),
...
def get_redirect_uri(provider: str) -> str:
	keys = sparrow.conf.get(f"{provider}_login")
	...
	return sparrow.utils.get_url(redirect_uri)
...
def get_info_via_oauth(
	...
	if not (info.get("email_verified") or info.get("email")):
		sparrow.throw(_("Email not verified with {0}").format(provider.title()))
...
def login_oauth_user(
	...
	if not (state and state["token"]):
		sparrow.respond_as_web_page(_("Invalid Request"), _("Token is missing"), http_status_code=417)
		return
	...
	try:
		...
		return sparrow.respond_as_web_page(
			"Signup is Disabled",
	...
	sparrow.local.login_manager.user = user
	sparrow.local.login_manager.post_login()

	# because of a GET request!
	sparrow.db.commit()

	if sparrow.utils.cint(generate_login_token):
		login_token = sparrow.generate_hash(length=32)
		sparrow.cache().set_value(
			f"login_token:{login_token}", sparrow.local.session.sid, expires_in_sec=120
		)

		sparrow.response["login_token"] = login_token

	else:
		redirect_to = state.get("redirect_to")
		redirect_post_login(
			desk_user=sparrow.local.response.get("message") == "Logged In",
			redirect_to=redirect_to,
			provider=provider,
		)
...
def get_user_record(user: str, data: dict) -> "User":
	try:
		return sparrow.get_doc("User", user)
	except sparrow.DoesNotExistError:
		if sparrow.get_website_settings("disable_signup"):
			raise SignupDisabledError

	user: "User" = sparrow.new_doc("User")
	...
	user.update(
		{
			...
			"gender": gender,
			"enabled": 1,
			"new_password": sparrow.generate_hash(),
...
def update_oauth_user(user: str, data: dict, provider: str):
	...
	if not user.enabled:
		sparrow.respond_as_web_page(_("Not Allowed"), _("User {0} is disabled").format(user.email))
		return False
	...
	if update_user_record:
		...
		if default_role := sparrow.db.get_single_value("Portal Settings", "default_role"):
...
def redirect_post_login(
	...
	if not redirect_to:
		...
		redirect_to = sparrow.utils.get_url(desk_uri if desk_user else "/me")

	sparrow.local.response["location"] = redirect_to
