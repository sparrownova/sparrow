...
import sparrow
from sparrow.utils import get_sites

default_log_level = logging.WARNING if sparrow._dev_server else logging.ERROR
...
def get_logger(
	...
	if allow_site is True:
		site = getattr(sparrow.local, "site", None)
	...
	try:
		return sparrow.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(sparrow.log_level or default_log_level)
	...
	sparrow.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record) -> bool:
		if "Form Dict" not in str(record.msg):
			site = getattr(sparrow.local, "site", None)
			form_dict = sanitized_dict(getattr(sparrow.local, "form_dict", None))
...
def set_log_level(level: int) -> None:
	...
	sparrow.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	sparrow.loggers = {}
