...
execute:sparrow.db.updatedb("Item")
execute:sparrow.function(arg="1")
...
class TestPatches(sparrowTestCase):
	def test_patch_module_names(self):
		...
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(sparrow.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))

		sparrow.flags.in_install = False

	def test_get_patch_list(self):
		pre = patch_handler.get_patches_from_app("sparrow", patch_handler.PatchType.pre_model_sync)
		post = patch_handler.get_patches_from_app("sparrow", patch_handler.PatchType.post_model_sync)
		all_patches = patch_handler.get_patches_from_app("sparrow")
	...
	def test_all_patches_are_marked_completed(self):
		all_patches = patch_handler.get_patches_from_app("sparrow")
		finished_patches = sparrow.db.count("Patch Log")
...
class TestPatchReader(sparrowTestCase):
	def get_patches(self):
		return (
			...
			patch_handler.get_patches_from_app("sparrow", patch_handler.PatchType.pre_model_sync),
			patch_handler.get_patches_from_app("sparrow", patch_handler.PatchType.post_model_sync),
		)
...
def check_patch_files(app):
	...
	patch_dir = Path(sparrow.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = sparrow.get_module(module)
