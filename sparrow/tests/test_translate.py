...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	sparrow.get_all("Language", pluck="name", filters=[["name", "not like", "en%"]]),
	k=5,
)
...
class TestTranslate(FrappeTestCase):
	...
	def tearDown(self):
		sparrow.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
			sparrow.set_user("Administrator")

	def test_extract_message_from_file(self):
		data = sparrow.translate.get_messages_from_file(translation_string_file)
		exp_filename = "apps/sparrow/sparrow/tests/translation_test_file.txt"
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for sparrow.translate.get_language

		Case 1: sparrow.form_dict._lang is set
		"""

		sparrow.form_dict._lang = first_lang

		with patch.object(sparrow.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: sparrow.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(sparrow.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: sparrow.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: sparrow.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: sparrow.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_python_extractor(self):
		...
		code = textwrap.dedent(
			...
			sparrow._("attr")
			_("name")
			sparrow._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(sparrow.get_app_path(app)) / "translations"
