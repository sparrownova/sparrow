...
UI_TEST_USER = "sparrow@example.com"


def whitelist_for_tests(fn):
	if sparrow.request and not (sparrow.flags.in_test or getattr(sparrow.local, "dev_server", 0)):
		sparrow.throw("Cannot run UI tests. Use a development server with `snova start`")

	return sparrow.whitelist()(fn)
...
def create_if_not_exists(doc):
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = sparrow.db.exists(doc.doctype, filters)
		if not name:
			d = sparrow.get_doc(doc)
...
@whitelist_for_tests
def clear_notes():
	for note in sparrow.get_all("Note", pluck="name"):
		sparrow.delete_doc("Note", note, force=True)
	...
	doc = sparrow.get_doc(
		{
			...
			"communication_date": sparrow.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@whitelist_for_tests
def setup_workflow():
	from sparrow.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if sparrow.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = sparrow.new_doc("Contact")
...
def create_contact_records():
	if sparrow.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if sparrow.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [(f"100{i}", f"Multiple ToDo {i}") for i in range(1, 1002)]

	sparrow.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = sparrow.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if sparrow.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return

	tour = sparrow.get_doc(
		{
			"doctype": "Form Tour",
...
def create_web_page(title, route, single_thread):
	web_page = sparrow.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": sparrow.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not sparrow.db.exists("Web Form", "update-profile-duplicate"):
		doc = sparrow.get_doc("Web Form", "edit-profile")
		_doc = sparrow.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if sparrow.get_all(name, {"title": "Test Grid Search"}):
		return
...
def insert_translations():
	...
	for doc in translation:
		if not sparrow.db.exists("doc"):
			sparrow.get_doc(doc).insert()


@whitelist_for_tests
def create_blog_post():

	blog_category = sparrow.get_doc(
		{"name": "general", "doctype": "Blog Category", "title": "general"}
	).insert(ignore_if_duplicate=True)

	blogger = sparrow.get_doc(
		{
...
def create_test_user(username=None):
	...
	if sparrow.db.exists("User", name):
		return

	user = sparrow.new_doc("User")
	user.email = name
	user.first_name = "Sparrow"
	user.new_password = sparrow.local.conf.admin_password
	...
	all_roles = set(sparrow.get_all("Role", pluck="name"))
...
def setup_tree_doctype():
	sparrow.delete_doc_if_exists("DocType", "Custom Tree")
	...
	if not sparrow.db.exists("Custom Tree", "All Trees"):
		sparrow.get_doc({"doctype": "Custom Tree", "tree": "All Trees"}).insert()


@whitelist_for_tests
def setup_image_doctype():
	sparrow.delete_doc_if_exists("DocType", "Custom Image")
...
def setup_default_view(view, force_reroute=None):
	sparrow.delete_doc_if_exists("Property Setter", "Event-main-default_view")
	sparrow.delete_doc_if_exists("Property Setter", "Event-main-force_re_route_to_default_view")
...
@whitelist_for_tests
def create_note():
	if not sparrow.db.exists("Note", "Routing Test"):
		sparrow.get_doc({"doctype": "Note", "title": "Routing Test"}).insert()


@whitelist_for_tests
def create_kanban():
	if not sparrow.db.exists("Custom Field", "Note-kanban"):
		sparrow.get_doc(
			{
				"is_system_generated": 0,
				"dt": "Note",
	...
	if not sparrow.db.exists("Kanban Board", "_Note _Kanban"):
		sparrow.get_doc(
			{
...
def create_todo(description):
	sparrow.get_doc({"doctype": "ToDo", "description": description}).insert()
...
def create_todo_with_attachment_limit(description):
	...
	return sparrow.get_doc({"doctype": "ToDo", "description": description}).insert()


@whitelist_for_tests
def create_admin_kanban():
	if not sparrow.db.exists("Kanban Board", "Admin Kanban"):
		sparrow.get_doc(
			{
				"doctype": "Kanban Board",
				"name": "Admin Kanban",
