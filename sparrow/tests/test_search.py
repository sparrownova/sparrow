...
class TestSearch(FrappeTestCase):
	...
	def test_search_field_sanitizer(self):
		...
		result = sparrow.response["results"][0]
	...
	def test_link_field_order(self):
		...
		result = sparrow.response["results"]
	...
	def test_link_search_in_foreign_language(self):
		try:
			...
			output = sparrow.response["values"]
	...
	def test_validate_and_sanitize_search_inputs(self):
		...
		# should not fail if function is called via sparrow.call with extra arguments
		args = ("Random DocType", "Random", "email", "2", "10", dict())
		kwargs = {"as_dict": False}
		self.assertListEqual(sparrow.call("sparrow.tests.test_search.get_data", *args, **kwargs), [])

		# should not fail if query has @ symbol in it
		search_link("User", "user@random", searchfield="name")
		self.assertListEqual(sparrow.response["results"], [])
	...
	def test_reference_doctype(self):
		...
		search_link(
			...
			query="sparrow.tests.test_search.query_with_reference_doctype",
		)
		self.assertListEqual(sparrow.response["results"], [])
...
def setup_test_link_field_order(TestCase):
	...
	# Create Tree doctype
	if not sparrow.db.exists("DocType", TestCase.tree_doctype_name):
		TestCase.tree_doc = sparrow.get_doc(
			{
				"doctype": "DocType",
		...
		TestCase.tree_doc = sparrow.get_doc("DocType", TestCase.tree_doctype_name)

	# Create root for the tree doctype
	if not sparrow.db.exists(TestCase.tree_doctype_name, {"random": TestCase.parent_doctype_name}):
		sparrow.get_doc(
