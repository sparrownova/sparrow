	...
	def test_get_user_pages_or_reports_with_permission_query(self):
		...
		sparrow.get_doc(
			dict(
				...
				script="""conditions = f"(`tabReport`.is_standard = 'Yes' or `tabReport`.owner = '{sparrow.session.user}')"
				""",
			)
		).insert()
		...
		allowed_reports = sparrow.cache().get_value("has_role:Report", user=sparrow.session.user)
