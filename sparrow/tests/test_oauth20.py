...
import sparrow
from sparrow.integrations.oauth2 import encode_params
...
class TestOAuth20(unittest.TestCase):
	def setUp(self):
		...
		client = sparrow.get_all("OAuth Client", fields=["*"])[0]
	...
	def test_login_using_authorization_code(self):
		...
		try:
			session.get(
				get_full_url("/api/method/sparrow.integrations.oauth2.authorize"),
		...
		token_response = requests.post(
			get_full_url("/api/method/sparrow.integrations.oauth2.get_token"),
	...
	def test_revoke_token(self):
		client = sparrow.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = requests.post(
			get_full_url("/api/method/sparrow.integrations.oauth2.revoke_token"),
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = sparrow.get_doc("OAuth Client", self.client_id)
...
def check_valid_openid_response(access_token=None):
	...
	openid_response = requests.get(
		get_full_url("/api/method/sparrow.integrations.oauth2.openid_profile"), headers=headers
	)
...
def get_full_url(endpoint):
	...
	return urljoin(sparrow.utils.get_url(), endpoint)


def update_client_for_auth_code_grant(client_id):
	client = sparrow.get_doc("OAuth Client", client_id)
	client.grant_type = "Authorization Code"
	client.response_type = "Code"
	client.save()
	sparrow.db.commit()
