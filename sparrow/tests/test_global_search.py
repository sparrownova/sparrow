...
class TestGlobalSearch(FrappeTestCase):
	def setUp(self):
		...
		self.assertTrue("__global_search" in sparrow.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		sparrow.db.delete("Property Setter", {"doc_type": "Event"})
	...
	def test_update_doc(self):
		...
		event = sparrow.get_doc("Event", sparrow.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		self.insert_test_events()
		event_name = sparrow.get_all("Event")[0].name
		event = sparrow.get_doc("Event", event_name)
		...
		sparrow.delete_doc("Event", event_name)
		global_search.sync_global_search()
		sparrow.db.commit()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = sparrow.get_doc({"doctype": "Event", "subject": text, "starts_on": now_datetime()})
			doc.insert()
