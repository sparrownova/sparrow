...
import sparrow
from sparrow.query_builder import Case
from sparrow.query_builder.builder import Function
from sparrow.query_builder.custom import ConstantColumn
from sparrow.query_builder.functions import (
	Cast_,
	Coalesce,
	CombineDatetime,
	Date,
...
def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(sparrow.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")
...
class TestCustomFunctionsMariaDB(SparrowTestCase):
	...
	def test_constant_column(self):
		query = sparrow.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
		self.assertEqual(query.get_sql(), "SELECT `name`,'John' `User` FROM `tabDocType`")

	def test_timestamp(self):
		note = sparrow.qb.DocType("Note")
		self.assertEqual(
	...
	def test_time(self):
		...
		select_query = sparrow.qb.from_(note).select(
			CombineDatetime(note.posting_date, note.posting_time)
		)
	...
	def test_cast(self):
		...
		self.assertEqual(
			sparrow.qb.from_("red").from_(note).select("other", Cast_(note.name, "varchar")).get_sql(),
	...
	def test_round(self):
		...
		query = sparrow.qb.from_(note).select(Round(note.price))
		self.assertEqual("select round(`price`,0) from `tabnote`", str(query).lower())

		query = sparrow.qb.from_(note).select(Round(note.price, 3))
		self.assertEqual("select round(`price`,3) from `tabnote`", str(query).lower())

	def test_truncate(self):
		note = sparrow.qb.DocType("Note")
		query = sparrow.qb.from_(note).select(Truncate(note.price, 3))
...
class TestBuilderBase:
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", sparrow.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", sparrow.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", sparrow.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = sparrow.qb.from_("ToDo").select("*").limit(1)
	...
	def test_agg_funcs(self):
		sparrow.db.truncate("Communication")
		sample_data = {
		...
		sparrow.get_doc(sample_data).insert()
		sample_data["rating"] = 3
		sparrow.get_doc(sample_data).insert()
		sample_data["rating"] = 4
		sparrow.get_doc(sample_data).insert()
		self.assertEqual(sparrow.qb.max("Communication", "rating"), 4)
		self.assertEqual(sparrow.qb.min("Communication", "rating"), 1)
		self.assertAlmostEqual(sparrow.qb.avg("Communication", "rating"), 2.666, places=2)
		self.assertEqual(sparrow.qb.sum("Communication", "rating"), 8.0)
		sparrow.db.rollback()
...
class TestParameterization(SparrowTestCase):
	...
	def test_set_conditions(self):
		DocType = sparrow.qb.DocType("DocType")
		query = sparrow.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_case(self):
		DocType = sparrow.qb.DocType("DocType")
		query = sparrow.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = sparrow.qb.DocType("DocType")
		query = sparrow.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
	...
	def test_named_parameter_wrapper(self):
		from sparrow.query_builder.terms import NamedParameterWrapper
...
class TestBuilderMaria(SparrowTestCase, TestBuilderBase):
	...
	def test_get_qb_type(self):
		from sparrow.query_builder import get_query_builder

		qb = get_query_builder(sparrow.db.db_type)
...
class TestBuilderPostgres(SparrowTestCase, TestBuilderBase):
	...
	def test_replace_tables(self):
		info_schema = sparrow.qb.Schema("information_schema")
		self.assertEqual(
			'SELECT * FROM "pg_stat_all_tables"',
			sparrow.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", sparrow.qb.Field("table_name").get_sql())
...
class TestMisc(SparrowTestCase):
	...
	def test_function_with_schema(self):
		from sparrow.query_builder import ParameterizedFunction

		x = ParameterizedFunction("rand", "45")
		x.schema = sparrow.qb.DocType("DocType")
		self.assertEqual("tabDocType.rand('45')", x.get_sql())

	def test_util_table(self):
		from sparrow.query_builder.utils import Table
