...
class TestPassword(FrappeTestCase):
	...
	def make_email_account(self, name="Test Email Account Password"):
		if not sparrow.db.exists("Email Account", name):
			...
			return sparrow.get_doc("Email Account", name)
	...
	def test_hashed_password(self, user="test@example.com"):
		...
		self.assertRaises(sparrow.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		sparrow.rename_doc(doc.doctype, old_name, new_name)

		new_doc = sparrow.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not get_password_list(doc))

		sparrow.rename_doc(doc.doctype, new_name, old_name)
