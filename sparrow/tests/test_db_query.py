...
from sparrow.database.utils import DefaultOrderBy
from sparrow.desk.reportview import get_filters_cond
...
def enable_permlevel_restrictions():
	sparrow.db.set_single_value("System Settings", "apply_perm_level_on_api_calls", 1)
	yield
...
class TestReportview(SparrowTestCase):
	...
	def test_child_table_field_syntax(self):
		note = sparrow.get_doc(
			doctype="Note",
			title=f"Test {sparrow.utils.random_string(8)}",
			content="test",
	...
	def test_child_table_join(self):
		sparrow.delete_doc_if_exists("DocType", "Parent DocType 1")
		sparrow.delete_doc_if_exists("DocType", "Parent DocType 2")
		sparrow.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		sparrow.get_doc(
			{
				"doctype": "DocType",
		...
		# clear records
		sparrow.db.delete("Parent DocType 1")
		sparrow.db.delete("Parent DocType 2")
		sparrow.db.delete("Child DocType")
		...
		# test query
		results1 = sparrow.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = sparrow.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_build_match_conditions(self):
		...
		test2user = sparrow.get_doc("User", "test2@example.com")
	...
	def test_none_filter(self):
		query = sparrow.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		sparrow.set_user("test2@example.com")
		self.assertRaises(sparrow.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(sparrow.DataError):
			DatabaseQuery("DocType").execute(
				fields=["name", "issingle", "if (issingle=1, (select name from tabUser), count(name))"],
				limit_start=0,
				limit_page_length=1,
	...
	def test_nested_permission(self):
		...
		from sparrow.core.page.permission_manager.permission_manager import update
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = sparrow.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = sparrow.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = sparrow.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = sparrow.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = sparrow.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = sparrow.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = sparrow.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = sparrow.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = sparrow.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(sparrow.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(sparrow.get_all("Nested DocType"))
		)
	...
	def test_is_set_is_not_set(self):
		...
		sparrow.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_prepare_select_args(self):
		# sparrow.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = sparrow.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = sparrow.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			response = execute_cmd("sparrow.desk.reportview.get")
	...
	def test_cast_name(self):
		...
		sparrow.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		sparrow.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		sparrow.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = sparrow.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from sparrow.desk.doctype.dashboard_settings.dashboard_settings import create_dashboard_settings
		...
		dashboard_settings = sparrow.db.sql(
			"""
				SELECT name
	...
	def test_virtual_doctype(self):
		...
		with patch("sparrow.controllers", new={sparrow.local.site: {"Virtual DocType": VirtualDocType}}):
			VirtualDocType.get_list = MagicMock()

			sparrow.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)
	...
	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", sparrow.get_all("User", {"name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", sparrow.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", sparrow.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", sparrow.get_all("User", {"name": ("in", [])}, run=0))
		self.assertIn("ifnull", sparrow.get_all("User", {"name": ("not in", ["a"])}, run=0))
...
class TestReportView(SparrowTestCase):
	...
	def test_get_count(self):
		...
		list_filter_response = execute_cmd("sparrow.desk.reportview.get_count")
		sparrow.local.form_dict = sparrow._dict(
			{"doctype": "DocType", "filters": {"show_title_field_in_link": 1}, "distinct": "true"}
		)
		dict_filter_response = execute_cmd("sparrow.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("sparrow.desk.reportview.get_count")
		current_value = sparrow.db.sql(
	...
	def test_reportview_get_aggregation(self):
		...
		sparrow.local.request = sparrow._dict(method="GET")
		sparrow.local.form_dict = sparrow._dict(
			{
				"doctype": "DocType",
...
def create_nested_doctype():
	if sparrow.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = sparrow.new_doc("Nested DocType")
		d.update(r)
