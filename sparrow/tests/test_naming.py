...
from sparrow.model.naming import (
...
class TestNaming(sparrowTestCase):
	...
	def test_field_autoname_name_sync(self):
		...
		country = sparrow.get_last_doc("Country")
	...
	def test_child_table_naming(self):
		...
		name = sparrow.generate_hash(length=10)

		doc = sparrow.new_doc(dt_with_child_autoname.name)
	...
	def test_format_autoname(self):
		...
		doc = sparrow.new_doc(doctype.name)
	...
	def test_format_autoname_for_consecutive_week_number(self):
		...
		todo_doctype = sparrow.get_doc("DocType", doctype)
		todo_doctype.autoname = "format:TODO-{WW}-{##}"
		todo_doctype.save()

		description = "Format"

		todo = sparrow.new_doc(doctype)
	...
	def test_revert_series(self):
		...
		sparrow.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 1)""", (series,))
		revert_series_if_last(key, name)
		current_index = sparrow.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]

		self.assertEqual(current_index.get("current"), 0)
		sparrow.db.delete("Series", {"name": series})

		series = f"TEST-{year}-"
		key = "TEST-.YYYY.-.#####"
		name = f"TEST-{year}-00002"
		sparrow.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 2)""", (series,))
		revert_series_if_last(key, name)
		current_index = sparrow.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
		...
		name = "TEST-00003"
		sparrow.db.delete("Series", {"name": series})
		sparrow.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 3)""", (series,))
		revert_series_if_last(key, name)
		current_index = sparrow.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
	...
	def test_naming_for_cancelled_and_amended_doc(self):
		submittable_doctype = sparrow.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		doc = sparrow.new_doc("Submittable Doctype")
		doc.save()
		...
		amended_doc = sparrow.copy_doc(doc)
	...
	def test_naming_validations(self):
		...
		# set name via prompt
		tag = sparrow.get_doc({"doctype": "Tag", "__newname": "Tag"})
		self.assertRaises(sparrow.NameError, tag.insert)

		# set by passing set_name as ToDo
		self.assertRaises(sparrow.NameError, make_invalid_todo)

		# set new name - Note
		note = sparrow.get_doc({"doctype": "Note", "title": "Note"})
		self.assertRaises(sparrow.NameError, note.insert)

		# case 2: set name with "New ---"
		tag = sparrow.get_doc({"doctype": "Tag", "__newname": "New Tag"})
		self.assertRaises(sparrow.NameError, tag.insert)
		...
		tag = sparrow.get_doc({"doctype": "Tag", "__newname": ""})
		self.assertRaises(sparrow.ValidationError, tag.insert)

	def test_autoincremented_naming(self):
		from sparrow.core.doctype.doctype.test_doctype import new_doctype

		doctype = "autoinc_doctype" + sparrow.generate_hash(length=5)
		dt = new_doctype(doctype, autoname="autoincrement").insert(ignore_permissions=True)

		for i in range(1, 20):
			self.assertEqual(sparrow.new_doc(doctype).save(ignore_permissions=True).name, i)
	...
	def test_naming_series_validation(self):
		dns = sparrow.get_doc("Document Naming Settings")
	...
	def test_naming_using_fields(self):
		...
		webhook = sparrow.new_doc("Webhook")
		...
		with patch_hooks(
			{
				"naming_series_variables": {
					"PM": ["sparrow.tests.test_naming.parse_naming_series_variable"],
				},
			},
		):
...
def make_invalid_todo():
	sparrow.get_doc({"doctype": "ToDo", "description": "Test"}).insert(set_name="ToDo")
