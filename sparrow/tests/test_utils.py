...
from sparrow.utils.dateutils import get_dates_from_timegrain
from sparrow.utils.diff import _get_value_from_version, get_version_diff, version_query
	...
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/sparrow/test.jpg">
			</div>
			<div style="background-image: url('/assets/sparrow/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/sparrow/test.jpg">' in html)
		self.assertTrue(
			f"style=\"background-image: url('{url}/assets/sparrow/bg.jpg') !important\"" in html
		)
...
class TestValidationUtils(SparrowTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://sparrow.io", throw=True))
		...
		self.assertRaises(sparrow.ValidationError, validate_url, "sparrow", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://sparrow.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(
			validate_url("bolo://sparrow.io", valid_schemes=("http", "https", "ftp", "ftps"))
		)
		self.assertRaises(
			sparrow.ValidationError, validate_url, "gopher://sparrow.io", valid_schemes="https", throw=True
		)
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@sparrow.com"))
		self.assertTrue(validate_email_address("someone@sparrow.com, anyone@sparrow.io"))
		...
		# Invalid with throw
		self.assertRaises(
			sparrow.InvalidEmailAddressError, validate_email_address, "someone.com", throw=True
		)
...
class TestImage(SparrowTestCase):
	def test_strip_exif_data(self):
		...
		original_image_content = open(
			"../apps/sparrow/sparrow/tests/data/exif_sample_image.jpg", mode="rb"
		).read()
	...
	def test_optimize_image(self):
		image_file_path = "../apps/sparrow/sparrow/tests/data/sample_image_for_optimization.jpg"
...
class TestDiffUtils(SparrowTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = sparrow.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(SparrowTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(sparrow.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				sparrow.utils.get_first_day_of_week("2020-12-25"), sparrow.utils.getdate("2020-12-21")
			)
			self.assertEqual(
				sparrow.utils.get_first_day_of_week("2020-12-20"), sparrow.utils.getdate("2020-12-14")
			)

		# Sunday as start of the week
		self.assertEqual(
			sparrow.utils.get_first_day_of_week("2020-12-25"), sparrow.utils.getdate("2020-12-20")
		)
		self.assertEqual(
			sparrow.utils.get_first_day_of_week("2020-12-21"), sparrow.utils.getdate("2020-12-20")
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			sparrow.utils.get_last_day_of_week("2020-12-24"), sparrow.utils.getdate("2020-12-26")
		)
		self.assertEqual(
			sparrow.utils.get_last_day_of_week("2020-12-28"), sparrow.utils.getdate("2021-01-02")
		)
...
class TestResponse(SparrowTestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				sparrow.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
...
class TestLinkTitle(SparrowTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		"""
		custom_doctype = sparrow.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		from sparrow.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = sparrow.local.response["_link_titles"]
...
class TestAppParser(SparrowTestCase):
	def test_app_name_parser(self):
		...
		sparrow_app = os.path.join(snova_path, "apps", "sparrow")
...
class TestIntrospectionMagic(SparrowTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(sparrow.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(sparrow.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(sparrow.get_newargs(f, args), {"b": 1})
...
class TestMiscUtils(SparrowTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(sparrow.local.site, get_sites())
...
class TestTBSanitization(SparrowTestCase):
	def test_traceback_sanitzation(self):
		try:
			...
			args = sparrow._dict({"password": "42", "pwd": "42", "safe": "safe_value"})
			raise Exception
		except Exception:
			traceback = sparrow.get_traceback(with_context=True)
