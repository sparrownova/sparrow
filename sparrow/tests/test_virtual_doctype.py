...
class VirtualDoctypeTest(Document):
	...
	def get_list(args):
		...
		return [sparrow._dict(doc) for name, doc in data.items()]
...
class TestVirtualDoctypes(SparrowTestCase):
	...
	def setUpClass(cls):
		...
		cls.addClassCleanup(sparrow.flags.pop, "allow_doctype_export", None)

		vdt = new_doctype(name=TEST_DOCTYPE_NAME, is_virtual=1, custom=0).insert()
		cls.addClassCleanup(vdt.delete)

		patch_virtual_doc = patch(
			"sparrow.controllers", new={sparrow.local.site: {TEST_DOCTYPE_NAME: VirtualDoctypeTest}}
		)
	...
	def test_insert_update_and_load_from_desk(self):
		...
		docname = sparrow.response.docs[0]["name"]

		doc = sparrow.get_doc(TEST_DOCTYPE_NAME, docname)
	...
	def test_multiple_doc_insert_and_get_list(self):
		doc1 = sparrow.get_doc(doctype=TEST_DOCTYPE_NAME, some_fieldname="first").insert()
		doc2 = sparrow.get_doc(doctype=TEST_DOCTYPE_NAME, some_fieldname="second").insert()
	...
	def test_delete_doc(self):
		doc = sparrow.get_doc(doctype=TEST_DOCTYPE_NAME, some_fieldname="data").insert()

		sparrow.delete_doc(doc.doctype, doc.name)
