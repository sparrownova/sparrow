...
class TestCORS(FrappeTestCase):
	def make_request_and_test(self, origin="http://example.com", absent=False):
		...
		sparrow.utils.set_request(method="OPTIONS", headers=headers)
	...
	def test_valid_origin(self):
		...
		sparrow.conf.allow_cors = "*"
		self.make_request_and_test()

		sparrow.conf.allow_cors = ["http://example.com", "https://example.com"]
		self.make_request_and_test()

	def test_invalid_origin(self):
		sparrow.conf.allow_cors = "http://example1.com"
		self.make_request_and_test(absent=True)

		sparrow.conf.allow_cors = ["http://example1.com", "https://example.com"]
		self.make_request_and_test(absent=True)
