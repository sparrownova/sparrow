...
class TestQueryReport(sparrowTestCase):
	...
	def test_report_for_duplicate_column_names(self):
		...
		try:
			...
			for i in range(1, 3):
				sparrow.get_doc({"doctype": "Doc A", "first_name": f"John{i}", "last_name": "Doe"}).insert()
				sparrow.get_doc({"doctype": "Doc B", "last_name": f"Doe{i}", "first_name": "John"}).insert()

			if not sparrow.db.exists("Report", "Doc A Report"):
				...
				report = sparrow.get_doc("Report", "Doc A Report")
