...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("sparrow.app.get_site_name", return_value=_site):
				header_patch = patch("sparrow.get_request_header", new=patch_request_header)
...
class TestResourceAPI(SparrowAPITestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(10):
			doc = sparrow.get_doc({"doctype": "ToDo", "description": sparrow.mock("paragraph")}).insert()
	...
	def tearDownClass(cls):
		for name in cls.GENERATED_DOCUMENTS:
			sparrow.delete_doc_if_exists(cls.DOCTYPE, name)
		sparrow.db.commit()
	...
	def test_get_list_dict(self):
		...
		json = sparrow._dict(response.json)
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		...
		data = {"description": sparrow.mock("paragraph"), "sid": self.sid}
	...
	def test_update_document(self):
		...
		generated_desc = sparrow.mock("paragraph")
		data = {"description": generated_desc, "sid": self.sid}
		random_doc = choice(self.GENERATED_DOCUMENTS)
		desc_before_update = sparrow.db.get_value(self.DOCTYPE, random_doc, "description")
	...
	def test_delete_document(self):
		...
		non_existent_doc = sparrow.generate_hash(length=12)
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "sparrow.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(SparrowAPITestCase):
	...
	def test_version(self):
		...
		self.assertEqual(Version(json.message), Version(sparrow.__version__))
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/sparrow.realtime.get_user_info
		response = self.get(f"{self.METHOD_PATH}/sparrow.realtime.get_user_info")
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get("/api/method/sparrow.auth.get_logged_user")
...
class TestReadOnlyMode(SparrowAPITestCase):
	...
	def test_blocked_writes(self):
		response = self.post(self.REQ_PATH, {"description": sparrow.mock("paragraph"), "sid": self.sid})
...
class TestWSGIApp(SparrowAPITestCase):
	def test_request_hooks(self):
		...
		with patch_hooks(
			{
				"before_request": ["sparrow.tests.test_api.before_request"],
				"after_request": ["sparrow.tests.test_api.after_request"],
			}
		):
