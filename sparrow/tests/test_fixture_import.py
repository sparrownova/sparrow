...
class TestFixtureImport(SparrowTestCase):
	...
	def insert_dummy_data_and_export(self, DocType: str, dummy_name_list: list[str]) -> str:
		for name in dummy_name_list:
			doc = sparrow.get_doc({"doctype": DocType, "member_name": name})
			doc.insert()
	...
	def test_fixtures_import(self):
		self.assertFalse(sparrow.db.exists("DocType", "temp_doctype"))
		...
		sparrow.db.truncate("temp_doctype")
		...
		self.assertEqual(sparrow.db.count("temp_doctype"), len(dummy_name_list))

		data = sparrow.get_all("temp_doctype", "member_name")
		sparrow.db.truncate("temp_doctype")
	...
	def test_singles_fixtures_import(self):
		self.assertFalse(sparrow.db.exists("DocType", "temp_singles"))
		...
		singles_doctype = sparrow.qb.DocType("Singles")
		truncate_query = (
			sparrow.qb.from_(singles_doctype).delete().where(singles_doctype.doctype == "temp_singles")
		)
		truncate_query.run()
		...
		data = sparrow.db.get_single_value("temp_singles", "member_name")
		truncate_query.run()
