...
class TestDBUpdate(SparrowTestCase):
	def test_db_update(self):
		...
		sparrow.model.meta.trim_tables("User")
		...
		table_columns = sparrow.db.get_table_columns_description(f"tab{doctype}")
		...
		for field_def in field_defs:
			...
			fallback_default = (
				"0" if field_def.get("fieldtype") in sparrow.model.numeric_fieldtypes else "NULL"
			)
	...
	def test_index_and_unique_constraints(self):
		...
		sparrow.db.add_index(doctype, ["email_signature(200)"])
		sparrow.db.updatedb(doctype)
	...
	def test_unique_index_on_install(self):
		...
		for dt in sparrow.get_all("DocType", {"is_virtual": 0, "issingle": 0}, pluck="name"):
			doctype = sparrow.get_meta(dt)
...
def get_fieldtype_from_def(field_def):
	fieldtuple = sparrow.db.type_map.get(field_def.fieldtype, ("", 0))
...
def get_other_fields_meta(meta):
	...
	optional_fields = sparrow.db.OPTIONAL_COLUMNS
	if meta.track_seen:
		optional_fields.append("_seen")

	child_table_fields_map = {}
	if meta.istable:
		child_table_fields_map.update({field: ("Data", 0) for field in sparrow.db.CHILD_TABLE_COLUMNS})
	...
	field_map = [
		sparrow._dict({"fieldname": field, "fieldtype": _type, "length": _length})
