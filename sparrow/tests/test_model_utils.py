...
class TestModelUtils(sparrowTestCase):
	def test_get_fetch_values(self):
		...
		full_name = sparrow.db.get_value("User", user, "full_name")
	...
	def test_get_permitted_fields(self):
		...
		todo_all_columns = sparrow.get_meta("ToDo").get_valid_columns()
		...
		with set_user("Guest"):
			...
			picked_doctype_all_columns = sparrow.get_meta(picked_doctype).get_valid_columns()
		...
		with set_user("Administrator"):
			...
			child_all_fields = sparrow.get_meta("Installed Application").get_valid_columns()
...
def set_user(user: str):
	past_user = sparrow.session.user or "Administrator"
	sparrow.set_user(user)
	yield
	sparrow.set_user(past_user)
