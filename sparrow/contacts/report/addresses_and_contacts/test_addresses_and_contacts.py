...
def get_custom_linked_doctype():
	if bool(sparrow.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if sparrow.flags.test_address_created:
		return

	address = sparrow.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	sparrow.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if sparrow.flags.test_contact_created:
		return
