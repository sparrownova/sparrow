...
from sparrow.utils import cstr, has_gravatar
...
class Contact(Document):
	def autoname(self):
		...
		if sparrow.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			sparrow.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = sparrow.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not sparrow.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not sparrow.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			sparrow.throw(_("Only one {0} can be set as primary.").format(sparrow.bold("Email ID")))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			sparrow.throw(
				_("Only one {0} can be set as primary.").format(sparrow.bold(sparrow.unscrub(fieldname)))
			)
...
def invite_user(contact):
	contact = sparrow.get_doc("Contact", contact)

	if not contact.email_id:
		sparrow.throw(_("Please set Email Address"))
...
def update_contact(doc, method):
	...
	contact_name = sparrow.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	...
	doctype = "Contact"
	if (
		not sparrow.get_meta(doctype).get_field(searchfield)
		and searchfield not in sparrow.db.DEFAULT_COLUMNS
	):
		return []
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = sparrow.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = sparrow.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return sparrow.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
