...
import sparrow
from sparrow.automation.doctype.auto_repeat.auto_repeat import (
...
class TestAutoRepeat(SparrowTestCase):
	...
	def test_daily_auto_repeat(self):
		...
		todo = sparrow.get_doc(doc.reference_doctype, doc.reference_document)
		self.assertEqual(todo.auto_repeat, doc.name)

		new_todo = sparrow.db.get_value(
			"ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name"
		)

		new_todo = sparrow.get_doc("ToDo", new_todo)
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date=None):
		...
		data = get_auto_repeat_entries(getdate(today()))
		create_repeated_entries(data)
		docnames = sparrow.get_all(doc.reference_doctype, {"auto_repeat": doc.name})
		self.assertEqual(len(docnames), 1)

		doc = sparrow.get_doc("Auto Repeat", doc.name)
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = sparrow.get_doc(dict(doctype=doctype, test="test submit on creation")).insert()
	...
	doc = sparrow.get_doc(
		{
			...
			"reference_document": args.reference_document or sparrow.db.get_value("ToDo", "name"),
...
def create_submittable_doctype(doctype, submit_perms=1):
	if sparrow.db.exists("DocType", doctype):
		return
	else:
		doc = sparrow.get_doc(
			{
