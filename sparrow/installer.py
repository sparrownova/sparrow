...
import sparrow
from sparrow.defaults import _clear_cache
from sparrow.utils import cint, is_git_url
...
def _is_scheduler_enabled() -> bool:
	...
	try:
		sparrow.connect()
		enable_scheduler = cint(sparrow.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		sparrow.db.close()
...
def _new_site(
	...
	"""Install a new Sparrow site"""

	from sparrow.utils import get_site_path, scheduler, touch_file
	...
	if not db_name:
		...
		db_name = "_" + hashlib.sha1(os.path.realpath(sparrow.get_site_path()).encode()).hexdigest()[:16]
	...
	scheduler_status = "disabled" if sparrow.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from sparrow.database import setup_database

	if not db_type:
		db_type = sparrow.conf.db_type or "mariadb"
	...
	sparrow.flags.in_install_db = True

	sparrow.flags.root_login = root_login
	sparrow.flags.root_password = root_password
	setup_database(force, source_sql, verbose, no_mariadb_socket)

	sparrow.conf.admin_password = sparrow.conf.admin_password or admin_password

	remove_missing_apps()

	sparrow.db.create_auth_table()
	sparrow.db.create_global_search_table()
	sparrow.db.create_user_settings_table()

	sparrow.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	from sparrow.exceptions import InvalidRemoteException
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from sparrow.model.sync import sync_for
	from sparrow.modules.utils import sync_customizations
	from sparrow.utils.fixtures import sync_fixtures

	sparrow.flags.in_install = name
	sparrow.flags.ignore_in_install = False

	sparrow.clear_cache()
	app_hooks = sparrow.get_hooks(app_name=name)
	installed_apps = sparrow.get_installed_apps()
	...
	if name not in sparrow.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = sparrow.get_attr(before_install)()
		if out is False:
			return

	for fn in sparrow.get_hooks("before_app_install"):
		sparrow.get_attr(fn)(name)
	...
	sparrow.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		sparrow.get_attr(after_install)()

	for fn in sparrow.get_hooks("after_app_install"):
		sparrow.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		sparrow.get_attr(after_sync)()  #

	sparrow.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		sparrow.db.set_global("installed_apps", json.dumps(installed_apps))
		sparrow.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = sparrow.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in sparrow.get_installed_apps():
	...
	if not (dry_run or no_backup):
		from sparrow.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	sparrow.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		sparrow.get_attr(before_uninstall)()

	for fn in sparrow.get_hooks("before_app_uninstall"):
		sparrow.get_attr(fn)(app_name)

	modules = sparrow.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		sparrow.get_attr(after_uninstall)()

	for fn in sparrow.get_hooks("after_app_uninstall"):
		sparrow.get_attr(fn)(app_name)

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	sparrow.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in sparrow.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					sparrow.delete_doc("DocType", doctype.name, ignore_on_trash=True)
				else:
		...
		if not dry_run:
			sparrow.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in sparrow.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				sparrow.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = sparrow.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if sparrow.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			sparrow.delete_doc("DocType", doctype, ignore_on_trash=True)
			sparrow.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from sparrow.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		sparrow.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	sparrow.db.commit()


def init_singles():
	singles = sparrow.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if sparrow.db.get_singles_dict(single):
			continue

		try:
			doc = sparrow.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, sparrow.DoesNotExistError):
...
def make_conf(
	...
	sites_path = sparrow.local.sites_path
	sparrow.destroy()
	sparrow.init(site, sites_path=sites_path)


def make_site_config(
	db_name=None, db_password=None, site_config=None, db_type=None, db_host=None, db_port=None
):
	sparrow.create_folder(os.path.join(sparrow.local.site_path))
...
def update_site_config(key, value, validate=True, site_config_path=None):
	...
	from sparrow.utils.synchronization import filelock
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(sparrow.local, "conf"):
		sparrow.local.conf[key] = value


def get_site_config_path():
	return os.path.join(sparrow.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = sparrow.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = sparrow.get_module_list(app)
	for module in modules:
		d = sparrow.new_doc("Module Def")
		d.app_name = app
...
def extract_files(site_name, file_path):
	...
	abs_site_path = os.path.abspath(sparrow.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	if sparrow.conf.db_type not in (None, "mariadb"):
		return False
	...
	with open(sql_file_path) as f:
		for line in f:
			if head in line:
				# 'line' (str) format: ('2056588823','2020-05-11 18:21:31.488367','2020-06-12 11:49:31.079506','Administrator','Administrator',0,'Installed Applications','installed_applications','Installed Applications',1,'sparrow','v10.1.71-74 (3c50d5e) (v10.x.x)','v10.x.x'),('855c640b8e','2020-05-11 18:21:31.488367','2020-06-12 11:49:31.079506','Administrator','Administrator',0,'Installed Applications','installed_applications','Installed Applications',2,'your_custom_app','0.0.1','master')
				line = line.strip().lstrip(head).rstrip(";").strip()
				app_rows = sparrow.safe_eval(line)
				...
				# 'all_apps' (list) format: [('sparrow', '12.x.x-develop ()', 'develop'), ('your_custom_app', '0.0.1', 'master')]
				...
				for app in all_apps:
					...
					if app_name == "sparrow":
						try:
							current_version = Version(sparrow.__version__)
						...
						if verbose and downgrade:
							print(f"Your site will be downgraded from Sparrow {backup_version} to {current_version}")
...
def partial_restore(sql_file_path, verbose=False):
	...
	if sparrow.conf.db_type in (None, "mariadb"):
		from sparrow.database.mariadb.setup_db import import_db_from_sql
	elif sparrow.conf.db_type == "postgres":
		import warnings

		from sparrow.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path, _raise=True):
	...
	if _raise and (missing_table or empty_file):
		raise sparrow.InvalidDatabaseFile
