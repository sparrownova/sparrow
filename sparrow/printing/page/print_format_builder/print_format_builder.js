sparrow.pages["print-format-builder"].on_page_load = function (wrapper) {
	sparrow.print_format_builder = new sparrow.PrintFormatBuilder(wrapper);
	sparrow.breadcrumbs.add("Setup", "Print Format");
};

sparrow.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = sparrow.get_route();
	if (route.length > 1) {
		sparrow.model.with_doc("Print Format", route[1], function () {
			sparrow.print_format_builder.print_format = sparrow.get_doc("Print Format", route[1]);
			sparrow.print_format_builder.refresh();
		});
	} else if (sparrow.route_options) {
		if (sparrow.route_options.make_new) {
			let { doctype, name, based_on, beta } = sparrow.route_options;
			sparrow.route_options = null;
			sparrow.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			sparrow.print_format_builder.print_format = sparrow.route_options.doc;
			sparrow.route_options = null;
			sparrow.print_format_builder.refresh();
		}
	}
...
sparrow.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = sparrow.ui.make_app_page({
			parent: this.parent,
		...
		this.page.main = $(
			'<div class="col-md-12 border print-format-builder-main sparrow-card"></div>'
	...
	show_start() {
		this.page.main.html(sparrow.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = sparrow.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			sparrow.model.with_doc("Print Format", name, function (doc) {
				sparrow.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = sparrow.ui.form.make_control({
		...
		this.name_input = sparrow.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				sparrow.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		sparrow.call({
			method: "sparrow.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
			...
			callback: (r) => {
				if (r.message) {
					...
					if (print_format.print_format_builder_beta) {
						sparrow.set_route("print-format-builder-beta", print_format.name);
					} else {
	...
	setup_print_format() {
		var me = this;
		sparrow.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = sparrow.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				sparrow.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(sparrow.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			sparrow.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						sparrow.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!in_list(sparrow.model.layout_fields, f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(sparrow.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						var field = sparrow.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = sparrow.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				sparrow.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = sparrow.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				sparrow.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		sparrow.call({
			...
			callback: function (r) {
				...
				sparrow.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
