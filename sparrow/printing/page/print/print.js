sparrow.pages["print"].on_page_load = function (wrapper) {
	sparrow.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new sparrow.ui.form.PrintView(wrapper);

	$(wrapper).bind("show", () => {
		const route = sparrow.get_route();
		const doctype = route[1];
		const docname = route.slice(2).join("/");
		if (!sparrow.route_options || !sparrow.route_options.frm) {
			sparrow.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = sparrow.get_doc(doctype, docname);
				sparrow.model.with_doctype(doctype, () => {
					frm.meta = sparrow.get_meta(route[1]);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = sparrow.route_options.frm.doctype
				? sparrow.route_options.frm
				: sparrow.route_options.frm.frm;
			sparrow.route_options.frm = null;
			print_view.show(print_view.frm);
		}
	});
};

sparrow.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${sparrow.render_template("print_skeleton_loading")}
		...
		this.print_settings = sparrow.model.get_doc(":Print Settings", "Print Settings");
		this.setup_menu();
		this.setup_toolbar();
		this.setup_sidebar();
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = sparrow.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (sparrow.model.can_create("Print Format")) {
	...
	set_breadcrumbs() {
		sparrow.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		sparrow
			.xcall("sparrow.printing.page.print.print.get_print_settings_to_show", {
	...
	edit_print_format() {
		...
		if (is_standard_but_editable) {
			sparrow.set_route("Form", "Print Format", print_format.name);
			return;
		}
		if (is_custom_format) {
			if (print_format.print_format_builder_beta) {
				...
				sparrow.set_route("print-format-builder", print_format.name);
			}
			return;
		}
		...
		sparrow.prompt(
			...
			(data) => {
				...
				sparrow.set_route("print-format-builder");
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find(".print-toolbar a.btn-default").each((i, el) => {
			sparrow.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	}
	...
	get_language_options() {
		return sparrow.get_languages();
	}

	set_default_print_language() {
		let print_format = this.get_print_format();
		this.lang_code =
			this.frm.doc.language || print_format.default_print_language || sparrow.boot.lang;
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = sparrow.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = sparrow.urllib.get_base_url();
		let print_css = sparrow.assets.bundled_asset(
			"print.bundle.css",
			sparrow.utils.is_rtl(this.lang_code)
		);
		this.$print_format_body
			.find("html")
			.attr("dir", sparrow.utils.is_rtl(this.lang_code) ? "rtl" : "ltr");
	...
	go_to_form_view() {
		...
		sparrow.set_route("Form", this.frm.doctype, this.frm.docname);
	}
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function (out) {
					sparrow.ui.form
						...
						.then(sparrow.ui.form.qz_success)
						.catch((err) => {
	...
	print_by_server() {
		...
		if (localStorage.getItem("network_printer")) {
			sparrow.call({
				method: "sparrow.utils.print_format.print_by_server",
				args: {
	...
	network_printer_setting_dialog(callback) {
		sparrow.call({
			method: "sparrow.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings",
			callback: function (r) {
				if (r.message) {
	...
	render_pdf() {
		...
		if (print_format.print_format_builder_beta) {
			...
			let w = window.open(`/api/method/sparrow.utils.weasyprint.download_pdf?${params}`);
			if (!w) {
				sparrow.msgprint(__("Please enable pop-ups"));
				return;
			}
		} else {
			this.render_page("/api/method/sparrow.utils.print_format.download_pdf?");
		}
	}
	...
	get_print_html(callback) {
		...
		this._req = sparrow.call({
			method: "sparrow.www.printview.get_html_and_style",
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		sparrow.call({
			method: "sparrow.www.printview.get_rendered_raw_commands",
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	refresh_print_options() {
		this.print_formats = sparrow.meta.get_print_formats(this.frm.doctype);
	...
	set_style(style) {
		sparrow.dom.set_style(style || sparrow.boot.print_css, "print-style");
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		sparrow.ui.form.qz_get_printer_list().then((data) => {
			...
			if (!(this.printer_list && this.printer_list.length)) {
				sparrow.throw(__("No Printer is Available."));
			}
		});
	}
};
