...
export default {
	...
	mounted() {
		if (!this.letterhead && sparrow.boot.sysdefaults.letter_head) {
			this.set_letterhead(sparrow.boot.sysdefaults.letter_head);
		}
	...
	methods: {
		toggle_edit_letterhead() {
			...
			if (!this.control) {
				this.control = sparrow.ui.form.make_control({
