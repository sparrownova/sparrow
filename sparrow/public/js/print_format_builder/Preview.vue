...
export default {
	...
	mounted() {
		this.doc_select = sparrow.ui.form.make_control({
		...
		this.preview_type = sparrow.ui.form.make_control({
	...
	methods: {
		...
		get_default_docname() {
			return sparrow.db.get_list(this.doctype, { limit: 1 }).then(doc => {
	...
	computed: {
		...
		url() {
			...
			let url =
				this.type == "PDF"
					? `/api/method/sparrow.utils.weasyprint.download_pdf`
					: "/printpreview";
