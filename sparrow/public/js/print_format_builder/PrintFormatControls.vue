...
export default {
	...
	mounted() {
		let method =
			"sparrow.printing.page.print_format_builder_beta.print_format_builder_beta.get_google_fonts";
	...
	methods: {
		...
		clone_field(df) {
			...
			if (cloned.custom) {
				...
				cloned.fieldname += "_" + sparrow.utils.get_random(8);
			}
			return cloned;
		}
	},
		...
		print_templates() {
			...
			for (let template of templates) {
				...
				if (template.field) {
					...
					df = {
						label: template.name,
						fieldname: sparrow.scrub(template.name)
					};
				}
				out.push({
...
.control-font :deep(.sparrow-control[data-fieldname="font"] label) {
	display: none;
}
</style>
