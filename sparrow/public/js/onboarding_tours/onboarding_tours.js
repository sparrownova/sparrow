sparrow.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new sparrow.Driver({
			className: "sparrow-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				sparrow.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!sparrow.boot.user.onboarding_status[this.tour.name] &&
								(sparrow.boot.user.onboarding_status[this.tour.name] = {});
							sparrow.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								sparrow.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							sparrow.call({
								method: "sparrow.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(sparrow.boot.user.onboarding_status),
				...
				if ($input) sparrow.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await sparrow.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: sparrow.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
sparrow.ui.init_onboarding_tour = () => {
	...
	typeof sparrow.boot.onboarding_tours == "undefined" && sparrow.boot.onboarding_tours == [];
	typeof sparrow.boot.user.onboarding_status == "undefined" &&
		sparrow.boot.user.onboarding_status == {};
	let route = sparrow.router.current_route;
	if (route[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		sparrow.boot.onboarding_tours &&
			sparrow.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (sparrow.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (sparrow.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(sparrow.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == sparrow.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == sparrow.ui.next_form_tour);
	if (current_tour) {
		...
		sparrow.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (sparrow.ui.currentTourInstance?.driver) {
		sparrow.ui.currentTourInstance.driver_steps = [];
		sparrow.ui.currentTourInstance.driver.reset(true);
		sparrow.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (sparrow.ui.currentTourInstance = new sparrow.ui.OnboardingTour());
