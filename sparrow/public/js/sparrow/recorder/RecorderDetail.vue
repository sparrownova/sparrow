...
export default {
	...
	methods: {
		...
		refresh: function() {
			sparrow.call("sparrow.recorder.get").then( r => this.requests = r.message);
		},
		update: function(message) {
			this.requests.push(JSON.parse(message));
		},
		clear: function() {
			sparrow.call("sparrow.recorder.delete").then(r => this.refresh());
		},
		start: function() {
			sparrow.call("sparrow.recorder.start").then(r => this.fetch_status());
		},
		stop: function() {
			sparrow.call("sparrow.recorder.stop").then(r => this.fetch_status());
		},
		fetch_status: function() {
			sparrow.call("sparrow.recorder.status").then(r => this.update_status(r.message));
		},
		update_status: function(result) {
			...
			if(this.status.status == "Active") {
				sparrow.realtime.on("recorder-dump-event", this.update);
			} else {
				sparrow.realtime.off("recorder-dump-event", this.update);
			}
		...
		export_data: function() {
			...
			sparrow.call("sparrow.recorder.export_data")
				.then((r) => {
