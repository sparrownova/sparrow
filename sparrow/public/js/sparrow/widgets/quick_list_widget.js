...
export default class QuickListWidget extends Widget {
	...
	setup_filter(doctype) {
		...
		this.filters = sparrow.utils.process_filter_expression(this.quick_list_filter);
	...
	setup_quick_list_item(doc) {
		const indicator = sparrow.get_indicator(doc, this.document_type);
			...
			<div class="quick-list-item">
				<div class="ellipsis left">
					...
					<div class="timestamp text-muted">
						${sparrow.datetime.prettyDate(doc.modified)}
					</div>
				</div>
			</div>
		`);
		...
		$(`<div class="right-arrow">${sparrow.utils.icon("right", "xs")}</div>`).appendTo(
			$quick_list_item
		);
		...
		$quick_list_item.click((e) => {
			...
			sparrow.set_route(`${sparrow.utils.get_form_link(this.document_type, doc.name)}`);
		});
	...
	set_body() {
		...
		sparrow.model.with_doctype(this.document_type, () => {
			let fields = ["name"];

			// get name of title field
			if (!this.title_field_name) {
				let meta = sparrow.get_meta(this.document_type);
			...
			this.has_status_field = sparrow.meta.has_field(this.document_type, "status");
			...
			if (this.has_status_field) {
				...
				let workflow_fieldname = sparrow.workflow.get_state_fieldname(this.document_type);
				workflow_fieldname && fields.push(workflow_fieldname);
			}

			fields.push("modified");

			let quick_list_filter = sparrow.utils.process_filter_expression(this.quick_list_filter);

			let args = {
				method: "sparrow.desk.reportview.get",
				args: {
	...
	set_footer() {
		...
		let filters = sparrow.utils.get_filter_from_json(this.quick_list_filter);

		let route = sparrow.utils.generate_route({ type: "doctype", name: this.document_type });
		this.see_all_button = $(`
