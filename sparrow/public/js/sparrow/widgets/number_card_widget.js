...
export default class NumberCardWidget extends Widget {
	...
	make_card() {
		sparrow.model.with_doc("Number Card", this.number_card_name || this.name).then((card) => {
			if (!card) {
	...
	create_number_card() {
		this.set_doc_args();
		return sparrow
			.xcall("sparrow.desk.doctype.number_card.number_card.create_number_card", {
				args: this.card_doc,
			})
			.then((doc) => {
				this.name = doc.name;
	...
	set_route() {
		...
		const route = sparrow.utils.generate_route({
			name: name,
		...
		if (is_document_type) {
			...
			sparrow.route_options = filters.reduce((acc, filter) => {
	...
	set_route_for_custom_card() {
		...
		if (this.data.route_options) {
			sparrow.route_options = this.data.route_options;
		}

		sparrow.set_route(this.data.route);
	}
	...
	get_settings(type) {
		...
		const settings_map = {
			...
			"Document Type": {
				method: "sparrow.desk.doctype.number_card.number_card.get_result",
				args: {
					doc: this.card_doc,
					filters: this.filters,
				},
	...
	get_filters() {
		const filters = sparrow.dashboard_utils.get_all_filters(this.card_doc);
		return filters;
	}
	...
	async get_data() {
		this.data = await sparrow.xcall(this.settings.method, this.settings.args);
	...
	get_number_for_doctype_card(res) {
		...
		if (this.card_doc.function !== "Count") {
			return sparrow.model.with_doctype(this.card_doc.document_type, () => {
				const based_on_df = sparrow.meta.get_docfield(
	...
	get_number_for_report_card(res) {
		...
		this.number = sparrow.report_utils.get_result_of_fn(this.card_doc.report_function, vals);
		this.set_formatted_number(col);
	}

	set_formatted_number(df) {
		const default_country = sparrow.sys_defaults.country;
		const shortened_number = sparrow.utils.shorten_number(this.number, default_country, 5);
		let number_parts = shortened_number.split(" ");

		const symbol = number_parts[1] || "";
		const formatted_number = $(sparrow.format(number_parts[0], df)).text();
		...
		return this.get_percentage_stats().then(() => {
			if (this.percentage_stat == 0 || this.percentage_stat == undefined) {
				...
				caret_html = `<span class="indicator-pill-round green">
						${sparrow.utils.icon("arrow-up-right", "xs")}
					</span>`;
				color_class = "green-stat";
			} else {
				caret_html = `<span class="indicator-pill-round red">
						${sparrow.utils.icon("arrow-down-left", "xs")}
					</span>`;
	...
	get_percentage_stats() {
		return sparrow
			.xcall("sparrow.desk.doctype.number_card.number_card.get_percentage_difference", {
				doc: this.card_doc,
				filters: this.filters,
				result: this.number,
			})
			.then((res) => {
				if (res !== undefined) {
					this.percentage_stat = sparrow.utils.shorten_number(res);
				}
			});
	}
		...
		let actions = [
			...
			},
			{
				...
				handler: () => {
					...
					sparrow.set_route("Form", "Number Card", number_card);
				},
			},
		];
