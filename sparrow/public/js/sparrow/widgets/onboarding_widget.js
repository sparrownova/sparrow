...
export default class OnboardingWidget extends Widget {
	async refresh() {
		sparrow.utils.load_video_player();
	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					...
					<div class="step-index step-skipped">${sparrow.utils.icon("tick", "xs")}</div>
					<div class="step-index step-complete">${sparrow.utils.icon("tick", "xs")}</div>
	...
	show_step(step) {
		...
		let actions = {
			...
			"Create Entry": (step) => {
				if (step.is_complete) {
					sparrow.set_route(`/app/List/${step.reference_document}`);
				} else {
		...
		const set_description = () => {
			let content = step.description
				? sparrow.markdown(step.description)
				: `<h1>${__(step.title)}</h1>`;

			if (step.action === "Create Entry") {
				// add a secondary action to view list
				content += `<p>
					<a href='/app/${sparrow.router.slug(step.reference_document)}'>
		...
		const toggle_video = () => {
			...
			let plyr = new sparrow.Plyr(video[0], {
				hideControls: true,
				resetOnEnd: true,
			});
	...
	go_to_page(step) {
		...
		sparrow.set_route(step.path).then(() => {
			let message =
			...
			let msg_dialog = sparrow.msgprint({
				message: message,
				title: title,
				primary_action: {
					action: () => {
	...
	open_report(step) {
		let route = sparrow.utils.generate_route({
		...
		let current_route = sparrow.get_route();

		sparrow.set_route(route).then(() => {
			let msg_dialog = sparrow.msgprint({
				message: __(step.report_description),
				title: __(step.reference_report),
				primary_action: {
					action: () => {
						sparrow.set_route(current_route).then(() => {
							this.mark_complete(step);
						});
						msg_dialog.hide();
					},
			...
			sparrow.msg_dialog.custom_onhide = () => this.mark_complete(step);
		});
	}
	...
	show_form_tour(step) {
		let route;
		if (step.is_single) {
			route = sparrow.router.slug(step.reference_document);
		} else {
			route = `${sparrow.router.slug(step.reference_document)}/new`;
		}

		let current_route = sparrow.get_route();

		sparrow.route_hooks = {};
		sparrow.route_hooks.after_load = (frm) => {
			const on_finish = () => {
				let msg_dialog = sparrow.msgprint({
	...
	update_settings(step) {
		...
		sparrow.route_hooks.after_save = (frm) => {
			let success = false;
			let args = {};
			...
			if (success) {
				...
				args.secondary_action = {
					action: () => sparrow.set_route(current_route),
					label: __("Go Back"),
				};
			...
			sparrow.msgprint(args);
			sparrow.msg_dialog.custom_onhide = () => custom_onhide();
		};

		sparrow.set_route("Form", step.reference_document);
	}
	...
	async create_entry(step) {
		...
		if (step.is_submittable) {
			sparrow.route_hooks.after_save = () => {
				sparrow.msgprint({
					message: __("Submit this document to complete this step."),
					title: __("Document Saved"),
				});
			};
			sparrow.route_hooks.after_submit = callback;
		} else {
			sparrow.route_hooks.after_save = callback;
		}

		sparrow.set_route("Form", step.reference_document, docname);
	}

	show_quick_entry(step) {
		let current_route = sparrow.get_route_str();
		sparrow.ui.form.make_quick_entry(
			step.reference_document,
			() => {
				if (sparrow.get_route_str() != current_route) {
					let success_dialog = sparrow.msgprint({
						message: __("Let's take you back to onboarding"),
						title: __("Document Saved"),
						primary_action: {
							action: () => {
	...
	show_video(step) {
		sparrow.help.show_video(step.video_url, step.title);
		this.mark_complete(step);
	}
	...
	update_step_status(step, status, value, callback) {
		...
		sparrow
			.call("sparrow.desk.desktop.update_onboarding_step", {
				name: step.name,
				field: status,
				value: value,
			})
	...
	show_success() {
		...
		let success_state_image =
			this.success_state_image || "/assets/sparrow/images/ui-states/success-color.png";
		let documentation = "";
		if (this.docs_url) {
			documentation = __(
	...
	is_dismissed() {
		...
		if (Object.keys(dismissed).includes(this.title)) {
			...
			let today = new Date();
			let diff = sparrow.datetime.get_hour_diff(today, last_hidden);
			return diff < 24;
		}
		return false;
	}
	...
	set_actions() {
		...
		dismiss.on("click", () => {
			let dismissed = JSON.parse(localStorage.getItem("dismissed-onboarding") || "{}");
			dismissed[this.title] = sparrow.datetime.now_datetime();
		...
		return sparrow.model
			...
			.then((onboarding_doc) => {
				if (onboarding_doc) {
					...
					const method =
						"sparrow.desk.doctype.onboarding_step.onboarding_step.get_onboarding_steps";
					return sparrow
	...
	async get_first_document(doctype) {
		const { message } = await sparrow.db.get_value(
			"Form Tour",
		...
		if (message.first_document) {
			await sparrow.db.get_list(doctype, { order_by: "creation" }).then((res) => {
