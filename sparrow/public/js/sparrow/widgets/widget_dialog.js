class WidgetDialog {
		...
		this.dialog = new sparrow.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${sparrow.utils.get_random(20)}`;
				}
	...
	get_title() {
		...
		let label = (action = action + " " + sparrow.model.unscrub(this.type));
		return __(label);
	}
...
class QuickListDialog extends WidgetDialog {
	...
	generate_filter_from_json() {
		if (this.values && this.values.quick_list_filter) {
			this.filters = sparrow.utils.get_filter_from_json(
...
class ShortcutDialog extends WidgetDialog {
			...
			},
			{
				...
				onchange: () => {
					...
					if (doctype && this.dialog.get_value("type") == "DocType") {
						sparrow.model.with_doctype(doctype, async () => {
							let meta = sparrow.get_meta(doctype);

							if (doctype && sparrow.boot.single_types.includes(doctype)) {
								this.hide_filters();
							} else if (doctype) {
							...
							if (sparrow.boot.calendars.includes(doctype)) views.push("Calendar");

							const response = await sparrow.db.get_value(
								"Kanban Board",
			...
			},
			{
				...
				depends_on: (state) => {
					if (this.dialog) {
						...
						let is_single = sparrow.boot.single_types.includes(doctype);
	...
	process_data(data) {
		...
		data.label = data.label ? data.label : sparrow.model.unscrub(data.link_to);
...
class NumberCardDialog extends WidgetDialog {
			...
			return [
				{
					...
					get_query: () => {
						return {
							query: "sparrow.desk.doctype.number_card.number_card.get_cards_for_user",
							filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type && sparrow.get_meta(this.document_type)) {
			sparrow.get_meta(this.document_type).fields.map((df) => {
		...
		return [
			{
				...
				get_query: () => {
					return {
						query: "sparrow.desk.doctype.custom_html_block.custom_html_block.get_custom_blocks_for_user",
					};
				},
			},
		];
