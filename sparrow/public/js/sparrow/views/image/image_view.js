/**
 * sparrow.views.ImageView
 */
sparrow.provide("sparrow.views");

sparrow.views.ImageView = class ImageView extends sparrow.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = sparrow.utils.escape_html(title);
	...
	get_attached_images() {
		return sparrow
			.call({
				method: "sparrow.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new sparrow.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
...
sparrow.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = sparrow.render_template("photoswipe_dom");
	...
	load_lib() {
		return new Promise((resolve) => {
			var asset_dir = "assets/sparrow/js/lib/photoswipe/";
			sparrow.require(
				[
