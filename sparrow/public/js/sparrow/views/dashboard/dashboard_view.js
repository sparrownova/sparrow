...
sparrow.views.DashboardView = class DashboardView extends sparrow.views.ListView {
	get view_name() {
		return "Dashboard";
	}
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.dashboard_settings =
				sparrow.get_user_settings(this.doctype)["dashboard_settings"] || null;
		});
	}
	...
	setup_dashboard_page() {
		...
		this.page.main.removeClass("sparrow-card");

		this.$dashboard_wrapper = this.$page.find(".dashboard-view");
		this.$chart_header = this.$page.find(".dashboard-header");

		sparrow.utils.bind_actions_with_object(this.$dashboard_page, this);
	}
	...
	render_empty_state() {
		...
		const empty_state_image = "/assets/sparrow/images/ui-states/list-empty-state.svg";
	...
	save_dashboard_customization() {
		...
		sparrow.model.user_settings.save(
			this.doctype,
			"dashboard_settings",
			this.dashboard_settings
		);
	...
	reset_dashboard_customization() {
		sparrow.confirm(__("Are you sure you want to reset all customizations?"), () => {
			this.dashboard_settings = null;
			sparrow.model.user_settings
		...
		const dialog = new sparrow.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							query: "sparrow.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user",
							filters: {
								document_type: this.doctype,
							},
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == "New Chart") {
					...
					sparrow
						.xcall(
							"sparrow.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart",
							{ args: chart }
						)
						.then((doc) => {
	...
	get_field_options() {
		...
		sparrow.get_meta(this.doctype).fields.map((df) => {
