...
sparrow.views.ReportView = class ReportView extends sparrow.views.ListView {
	get view_name() {
		return "Report";
	}
	...
	setup_events() {
		...
		sparrow.realtime.on("list_update", (data) => this.on_update(data));
	}
	...
	setup_sort_selector() {
		this.sort_selector = new sparrow.ui.SortSelector({
		...
		this.group_by_control = new sparrow.ui.GroupBy(this);
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse sparrow.route_options if this is a Custom Report
	...
	set_dirty_state_for_custom_report() {
		...
		if (!sparrow.utils.deep_equal(current_settings, report_settings)) {
	...
	prepare_data(r) {
		let data = r.message || {};
		data = sparrow.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== sparrow.session.user;
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			// numeric values in y
			if (col.docfield && sparrow.model.numeric_fieldtypes.includes(col.docfield.fieldtype)) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
					value: col.id,
			...
			fields: [
				...
				},
				{
					...
					default: defaults.chart_type
						? sparrow.utils.to_title_case(defaults.chart_type)
						: "Bar",
				},
			],
			primary_action: (data) => {
	...
	make_chart() {
		...
		this.chart = new sparrow.Chart(this.$charts_wrapper.find(".charts-inner-wrapper")[0], {
			...
			tooltipOptions: {
				formatTooltipY: (value) => {
					return sparrow.format(value, get_df(this.chart_args.y_axes[0]), {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === "Text Editor") {
			...
			// make control
			control = sparrow.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true,
			});
	...
	is_editable(df, data) {
		if (
			df &&
			sparrow.model.can_write(this.doctype) &&
			...
			sparrow.model.is_non_std_field(df.fieldname)
		)
			return true;
		return false;
	}
	...
	set_default_fields() {
		...
		const fields = this.meta.fields.filter((df) => {
			return (
				...
				sparrow.perm.has_perm(this.doctype, df.permlevel, "read") &&
				sparrow.model.is_value_type(df.fieldtype) &&
				!df.report_hide
			);
		});
	...
	get_fields() {
		let fields = this.fields.map((f) => {
			let column_name = sparrow.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields = this.get_unique_cdt_in_view().map(
			(cdt) => sparrow.model.get_full_column_name("name", cdt) + " as " + `'${cdt}:name'`
		);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = sparrow.meta.get_docfield(doctype, fieldname);
		if (
			df &&
			df.fieldtype === "Currency" &&
			df.options &&
			!df.options.includes(":") &&
			sparrow.meta.has_field(doctype, df.options)
		) {
			...
			const field_label = sparrow.meta.get_label(doctype, field[0]);
			sparrow.show_alert(
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === "name") {
			this.refresh();
			sparrow.throw(__("Cannot remove ID field"));
		}
	...
	get_columns_for_picker() {
		...
		const standard_fields_filter = (df) => !in_list(sparrow.model.no_value_type, df.fieldtype);

		let doctype_fields = sparrow.meta
			.get_docfields(this.doctype)
			.filter(standard_fields_filter);

		// filter out docstatus field from picker
		let std_fields = sparrow.model.std_fields.filter((df) => df.fieldname !== "docstatus");

		// add status field derived from docstatus, if status is not a standard field
		let has_status_values = false;

		if (this.data) {
			has_status_values = sparrow.get_indicator(this.data[0], this.doctype);
		}

		if (!sparrow.meta.has_field(this.doctype, "status") && has_status_values) {
			doctype_fields = [
				{
					label: __("Status"),
					fieldname: "docstatus",
		...
		const table_fields = sparrow.meta.get_table_fields(this.doctype);

		table_fields.forEach((df) => {
			const cdt = df.options;
			const child_table_fields = sparrow.meta
				.get_docfields(cdt)
				.filter(standard_fields_filter);
	...
	get_dialog_fields() {
		...
		const table_fields = sparrow.meta.get_table_fields(this.doctype).filter((df) => !df.hidden);
		...
		for (let f of this.fields) {
			...
			if (f[0] !== "docstatus") {
				...
				if (
					...
					!sparrow.meta.has_field(this.doctype, "status")
				) {
	...
	build_column(c) {
		...
		let docfield = sparrow.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		if (!docfield) {
			docfield = sparrow.model.get_std_field(fieldname, true);
			...
			if (docfield) {
				...
				if (fieldname == "docstatus" && !sparrow.meta.has_field(this.doctype, "status")) {
		...
		const editable =
			sparrow.model.is_non_std_field(fieldname) &&
			!docfield.read_only &&
			!docfield.is_virtual;

		const align = (() => {
			const is_numeric = sparrow.model.is_numeric_field(docfield);
			if (is_numeric) {
				return "right";
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				return sparrow.format(value, column.docfield, { always_show_decimals: true }, doc);
			},
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: (value) => {
						let formatted_value = sparrow.format(
							value,
							col.docfield,
							{
								always_show_decimals: true,
	...
	format_total_cell(formatted_value, df) {
		...
		if (sparrow.model.is_numeric_field(df.docfield)) {
	...
	build_row(d) {
		return this.columns.map((col) => {
			...
			if (col.field === "docstatus" && !sparrow.meta.has_field(this.doctype, "status")) {
				// get status from docstatus
				let status = sparrow.get_indicator(d, this.doctype);
				if (status) {
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return sparrow.call({
				method: "sparrow.desk.reportview.save_report",
				args: {
					name: name,
					doctype: this.doctype,
				...
				callback: (r) => {
					if (r.exc) {
						sparrow.msgprint(__("Report was not saved (there were errors)"));
						return;
					}
					if (r.message != this.report_name) {
						// Rerender the reports dropdown,
						// so that this report is included in the dropdown as well.
						sparrow.boot.user.all_reports[r.message] = {
							ref_doctype: this.doctype,
							report_type: "Report Builder",
							title: r.message,
						};

						sparrow.set_route("List", this.doctype, "Report", r.message);
						return;
					}
	...
	delete_report() {
		return sparrow.call({
			method: "sparrow.desk.reportview.delete_report",
	...
	get_report_doc() {
		return new Promise((resolve) => {
			sparrow.model.with_doc("Report", this.report_name, () => {
				resolve(sparrow.get_doc("Report", this.report_name));
			});
		});
	}
		...
		return (
			...
			filters
				.map((f) => {
					...
					const docfield = sparrow.meta.get_docfield(doctype, fieldname);
					const label = `<b>${__(sparrow.meta.get_label(doctype, fieldname))}</b>`;
					switch (condition) {
						case "=":
							return __("{0} is equal to {1}", [
								label,
								sparrow.format(value, docfield),
							]);
						case "!=":
						...
						case "Between":
							return __("{0} is between {1} and {2}", [
								__(label),
								sparrow.format(value[0], docfield),
								sparrow.format(value[1], docfield),
							]);
						case "Timespan":
						...
						case "in":
							return __("{0} is one of {1}", [
								__(label),
								sparrow.utils.comma_or(
									value.map((v) => sparrow.format(v, docfield))
								),
							]);
						case "not in":
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && sparrow.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					sparrow.ui.get_print_settings(false, (print_settings) => {
			...
			},
			{
				...
				action: () => {
					...
					sparrow.utils.setup_search(d.$body, ".unit-checkbox", ".label-area");
					d.show();
				},
			},
		];

		if (sparrow.model.can_export(this.doctype)) {
			items.push({
				label: __("Export"),
				action: () => {
					...
					const d = new sparrow.ui.Dialog({
						...
						primary_action: (data) => {
							args.cmd = "sparrow.desk.reportview.export_query";
		...
		items.push({
			...
			action: () => {
				if (this.report_name) {
					...
					sparrow.msgprint(__("Please save the report first"));
				}
			},
		});
		...
		const can_edit_or_delete = (action) => {
			...
			return (
				...
				(sparrow.model[method]("Report") || this.report_doc.owner === sparrow.session.user)
			);
		};
		...
		if (can_edit_or_delete("delete")) {
			items.push({
				label: __("Delete"),
				action: () =>
					sparrow.confirm("Are you sure you want to delete this report?", () =>
						this.delete_report()
					),
		...
		// user permissions
		if (this.report_name && sparrow.model.can_set_user_permissions("Report")) {
			items.push({
				...
				action: () => {
					...
					sparrow.set_route("List", "User Permission", args);
				},
			});
		}
