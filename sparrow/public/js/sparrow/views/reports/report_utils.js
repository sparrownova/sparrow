sparrow.provide("sparrow.report_utils");

sparrow.report_utils = {
	make_chart_options: function (
		...
		let datasets = y_fields.map((y_field) => ({
			name: sparrow.model.unscrub(y_field),
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = sparrow.report_utils.prepare_field_from_column(field);
					return { label: field.label, value: field.fieldname };
				}
			});
		}
	...
	get_report_filters: function (report_name) {
		if (sparrow.query_reports[report_name]) {
			let filters = sparrow.query_reports[report_name].filters;
		...
		return sparrow
			...
			.then((r) => {
				sparrow.dom.eval(r.script || "");
				return sparrow.after_ajax(() => {
					if (
						sparrow.query_reports[report_name] &&
						!sparrow.query_reports[report_name].filters &&
						r.filters
					) {
						return (sparrow.query_reports[report_name].filters = r.filters);
					}
					return (
						sparrow.query_reports[report_name] &&
						sparrow.query_reports[report_name].filters
