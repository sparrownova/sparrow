...
sparrow.provide("sparrow.query_reports");

sparrow.standard_pages["query-report"] = function () {
	var wrapper = sparrow.container.add_page("query-report");
	...
	sparrow.query_report = new sparrow.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind("show", function () {
		sparrow.query_report.show();
	});
};

sparrow.views.QueryReport = class QueryReport extends sparrow.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = sparrow.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = sparrow.utils.throttle(this.refresh, 300);
	...
	update_url_with_filters() {
		if (sparrow.get_route_str() == this.page_name) {
	...
	get_no_result_message() {
		return `<div class="msg-box no-border">
			<div>
				<img src="/assets/sparrow/images/ui-states/list-empty-state.svg" alt="Generic Empty State" class="null-state">
			</div>
	...
	setup_events() {
		sparrow.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					sparrow.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (sparrow.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, sparrow.route_options);

		if (this.report_name !== sparrow.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (sparrow.has_route_options()) {
	...
	add_card_button_to_toolbar() {
		if (!sparrow.model.can_create("Number Card")) return;
	...
	add_chart_buttons_to_toolbar(show) {
		if (!sparrow.model.can_create("Dashboard Chart")) return;
		if (show) {
	...
	add_card_to_dashboard() {
		let field_options = sparrow.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = sparrow.dashboard_utils.get_dashboard_link_field();
		const set_standard = sparrow.boot.developer_mode;
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			sparrow.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"sparrow.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || sparrow.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"sparrow.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		sparrow.xcall(method, { args: args }).then(() => {
			let message;
			if (dashboard_name) {
			...
			sparrow.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	get_report_doc() {
		return sparrow.model
			.with_doc("Report", this.report_name)
			.then((doc) => {
				this.report_doc = doc;
			})
			.then(() => sparrow.model.with_doctype(this.report_doc.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (sparrow.query_reports[this.report_name]) {
				this.report_settings = sparrow.query_reports[this.report_name];
				resolve();
			} else {
				sparrow
					.xcall("sparrow.desk.query_report.get_script", {
						report_name: this.report_name,
					})
					.then((settings) => {
						sparrow.dom.eval(settings.script || "");
						sparrow.after_ajax(() => {
							...
							sparrow.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings(custom_report_name) {
		let report_script_name =
			this.report_doc.report_type === "Custom Report"
				...
				: this.report_name;
		return sparrow.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			sparrow.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = sparrow.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					sparrow.throw(
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = sparrow.route_options;
		...
		if (route_options) {
			...
			return sparrow.run_serially(promises);
		}
	}
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			this.last_ajax = sparrow.call({
				method: "sparrow.desk.query_report.run",
				type: "GET",
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = sparrow.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			sparrow.utils.build_summary_item(summary).appendTo(this.$summary);
		});
	...
	get_query_params() {
		const query_string = sparrow.utils.get_query_string(sparrow.get_route_str());
		const query_params = sparrow.utils.get_query_params(query_string);
		return query_params;
	}
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
			this.page.add_inner_button(__("Download Report"), function () {
				window.open(
					sparrow.urllib.get_full_url(
						"/api/method/sparrow.core.doctype.prepared_report.prepared_report.download_attachment?" +
							"dn=" +
			...
			let pretty_diff = sparrow.datetime.comment_when(doc.report_end_time);
			const days_old = sparrow.datetime.get_day_diff(
				sparrow.datetime.now_datetime(),
				doc.report_end_time
			);
			if (days_old > 1) {
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					sparrow.set_route(sparrow.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		sparrow
			.xcall(
				"sparrow.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = sparrow.warn(
					...
					sparrow.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		sparrow
			.xcall("sparrow.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				sparrow.call({
					method: "sparrow.desk.query_report.background_enqueue_run",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
				...
				let alert_message =
					...
					`</a>`;
				sparrow.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: sparrow.boot.lang,
				translations: sparrow.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: "fixed",
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: sparrow.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: sparrow.utils.report_column_total,
				},
			};
	...
	get_chart_options(data) {
		...
		options.axisOptions = {
			...
			numberFormatter: sparrow.utils.format_chart_axis_number,
		};
		options.height = 280;
		return options;
	}
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new sparrow.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = sparrow.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new sparrow.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		return columns.map((column) => {
			column = sparrow.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = sparrow.datetime.user_to_obj(keyword);
					const cellValue = sparrow.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = sparrow.get_meta(this.report_doc.ref_doctype);
		sparrow.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		sparrow.call("sparrow.core.doctype.access_log.access_log.make_access_log", {
	...
	pdf_report(print_settings) {
		const base_url = sparrow.urllib.get_base_url();
		const print_css = sparrow.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = sparrow.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = sparrow.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: sparrow.boot.lang,
			layout_direction: sparrow.utils.is_rtl() ? "rtl" : "ltr",
			can_use_smaller_font: this.report_doc.is_standard === "Yes" && custom_format ? 0 : 1,
		});
		...
		sparrow.render_pdf(html, print_settings);
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				const docfield = sparrow.query_report.get_filter(fieldname).df;
				const value = applied_filters[fieldname];
				return `<h6>${__(docfield.label)}: ${sparrow.format(value, docfield)}</h6>`;
			})
			.join("");
	}
	...
	export_report() {
		...
		this.export_dialog = sparrow.prompt(
			...
			({ file_format, include_indentation }) => {
				...
				if (file_format === "CSV") {
					...
					sparrow.tools.downloadify(out, null, this.report_name);
				} else {
					let filters = this.get_filter_values(true);
					if (sparrow.urllib.get_dict("prepared_report_name")) {
						filters = Object.assign(
							sparrow.urllib.get_dict("prepared_report_name"),
							filters
						);
					}
					...
					const args = {
						cmd: "sparrow.desk.query_report.export_query",
						report_name: this.report_name,
					...
					open_url_post(sparrow.request.url, args);
				}
			},
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = sparrow.utils.get_formatted_duration(cell.content);
				}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __("Edit"),
				action: () => sparrow.set_route("Form", "Report", this.report_name),
				condition: () => sparrow.user.is_report_manager(),
				standard: true,
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = sparrow.ui.get_print_settings(
						false,
				...
				condition: () => sparrow.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => sparrow.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("Setup Auto Email"),
				action: () =>
					sparrow.set_route("List", "Auto Email Report", { report: this.report_name }),
				standard: true,
			},
			{
				label: __("Add Column"),
				action: () => {
					let d = new sparrow.ui.Dialog({
						...
						fields: [
							{
								...
								change: () => {
									...
									sparrow.model.with_doctype(doctype, () => {
										let options = sparrow.meta
											.get_docfields(doctype)
											.filter(sparrow.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = sparrow.meta.get_docfield(values.doctype, values.field);
							...
							custom_columns.push({
								fieldname: this.columns
									...
									? df.fieldname + "-" + sparrow.scrub(values.doctype)
									: df.fieldname,
							...
							sparrow.call({
								method: "sparrow.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
									names: Array.from(
			...
			},
			{
				...
				condition: () => sparrow.model.can_set_user_permissions("Report"),
				standard: true,
			},
		];

		if (sparrow.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new sparrow.ui.Dialog({
						...
						primary_action: (values) => {
							sparrow.call({
								method: "sparrow.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									sparrow.set_route("query-report", r.message);
								},
							});
						},
					});
	...
	add_custom_column(
		...
		this.data.forEach((row) => {
			if (column[0].fieldname.includes("-")) {
				row[column_field + "-" + sparrow.scrub(new_column_data.doctype)] =
...
Object.defineProperty(sparrow, "query_report_filters_by_name", {
	get() {
		console.warn(
			"[Query Report] sparrow.query_report_filters_by_name is deprecated. Please use the new api: sparrow.query_report.get_filter_value(fieldname) and sparrow.query_report.set_filter_value(fieldname, value)"
		);
		return null;
	},
});
