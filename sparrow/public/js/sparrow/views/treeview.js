...
sparrow.provide("sparrow.treeview_settings");
sparrow.provide("sparrow.views.trees");
window.cur_tree = null;

sparrow.views.TreeFactory = class TreeFactory extends sparrow.views.Factory {
	make(route) {
		sparrow.model.with_doctype(route[1], function () {
			var options = {
				doctype: route[1],
				meta: sparrow.get_meta(route[1]),
			};

			if (
				!sparrow.treeview_settings[route[1]] &&
				!sparrow.meta.get_docfield(route[1], "is_group")
			) {
				sparrow.msgprint(__("Tree view is not available for {0}", [route[1]]));
				return false;
			}
			$.extend(options, sparrow.treeview_settings[route[1]] || {});
			sparrow.views.trees[options.doctype] = new sparrow.views.TreeView(options);
		});
	}
	...
	on_show() {
		...
		let treeview = sparrow.views.trees[route[1]];
...
sparrow.views.TreeView = class TreeView {
	constructor(opts) {
		var me = this;
		...
		this.get_tree_nodes = me.opts.get_tree_nodes || "sparrow.desk.treeview.get_children";
	...
	get_permissions() {
		this.can_read = sparrow.model.can_read(this.doctype);
		this.can_create =
			sparrow.boot.user.can_create.indexOf(this.doctype) !== -1 ||
			sparrow.boot.user.in_create.indexOf(this.doctype) !== -1;
		this.can_write = sparrow.model.can_write(this.doctype);
		this.can_delete = sparrow.model.can_delete(this.doctype);
	}
	make_page() {
		var me = this;
		this.parent = sparrow.container.add_page(this.page_name);
		sparrow.ui.make_app_page({ parent: this.parent, single_column: true });
		...
		this.page.main.addClass("sparrow-card");
	...
	make_filters() {
		var me = this;
		sparrow.treeview_settings.filters = [];
		$.each(this.opts.filters || [], function (i, filter) {
			if (sparrow.route_options && sparrow.route_options[filter.fieldname]) {
				filter.default = sparrow.route_options[filter.fieldname];
			}
	...
	rebuild_tree() {
		...
		sparrow.call({
			method: "sparrow.utils.nestedset.rebuild_tree",
			args: {
				doctype: me.doctype,
	...
	select_node(node) {
		...
		if (this.opts.view_template) {
			...
			$(
				sparrow.render_template(me.opts.view_template, {
					data: node.data,
					doctype: me.doctype,
				})
		...
		var toolbar = [
			{
				...
				click: function (node) {
					sparrow.set_route("Form", me.doctype, node.label);
				},
			},
			{
				label: __("Add Child"),
			...
			},
			{
				...
				condition: function (node) {
					...
					if (me.doctype && sparrow.get_meta(me.doctype)) {
						if (!sparrow.get_meta(me.doctype).allow_rename) allow_rename = false;
					}
					return !node.is_root && me.can_write && allow_rename;
				},
				click: function (node) {
					sparrow.model.rename_doc(me.doctype, node.label, function (new_name) {
			...
			},
			{
				...
				click: function (node) {
					sparrow.model.delete_doc(me.doctype, node.label, function () {
						node.parent.remove();
					});
				},
				btnClass: "hidden-xs",
	...
	new_node() {
		...
		if (!(node && node.expandable)) {
			sparrow.msgprint(__("Select a group node first."));
			return;
		}
		...
		d.set_primary_action(__("Create New"), function () {
			...
			d.hide();
			sparrow.dom.freeze(__("Creating {0}", [me.doctype]));

			$.extend(args, v);
			return sparrow.call({
				method: me.opts.add_tree_node || "sparrow.desk.treeview.add_node",
				args: args,
				callback: function (r) {
					if (!r.exc) {
	...
	print_tree() {
		if (!sparrow.model.can_print(this.doctype)) {
			sparrow.msgprint(__("You are not allowed to print this report"));
			return false;
		}
		var tree = $(".tree:visible").html();
		var me = this;
		sparrow.ui.get_print_settings(false, function (print_settings) {
			var title = __(me.docname || me.doctype);
			sparrow.render_tree({ title: title, tree: tree, print_settings: print_settings });
			sparrow.call({
				method: "sparrow.core.doctype.access_log.access_log.make_access_log",
				args: {
					doctype: me.doctype,
	...
	set_menu_item() {
		...
		this.menu_items = [
			{
				label: __("View List"),
				action: function () {
					sparrow.set_route("List", me.doctype);
				},
			},
			{
				label: __("Print"),
		...
		if (
			...
			sparrow.meta.has_field(me.doctype, "lft") &&
			sparrow.meta.has_field(me.doctype, "rgt")
		) {
