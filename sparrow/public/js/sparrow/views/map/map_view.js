/**
 * sparrow.views.MapView
 */
sparrow.provide("sparrow.utils.utils");
sparrow.provide("sparrow.views");

sparrow.views.MapView = class MapView extends sparrow.views.ListView {
	get view_name() {
		return "Map";
	}
	...
	render_map_view() {
		...
		this.map = L.map(this.map_id).setView(
			sparrow.utils.map_defaults.center,
			sparrow.utils.map_defaults.zoom
		);
	...
	get_coords() {
		let get_coords_method =
			(this.settings && this.settings.get_coords_method) || "sparrow.geo.utils.get_coords";
	...
	get required_libs() {
		return [
			"assets/sparrow/js/lib/leaflet/leaflet.css",
			"assets/sparrow/js/lib/leaflet/leaflet.js",
		];
	}
};
