...
sparrow.provide("sparrow.interaction_settings");

sparrow.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		sparrow.model.with_doctype("Event", () => {
			let categories = sparrow.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new sparrow.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = sparrow.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = sparrow.session.user;
		}
		return sparrow.call({
	...
	add_attachments(doc, attachments) {
		sparrow.call({
			method: "sparrow.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
