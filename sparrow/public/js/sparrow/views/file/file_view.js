...
sparrow.views.FileView = class FileView extends sparrow.views.ListView {
	static load_last_view() {
		const route = sparrow.get_route();
		if (route.length === 2) {
			const view_user_settings = sparrow.get_user_settings("File", "File");
			sparrow.set_route(
				"List",
				"File",
				view_user_settings.last_folder || sparrow.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			sparrow.views.FileView.grid_view = !sparrow.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (sparrow.views.FileView.grid_view) {
			Object.assign(args, {
		...
		const items = [
			{
				label: __("Home"),
				action: () => {
					sparrow.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					sparrow.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								sparrow.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							sparrow.call({
								method: "sparrow.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new sparrow.ui.FileUploader({
						...
						on_success: (file) => {
							sparrow.show_alert(__("Unzipping files..."));
							sparrow
								.call("sparrow.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										sparrow.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				sparrow.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => sparrow.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/sparrow.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => sparrow.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (sparrow.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
	...
	before_render() {
		super.before_render();
		sparrow.model.user_settings.save("File", "grid_view", sparrow.views.FileView.grid_view);
	...
	get_header_html() {
		...
		let header_selector_html = !sparrow.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !sparrow.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === sparrow.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = sparrow.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? sparrow.form.formatters.FileSize(file.file_size) : "";
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					sparrow.file_manager.paste(folder_name);
					sparrow.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!sparrow.file_manager.can_paste ||
			sparrow.file_manager.old_folder === this.current_folder;
...
sparrow.views.FileView.grid_view = sparrow.get_user_settings("File").grid_view || false;
