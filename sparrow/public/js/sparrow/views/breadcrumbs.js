...
sparrow.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[sparrow.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return sparrow.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[sparrow.breadcrumbs.current_page()];
	...
	get last_route() {
		return sparrow.route_history.slice(-2)[0];
	},
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (
			...
			(breadcrumbs.module_info.blocked ||
				!sparrow.visible_modules.includes(breadcrumbs.module_info.module))
		) {
			return;
		}

		this.append_breadcrumb_element(
			`/app/${sparrow.router.slug(breadcrumbs.workspace)}`,
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			if (
				breadcrumbs.module &&
				sparrow.boot.module_wise_workspaces[breadcrumbs.module]?.includes(last_workspace)
			) {
		...
		if (breadcrumbs.module) {
			...
			breadcrumbs.module_info = sparrow.get_module(breadcrumbs.module);

			// set workspace
			if (
				breadcrumbs.module_info &&
				sparrow.boot.module_wise_workspaces[breadcrumbs.module]
			) {
				breadcrumbs.workspace = sparrow.boot.module_wise_workspaces[breadcrumbs.module][0];
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = sparrow.get_doc("DocType", doctype);
		if (
			(doctype === "User" && !sparrow.user.has_role("System Manager")) ||
			doctype_meta?.issingle
		) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = sparrow.router.slug(sparrow.router.doctype_layout || doctype);
			if (doctype_meta?.is_tree) {
				let view = sparrow.model.user_settings[doctype].last_view || "Tree";
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		const docname = sparrow.get_route().slice(2).join("/");
		let form_route = `/app/${sparrow.router.slug(doctype)}/${docname}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				sparrow.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	},
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = sparrow.get_route()[1];
		let dashboard_route = `/app/${sparrow.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`).appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!sparrow.visible_modules) {
			sparrow.visible_modules = $.map(sparrow.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete sparrow.breadcrumbs.all[old_route_str];
		this.update();
	},
