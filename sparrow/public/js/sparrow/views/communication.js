...
sparrow.last_edited_communication = {};
const separator_element = "<div>---</div>";

sparrow.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	make() {
		...
		this.dialog = new sparrow.ui.Dialog({
	...
	get_fields() {
		const fields = [
			...
			{
				fieldtype: "Button",
				label: sparrow.utils.icon("down", "xs"),
			...
			},
			{
				...
				fieldname: "content",
				onchange: sparrow.utils.debounce(this.save_as_draft.bind(this), 300),
			},
			{
				fieldtype: "Button",
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: sparrow.boot.user.send_me_a_copy,
			},
			{
		...
		const email_accounts = sparrow.boot.email_accounts.filter((account) => {
			return (
	...
	toggle_more_options(show_options) {
		...
		const label = sparrow.utils.icon(show_options ? "up-line" : "down", "xs");
	...
	setup_multiselect_queries() {
		["recipients", "cc", "bcc"].forEach((field) => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				sparrow.call({
					method: "sparrow.email.get_contact_list",
					args: { txt },
					callback: (r) => {
	...
	setup_email_template() {
		...
		function add_template() {
			...
			sparrow.call({
				method: "sparrow.email.doctype.email_template.email_template.get_email_template",
				args: {
		...
		sparrow.utils.add_select_group_button(clear_and_add_template, email_template_actions);
	}
	...
	get_last_edited_communication(clear) {
		if (!sparrow.last_edited_communication[this.doctype]) {
			sparrow.last_edited_communication[this.doctype] = {};
		}

		if (clear || !sparrow.last_edited_communication[this.doctype][this.key]) {
			sparrow.last_edited_communication[this.doctype][this.key] = {};
		}

		return sparrow.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = this.subject ? sparrow.utils.html2text(this.subject) : "";
	...
	setup_print_language() {
		...
		$(fields.language_sel.input).empty().add_options(sparrow.get_languages());
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = sparrow.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		$(`
			...
			<p class='add-more-attachments'>
				<button class='btn btn-xs btn-default'>
					${sparrow.utils.icon("small-add", "xs")}&nbsp;
					${__("Add Attachment")}
				</button>
			</p>
		...
		attach
			...
			.on("click", () => new sparrow.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = sparrow.urllib.get_full_url(f.file_url);
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on("click", () => {
			...
			sparrow.db.set_value("User", sparrow.session.user, "send_me_a_copy", val);
			sparrow.boot.user.send_me_a_copy = val;
		});
	}
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			sparrow.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !sparrow.model.can_email(this.doc.doctype, this.frm)) {
			sparrow.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}

		return sparrow.call({
			method: "sparrow.core.doctype.communication.email.make",
			args: {
				...
				sender_full_name: form_values.sender ? sparrow.user.full_name() : undefined,
			...
			callback(r) {
				if (!r.exc) {
					sparrow.utils.play_sound("email");

					if (r.message["emails_not_sent_to"]) {
						sparrow.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								sparrow.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					}
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find(".form-print-wrapper").is(":visible")) {
			...
			return (
				sparrow.model.get_doc(":Print Settings", "Print Settings") || { with_letterhead: 1 }
			).with_letterhead
				? 1
				: 0;
		}
	...
	async get_signature(sender_email) {
		let signature = sparrow.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await sparrow.db.get_list("Email Account", {
				filters: filters,
		...
		if (!sparrow.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = sparrow.datetime.global_date_format(
	...
	html2text(html) {
		...
		const text = sparrow.utils.html2text(html);
		return text.replace(/\n{3,}/g, "\n\n");
	}
};
