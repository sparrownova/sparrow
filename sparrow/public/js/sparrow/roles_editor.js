sparrow.RoleEditor = class {
	constructor(wrapper, frm, disable) {
		...
		this.multicheck = sparrow.ui.form.make_control({
			...
			df: {
				...
				get_data: () => {
					return sparrow
						.xcall("sparrow.core.doctype.user.user.get_all_roles")
						.then((roles) => {
	...
	show_permissions(role) {
		...
		return sparrow
			.xcall("sparrow.core.doctype.user.user.get_perm_info", { role })
			.then((permissions) => {
						...
						<table class="user-perm">
							<thead>
								<tr>
									...
									<th> ${__("Level")} </th>
									${sparrow.perm.rights.map((p) => `<th> ${__(sparrow.unscrub(p))}</th>`).join("")}
								</tr>
							</thead>
							<tbody></tbody>
						</table>
									...
									.map(
										(p) =>
											`<td class="text-muted bold">${
												perm[p] ? sparrow.utils.icon("check", "xs") : "-"
											}</td>`
									)
									.join("")}
							</tr>
	...
	make_perm_dialog() {
		this.perm_dialog = new sparrow.ui.Dialog({
	...
	set_roles_in_table() {
		...
		roles.map((role_doc) => {
			if (!checked_options.includes(role_doc.role)) {
				sparrow.model.clear_doc(role_doc.doctype, role_doc.name);
			}
		});
		checked_options.map((role) => {
			if (!roles.find((d) => d.role === role)) {
				let role_doc = sparrow.model.add_child(this.frm.doc, "Has Role", "roles");
				role_doc.role = role;
			}
		});
	}
