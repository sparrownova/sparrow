...
sparrow.provide("sparrow.web_form");
import EventEmitterMixin from "../../sparrow/event_emitter";

export default class WebForm extends sparrow.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		sparrow.web_form = this;
		sparrow.web_form.events = {};
		Object.assign(sparrow.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		sparrow.init_client_script && sparrow.init_client_script();
		this.setup_listeners();
		sparrow.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = sparrow.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${sparrow.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _section.find(".sparrow-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		sparrow.form_dirty = false;

		sparrow.call({
			type: "POST",
			method: "sparrow.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					sparrow.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						sparrow.call({
							type: "POST",
							method: "sparrow.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			sparrow.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
