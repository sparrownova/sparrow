...
sparrow.ready(function () {
	let web_form_doc = sparrow.web_form_doc;
	let reference_doc = sparrow.reference_doc;
	...
	function show_login_prompt() {
		if (sparrow.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new sparrow.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(sparrow.form.formatters.FileSize($(this).text()));
		});
	}
