...
sparrow.provide("sparrow.web_form_list");

export default class WebFormList {
	constructor(opts) {
		Object.assign(this, opts);
		sparrow.web_form_list = this;
	...
	make_filters() {
		...
		sparrow
			.call("sparrow.website.doctype.web_form.web_form.get_web_form_filters", {
			...
			.then((response) => {
				...
				fields.forEach((field) => {
					...
					let input = sparrow.ui.form.make_control({
						df: {
	...
	fetch_data() {
		if (this.condition_json && JSON.parse(this.condition_json)) {
			let filter = sparrow.utils.get_filter_from_json(this.condition_json);
			filter = sparrow.utils.get_filter_as_json(filter);
			this.filters = Object.assign(this.filters, JSON.parse(filter));
		}

		let args = {
			method: "sparrow.www.list.get_list_data",
			args: {
	...
	more() {
		...
		this.fetch_data().then((res) => {
			if (res.message.length === 0) {
				sparrow.msgprint(__("No more items to display"));
			}
	...
	make_table_body() {
		if (this.data.length) {
			...
			this.wrapper.empty();
			sparrow.has_permission(this.doctype, "", "create", () => {
		...
		let empty_state = $(`
			<div class="no-result text-muted flex justify-center align-center">
				<div class="text-center">
					<div>
						<img
							src="/assets/sparrow/images/ui-states/list-empty-state.svg"
	...
	append_rows(row_data) {
		...
		row_data.forEach((data_item) => {
			...
			let row = new sparrow.ui.WebFormListRow({
				row: $row_element,
				doc: data_item,
	...
	make_actions() {
		...
		sparrow.has_permission(this.doctype, "", "delete", () => {
	...
	delete_rows() {
		...
		sparrow
			.call({
				type: "POST",
				method: "sparrow.website.doctype.web_form.web_form.delete_multiple",
				args: {
					web_form_name: this.web_form_name,
...
sparrow.ui.WebFormListRow = class WebFormListRow {
	...
	make_row() {
		...
		this.columns.forEach((field) => {
			let formatter = sparrow.form.get_formatter(field.fieldtype);
			let value =
