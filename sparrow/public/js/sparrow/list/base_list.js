...
sparrow.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || sparrow.router.doctype_layout || __(this.doctype);
		this.meta = sparrow.get_meta(this.doctype);
		this.settings = sparrow.listview_settings[this.doctype] || {};
		this.user_settings = sparrow.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = 20;
		this.data = [];
		this.method = "sparrow.desk.reportview.get";

		this.can_create = sparrow.model.can_create(this.doctype);
	...
	get_list_view_settings() {
		return sparrow
			.call("sparrow.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(sparrow.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			sparrow.model.std_fields_list.includes(fieldname) ||
			sparrow.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = sparrow.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!sparrow.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return sparrow.model.with_doctype(this.doctype);
	}
	...
	setup_page() {
		...
		!this.hide_card_layout && this.page.main.addClass("sparrow-card");
	...
	setup_view_menu() {
		...
		if (sparrow.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			/* @preserve
			...
			this.views_list = new sparrow.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		sparrow.breadcrumbs.add(this.meta.module, this.doctype);
	}

	setup_side_bar() {
		if (this.hide_sidebar || !sparrow.boot.desk_settings.list_sidebar) return;
		this.list_sidebar = new sparrow.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	get_fields() {
		...
		return this.fields.map((f) => sparrow.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return sparrow.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	refresh() {
		...
		return sparrow.call(args).then((r) => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		...
		Object.assign(sparrow.boot.user_info, data.user_info);
		delete data.user_info;

		data = !Array.isArray(data) ? sparrow.utils.dict(data.keys, data.values) : data;
...
class FilterArea {
	...
	clear(refresh = true) {
		...
		return sparrow.run_serially(promises).then(() => {
			this.trigger_refresh = true;
		});
	}
			...
			doctype_fields
				.filter(
					(df) =>
						...
						(df.in_standard_filter && sparrow.model.is_value_type(df.fieldtype))
				)
				.map((df) => {
	...
	make_filter_list() {
		...
		this.filter_list = new sparrow.ui.FilterGroup({
...
sparrow.views.is_valid = (view_mode) => sparrow.views.view_modes.includes(view_mode);
