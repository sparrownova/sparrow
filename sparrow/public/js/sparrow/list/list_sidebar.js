...
sparrow.views.ListSidebar = class ListSidebar {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		var sidebar_content = sparrow.render_template("list_sidebar", { doctype: this.doctype });
	...
	setup_views() {
		...
		if (this.doctype === "Communication" && sparrow.boot.email_accounts.length) {
			this.sidebar.find('.list-link[data-view="Inbox"]').removeClass("hide");
			show_list_link = true;
		}

		if (sparrow.treeview_settings[this.doctype] || sparrow.get_meta(this.doctype).is_tree) {
			this.sidebar.find(".tree-link").removeClass("hide");
		}

		this.current_view = "List";
		var route = sparrow.get_route();
		if (route.length > 2 && sparrow.views.view_modes.includes(route[2])) {
	...
	setup_kanban_boards() {
		...
		sparrow.views.KanbanView.setup_dropdown_in_sidebar(this.doctype, $dropdown);
	}

	setup_keyboard_shortcuts() {
		this.sidebar.find(".list-link > a, .list-link > .btn-group > a").each((i, el) => {
			sparrow.ui.keys.get_shortcut_group(this.page).add($(el));
		});
	}

	setup_list_group_by() {
		this.list_group_by = new sparrow.views.ListGroupBy({
			doctype: this.doctype,
			sidebar: this,
			list_view: this.list_view,
			page: this.page,
	...
	get_stats() {
		...
		sparrow.call({
			method: "sparrow.desk.reportview.get_sidebar_stats",
			type: "GET",
			args: {
				stats: me.stats,
				doctype: me.doctype,
			...
			callback: function (r) {
				...
				sparrow.utils.setup_search(stats_dropdown, ".stat-link", ".stat-label");
			},
		});
	}
	...
	render_stat(stats) {
		...
		let tag_list = $(sparrow.render_template("list_sidebar_stat", args)).on(
			"click",
			".stat-link",
			(e) => {
	...
	add_insights_banner() {
		try {
			...
			const link = "https://sparrow.io/s/insights";
			const cta = "Sparrow Insights";
