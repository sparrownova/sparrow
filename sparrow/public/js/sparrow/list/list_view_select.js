...
sparrow.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = sparrow.utils.to_title_case(route[2] || "");
		if (route.length > 2 && sparrow.views.view_modes.includes(view_name)) {
	...
	setup_views() {
		const views = {
			...
			Calendar: {
				condition: sparrow.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && sparrow.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(sparrow.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => sparrow.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					sparrow.treeview_settings[this.doctype] ||
					sparrow.get_meta(this.doctype).is_tree,
		...
		sparrow.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => sparrow.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return sparrow.utils.to_title_case(sparrow.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(sparrow.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			sparrow.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						sparrow.set_route("list", doctype, "kanban", board.name);
					} else {
						sparrow.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			sparrow.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			sparrow.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					sparrow.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return sparrow.db
			...
			.then((result) => {
				...
				if (sparrow.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = sparrow.boot.email_accounts;
	...
	slug() {
		return sparrow.router.slug(sparrow.router.doctype_layout || this.doctype);
	}
};
