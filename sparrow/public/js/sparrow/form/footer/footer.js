...
sparrow.ui.form.Footer = class FormFooter {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make() {
		this.wrapper = $(sparrow.render_template("form_footer", {})).appendTo(this.parent);
		this.wrapper.find(".btn-save").click(() => {
			this.frm.save("Save", null, this);
		});
	}
	make_comment_box() {
		this.frm.comment_box = sparrow.ui.form.make_control({
			...
			on_submit: (comment) => {
				if (strip_html(comment).trim() != "" || comment.includes("img")) {
					...
					sparrow
						.xcall("sparrow.desk.form.utils.add_comment", {
							reference_doctype: this.frm.doctype,
							reference_name: this.frm.docname,
							content: comment,
							comment_email: sparrow.session.user,
							comment_by: sparrow.session.user_fullname,
						})
						.then((comment) => {
							let comment_item =
