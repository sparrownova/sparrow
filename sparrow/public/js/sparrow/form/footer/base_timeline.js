...
class BaseTimeline {
	...
	add_action_button(label, action, icon = null, btn_class = null) {
		let icon_element = icon ? sparrow.utils.icon(icon, "xs") : null;
	...
	get_timeline_item(item) {
		...
		if (item.icon) {
			timeline_item.append(`
				<div class="timeline-badge" title='${item.title || sparrow.utils.to_title_case(item.icon)}'>
					${sparrow.utils.icon(item.icon, item.icon_size || "md")}
				</div>
			`);
		...
		timeline_item.append(
			`<div class="timeline-content ${item.is_card ? "sparrow-card" : ""}">`
		);
