...
sparrow.provide("sparrow.model.docinfo");
...
sparrow.ui.form.Controller = class FormController {
	constructor(opts) {
		$.extend(this, opts);
	}
};
...
sparrow.ui.form.Form = class SparrowForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.sections = [];
		this.grids = [];
		this.cscript = new sparrow.ui.form.Controller({ frm: this });
		this.events = {};
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = sparrow.get_doc("DocType Layout", doctype_layout_name);
		this.undo_manager = new UndoManager({ frm: this });
		this.setup_meta(doctype);
		this.debounced_reload_doc = sparrow.utils.debounce(this.reload_doc.bind(this), 1000);
	...
	setup_meta() {
		this.meta = sparrow.get_doc("DocType", this.doctype);

		if (this.meta.istable) {
			this.meta.in_dialog = 1;
		}

		this.perm = sparrow.perm.get_perm(this.doctype); // for create
	...
	setup() {
		this.fields = [];
		this.fields_dict = {};
		this.state_fieldname = sparrow.workflow.get_state_fieldname(this.doctype);
		...
		this.toolbar = new sparrow.ui.form.Toolbar({
			frm: this,
			page: this.page,
		});
		...
		this.script_manager = new sparrow.ui.form.ScriptManager({
			frm: this,
		});
		this.script_manager.setup();
		this.watch_model_updates();

		if (!this.meta.hide_toolbar && sparrow.boot.desk_settings.timeline) {
			// this.footer_tab = new sparrow.ui.form.Tab(this.layout, {
			// 	label: __("Activity"),
			// 	fieldname: 'timeline'
			// });

			this.footer = new sparrow.ui.form.Footer({
				frm: this,
	...
	setup_std_layout() {
		...
		// layout
		this.layout = new sparrow.ui.form.Layout({
			parent: this.body,
		...
		this.dashboard = new sparrow.ui.form.Dashboard(dashboard_parent, this);

		this.tour = new sparrow.ui.form.FormTour({
			frm: this,
		});

		// workflow state
		this.states = new sparrow.ui.form.States({
			frm: this,
		});
	}
	...
	watch_model_updates() {
		...
		// on table fields
		var table_fields = sparrow.get_children("DocType", me.doctype, "fields", {
			fieldtype: ["in", sparrow.model.table_fields],
		});

		// using $.each to preserve df via closure
		$.each(table_fields, function (i, df) {
			sparrow.model.on(df.options, "*", function (fieldname, value, doc) {
	...
	setup_file_drop() {
		...
		this.$wrapper.on("dragenter dragover", false).on("drop", function (e) {
			...
			if (me.doc.__islocal) {
				sparrow.msgprint(__("Please save before attaching."));
				throw "attach error";
			}
		...
		this.fields.map((field) => {
			if (field.df.fieldtype === "Markdown Editor") {
				this.set_df_property(field.df.fieldname, "autocompletions", () => {
					...
					return attachments
						.filter((file) => sparrow.utils.is_image_file(file.file_url))
						.map((file) => {
							return {
	...
	refresh(docname) {
		...
		if (this.docname) {
			...
			// set the doc
			this.doc = sparrow.get_doc(this.doctype, this.docname);

			// check permissions
			this.fetch_permissions();
			if (!this.has_read_permission()) {
				sparrow.show_not_permitted(__(this.doctype) + " " + __(cstr(this.docname)));
				return;
			}
			...
			// read only (workflow)
			this.read_only = sparrow.workflow.is_read_only(this.doctype, this.docname);
			if (this.read_only) {
				this.set_read_only(true);
				sparrow.show_alert(__("This form is not editable due to a Workflow."));
			}
	...
	setup_doctype_actions() {
		if (this.meta.actions) {
			for (let action of this.meta.actions) {
				sparrow.ui.form.on(this.doctype, "refresh", () => {
					if (!this.is_new()) {
						if (!action.hidden) {
	...
	execute_action(action) {
		if (typeof action === "string") {
			...
			if (typeof action === "string") {
				sparrow.throw(`Action ${action} not found`);
			}
		}
		if (action.action_type === "Server Action") {
			return sparrow.xcall(action.action, { doc: this.doc }).then((doc) => {
				if (doc.doctype) {
					// document is returned by the method,
					// apply the changes locally and refresh
					sparrow.model.sync(doc);
					this.refresh();
				}
			...
			return sparrow.set_route(action.action);
		}
	}
	...
	initialize_new_doc() {
		...
		this.script_manager.trigger("before_load", this.doctype, this.docname).then(() => {
			...
			sparrow.after_ajax(function () {
				me.trigger_link_fields();
			});

			sparrow.breadcrumbs.add(me.meta.module, me.doctype);
		});
	...
	render_form(switched) {
		if (!this.meta.istable) {
			...
			if (sparrow.boot.desk_settings.form_sidebar) {
				this.sidebar = new sparrow.ui.form.Sidebar({
					frm: this,
					page: this.page,
				});
				this.sidebar.make();
	...
	run_after_load_hook() {
		if (sparrow.route_hooks.after_load) {
			let route_callback = sparrow.route_hooks.after_load;
			delete sparrow.route_hooks.after_load;
	...
	refresh_header(switched) {
		...
		if (!this.meta.in_dialog || this.in_form) {
			sparrow.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
		}
	...
	validate_and_save(save_action, callback, btn, on_error, resolve, reject) {
		...
		var after_save = function (r) {
			...
			if (!r.exc) {
				if (["Save", "Update", "Amend"].indexOf(save_action) !== -1) {
					sparrow.utils.play_sound("click");
				}

				me.script_manager.trigger("after_save");

				if (sparrow.route_hooks.after_save) {
					let route_callback = sparrow.route_hooks.after_save;
					delete sparrow.route_hooks.after_save;
		...
		if (save_action != "Update") {
			// validate
			sparrow.validated = true;
			sparrow
				.run_serially([
					() => this.script_manager.trigger("validate"),
					() => this.script_manager.trigger("before_save"),
					() => {
						if (!sparrow.validated) {
							fail();
							return;
						}

						sparrow.ui.form.save(me, save_action, after_save, btn);
					},
				])
				.catch(fail);
		} else {
						...
						me.save(
							...
							function (r) {
								if (r.exc) {
									...
									sparrow.utils.play_sound("submit");
									callback && callback();
									me.script_manager
										.trigger("on_submit")
										.then(() => resolve(me))
										.then(() => {
											if (sparrow.route_hooks.after_submit) {
												let route_callback =
													sparrow.route_hooks.after_submit;
												delete sparrow.route_hooks.after_submit;
												route_callback(me);
											}
										});
								}
	...
	savecancel(btn, callback, on_error) {
		...
		sparrow
			.call({
				method: "sparrow.desk.form.linked_with.get_submitted_linked_docs",
				args: {
	...
	_cancel_all(r, btn, callback, on_error) {
		...
		for (let doctype of doctypes) {
			if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
				let docnames = links
					...
					.map((link) => sparrow.utils.get_form_link(link.doctype, link.name, true))
					.join(", ");
		...
		let can_cancel = links.every((link) => sparrow.model.can_cancel(link.doctype));
		if (can_cancel) {
		...
		let d = new sparrow.ui.Dialog(
			{
				...
				fields: [
					{
						fieldtype: "HTML",
						options: `<p class="sparrow-confirm-message">${confirm_message}</p>`,
					},
				],
			},
			() => me.handle_save_fail(btn, on_error)
		...
		if (can_cancel) {
			d.set_primary_action(__("Cancel All"), () => {
				d.hide();
				sparrow.call({
					method: "sparrow.desk.form.linked_with.cancel_all_linked_docs",
					args: {
						docs: links,
			...
			me.script_manager.trigger("before_cancel").then(() => {
				...
				var after_cancel = function (r) {
					if (r.exc) {
						...
						sparrow.utils.play_sound("cancel");
						me.refresh();
						callback && callback();
						me.script_manager.trigger("after_cancel");
					}
				};
				sparrow.ui.form.save(me, "cancel", after_cancel, btn);
			});
		};
	...
	savetrash() {
		...
		sparrow.model.delete_doc(this.doctype, this.docname, function () {
			window.history.back();
		});
	}
	...
	amend_doc() {
		if (!this.fields_dict["amended_from"]) {
			sparrow.msgprint(__('"amended_from" field must be present to do an amendment.'));
			return;
		}

		sparrow
			.xcall("sparrow.client.is_document_amended", {
			...
			.then((is_amended) => {
				...
				var fn = function (newdoc) {
					...
					if (me.fields_dict && me.fields_dict["amendment_date"])
						newdoc.amendment_date = sparrow.datetime.obj_to_str(new Date());
				};
				this.copy_doc(fn, 1);
				sparrow.utils.play_sound("click");
			});
	...
	validate_form_action(action, resolve) {
		...
		var perms = sparrow.perm.get_perm(this.doc.doctype)[0];
		...
		if (
			(sparrow.workflow.is_read_only(this.doctype, this.docname) &&
	...
	show_submit_message() {
		if (
			...
			!this.is_dirty() &&
			!this.is_new() &&
			!sparrow.model.has_workflow(this.doctype) && // show only if no workflow
	...
	fetch_permissions() {
		...
		this.perm = sparrow.perm.get_perm(dt, this.doc);
	}
	...
	check_doctype_conflict(docname) {
		if (this.doctype == "DocType" && docname == "DocType") {
			sparrow.msgprint(__("Allowing DocType, DocType. Be careful!"));
		} else if (this.doctype == "DocType") {
			if (sparrow.views.formview[docname] || sparrow.pages["List/" + docname]) {
				window.location.reload();
				//	sparrow.msgprint(__("Cannot open {0} when its instance is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		} else {
			if (
				sparrow.views.formview.DocType &&
				sparrow.views.formview.DocType.frm.opendocs[this.doctype]
			) {
				window.location.reload();
				//	sparrow.msgprint(__("Cannot open instance when its {0} is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		}
	}
	...
	rename_notify(dt, old, name) {
		...
		// cleanup
		if (this && this.opendocs[old] && sparrow.meta.docfield_copy[dt]) {
			// delete docfield copy
			sparrow.meta.docfield_copy[dt][name] = sparrow.meta.docfield_copy[dt][old];
			delete sparrow.meta.docfield_copy[dt][old];
		}
		...
		sparrow.re_route[sparrow.router.get_sub_path()] = `${encodeURIComponent(
			sparrow.router.slug(this.doctype)
		)}/${encodeURIComponent(name)}`;
		!sparrow._from_link && sparrow.set_route("Form", this.doctype, name);
	}
	...
	print_doc() {
		...
		sparrow.set_route("print", this.doctype, this.doc.name);
	}

	navigate_records(prev) {
		let filters, sort_field, sort_order;
		let list_view = sparrow.get_list_view(this.doctype);
		if (list_view) {
			filters = list_view.get_filters_for_args();
			sort_field = list_view.sort_by;
			sort_order = list_view.sort_order;
		} else {
			let list_settings = sparrow.get_user_settings(this.doctype)["List"];
			if (list_settings) {
		...
		sparrow.call("sparrow.desk.form.utils.get_next", args).then((r) => {
			if (r.message) {
				sparrow.set_route("Form", this.doctype, r.message);
	...
	rename_doc() {
		sparrow.model.rename_doc(this.doctype, this.docname, () => this.refresh_header());
	}
	...
	email_doc(message) {
		new sparrow.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	copy_doc(onload, from_amend) {
		...
		var newdoc = sparrow.model.copy_doc(this.doc, from_amend);
	...
	reload_doc() {
		...
		if (!this.doc.__islocal) {
			sparrow.model.remove_from_locals(this.doctype, this.docname);
			return sparrow.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	has_perm(ptype) {
		return sparrow.perm.has_perm(this.doctype, 0, ptype, this.doc);
	}
	...
	get_docinfo() {
		return sparrow.model.docinfo[this.doctype][this.docname];
	}
	...
	set_footnote(txt) {
		this.footnote_area = sparrow.utils.set_footnote(this.footnote_area, this.body, txt);
	}
	...
	scroll_to_element() {
		if (sparrow.route_options && sparrow.route_options.scroll_to) {
			var scroll_to = sparrow.route_options.scroll_to;
			delete sparrow.route_options.scroll_to;
			...
			if (selector.length) {
				sparrow.utils.scroll_to(selector);
			}
		} else if (window.location.hash) {
			if ($(window.location.hash).length) {
				sparrow.utils.scroll_to(window.location.hash, true, 200, null, null, true);
			} else {
	...
	show_success_action() {
		...
		const success_action = new sparrow.ui.form.SuccessAction(this);
		success_action.show();
	}
	...
	set_currency_labels(fields_list, currency, parentfield) {
		...
		$.each(fields_list, function (i, fname) {
			var docfield = sparrow.meta.docfield_map[doctype][fname];
			if (docfield) {
	...
	field_map(fnames, fn) {
		...
		for (var i = 0, l = fnames.length; i < l; i++) {
			...
			var field = sparrow.meta.get_docfield(this.doctype, fieldname, this.docname);
			if (field) {
				fn(field);
	...
	get_docfield(fieldname1, fieldname2) {
		if (fieldname2) {
			...
			return sparrow.meta.get_docfield(doctype, fieldname2, this.docname);
		} else {
			// for parent
			return sparrow.meta.get_docfield(this.doctype, fieldname1, this.docname);
		}
	}
	...
	set_df_property(fieldname, property, value, docname, table_field, table_row_name = null) {
		...
		if (!docname || !table_field) {
			...
			const filtered_fields = sparrow.utils.filter_dict(grid.docfields, {
				fieldname: table_field,
			});
			if (filtered_fields.length) {
				df = sparrow.meta.get_docfield(
	...
	get_files() {
		return this.attachments
			? sparrow.utils.sort(this.attachments.get_attachments(), "file_name", "string")
			: [];
	}
	...
	clear_table(fieldname) {
		sparrow.model.clear_table(this.doc, fieldname);
	}

	add_child(fieldname, values) {
		var doc = sparrow.model.add_child(
			this.doc,
			sparrow.meta.get_docfield(this.doctype, fieldname).options,
			fieldname
		);
		if (values) {
	...
	set_value(field, value, if_missing, skip_dirty_trigger = false) {
		...
		var _set = function (f, v) {
			...
			if (fieldobj) {
				if (!if_missing || !sparrow.model.has_value(me.doctype, me.doc.name, f)) {
					if (
						sparrow.model.table_fields.includes(fieldobj.df.fieldtype) &&
						$.isArray(v)
					) {
						// set entire child table from specified array as value
						sparrow.model.clear_table(me.doc, fieldobj.df.fieldname);

						const standard_fields = [
							...sparrow.model.std_fields_list,
							...sparrow.model.child_table_field_list,
						];
						v.forEach((d, idx) => {
							let child = sparrow.model.add_child(
								me.doc,
								fieldobj.df.options,
						...
						return sparrow.model.set_value(
							me.doctype,
							me.doc.name,
							f,
							v,
				...
				sparrow.msgprint(__("Field {0} not found.", [f]));
				throw "frm.set_value";
			}
		};
		...
		if (typeof field == "string") {
			...
			return sparrow.run_serially(tasks);
		}
	}
	...
	call(opts, args, callback) {
		...
		if (!opts.doc) {
			if (opts.method.indexOf(".") === -1)
				opts.method = sparrow.model.get_server_module_name(me.doctype) + "." + opts.method;
				...
				if ($.isPlainObject(r.message)) {
					if (opts.child) {
						...
						var std_field_list = ["doctype"]
							.concat(sparrow.model.std_fields_list)
							.concat(sparrow.model.child_table_field_list);
	...
	set_read_only() {
		const docperms = sparrow.perm.get_perm(this.doc.doctype);
	...
	get_formatted(fieldname) {
		return sparrow.format(
			this.doc[fieldname],
			sparrow.meta.get_docfield(this.doctype, fieldname, this.docname),
			{ no_icon: true },
			this.doc
		);
	}
	...
	open_grid_row() {
		return sparrow.ui.form.get_open_grid_form();
	}
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		sparrow.meta.get_table_fields(this.doctype).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		var doctype;
		if (sparrow.meta.docfield_map[this.doctype][fieldname]) {
			doctype = this.doctype;
		} else {
			sparrow.meta.get_table_fields(this.doctype).every(function (df) {
				if (sparrow.meta.docfield_map[df.options][fieldname]) {
					doctype = df.options;
					return false;
				} else {
					return true;
		...
		sparrow.meta.docfield_map[doctype][fieldname].formatter = function (
			value,
			df,
			options,
			doc
			...
			if (value) {
				...
				if (get_text) {
					label = get_text(doc);
				} else if (sparrow.form.link_formatters[df.options]) {
					label = sparrow.form.link_formatters[df.options](value, doc);
				} else {
					label = value;
				}
				...
				return `
						<a class="indicator ${get_color(doc || {})}"
							href="/app/${sparrow.router.slug(df.options)}/${escaped_name}"
							data-doctype="${df.options}"
							data-name="${sparrow.utils.escape_html(value)}">
							${label}
						</a>
					`;
			} else {
	...
	can_create(doctype) {
		...
		if (!sparrow.model.can_create(doctype)) {
			return false;
		}
	...
	make_new(doctype) {
		...
		if (this.make_methods && this.make_methods[doctype]) {
			...
			sparrow.model.with_doctype(doctype, function () {
				let new_doc = sparrow.model.get_new_doc(doctype, null, null, true);

				// set link fields (if found)
				me.set_link_field(doctype, new_doc);

				sparrow.ui.form.make_quick_entry(doctype, null, null, new_doc);
				// sparrow.set_route('Form', doctype, new_doc.name);
			});
		}
	}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		sparrow.get_meta(doctype).fields.forEach(function (df) {
	...
	update_in_all_rows(table_fieldname, fieldname, value) {
		...
		sparrow.model
			...
			.filter((child) => !sparrow.model.has_value(child.doctype, child.name, fieldname))
			.forEach((child) =>
				sparrow.model.set_value(child.doctype, child.name, fieldname, value)
			);
	}
	...
	scroll_to_field(fieldname, focus = true) {
		...
		// scroll to input
		sparrow.utils.scroll_to($el, true, 15);
		...
		let control_element = $el.closest(".sparrow-control");
	...
	setup_docinfo_change_listener() {
		...
		if (this.doc && !this.is_new()) {
			sparrow.socketio.doc_subscribe(doctype, docname);
		}
		sparrow.realtime.off("docinfo_update");
		sparrow.realtime.on("docinfo_update", ({ doc, key, action = "update" }) => {
			if (
			...
			let doc_list = sparrow.model.docinfo[doctype][docname][key] || [];
			let docindex = doc_list.findIndex((old_doc) => {
				return old_doc.name === doc.name;
			});

			if (action === "add") {
				sparrow.model.docinfo[doctype][docname][key].push(doc);
			}
			if (docindex > -1) {
				if (action === "update") {
					sparrow.model.docinfo[doctype][docname][key].splice(docindex, 1, doc);
				}
				if (action === "delete") {
					sparrow.model.docinfo[doctype][docname][key].splice(docindex, 1);
				}
			}
			...
			if (
				!(
					...
					doc.doctype === "Comment" &&
					doc.owner === sparrow.session.user
				)
			) {
	...
	set_fields_as_options(
		...
		return new Promise((resolve) => {
			sparrow.model.with_doctype(reference_doctype, () => {
				sparrow.get_meta(reference_doctype).fields.map((df) => {
					filter_function(df) &&
	...
	get_involved_users() {
		...
		return involved_users
			.uniqBy((u) => u)
			.filter((user) => !["Administrator", sparrow.session.user].includes(user))
			.filter(Boolean);
	}
};

sparrow.validated = 0;
// Proxy for sparrow.validated
Object.defineProperty(window, "validated", {
	get: function () {
		console.warn(
			"Please use `sparrow.validated` instead of `validated`. It will be deprecated soon."
		); // eslint-disable-line
		return sparrow.validated;
	},
	set: function (value) {
		console.warn(
			"Please use `sparrow.validated` instead of `validated`. It will be deprecated soon."
		); // eslint-disable-line
		sparrow.validated = value;
		return sparrow.validated;
	},
});
