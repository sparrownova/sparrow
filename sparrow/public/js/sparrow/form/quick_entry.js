...
sparrow.quick_edit = function (doctype, name) {
	sparrow.db.get_doc(doctype, name).then((doc) => {
		sparrow.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

sparrow.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (sparrow.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	sparrow.quick_entry = new sparrow.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return sparrow.quick_entry.setup();
};

sparrow.ui.form.QuickEntryForm = class QuickEntryForm {
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = sparrow.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.dialog.onhide = () => (sparrow.quick_entry = null);
		this.dialog.show();
	...
	insert() {
		...
		return new Promise((resolve) => {
			me.update_doc();
			sparrow.call({
				method: "sparrow.client.save",
				args: {
					doc: me.dialog.doc,
				},
				callback: function (r) {
					if (sparrow.model.is_submittable(me.doctype)) {
						...
						me.dialog.hide();
						// delete the old doc
						sparrow.model.clear_doc(me.dialog.doc.doctype, me.dialog.doc.name);
						me.dialog.doc = r.message;
						if (sparrow._from_link) {
							sparrow.ui.form.update_calling_link(me.dialog.doc);
						} else {
							if (me.after_insert) {
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			sparrow.run_serially([() => sparrow.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			sparrow.route_options = sparrow.route_options || {};
			sparrow.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		sparrow.set_route("Form", this.doctype, this.doc.name);
	}
