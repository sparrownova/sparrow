...
sparrow.ui.form.LinkSelector = class LinkSelector {
	constructor(opts) {
		...
		if (this.doctype != "[Select]") {
			sparrow.model.with_doctype(this.doctype, function (r) {
				me.make();
			});
		} else {
			this.make();
				...
				if (r.values.length) {
					...
					$(
						'<p><br><span class="text-muted">' +
							__("No Results") +
							"</span>" +
							(sparrow.model.can_create(me.doctype)
	...
	set_in_grid(value) {
		return new Promise((resolve) => {
			if (this.qty_fieldname) {
						...
						if (!updated) {
							...
							sparrow.run_serially([
								...
								() => {
									...
									return sparrow.model.set_value(d.doctype, d.name, args);
								},
								() => sparrow.show_alert(__("Added {0} ({1})", [value, data.qty])),
								() => resolve(),
							]);
						}
					},
				...
				sparrow.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					sparrow.show_alert(__("{0} {1} added", [this.dynamic_link_reference, value]));
					resolve();
				});
			} else {
				let d = this.target.add_new_row();
				sparrow.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					sparrow.show_alert(__("{0} added", [value]));
					resolve();
				});
			}
		});
...
sparrow.link_search = function (doctype, args, callback, btn) {
	if (!args) {
		args = {
			txt: "",
		};
