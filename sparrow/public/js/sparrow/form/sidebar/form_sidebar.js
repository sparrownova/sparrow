...
sparrow.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make() {
		var sidebar_content = sparrow.render_template("form_sidebar", {
		...
		sparrow.ui.form.setup_user_image_event(this.frm);
	...
	bind_events() {
		...
		this.comments.on("click", function () {
			sparrow.utils.scroll_to(me.frm.footer.wrapper.find(".comment-box"), true);
		});

		this.like_icon.on("click", function () {
			sparrow.ui.toggle_like(me.like_wrapper, me.frm.doctype, me.frm.doc.name, function () {
				me.refresh_like();
			});
		});
	}
	...
	setup_keyboard_shortcuts() {
		...
		sparrow.ui.keys.get_shortcut_group(this.page).add(assignment_link);
	}
	...
	refresh() {
		if (this.frm.doc.__islocal) {
			...
			this.sidebar
				...
				.html(
					__(
						"{0} edited this {1}",
						[
							sparrow.user.full_name(this.frm.doc.modified_by).bold(),
			...
			this.sidebar
				...
				.html(
					__(
						"{0} created this {1}",
						[
							sparrow.user.full_name(this.frm.doc.owner).bold(),
			...
			sparrow.ui.form.set_user_image(this.frm);
		}
	}
			...
			sparrow.call({
				...
				callback: function (res) {
					...
					me.sidebar.find(".auto-repeat-status").on("click", function () {
						sparrow.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				},
			});
		}
	...
	make_tags() {
		...
		this.frm.tags = new sparrow.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new sparrow.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new sparrow.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new sparrow.ui.form.Share({
			frm: this.frm,
	...
	make_like() {
		...
		sparrow.ui.setup_like_popover(this.sidebar.find(".form-stats-likes"), ".like-icon");
	}
	...
	make_follow() {
		...
		this.follow_button.on("click", () => {
			...
			sparrow
				.call("sparrow.desk.form.document_follow.update_follow", {
					doctype: this.frm.doctype,
					doc_name: this.frm.doc.name,
					following: !is_followed,
				})
				.then(() => {
					sparrow.model.set_docinfo(
						this.frm.doctype,
						this.frm.doc.name,
	...
	refresh_like() {
		...
		const liked = sparrow.ui.is_liked(this.frm.doc);
		this.like_wrapper
	...
	make_review() {
		...
		if (sparrow.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new sparrow.ui.form.Review({
	...
	reload_docinfo(callback) {
		sparrow.call({
			method: "sparrow.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,
