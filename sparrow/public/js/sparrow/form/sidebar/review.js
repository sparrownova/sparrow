...
sparrow.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = sparrow.boot.points;
	...
	update_points() {
		return sparrow
			.xcall("sparrow.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: sparrow.session.user,
			})
			.then((data) => {
				sparrow.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new sparrow.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return sparrow.msgprint(__("You do not have enough points"));
				}
				sparrow
					.xcall("sparrow.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = sparrow.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${sparrow.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${sparrow.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${sparrow.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${sparrow.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
