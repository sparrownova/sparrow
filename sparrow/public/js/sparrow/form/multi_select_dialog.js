sparrow.ui.form.MultiSelectDialog = class MultiSelectDialog {
	constructor(opts) {
		...
		if (!this.for_select) {
			sparrow.model.with_doctype(this.doctype, () => this.init());
		} else {
			this.init();
		}
	}
	...
	make_new_document(e) {
		...
		if (e) {
			...
			sparrow.new_doc(this.doctype, true);
		}
	}
	...
	setup_child_datatable() {
		...
		this.child_datatable = new sparrow.DataTable(this.$child_wrapper.get(0), {
	...
	get_primary_filters() {
		...
		if ($.isArray(this.setters)) {
			...
			Object.keys(this.setters).forEach((setter, index) => {
				let df_prop = sparrow.meta.docfield_map[this.doctype][setter];
		...
		if (this.allow_child_item_selection) {
			this.child_doctype = sparrow.meta.get_docfield(
				this.doctype,
	...
	bind_events() {
		...
		this.$parent.find(".input-with-feedback").on("change", () => {
			sparrow.flags.auto_scroll = false;
			if (this.is_child_selection_enabled()) {
				this.show_child_results();
			} else {
				this.get_results();
				...
				${
					head
						? `<span class="ellipsis text-muted" title="${__(
								sparrow.model.unscrub(column)
						  )}">${__(sparrow.model.unscrub(column))}</span>`
						: column !== "name"
	...
	render_result_list(results, more = 0, empty = true) {
		...
		if (!sparrow.flags.auto_scroll && empty) {
			this.empty_list();
		}
		more_btn.hide();
	...
	async perform_search(args) {
		const res = await sparrow.call({
			type: "GET",
			method: "sparrow.desk.search.search_widget",
			no_spinner: true,
			args: args,
		});
