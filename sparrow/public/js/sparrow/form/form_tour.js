sparrow.ui.form.FormTour = class FormTour {
	constructor({ frm }) {
		this.frm = frm;
		this.driver_steps = [];
	}
	...
	init_driver() {
		...
		sparrow.router.on("change", () => this.driver.reset());
	...
	async init({ tour_name, on_finish }) {
		if (tour_name) {
			...
			const doctype_tour_exists = await sparrow.db.exists("Form Tour", this.frm.doctype);
			if (doctype_tour_exists) {
				this.tour = await sparrow.db.get_doc("Form Tour", this.frm.doctype);
			} else {
				this.tour = { steps: sparrow.tour[this.frm.doctype] };
			}
		}
	...
	get_step(step_info, on_next, on_prev) {
		...
		let element = `.sparrow-control[data-fieldname='${fieldname}']`;
		...
		if (is_table_field) {
			...
			element = `.grid-row-open .sparrow-control[data-fieldname='${fieldname}']`;
		}

		return {
			element,
			name,
			popover: { title, description, position: sparrow.router.slug(position || "Bottom") },
			onNext: on_next,
			onPrevious: on_prev,
		};
	}
	...
	add_new_row_step(step) {
		const $add_row = `.sparrow-control[data-fieldname='${step.fieldname}'] .grid-add-row`;
		const add_row_step = {
			element: $add_row,
	...
	expand_row_and_proceed(step, start_from) {
		...
		sparrow.utils.sleep(300).then(() => this.driver.start(start_from));
	}
	...
	open_first_row_of(fieldname) {
		...
		$($close_row).one("click", () => {
			...
			sparrow.utils.scroll_to(next_element, true, 150, null, () => {
				this.driver.moveNext();
				sparrow.flags.disable_auto_scroll = false;
			});
			sparrow.flags.disable_auto_scroll = true;
		});
	}
