...
sparrow.ui.form.Toolbar = class Toolbar {
	constructor(opts) {
		$.extend(this, opts);
		this.refresh();
	...
	set_title() {
		if (this.frm.is_new()) {
			...
			if (
				...
				this.page.$sub_title_area.on("click", (event) => {
					...
					sparrow.utils.copy_to_clipboard(this.frm.docname);
				});
			}
		} else {
		...
		if (this.frm.meta.title_field) {
			sparrow.utils.set_title(title + " - " + this.frm.docname);
		}
	...
	rename_document_title(input_name, input_title, merge = false) {
		...
		let rename_document = () => {
			return sparrow
				.xcall("sparrow.model.rename_doc.update_document_title", {
					doctype,
					docname,
					name: input_name,
					title: input_title,
				...
				.then((new_docname) => {
					...
					if (input_name && new_docname == docname) {
						sparrow.socketio.doc_subscribe(doctype, input_name);
						sparrow.realtime.on("doc_update", (data) => {
	...
	set_indicator() {
		var indicator = sparrow.get_indicator(this.frm.doc);
		if (
	...
	make_viewers() {
		...
		this.frm.viewers = new sparrow.ui.form.FormViewers({
			frm: this.frm,
	...
	make_menu_items() {
		...
		const is_submittable = sparrow.model.is_submittable(this.frm.doc.doctype);
		...
		if (
			...
			if (sparrow.model.can_print(null, me.frm) && !this.frm.meta.issingle) {
		...
		// email
		if (sparrow.model.can_email(null, me.frm) && me.frm.doc.docstatus < 2) {
			this.page.add_menu_item(
				__("Email"),
				function () {
					me.frm.email_doc();
		...
		// duplicate
		if (in_list(sparrow.boot.user.can_create, me.frm.doctype) && !me.frm.meta.allow_copy) {
			this.page.add_menu_item(
				__("Duplicate"),
				function () {
					me.frm.copy_doc();
		...
		this.page.add_menu_item(
			...
			function () {
				sparrow.utils.copy_to_clipboard(JSON.stringify(me.frm.doc));
			},
			true
		);
		...
		if (
			...
			sparrow.model.can_delete(me.frm.doctype)
		) {
			this.page.add_menu_item(
				__("Delete"),
				function () {
		...
		if (this.can_repeat()) {
			this.page.add_menu_item(
				__("Repeat"),
				function () {
					sparrow.utils.new_auto_repeat_prompt(me.frm);
				},
				true
			);
		}
		...
		if (p[CREATE] && !this.frm.meta.issingle && !this.frm.meta.in_create) {
			this.page.add_menu_item(
				...
				function () {
					sparrow.new_doc(me.frm.doctype, true);
				},
				true,
				{
					shortcut: "Ctrl+B",
	...
	make_customize_buttons() {
		...
		if (sparrow.model.can_create("DocType")) {
			if (sparrow.boot.developer_mode === 1 && !is_doctype_form) {
				// edit doctype
				this.page.add_menu_item(
					__("Edit DocType"),
					() => {
						sparrow.set_route("Form", "DocType", this.frm.doctype);
					},
					true
				);
			}
	...
	has_workflow() {
		if (this._has_workflow === undefined)
			this._has_workflow = sparrow.get_list("Workflow", {
	...
	show_linked_with() {
		if (!this.frm.linked_with) {
			this.frm.linked_with = new sparrow.ui.form.LinkedWith({
				frm: this.frm,
			});
		}
	...
	set_page_actions(status) {
		...
		if (status === "Edit") {
			...
			if (this.has_workflow()) {
				sparrow
					.xcall("sparrow.model.workflow.can_cancel_document", {
