...
sparrow.ui.form.ControlTable = class ControlTable extends sparrow.ui.form.Control {
	make() {
		super.make();
		...
		this.$wrapper.on("paste", ":text", (e) => {
			...
			let data = sparrow.utils.csv_to_array(pasted_data, "\t");
	...
	get_field(field_name) {
		...
		this.grid.meta.fields.some((field) => {
			if (sparrow.model.no_value_type.includes(field.fieldtype)) {
				return false;
			}
