sparrow.provide("sparrow.utils.utils");

sparrow.ui.form.ControlGeolocation = class ControlGeolocation extends sparrow.ui.form.ControlData {
	...
	set_disp_area(value) {
		...
		this.map_id = sparrow.dom.get_unique_id();
		this.map_area = $(
		...
		if (this.frm) {
			...
			$(document).on("sparrow.ui.Dialog:shown", () => {
				this.make_map(value);
			});
		}
	}
	...
	bind_leaflet_map() {
		...
		L.Icon.Default.imagePath = "/assets/sparrow/images/leaflet/";
		this.map = L.map(this.map_id);
		this.map.setView(sparrow.utils.map_defaults.center, sparrow.utils.map_defaults.zoom);

		L.tileLayer(sparrow.utils.map_defaults.tiles, sparrow.utils.map_defaults.options).addTo(
			this.map
		);
	...
	bind_leaflet_draw_control() {
		if (
			!sparrow.perm.has_perm(this.doctype, this.df.permlevel, "write", this.doc) ||
			this.df.read_only
		) {
			return;
		}
	...
	get_leaflet_controls() {
		return new L.Control.Draw({
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: sparrow.ui.color.get("blue"),
						weight: 10,
					},
				},
				polygon: {
					...
					drawError: {
						color: sparrow.ui.color.get("orange"), // Color the shape will turn when intersects
