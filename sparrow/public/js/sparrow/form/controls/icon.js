...
sparrow.ui.form.ControlIcon = class ControlIcon extends sparrow.ui.form.ControlData {
	make_input() {
	...
	get_all_icons() {
		sparrow.symbols = [];
		$("#sparrow-symbols > symbol[id]").each(function () {
			this.id.includes("icon-") && sparrow.symbols.push(this.id.replace("icon-", ""));
		});
	}
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(
				`<div class="selected-icon">${sparrow.utils.icon("folder-normal", "md")}</div>`
			);
