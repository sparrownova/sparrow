sparrow.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();

		// if developer_mode=1, show fieldname as tooltip
		if (sparrow.boot.user && sparrow.boot.developer_mode === 1 && this.$wrapper) {
	...
	make_wrapper() {
		this.$wrapper = $("<div class='sparrow-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = sparrow.perm.get_field_display_status(
			this.df,
			sparrow.model.get_doc(this.doctype, this.docname),
		...
		let value = sparrow.model.get_value(this.doctype, this.docname, this.df.fieldname);
	...
	show_translatable_button(value) {
		...
		if (
			...
			!sparrow.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!sparrow.views || !sparrow.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new sparrow.views.TranslationManager({
						df: this.df,
