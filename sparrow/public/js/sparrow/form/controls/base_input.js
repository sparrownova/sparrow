sparrow.ui.form.ControlInput = class ControlInput extends sparrow.ui.form.Control {
	...
	make_wrapper() {
		if (this.only_input) {
			this.$wrapper = $('<div class="form-group sparrow-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $(
				`<div class="sparrow-control">
	...
	refresh_input() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if (me.frm) {
				me.value = sparrow.model.get_value(me.doctype, me.docname, me.df.fieldname);
			} else if (me.doc) {
				...
				if (is_fetch_from_read_only) {
					$(me.disp_area).attr(
						...
						__(
							...
							me.df.fetch_from.split(".").map((value) => __(sparrow.unscrub(value)))
						)
					);
				}
			}
	...
	set_disp_area(value) {
		...
		if (this.df.fieldtype === "Data") {
			value = sparrow.utils.escape_html(value);
		}
		let doc = this.doc || (this.frm && this.frm.doc);
		let display_value = sparrow.format(value, this.df, { no_icon: true, inline: true }, doc);
