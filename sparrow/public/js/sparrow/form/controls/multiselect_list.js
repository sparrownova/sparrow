sparrow.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	sparrow.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${sparrow.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
