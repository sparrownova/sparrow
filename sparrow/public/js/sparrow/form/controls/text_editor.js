...
sparrow.ui.form.ControlTextEditor = class ControlTextEditor extends sparrow.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			sparrow.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = sparrow.ui.keys && sparrow.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: sparrow.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "sparrow.desk.search.get_names_for_mentions";
				let values = await sparrow.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? sparrow.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return sparrow.dom.remove_script_and_style(value);
	}
