sparrow.provide("sparrow.phone_call");

sparrow.ui.form.ControlData = class ControlData extends sparrow.ui.form.ControlInput {
	...
	make_input() {
		...
		this.$input.on("paste", (e) => {
			...
			if (maxlength && pasted_data.length > maxlength) {
				...
				if (this.frm && sparrow.model.can_write(this.frm.doctype)) {
					let doctype_edit_link = null;
					if (this.frm.meta.custom) {
						doctype_edit_link = sparrow.utils.get_form_link(
							"DocType",
							this.frm.doctype,
							true,
							__("this form")
		...
		this.$wrapper.find(".control-input").append(
			`<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Open Link")}" target="_blank">
					${sparrow.utils.icon("link-url", "sm")}
				</a>
			</span>`
		);
		...
		if (this.df.with_copy_button) {
			this.$wrapper
				...
				.append(
					...
					${sparrow.utils.icon("clipboard", "sm")}
				</button>`
				)
				.find(".action-btn")
				.click(() => {
					sparrow.utils.copy_to_clipboard(this.value);
				});
		}
	}
	...
	bind_change_event() {
		...
		if (this.constructor.trigger_change_on_input_event && !this.in_grid()) {
			...
			this.$input.on("input", sparrow.utils.debounce(change_handler, 500));
		}
	}
	setup_autoname_check() {
		if (!this.df.parent) return;
		this.meta = sparrow.get_meta(this.df.parent);
		if (
			this.meta &&
			((this.meta.autoname &&
	...
	validate(v) {
		...
		if (this.df.options == "Phone") {
			...
			var email_list = sparrow.utils.split_emails(v);
			if (!email_list) {
				return "";
			} else {
...
sparrow.ui.form.ControlReadOnly = sparrow.ui.form.ControlData;
