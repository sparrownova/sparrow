sparrow.ui.form.FormViewers = class FormViewers {
	...
	refresh() {
		...
		let currently_viewing = users.current.filter((user) => user != sparrow.session.user);
		let avatar_group = sparrow.avatar_group(currently_viewing, 5, {
			align: "left",
			overlap: true,
		});
...
sparrow.ui.form.FormViewers.set_users = function (data, type) {
	const doctype = data.doctype;
	const docname = data.docname;
	const docinfo = sparrow.model.get_docinfo(doctype, docname);
	...
	const set_and_refresh = () => {
		...
		sparrow.model.set_docinfo(doctype, docname, type, info);
	...
	for (let user of users) {
		if (!sparrow.boot.user_info[user]) unknown_users.push(user);
	}
	...
	if (unknown_users.length === 0) {
		...
		sparrow
			.xcall("sparrow.desk.form.load.get_user_info_for_viewers", { users: unknown_users })
			.then((data) => {
				Object.assign(sparrow.boot.user_info, data);
				set_and_refresh();
			});
	}
};
