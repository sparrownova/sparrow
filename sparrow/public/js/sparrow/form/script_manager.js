...
sparrow.provide("sparrow.ui.form.handlers");
...
sparrow.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!sparrow.ui.form.handlers[doctype]) {
		sparrow.ui.form.handlers[doctype] = {};
	}
	if (!sparrow.ui.form.handlers[doctype][fieldname]) {
		sparrow.ui.form.handlers[doctype][fieldname] = [];
	}
	return sparrow.ui.form.handlers[doctype][fieldname];
};

sparrow.ui.form.on = sparrow.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = sparrow.ui.form.get_event_handler_list(doctype, fieldname);
...
sparrow.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = sparrow.ui.form.get_event_handler_list(doctype, fieldname);
...
sparrow.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

sparrow.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = sparrow.get_doc(doctype, name);
	...
	get_handlers(event_name, doctype) {
		...
		if (sparrow.ui.form.handlers[doctype] && sparrow.ui.form.handlers[doctype][event_name]) {
			$.each(sparrow.ui.form.handlers[doctype][event_name], function (i, fn) {
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (sparrow.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					sparrow.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && sparrow.dom.set_style(doctype.__css);

		this.trigger("setup");
	}
	log_error(caller, e) {
		sparrow.show_alert({ message: __("Error in Client Script."), indicator: "error" });
