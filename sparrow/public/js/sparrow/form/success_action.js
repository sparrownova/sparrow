...
sparrow.provide("sparrow.success_action");

sparrow.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = sparrow.boot.success_action.find(
	...
	show_alert() {
		sparrow.db.get_list(this.form.doctype, { limit: 2 }).then((result) => {
	...
	get default_actions() {
		return {
			new: {
				label: __("New"),
				action: (frm) => sparrow.new_doc(frm.doctype),
			},
			print: {
				label: __("Print"),
			...
			list: {
				label: __("View All"),
				action: (frm) => {
					sparrow.set_route("List", frm.doctype);
				},
			},
		};
	}
