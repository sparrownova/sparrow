...
sparrow.provide("sparrow.form.formatters");

sparrow.form.link_formatters = {};

sparrow.form.formatters = {
	...
	_apply_custom_formatter: function (value, df) {
		...
		example:
			sparrow.meta.docfield_map[df.parent][df.fieldname].formatter = (value) => {
		...
		*/
		...
		if (df) {
			const std_df =
				sparrow.meta.docfield_map[df.parent] &&
				sparrow.meta.docfield_map[df.parent][df.fieldname];
	...
	Data: function (value, df) {
		...
		return sparrow.form.formatters._apply_custom_formatter(value, df);
	},
	Autocomplete: function (value, df) {
		return __(sparrow.form.formatters["Data"](value, df));
	},
	...
	Float: function (value, docfield, options, doc) {
		...
		var precision =
			docfield.precision ||
			cint(sparrow.boot.sysdefaults && sparrow.boot.sysdefaults.float_precision) ||
			null;
		if (docfield.options && docfield.options.trim()) {
			...
			return sparrow.form.formatters.Currency(value, docfield, options, doc);
		} else {
			// show 1.000000 as 1
			...
			return sparrow.form.formatters._right(format_number(value, null, precision), options);
		}
	},
	Int: function (value, docfield, options) {
		return sparrow.form.formatters._right(value == null ? "" : cint(value), options);
	},
	Percent: function (value, docfield, options) {
		const precision =
			...
			cint(sparrow.boot.sysdefaults && sparrow.boot.sysdefaults.float_precision) ||
			2;
		return sparrow.form.formatters._right(flt(value, precision) + "%", options);
	},
	...
	Currency: function (value, docfield, options, doc) {
		var currency = sparrow.meta.get_field_currency(docfield, doc);
		...
		if (typeof docfield.precision == "number") {
			...
			precision = cint(
				docfield.precision || sparrow.boot.sysdefaults.currency_precision || 2
			);
		}
		...
		if (precision > 2) {
			...
			if (decimals.length < 3 || decimals.length < precision) {
				const fraction =
					sparrow.model.get_value(":Currency", currency, "fraction_units") || 100; // if not set, minimum 2.
		...
		if (options && options.only_value) {
			...
			return sparrow.form.formatters._right(value, options);
		}
	},
	...
	Link: function (value, docfield, options, doc) {
		...
		let link_title = sparrow.utils.get_link_title(doctype, value);
		...
		if (sparrow.form.link_formatters[doctype]) {
			// don't apply formatters in case of composite (parent field of same type)
			if (doc && doctype !== doc.doctype) {
				value = sparrow.form.link_formatters[doctype](value, doc, docfield);
			}
		}
	...
	Date: function (value) {
		if (!sparrow.datetime.str_to_user) {
			return value;
		}
		if (value) {
			value = sparrow.datetime.str_to_user(value, false, true);
			// handle invalid date
	...
	DateRange: function (value) {
		if (Array.isArray(value)) {
			return __("{0} to {1}", [
				sparrow.datetime.str_to_user(value[0]),
				sparrow.datetime.str_to_user(value[1]),
			]);
		} else {
			return value || "";
		}
	...
	Datetime: function (value) {
		if (value) {
			return moment(sparrow.datetime.convert_to_user_tz(value)).format(
				sparrow.boot.sysdefaults.date_format.toUpperCase() +
					" " +
					(sparrow.boot.sysdefaults.time_format || "HH:mm:ss")
			);
		} else {
			return "";
		}
	...
	Text: function (value, df) {
		if (value) {
			...
			if (!match) {
				value = sparrow.utils.replace_newlines(value);
			}
		}

		return sparrow.form.formatters.Data(value, df);
	},
	Time: function (value) {
		if (value) {
			value = sparrow.datetime.str_to_user(value, true);
		}
	...
	Duration: function (value, docfield) {
		if (value) {
			let duration_options = sparrow.utils.get_duration_options(docfield);
			value = sparrow.utils.get_formatted_duration(value, duration_options);
		}
	...
	SmallText: function (value) {
		return sparrow.form.formatters.Text(value);
	},
	TextEditor: function (value) {
		let formatted_value = sparrow.form.formatters.Text(value);
	...
	WorkflowState: function (value) {
		var workflow_state = sparrow.get_doc("Workflow State", value);
		if (workflow_state) {
			return repl(
	...
	TableMultiSelect: function (rows, df, options) {
		...
		const formatted_values = rows.map((row) => {
			...
			return sparrow.format(value, link_field, options, row);
		});
	...
	Icon: (value) => {
		return value
			? `<div>
			<div class="selected-icon">${sparrow.utils.icon(value, "md")}</div>
...
sparrow.form.get_formatter = function (fieldtype) {
	if (!fieldtype) fieldtype = "Data";
	return sparrow.form.formatters[fieldtype.replace(/ /g, "")] || sparrow.form.formatters.Data;
};

sparrow.format = function (value, df, options, doc) {
	...
	var formatter = df.formatter || sparrow.form.get_formatter(fieldtype);

	var formatted = formatter(value, df, options, doc);

	if (typeof formatted == "string") formatted = sparrow.dom.remove_script_and_style(formatted);

	return formatted;
};

sparrow.get_format_helper = function (doc) {
	var helper = {
		get_formatted: function (fieldname) {
			var df = sparrow.meta.get_docfield(doc.doctype, fieldname);
			if (!df) {
				console.log("fieldname not found: " + fieldname);
			}
			return sparrow.format(doc[fieldname], df, { inline: 1 }, doc);
		},
	};
	$.extend(helper, doc);
	return helper;
...
sparrow.form.link_formatters["User"] = function (value, doc, docfield) {
