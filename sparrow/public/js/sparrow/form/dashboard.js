...
sparrow.ui.form.Dashboard = class FormDashboard {
	...
	setup_dashboard_sections() {
		...
		this.heatmap_area = this.make_section({
			...
			is_dashboard_section: 1,
			body_html: `
				<div id="heatmap-${sparrow.model.scrub(this.frm.doctype)}" class="heatmap"></div>
	...
	refresh() {
		this.reset();
		if (this.frm.doc.__islocal || !sparrow.boot.desk_settings.dashboard) {
			return;
		}
	...
	render_links() {
		...
		$(sparrow.render_template("form_links", this.data)).appendTo(transactions_area_body);
	...
	render_report_links() {
		...
		if (this.data.reports && this.data.reports.length) {
			$(sparrow.render_template("report_links", this.data)).appendTo(parent);
			// bind reports
	...
	open_report($link) {
		...
		sparrow.provide("sparrow.route_options");
		sparrow.route_options[fieldname] = this.frm.doc.name;
		sparrow.set_route("query-report", report);
	}
	...
	open_document_list($link, show_open) {
		...
		if (
			...
			if (names.length) {
				sparrow.route_options = { name: ["in", names] };
			} else {
				return false;
			}
		} else if (this.data.fieldname) {
			sparrow.route_options = this.get_document_filter(doctype);
			if (show_open && sparrow.ui.notifications) {
				sparrow.ui.notifications.show_open_count_list(doctype);
			}
		}

		sparrow.set_route("List", doctype, "List");
	}
	...
	set_open_count() {
		...
		let method = this.data.method || "sparrow.desk.notifications.get_open_count";
		sparrow.call({
			type: "GET",
			method: method,
			args: {
	...
	// heatmap
	render_heatmap() {
		this.heatmap = new sparrow.Chart("#heatmap-" + sparrow.model.scrub(this.frm.doctype), {
			type: "heatmap",
	...
	render_graph(args) {
		...
		this.chart = new sparrow.Chart(".form-graph", args);
		if (!this.chart) {
			this.hide();
		}
	}
