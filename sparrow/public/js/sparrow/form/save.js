...
sparrow.ui.form.save = function (frm, action, callback, btn) {
	...
	var save = function () {
		...
		if ((action !== "Save" || frm.is_dirty()) && check_mandatory()) {
			_call({
				method: "sparrow.desk.form.save.savedocs",
			...
			!frm.is_dirty() &&
				sparrow.show_alert({ message: __("No changes in document"), indicator: "orange" });
	...
	var remove_empty_rows = function () {
		...
		*/
		const docs = sparrow.model.get_all_docs(frm.doc);

		// we should only worry about table data
		const tables = docs.filter((d) => {
			return sparrow.model.is_table(d.doctype);
		});

		let modified_table_fields = [];

		tables.map((doc) => {
			const cells = sparrow.meta.docfield_list[doc.doctype] || [];
			...
			if (is_empty_row(in_list_view_cells)) {
				sparrow.model.clear_doc(doc.doctype, doc.name);
	...
	var cancel = function () {
		...
		var workflow_state_fieldname = sparrow.workflow.get_state_fieldname(frm.doctype);
		...
		_call({
			method: "sparrow.desk.form.save.cancel",
			args: args,
			callback: function (r) {
				$(document).trigger("save", [frm.doc]);
				callback(r);
	...
	var check_mandatory = function () {
		...
		$.each(sparrow.model.get_all_docs(frm.doc), function (i, doc) {
			var error_fields = [];
			var folded = false;

			$.each(sparrow.meta.docfield_list[doc.doctype] || [], function (i, docfield) {
				if (docfield.fieldname) {
					const df = sparrow.meta.get_docfield(doc.doctype, docfield.fieldname, doc.name);
					...
					if (
						...
						!sparrow.model.has_value(doc.doctype, doc.name, df.fieldname)
					) {
						has_errors = true;
			...
			if (error_fields.length) {
				...
				if (meta.istable) {
					const table_field = sparrow.meta.docfield_map[doc.parenttype][doc.parentfield];

					const table_label = __(
						table_field.label || sparrow.unscrub(table_field.fieldname)
					).bold();
	...
	let is_docfield_mandatory = function (doc, df) {
		...
		let parent = sparrow.get_meta(df.parent);
		...
		if (typeof expression === "boolean") {
			...
			try {
				...
				sparrow.throw(__('Invalid "mandatory_depends_on" expression'));
			}
		} else {
	...
	var _call = function (opts) {
		...
		if (frm.is_new()) {
			sparrow.ui.form.remove_old_form_route();
		}
		sparrow.ui.form.is_saving = true;
		...
		return sparrow.call({
			...
			always: function (r) {
				$(btn).prop("disabled", false);
				sparrow.ui.form.is_saving = false;

				if (r) {
					var doc = r.docs && r.docs[0];
					if (doc) {
						sparrow.ui.form.update_calling_link(doc);
					}
				}
			},
		});
...
sparrow.ui.form.remove_old_form_route = () => {
	let current_route = sparrow.get_route().join("/");
	sparrow.route_history = sparrow.route_history.filter(
		(route) => route.join("/") !== current_route
	);
};

sparrow.ui.form.update_calling_link = (newdoc) => {
	if (!sparrow._from_link) return;
	var doc = sparrow.get_doc(sparrow._from_link.doctype, sparrow._from_link.docname);

	let is_valid_doctype = () => {
		if (sparrow._from_link.df.fieldtype === "Link") {
			return newdoc.doctype === sparrow._from_link.df.options;
		} else {
			// dynamic link, type is dynamic
			return newdoc.doctype === doc[sparrow._from_link.df.options];
		}
	};

	if (is_valid_doctype()) {
		sparrow.model.with_doctype(newdoc.doctype, () => {
			let meta = sparrow.get_meta(newdoc.doctype);
			// set value
			if (doc && doc.parentfield) {
				//update values for child table
				$.each(
					sparrow._from_link.frm.fields_dict[doc.parentfield].grid.grid_rows,
					function (index, field) {
						if (field.doc && field.doc.name === sparrow._from_link.docname) {
							if (meta.title_field && meta.show_title_field_in_link) {
								sparrow.utils.add_link_title(
									newdoc.doctype,
									newdoc.name,
									newdoc[meta.title_field]
								);
							}
							sparrow._from_link.set_value(newdoc.name);
						}
					}
				);
			} else {
			...
			// refresh field
			sparrow._from_link.refresh();

			// if from form, switch
			if (sparrow._from_link.frm) {
				sparrow
					.set_route(
						"Form",
						sparrow._from_link.frm.doctype,
						sparrow._from_link.frm.docname
					)
					.then(() => {
						sparrow.utils.scroll_to(sparrow._from_link_scrollY);
					});
			}

			sparrow._from_link = null;
		});
	}
};
