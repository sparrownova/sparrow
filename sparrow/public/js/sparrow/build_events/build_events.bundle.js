...
sparrow.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						sparrow.assets.executed_ = sparrow.assets.executed_.filter(
		...
		// update assets json
		sparrow.call("sparrow.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				sparrow.boot.assets_json = r.message;

				if (sparrow.hot_update) {
					sparrow.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
