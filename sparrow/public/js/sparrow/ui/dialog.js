...
sparrow.ui.open_dialogs = [];

sparrow.ui.Dialog = class Dialog extends sparrow.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		this.$wrapper = sparrow.get_modal("", "");
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				if (sparrow.ui.open_dialogs[sparrow.ui.open_dialogs.length - 1] === me) {
					sparrow.ui.open_dialogs.pop();
					if (sparrow.ui.open_dialogs.length) {
						window.cur_dialog =
							sparrow.ui.open_dialogs[sparrow.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				sparrow.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("sparrow.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(sparrow.utils.icon(icon));
