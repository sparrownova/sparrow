sparrow.provide("sparrow.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
sparrow.ui.keys.shortcut_groups = shortcut_groups;

sparrow.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new sparrow.ui.keys.AltShortcutGroup());
	}
...
sparrow.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (sparrow.ui.keys.key_map[e.which] || "").toLowerCase();
...
sparrow.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = sparrow.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
