...
sparrow.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
			...
			sparrow.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(sparrow.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(sparrow.search.utils.get_frequent_links());
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				sparrow.route_options = item.route_options;
			}
			...
			if (item.onclick) {
				...
				if (event.ctrlKey || event.metaKey) {
					sparrow.open_in_new_tab = true;
				}
				sparrow.set_route(item.route);
			}
			$input.val("");
			$input.trigger("blur");
		});
		...
		sparrow.search.utils.setup_recent();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function () {
				...
				sparrow.msgprint(txt, __("Search Help"));
			},
		});
	}
	...
	build_options(txt) {
		var options = sparrow.search.utils
			.get_creatables(txt)
			.concat(
				sparrow.search.utils.get_search_in_list(txt),
				sparrow.search.utils.get_doctypes(txt),
				sparrow.search.utils.get_reports(txt),
				sparrow.search.utils.get_pages(txt),
				sparrow.search.utils.get_workspaces(txt),
				sparrow.search.utils.get_dashboards(txt),
				sparrow.search.utils.get_recent_pages(txt || ""),
				sparrow.search.utils.get_executables(txt)
			);
		if (txt.charAt(0) === "#") {
			options = sparrow.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${sparrow.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	sparrow.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [sparrow.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			var meta = sparrow.get_meta(sparrow.container.page.list_view.doctype);
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function () {
						sparrow.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: sparrow.utils.get_random(16),
				onclick: function () {
					sparrow.msgprint(sparrow.utils.get_random(16), __("Result"));
				},
			});
		}
	}
