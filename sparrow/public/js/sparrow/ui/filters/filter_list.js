sparrow.ui.FilterGroup = class {
	...
	add_filters(filters) {
		...
		return sparrow.run_serially(promises).then(() => this.update_filters());
	}
	...
	_push_new_filter(doctype, fieldname, condition, value, hidden = false) {
		...
		let filter = new sparrow.ui.Filter(args);
	...
	filter_exists(filter_value) {
		...
		this.filters
			...
			.map((f) => {
				...
				let equal = sparrow.utils.arrays_equal;
