...
sparrow.ui.color = {
	get: function (color_name, shade) {
		...
		return sparrow.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(sparrow.ui.color_map);
		if (color_names.includes(color_name)) {
			return sparrow.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(sparrow.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(sparrow.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in sparrow.ui.color_map) {
			const colors = sparrow.ui.color_map[key];
...
sparrow.ui.color_map = sparrow.ui.color.get_color_map();
