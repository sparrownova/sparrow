...
sparrow.provide("sparrow.ui.keys.handlers");

sparrow.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = sparrow.ui.keys.get_key(e);
		if (sparrow.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = sparrow.ui.keys.handlers[key].length; i < l; i++) {
				var handler = sparrow.ui.keys.handlers[key][i];
...
sparrow.ui.keys.standard_shortcuts = standard_shortcuts;
sparrow.ui.keys.add_shortcut = ({
	...
	sparrow.ui.keys.off(shortcut, page);
	// attach new handler
	sparrow.ui.keys.on(shortcut, handler);
...
sparrow.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (sparrow.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(sparrow.utils.to_title_case)
					.join("+");
				if (sparrow.utils.is_mac()) {
	...
	let dialog = new sparrow.ui.Dialog({
		...
		on_hide() {
			sparrow.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	sparrow.ui.keys.is_dialog_shown = true;
};

sparrow.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = sparrow.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
sparrow.ui.keys.on = function (key, handler) {
	if (!sparrow.ui.keys.handlers[key]) {
		sparrow.ui.keys.handlers[key] = [];
	}
	sparrow.ui.keys.handlers[key].push(handler);
};

sparrow.ui.keys.off = function (key, page) {
	let handlers = sparrow.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	sparrow.ui.keys.handlers[key] = handlers.filter((h) => {
...
sparrow.ui.keys.add_shortcut({
	shortcut: "ctrl+s",
	action: function (e) {
		sparrow.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
sparrow.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

sparrow.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

sparrow.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

sparrow.ui.keys.on("ctrl+down", function (e) {
	var grid_row = sparrow.ui.form.get_open_grid_form();
	grid_row &&
...
sparrow.ui.keys.on("ctrl+up", function (e) {
	var grid_row = sparrow.ui.form.get_open_grid_form();
	grid_row &&
		grid_row.toggle_view(false, function () {
			grid_row.open_prev();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	sparrow.ui.keys.key_map[65 + i] = letter;
});
