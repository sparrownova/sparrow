// sparrow.ui.Capture
// Author - Achilles Rasquinha <achilles@sparrow.io>
 ...
 *
 * @example
 * sparrow._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
sparrow._.get_data_uri = (element) => {
...
/**
 * @description Sparrow's Capture object.
 *
 * @example
 * const capture = sparrow.ui.Capture()
 * capture.show()
 *
 ...
 */
sparrow.ui.Capture = class {
	constructor(options = {}) {
		this.options = sparrow.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...sparrow.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(sparrow.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (sparrow.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(sparrow.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					sparrow.show_alert(sparrow.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = sparrow._.get_data_uri(me.video);
...
sparrow.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
sparrow.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
sparrow.ui.Capture.TEMPLATE = `
<div class="sparrow-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${sparrow.ui.Capture.ERR_MESSAGE}</video>
	</div>
