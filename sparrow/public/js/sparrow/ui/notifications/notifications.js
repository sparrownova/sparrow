...
sparrow.ui.Notifications = class Notifications {
	constructor() {
		this.tabs = {};
		this.notification_settings = sparrow.boot.notification_settings;
		this.make();
	}
	...
	make() {
		...
		this.user = sparrow.session.user;
	...
	setup_headers() {
		...
		$(`<span class="notification-settings pull-right" data-action="go_to_settings">
			${sparrow.utils.icon("setting-gear")}
		</span>`)
			.on("click", (e) => {
				e.stopImmediatePropagation();
				this.dropdown.dropdown("hide");
				sparrow.set_route("Form", "Notification Settings", sparrow.session.user);
			})
		...
		$(`<span class="mark-all-read pull-right" data-action="mark_all_as_read">
			${sparrow.utils.icon("mark-as-read")}
		</span>`)
	...
	mark_all_as_read(e) {
		...
		sparrow.call("sparrow.desk.doctype.notification_log.notification_log.mark_all_as_read");
	}
...
sparrow.ui.notifications = {
	get_notification_config() {
		return sparrow.xcall("sparrow.desk.notifications.get_notification_info").then((r) => {
			sparrow.ui.notifications.config = r;
			return r;
		});
	},

	show_open_count_list(doctype) {
		if (!sparrow.ui.notifications.config) {
	...
	route_to_list_with_filters(doctype) {
		let filters = sparrow.ui.notifications.config["conditions"][doctype];
		if (filters && $.isPlainObject(filters)) {
			if (!sparrow.route_options) {
				sparrow.route_options = {};
			}
			$.extend(sparrow.route_options, filters);
		}
		sparrow.set_route("List", doctype);
	},
};
...
class NotificationsView extends BaseNotificationsView {
	make() {
		...
		this.get_notifications_list(this.max_length).then((r) => {
			...
			sparrow.update_user_info(r.message.user_info);
	...
	mark_as_read(docname, $el) {
		sparrow
			.call("sparrow.desk.doctype.notification_log.notification_log.mark_as_read", {
				docname: docname,
			})
			.then(() => {
	...
	get_dropdown_item_html(field) {
		...
		message = title
			? message.replace(title[1], sparrow.ellipsis(strip_html(title[1]), 100))
			: message;

		let timestamp = sparrow.datetime.comment_when(field.creation);
		...
		let user_avatar = sparrow.avatar(user, "avatar-medium user-avatar");
			...
			if (this.dropdown_items.length) {
				...
				this.container.append(
					...
					<div class="text-center">
						<img src="/assets/sparrow/images/ui-states/notification-empty-state.svg" alt="Generic Empty State" class="null-state">
	...
	get_notifications_list(limit) {
		return sparrow.call(
			"sparrow.desk.doctype.notification_log.notification_log.get_notification_logs",
			{ limit: limit }
		);
	}
	...
	get_item_link(notification_doc) {
		...
		return sparrow.utils.get_form_link(link_doctype, link_docname);
	}
	...
	toggle_seen(flag) {
		sparrow.call(
			"sparrow.desk.doctype.notification_settings.notification_settings.set_seen_value",
			{
				value: cint(flag),
	...
	setup_notification_listeners() {
		sparrow.realtime.on("notification", () => {
			this.toggle_notification_icon(false);
			this.update_dropdown();
		});

		sparrow.realtime.on("indicator_hide", () => {
		...
		this.parent.on("show.bs.dropdown", () => {
			...
			if (this.notifications_icon.find(".notifications-unseen").is(":visible")) {
				this.toggle_notification_icon(true);
				sparrow.call(
					"sparrow.desk.doctype.notification_log.notification_log.trigger_indicator_hide"
				);
			}
		});
	}
...
class EventsView extends BaseNotificationsView {
	make() {
		let today = sparrow.datetime.get_today();
		sparrow
			.xcall("sparrow.desk.doctype.event.event.get_events", {
				start: today,
				end: today,
			})
			.then((event_list) => {
	...
	render_events_html(event_list) {
		...
		if (event_list.length) {
			let get_event_html = (event) => {
				let time = __("All Day");
				if (!event.all_day) {
					let start_time = sparrow.datetime.get_time(event.starts_on);
					let days_diff = sparrow.datetime.get_day_diff(event.ends_on, event.starts_on);
					let end_time = sparrow.datetime.get_time(event.ends_on);
					if (days_diff > 1) {
				...
				if (event.particpants) {
					particpants = sparrow.avatar_group(event.particpants, 3);
				}
			...
			html = `
				<div class="notification-null-state">
					<div class="text-center">
					<img src="/assets/sparrow/images/ui-states/event-empty-state.svg" alt="Generic Empty State" class="null-state">
