...
sparrow.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = sparrow.ui.keys.get_key(e);
			let increment_by;
	...
	toggle_theme(theme) {
		...
		sparrow.show_alert(__("Theme Changed"), 3);

		sparrow.xcall("sparrow.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
sparrow.ui.add_system_theme_switch_listener = () => {
	sparrow.ui.dark_theme_media_query.addEventListener("change", () => {
		sparrow.ui.set_theme();
	});
};

sparrow.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

sparrow.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = sparrow.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
