...
sparrow.ui.Tree = class {
	constructor({
		...
		if (!icon_set) {
			this.icon_set = {
				open: sparrow.utils.icon("folder-open", "md"),
				closed: sparrow.utils.icon("folder-normal", "md"),
				leaf: sparrow.utils.icon("primitive-dot", "xs"),
			};
		}
	...
	get_all_nodes(value, is_root, label) {
		...
		return new Promise((resolve) => {
			sparrow.call({
				method: "sparrow.desk.treeview.get_all_nodes",
				args: args,
				callback: (r) => {
	...
	on_node_click(node) {
		this.expand_node(node);
		sparrow.dom.activate(this.wrapper, node.$tree_link, "tree-link");
