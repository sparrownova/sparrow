 ...
 *
 * @returns {sparrow.ui.Page}
 */

/**
 * @typedef {Object} sparrow.ui.Page
 */

sparrow.ui.make_app_page = function (opts) {
	opts.parent.page = new sparrow.ui.Page(opts);
	return opts.parent.page;
};

sparrow.ui.pages = {};

sparrow.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		this.make();
		sparrow.ui.pages[sparrow.get_route_str()] = this;
	}
	...
	load_lib(callback) {
		sparrow.require(this.required_libs, callback);
	}

	add_main_section() {
		$(sparrow.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_page() {
		...
		this.card_layout && this.main.addClass("sparrow-card");
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (sparrow.utils.is_xs() || sparrow.utils.is_sm()) {
	...
	update_sidebar_icon() {
		...
		sidebar_toggle_icon.html(
			sparrow.utils.icon(is_sidebar_visible ? "sidebar-collapse" : "sidebar-expand", "md")
		);
	}
	...
	add_action_icon(icon, click, css_class = "", tooltip_label) {
		...
		button.click(click);
		button
			.attr("title", __(tooltip_label || sparrow.unscrub(icon)))
	...
	get_icon_label(icon, label) {
		...
		return `${icon ? sparrow.utils.icon(icon_name, size) : ""} <span class="hidden-xs"> ${__(
			label
		)} </span>`;
	}
	...
	set_action(btn, opts) {
		...
		sparrow.ui.keys.get_shortcut_group(this).add(btn, text_span.length ? text_span : btn);
	}
	...
	add_dropdown_item({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${sparrow.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			sparrow.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
	...
	set_title(title, icon = null, strip = true, tab_title = "") {
		...
		this.title = title;
		sparrow.utils.set_title(tab_title || title);
		if (icon) {
			title = `${sparrow.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || "btn-default"} ${opts.btn_size || "btn-sm"} ellipsis">
				${opts.icon ? sparrow.utils.icon(opts.icon) : ""}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		sparrow.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = sparrow.ui.form.make_control({
			df: df,
