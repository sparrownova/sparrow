...
sparrow.ui.Tags = class {
	...
	bind() {
		...
		const select_tag = function () {
			const tagValue = sparrow.utils.xss_sanitise(me.$input.val());
			me.addTag(tagValue);
			me.$input.val("");
		};
	...
	removeTag(label) {
		label = sparrow.utils.xss_sanitise(label);
	...
	get_tag(label) {
		let $tag = sparrow.get_data_pill(label, label, (target, pill_wrapper) => {
