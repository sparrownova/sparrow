...
sparrow.ui.is_liked = function (doc) {
	var liked = sparrow.ui.get_liked_by(doc);
	return liked.indexOf(sparrow.session.user) === -1 ? false : true;
};

sparrow.ui.get_liked_by = function (doc) {
...
sparrow.ui.toggle_like = function ($btn, doctype, name, callback) {
	var add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css("pointer-events", "none");

	sparrow.call({
		method: "sparrow.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
			...
			if (!r.exc) {
				...
				if (doc) {
					var liked_by = JSON.parse(doc._liked_by || "[]"),
						idx = liked_by.indexOf(sparrow.session.user);
					if (add === "Yes") {
						if (idx === -1) liked_by.push(sparrow.session.user);
					} else {
						if (idx !== -1) {
...
sparrow.ui.click_toggle_like = function () {
	var $btn = $(this);
	...
	sparrow.ui.toggle_like($btn, doctype, name, function () {
		if (not_liked) {
...
sparrow.ui.setup_like_popover = ($parent, selector, check_not_liked = true) => {
	if (sparrow.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach((user) => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${sparrow.avatar(user, "avatar-xs")}
							<span>${sparrow.user.full_name(user)}</span>
						</li>
					`);
				});
				...
				liked_by_list.children("li").click((ev) => {
					...
					sparrow.set_route(link_base + user);
				});
