sparrow.ui.SortSelector = class SortSelector {
	// parent:
	// change:
	// args:
	...
	make() {
		...
		this.wrapper = $(sparrow.render_template("sort_selector", this.args)).appendTo(this.parent);
		this.bind_events();
	}
	bind_events() {
		var me = this;
		...
		this.wrapper.find(".btn-order").on("click", function () {
			...
			btn.find(".sort-order").html(sparrow.utils.icon(icon_name, "sm"));
		...
		if (!this.args.options) {
			...
			meta.fields.forEach(function (df) {
				if (
					...
					sparrow.perm.has_perm(me.doctype, df.permlevel, "read")
				) {
	...
	get_label(fieldname) {
		if (fieldname === "idx") {
			...
			return this.labels[fieldname] || sparrow.meta.get_label(this.doctype, fieldname);
		}
	}
	get_sql_string() {
