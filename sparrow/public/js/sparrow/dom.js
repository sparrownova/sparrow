...
sparrow.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + sparrow.dom.id_count;
		sparrow.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + sparrow.dom.id_count;
		$ele.attr("id", id);
		sparrow.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = sparrow.dom.by_id(parent);
	...
	freeze: function (msg, css_class) {
		...
		sparrow.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!sparrow.dom.freeze_count) return; // anything open?
		sparrow.dom.freeze_count--;
		if (!sparrow.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				sparrow.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
sparrow.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
sparrow.run_serially = function (tasks) {
...
sparrow.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
sparrow.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

sparrow.scrub = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

sparrow.unscrub = function (txt) {
	return sparrow.model.unscrub(txt);
};

sparrow.get_data_pill = (label, target_id = null, remove_action = null, image = null) => {
...
sparrow.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${sparrow.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${sparrow.utils.icon("close-alt", "sm", "close-alt")}
						</button>
					</div>
				</div>
...
sparrow.is_online = function () {
	if (sparrow.boot.developer_mode == 1) {
...
sparrow.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + sparrow.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = sparrow.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = sparrow.assets.bundled_asset("desk.bundle.css");
