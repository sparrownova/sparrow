...
sparrow.avatar = function (
	...
	if (user) {
		// desk
		user_info = sparrow.user_info(user);
	} else {
		// website
		let full_name = title || sparrow.get_cookie("full_name");
		user_info = {
			image: image_url === null ? sparrow.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: sparrow.get_abbr(full_name),
			color: sparrow.get_palette(full_name),
		};
	}
...
sparrow.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let abbr = sparrow.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = sparrow.get_palette(title);
...
sparrow.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			sparrow.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => sparrow.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${sparrow.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
sparrow.ui.scroll = function (element, animate, additional_offset) {
...
sparrow.get_palette = function (txt) {
	if (!txt) return sparrow.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return sparrow.palette[idx % 8];
};

sparrow.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
sparrow.gravatars = {};
sparrow.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!sparrow.gravatars[email_id]) {
		// TODO: check if gravatar exists
		sparrow.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return sparrow.gravatars[email_id];
};
...
sparrow.get_cookie = function getCookie(name) {
	return sparrow.get_cookies()[name];
};

sparrow.get_cookies = function getCookies() {
...
sparrow.is_mobile = function () {
	return $(document).width() < 768;
};

sparrow.utils.xss_sanitise = function (string, options) {
...
sparrow.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = sparrow.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(sparrow.utils.xss_sanitise(url, { strategies: ["js"] }));
};

sparrow.utils.strip_url = (url) => {
...
sparrow.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
		...
		},
		{
			...
			label: __("Start Date"),
			reqd: 1,
			default: sparrow.datetime.nowdate(),
		},
		{
			fieldname: "end_date",
			fieldtype: "Date",
	...
	sparrow.prompt(
		...
		function (values) {
			sparrow.call({
				method: "sparrow.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
sparrow.utils.get_page_view_count = function (route) {
	return sparrow.call("sparrow.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};
