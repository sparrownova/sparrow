sparrow.provide("sparrow.contacts");

$.extend(sparrow.contacts, {
	...
	render_address_and_contact: function (frm) {
		...
		if (frm.fields_dict["address_html"] && "addr_list" in frm.doc.__onload) {
			$(frm.fields_dict["address_html"].wrapper)
				.html(sparrow.render_template("address_list", frm.doc.__onload))
				.find(".btn-address")
				.on("click", function () {
					sparrow.new_doc("Address");
				});
		}
		...
		if (frm.fields_dict["contact_html"] && "contact_list" in frm.doc.__onload) {
			$(frm.fields_dict["contact_html"].wrapper)
				.html(sparrow.render_template("contact_list", frm.doc.__onload))
				.find(".btn-contact")
				.on("click", function () {
	...
	get_last_doc: function (frm) {
		const reverse_routes = sparrow.route_history.slice().reverse();
	...
	get_address_display: function (frm, address_field, display_field) {
		...
		sparrow
			.xcall("sparrow.contacts.doctype.address.address.get_address_display", {
