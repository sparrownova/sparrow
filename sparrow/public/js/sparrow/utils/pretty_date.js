function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = sparrow.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff = (new Date(sparrow.datetime.now_datetime()).getTime() - date.getTime()) / 1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = sparrow.datetime.str_to_user ? sparrow.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="sparrow-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
sparrow.datetime.comment_when = comment_when;
sparrow.datetime.prettyDate = prettyDate;

sparrow.datetime.refresh_when = function () {
	if (jQuery) {
		$(".sparrow-timestamp").each(function () {
