...
sparrow.provide("sparrow.tools");

sparrow.tools.downloadify = function (data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		sparrow.msgprint(
			__("Export not allowed. You need {0} role to export.", [sparrow.utils.comma_or(roles)])
		);
		return;
	}

	var filename = title + ".csv";
	var csv_data = sparrow.tools.to_csv(data);
...
sparrow.markdown = function (txt) {
	if (!sparrow.md2html) {
		sparrow.md2html = new showdown.Converter();
	}
	...
	return sparrow.md2html.makeHtml(txt);
};

sparrow.tools.to_csv = function (data) {
	var res = [];
