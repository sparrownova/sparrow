...
export default {
	...
	methods: {
		...
		get_files_in_folder(folder, start) {
			return sparrow
				.call("sparrow.core.api.file.get_files_in_folder", {
					folder,
					start,
		...
		search_by_name: sparrow.utils.debounce(function() {
			if (this.search_text === "") {
			...
			sparrow
				.call(
					"sparrow.core.api.file.get_files_by_search_text",
					{
		...
		make_file_node(file) {
			...
			let label = sparrow.utils.file_name_ellipsis(filename, 40);
			return {
				label: label,
				filename: filename,
