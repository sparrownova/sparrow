...
export default class GoogleDrivePicker {
	...
	loadGapi() {
		...
		if (!sparrow.boot.user.google_drive_token) {
	...
	handleAuthResult(authResult) {
		...
		if (authResult && !authResult.error) {
			sparrow.boot.user.google_drive_token = authResult.access_token;
			this.createPicker();
		} else {
	...
	createPicker() {
		...
		if (this.pickerApiLoaded && sparrow.boot.user.google_drive_token) {
			...
			this.picker = new google.picker.PickerBuilder()
				...
				.setOAuthToken(sparrow.boot.user.google_drive_token)
				.addView(this.view)
				.setLocale(sparrow.boot.lang)
