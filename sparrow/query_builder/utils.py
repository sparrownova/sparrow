...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(sparrow.conf.db_type or "mariadb")
...
def get_query(*args, **kwargs) -> QueryBuilder:
	from sparrow.database.query import Engine
...
def DocType(*args, **kwargs):
	return sparrow.qb.DocType(*args, **kwargs)


def Table(*args, **kwargs):
	return sparrow.qb.Table(*args, **kwargs)


def patch_query_execute():
	"""Patch the Query Builder with helper execute method
	This excludes the use of `sparrow.db.sql` method while
	...
	def execute_query(query, *args, **kwargs):
		...
		result = sparrow.db.sql(query, params, *args, **kwargs)  # nosemgrep
	...
	def prepare_query(query):
		...
		if sparrow.flags.in_safe_exec:
			from sparrow.utils.safe_exec import check_safe_sql_query
			...
			if not check_safe_sql_query(query, throw=False):
				...
				if len(callstack) >= 3 and ".py" in callstack[2].filename:
					...
					raise sparrow.PermissionError("Only SELECT SQL allowed in scripting")
		return query, param_collector.get_parameters()

	builder_class = sparrow.qb._BuilderClasss
	...
	sparrow._qb_patched[sparrow.conf.db_type] = True


def patch_query_aggregation():
	"""Patch aggregation functions to sparrow.qb"""
	from sparrow.query_builder.functions import _avg, _max, _min, _sum
