...
import sparrow
from sparrow import _, get_module_path
from sparrow.core.doctype.access_log.access_log import make_access_log
from sparrow.core.doctype.document_share_key.document_share_key import is_expired
from sparrow.utils import cint, escape_html, strip_html
...
def get_context(context):
	...
	if not ((sparrow.form_dict.doctype and sparrow.form_dict.name) or sparrow.form_dict.doc):
		return {
			"body": f"""
				<h1>Error</h1>
				<p>Parameters doctype and name required</p>
				<pre>{escape_html(sparrow.as_json(sparrow.form_dict, indent=2))}</pre>
				"""
		}

	if sparrow.form_dict.doc:
		doc = sparrow.form_dict.doc
	else:
		doc = sparrow.get_doc(sparrow.form_dict.doctype, sparrow.form_dict.name)

	set_link_titles(doc)

	settings = sparrow.parse_json(sparrow.form_dict.settings)

	letterhead = sparrow.form_dict.letterhead or None
	...
	make_access_log(
		doctype=sparrow.form_dict.doctype, document=sparrow.form_dict.name, file_type="PDF", method="Print"
	)
	...
	body = get_rendered_template(
		...
		meta=meta,
		trigger_print=sparrow.form_dict.trigger_print,
		no_letterhead=sparrow.form_dict.no_letterhead,
		letterhead=letterhead,
		settings=settings,
	)
	print_style = get_print_style(sparrow.form_dict.style, print_format)

	return {
		"body": body,
		"print_style": print_style,
		"comment": sparrow.session.user,
		"title": sparrow.utils.strip_html(doc.get_title() or doc.name),
		"lang": sparrow.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"doctype": sparrow.form_dict.doctype,
		"name": sparrow.form_dict.name,
		"key": sparrow.form_dict.get("key"),
	}
...
def get_print_format_doc(print_format_name, meta):
	...
	if not print_format_name:
		print_format_name = sparrow.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return sparrow.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	print_settings = sparrow.get_single("Print Settings").as_dict()
	...
	if not sparrow.flags.ignore_print_permissions:
		validate_print_permission(doc)

	if doc.meta.is_submittable:
		if doc.docstatus.is_draft() and not cint(print_settings.allow_print_for_draft):
			sparrow.throw(_("Not allowed to print draft documents"), sparrow.PermissionError)

		if doc.docstatus.is_cancelled() and not cint(print_settings.allow_print_for_cancelled):
			sparrow.throw(_("Not allowed to print cancelled documents"), sparrow.PermissionError)
	...
	letter_head = sparrow._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = sparrow.utils.jinja.render_template(
			letter_head.content, {"doc": doc.as_dict()}
		)

	if letter_head.footer:
		letter_head.footer = sparrow.utils.jinja.render_template(
...
def set_title_values_for_link_and_dynamic_link_fields(meta, doc, parent_doc=None):
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = sparrow.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta, doc):
	for field in meta.get_table_fields():
		...
		_meta = sparrow.get_meta(field.options)
	...
	for field in meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, sparrow.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(doc, str) and isinstance(name, str):
		doc = sparrow.get_doc(doc, name)

	if isinstance(doc, str):
		doc = sparrow.get_doc(json.loads(doc))

	print_format = get_print_format_doc(print_format, meta=meta or sparrow.get_meta(doc.doctype))
	set_link_titles(doc)
	...
	try:
		html = get_rendered_template(
			...
			settings=sparrow.parse_json(settings),
		)
	except sparrow.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc, name=None, print_format=None, meta=None, lang=None):
	...
	if not print_format or (print_format and not print_format.raw_printing):
		sparrow.throw(
			_("{0} is not a raw printing format.").format(print_format), sparrow.TemplateNotFoundError
		)
...
def validate_print_permission(doc):
	for ptype in ("read", "print"):
		if sparrow.has_permission(doc.doctype, ptype, doc) or sparrow.has_website_permission(doc):
			return

	key = sparrow.form_dict.key
	if key and isinstance(key, str):
		validate_key(key, doc)
	else:
		raise sparrow.PermissionError(_("You do not have permission to view this document"))


def validate_key(key, doc):
	document_key_expiry = sparrow.get_cached_value(
		"Document Share Key",
	...
	if document_key_expiry is not None:
		if is_expired(document_key_expiry[0]):
			raise sparrow.exceptions.LinkExpired
		else:
			return

	# TODO: Deprecate this! kept it for backward compatibility
	if sparrow.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return

	raise sparrow.exceptions.InvalidKeyError
...
def get_letter_head(doc, no_letterhead, letterhead=None):
	if no_letterhead:
		return {}
	if letterhead:
		return sparrow.db.get_value("Letter Head", letterhead, ["content", "footer"], as_dict=True)
...
def get_print_format(doctype, print_format):
	if print_format.disabled:
		sparrow.throw(
			_("Print Format {0} is disabled").format(print_format.name), sparrow.DoesNotExistError
		)
	...
	if os.path.exists(path):
		...
		sparrow.throw(_("No template found at path: {0}").format(path), sparrow.TemplateNotFoundError)
...
def make_layout(doc, meta, format_data=None):
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/sparrow/images/signature-placeholder.png"
...
def get_print_style(style=None, print_format=None, for_legacy=False):
	...
	css = sparrow.get_template("templates/styles/standard.css").render(context)

	if style and sparrow.db.exists("Print Style", style):
		css = css + "\n" + sparrow.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data, table_meta, df):
	...
	columns = []
	doc = data[0] or sparrow.new_doc(df.options)
