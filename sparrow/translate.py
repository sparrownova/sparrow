...
def get_language(lang_list: list = None) -> str:
	"""Set `sparrow.local.lang` from HTTP headers at beginning of request
	...
	"""
	is_logged_in = sparrow.session.user != "Guest"

	# fetch language from form_dict
	if sparrow.form_dict._lang:
		language = get_lang_code(sparrow.form_dict._lang or get_parent_language(sparrow.form_dict._lang))
		if language:
			return language
	...
	accept_language = list(sparrow.request.accept_languages.values())
	...
	return sparrow.db.get_default("lang") or "en"
...
def get_user_lang(user: str = None) -> str:
	"""Set sparrow.local.lang from user preferences on session beginning or resumption"""
	user = user or sparrow.session.user
	lang = sparrow.cache().hget("lang", user)
...
def get_lang_code(lang: str) -> str | None:
	return sparrow.db.get_value("Language", {"name": lang}) or sparrow.db.get_value(
...
def set_default_language(lang):
	...
	if sparrow.db.get_default("lang") != lang:
		sparrow.db.set_default("lang", lang)
	sparrow.local.lang = lang


def get_lang_dict():
	"""Returns all languages in dict format, full name is the key e.g. `{"english":"en"}`"""
	return dict(
		sparrow.get_all("Language", fields=["language_name", "name"], order_by="modified", as_list=True)
	)
...
def get_dict(fortype: str, name: str | None = None) -> dict[str, str]:
	...
	translation_assets = cache.hget("translation_assets", sparrow.local.lang) or {}
	...
	if asset_key not in translation_assets:
		...
		if fortype == "doctype":
			...
			apps = sparrow.get_all_apps(True)
			for app in apps:
			...
			messages += get_messages_from_include_files()
			messages += (
				sparrow.qb.from_("Print Format").select(PseudoColumn("'Print Format:'"), "name")
			).run()
			messages += (sparrow.qb.from_("DocType").select(PseudoColumn("'DocType:'"), "name")).run()
			messages += sparrow.qb.from_("Role").select(PseudoColumn("'Role:'"), "name").run()
			messages += (sparrow.qb.from_("Module Def").select(PseudoColumn("'Module:'"), "name")).run()
			messages += (
				sparrow.qb.from_("Workspace Shortcut")
				.where(Field("format").isnotnull())
				.select(PseudoColumn("''"), "format")
			).run()
			messages += (sparrow.qb.from_("Onboarding Step").select(PseudoColumn("''"), "title")).run()
		...
		cache.hset("translation_assets", sparrow.local.lang, translation_assets)

	translation_map: dict = translation_assets[asset_key]

	translation_map.update(get_user_translations(sparrow.local.lang))
...
def get_messages_for_boot():
	...
	messages = get_all_translations(sparrow.local.lang)
...
def get_dict_from_hooks(fortype, name):
	...
	hooks = sparrow.get_hooks("get_translated_dict")
	for (hook_fortype, fortype_name) in hooks:
		if hook_fortype == fortype and fortype_name == name:
			for method in hooks[(hook_fortype, fortype_name)]:
				translated_dict.update(sparrow.get_attr(method)())

	return translated_dict


def make_dict_from_messages(messages, full_dict=None, load_user_translation=True):
	"""Returns translated messages as a dict in Language specified in `sparrow.local.lang`
	...
	if full_dict is None:
		if load_user_translation:
			full_dict = get_all_translations(sparrow.local.lang)
		else:
			full_dict = get_translations_from_apps(sparrow.local.lang)
...
def get_lang_js(fortype: str, name: str) -> str:
	...
	"""
	return f"\n\n$.extend(sparrow._messages, {json.dumps(get_dict(fortype, name))})"
...
def get_all_translations(lang: str) -> dict[str, str]:
	...
	try:
		return sparrow.cache().hget(MERGED_TRANSLATION_KEY, lang, generator=_merge_translations)
	except Exception:
	...
	if os.path.exists(path):
		...
		for item in csv_content:
			if len(item) == 3 and item[2]:
				...
				sparrow.log_error(message=msg, title="Error in translation file")
				if throw:
					sparrow.throw(msg, title="Error in translation file")
...
def get_user_translations(lang):
	...
	return sparrow.cache().hget(USER_TRANSLATION_KEY, lang, generator=_read_from_db)


def clear_cache():
	"""Clear all translation assets from :meth:`sparrow.cache`"""
	cache = sparrow.cache()
...
def get_messages_for_app(app, deduplicate=True):
	...
	messages = []
	modules = [sparrow.unscrub(m) for m in sparrow.local.app_modules[app]]
	...
	if modules:
		if isinstance(modules, str):
			modules = [modules]
		filtered_doctypes = (
			sparrow.qb.from_("DocType").where(Field("module").isin(modules)).select("name").run(pluck=True)
		)
		...
		# pages
		filtered_pages = (
			sparrow.qb.from_("Page").where(Field("module").isin(modules)).select("name", "title").run()
		)
...
def get_messages_from_navbar():
	...
	labels = sparrow.get_all("Navbar Item", filters={"item_label": ("is", "set")}, pluck="item_label")
...
def get_messages_from_doctype(name):
	...
	if not meta.custom:
		doctype_file_path = sparrow.get_module_path(meta.module, "doctype", meta.name, meta.name)
...
def get_messages_from_workflow(doctype=None, app_name=None):
	...
	workflows = []
	if doctype:
		workflows = sparrow.get_all("Workflow", filters={"document_type": doctype})
	else:
		fixtures = sparrow.get_hooks("fixtures", app_name=app_name) or []
		for fixture in fixtures:
			if isinstance(fixture, str) and fixture == "Worflow":
				workflows = sparrow.get_all("Workflow")
				break
			elif isinstance(fixture, dict) and fixture.get("dt", fixture.get("doctype")) == "Workflow":
				workflows.extend(sparrow.get_all("Workflow", filters=fixture.get("filters")))

	messages = []
	document_state = DocType("Workflow Document State")
	for w in workflows:
		states = sparrow.db.get_values(
			document_state,
...
def get_messages_from_page(name):
	"""Returns all translatable strings from a :class:`sparrow.core.doctype.Page`"""
	return _get_messages_from_page_or_report("Page", name)


def get_messages_from_report(name):
	"""Returns all translatable strings from a :class:`sparrow.core.doctype.Report`"""
	report = sparrow.get_doc("Report", name)
	messages = _get_messages_from_page_or_report(
		"Report", name, sparrow.db.get_value("DocType", report.ref_doctype, "module")
	)
...
def _get_messages_from_page_or_report(doctype, name, module=None):
	if not module:
		module = sparrow.db.get_value(doctype, name, "module")

	doc_path = sparrow.get_module_path(module, doctype, name)

	messages = get_messages_from_file(os.path.join(doc_path, sparrow.scrub(name) + ".py"))
...
def get_server_messages(app):
	"""Extracts all translatable strings (tagged with :func:`sparrow._`) from Python modules
	inside an app"""
	messages = []
	file_extensions = (".py", ".html", ".js", ".vue")
	app_walk = os.walk(sparrow.get_pymodule_path(app))
...
def get_messages_from_include_files(app_name=None):
	...
	messages = []
	app_include_js = sparrow.get_hooks("app_include_js", app_name=app_name) or []
	web_include_js = sparrow.get_hooks("web_include_js", app_name=app_name) or []
	include_js = app_include_js + web_include_js

	for js_path in include_js:
		file_path = bundled_asset(js_path)
		relative_path = os.path.join(sparrow.local.sites_path, file_path.lstrip("/"))
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	"""
	sparrow.flags.setdefault("scanned_files", set())
	# TODO: Find better alternative
	# To avoid duplicate scan
	if path in sparrow.flags.scanned_files:
		return []
...
def extract_messages_from_code(code):
	...
	try:
		code = sparrow.as_unicode(render_include(code))
...
def update_translations(lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	for key, value in zip(
		sparrow.get_file_items(untranslated_file, ignore_empty_lines=False),
		sparrow.get_file_items(translated_file, ignore_empty_lines=False),
	):
...
def import_translations(lang, path):
	...
	for app in sparrow.get_all_apps(True):
...
def rebuild_all_translation_files():
	...
	for lang in get_all_languages():
		for app in sparrow.get_all_apps():
...
def write_translations_file(app, lang, full_dict=None, app_messages=None):
	...
	tpath = sparrow.get_pymodule_path(app, "translations")
	sparrow.create_folder(tpath)
	write_csv_file(
...
def send_translations(translation_dict):
	"""Append translated dict in `sparrow.local.response`"""
	if "__messages" not in sparrow.local.response:
		sparrow.local.response["__messages"] = {}

	sparrow.local.response["__messages"].update(translation_dict)
...
def rename_language(old_name, new_name):
	if not sparrow.db.exists("Language", new_name):
		return

	language_in_system_settings = sparrow.db.get_single_value("System Settings", "language")
	if language_in_system_settings == old_name:
		sparrow.db.set_single_value("System Settings", "language", new_name)
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = sparrow.db.get_values(
...
def get_translator_url():
	return sparrow.get_hooks()["translator_url"][0]
...
def get_all_languages(with_language_name=False):
	...
	def get_language_codes():
		return sparrow.get_all("Language", filters={"enabled": 1}, pluck="name")

	def get_all_language_with_name():
		return sparrow.get_all("Language", ["language_code", "language_name"], {"enabled": 1})

	if not sparrow.db:
		sparrow.connect()

	if with_language_name:
		return sparrow.cache().get_value("languages_with_name", get_all_language_with_name)
	else:
		return sparrow.cache().get_value("languages", get_language_codes)


def get_preferred_language_cookie():
	return sparrow.request.cookies.get("preferred_language")


def get_translated_doctypes():
	dts = sparrow.get_all("DocType", {"translated_doctype": 1}, pluck="name")
...
def print_language(language: str):
	...
	with print_language("de"):
	    html = sparrow.get_print( ... )
	```
	"""
	if not language or language == sparrow.local.lang:
		# do nothing
		yield
		return

	# remember original values
	_lang = sparrow.local.lang
	_jenv = sparrow.local.jenv

	# set language, empty any existing lang_full_dict and jenv
	sparrow.local.lang = language
	sparrow.local.jenv = None

	yield

	# restore original values
	sparrow.local.lang = _lang
	sparrow.local.jenv = _jenv
