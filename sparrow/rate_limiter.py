...
def apply():
	rate_limit = sparrow.conf.rate_limit
	if rate_limit:
		sparrow.local.rate_limiter = RateLimiter(rate_limit["limit"], rate_limit["window"])
		sparrow.local.rate_limiter.apply()


def update():
	if hasattr(sparrow.local, "rate_limiter"):
		sparrow.local.rate_limiter.update()


def respond():
	if hasattr(sparrow.local, "rate_limiter"):
		return sparrow.local.rate_limiter.respond()
...
class RateLimiter:
	def __init__(self, limit, window):
		...
		timestamp = int(sparrow.utils.now_datetime().timestamp())

		self.window_number, self.spent = divmod(timestamp, self.window)
		self.key = sparrow.cache().make_key(f"rate-limit-counter-{self.window_number}")
		self.counter = cint(sparrow.cache().get(self.key))
	...
	def reject(self):
		raise sparrow.TooManyRequestsError

	def update(self):
		self.end = datetime.utcnow()
		self.duration = int((self.end - self.start).total_seconds() * 1000000)

		pipeline = sparrow.cache().pipeline()
	...
	def ratelimit_decorator(fun):
		...
		def wrapper(*args, **kwargs):
			...
			if (
				methods != "ALL"
				and sparrow.request
				and sparrow.request.method
				and sparrow.request.method.upper() not in methods
			):
				return sparrow.call(fun, **sparrow.form_dict or kwargs)

			_limit = limit() if callable(limit) else limit

			ip = sparrow.local.request_ip if ip_based is True else None

			user_key = sparrow.form_dict[key] if key else None
			...
			if not identity:
				sparrow.throw(_("Either key or IP flag is required."))

			cache_key = f"rl:{sparrow.form_dict.cmd}:{identity}"

			value = sparrow.cache().get(cache_key) or 0
			if not value:
				sparrow.cache().setex(cache_key, seconds, 0)

			value = sparrow.cache().incrby(cache_key, 1)
			if value > _limit:
				sparrow.throw(
