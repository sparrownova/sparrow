...
class TestEnergyPointLog(SparrowTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		settings = sparrow.get_single("Energy Point Settings")
		settings.enabled = 1
		settings.save()
	...
	def setUp(self):
		sparrow.cache().delete_value("energy_point_rule_map")

	def tearDown(self):
		sparrow.set_user("Administrator")
		sparrow.db.delete("Energy Point Log")
		sparrow.db.delete("Energy Point Rule")
		sparrow.cache().delete_value("energy_point_rule_map")
	...
	def test_revert_points_on_cancelled_doc(self):
		...
		energy_point_logs = sparrow.get_all("Energy Point Log")
	...
	def test_allow_creation_of_new_log_if_the_previous_log_was_reverted(self):
		...
		log_name = sparrow.db.exists("Energy Point Log", {"reference_name": created_todo.name})
		sparrow.get_doc("Energy Point Log", log_name).revert("Just for test")
...
def create_energy_point_rule_for_todo(
	...
	name = "ToDo Closed"
	point_rule_exists = sparrow.db.exists("Energy Point Rule", name)

	if point_rule_exists:
		return sparrow.get_doc("Energy Point Rule", name)
