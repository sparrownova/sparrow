...
import sparrow
import sparrow.model
import sparrow.utils
from sparrow import _
from sparrow.desk.reportview import validate_args
from sparrow.model.db_query import check_parent_permission
from sparrow.utils import get_safe_filters
...
def get_list(
	...
	validate_args(args)
	return sparrow.get_list(**args)


@sparrow.whitelist()
def get_count(doctype, filters=None, debug=False, cache=False):
	return sparrow.db.count(doctype, get_safe_filters(filters), debug, cache)
...
def get(doctype, name=None, filters=None, parent=None):
	...
	if name:
		...
		doc = sparrow.get_doc(doctype, sparrow.parse_json(filters))
	else:
		doc = sparrow.get_doc(doctype)  # single
...
def get_value(doctype, fieldname, filters=None, as_dict=True, debug=False, parent=None):
	...
	if not sparrow.has_permission(doctype, parent_doctype=parent):
		sparrow.throw(_("No permission for {0}").format(_(doctype)), sparrow.PermissionError)
	...
	try:
		fields = sparrow.parse_json(fieldname)
	...
	if sparrow.get_meta(doctype).issingle:
		value = sparrow.db.get_values_from_single(fields, filters, doctype, as_dict=as_dict, debug=debug)
	else:
		value = get_list(
			doctype,
			filters=filters,
...
def get_single_value(doctype, field):
	if not sparrow.has_permission(doctype):
		sparrow.throw(_("No permission for {0}").format(_(doctype)), sparrow.PermissionError)

	return sparrow.db.get_single_value(doctype, field)
...
def set_value(doctype, name, fieldname, value=None):
	...
	if fieldname in (sparrow.model.default_fields + sparrow.model.child_table_fields):
		sparrow.throw(_("Cannot edit standard fields"))
	...
	if not sparrow.get_meta(doctype).istable:
		doc = sparrow.get_doc(doctype, name)
		doc.update(values)
	else:
		doc = sparrow.db.get_value(doctype, name, ["parenttype", "parent"], as_dict=True)
		doc = sparrow.get_doc(doc.parenttype, doc.parent)
...
def insert_many(docs=None):
	...
	if len(docs) > 200:
		sparrow.throw(_("Only 200 inserts allowed in one request"))
...
def rename_doc(doctype, old_name, new_name, merge=False):
	...
	new_name = sparrow.rename_doc(doctype, old_name, new_name, merge=merge)
	return new_name
...
def cancel(doctype, name):
	...
	wrapper = sparrow.get_doc(doctype, name)
	wrapper.cancel()
...
def bulk_update(docs):
	...
	for doc in docs:
		doc.pop("flags", None)
		try:
			existing_doc = sparrow.get_doc(doc["doctype"], doc["docname"])
			existing_doc.update(doc)
			existing_doc.save()
		except Exception:
			failed_docs.append({"doc": doc, "exc": sparrow.utils.get_traceback()})
...
def has_permission(doctype, docname, perm_type="read"):
	...
	return {"has_permission": sparrow.has_permission(doctype, perm_type.lower(), docname)}
...
def get_doc_permissions(doctype, docname):
	...
	return {"permissions": sparrow.permissions.get_doc_permissions(doc)}
...
def get_password(doctype, name, fieldname):
	...
	return sparrow.get_doc(doctype, name).get_password(fieldname)


@sparrow.whitelist()
def get_js(items):
	"""Load JS code files.  Will also append translations
	and extend `sparrow._messages`
	...
	for src in items:
		...
		if ".." in src or src[0] != "assets":
			sparrow.throw(_("Invalid file path: {0}").format("/".join(src)))

		contentpath = os.path.join(sparrow.local.sites_path, *src)
		with open(contentpath) as srcfile:
			code = sparrow.utils.cstr(srcfile.read())
...
def get_time_zone():
	...
	return {"time_zone": sparrow.defaults.get_defaults().get("time_zone")}
...
def is_document_amended(doctype, docname):
	if sparrow.permissions.has_permission(doctype):
		try:
			return sparrow.db.exists(doctype, {"amended_from": docname})
		except sparrow.db.InternalError:
			pass
...
def validate_link(doctype: str, docname: str, fields=None):
	if not isinstance(doctype, str):
		sparrow.throw(_("DocType must be a string"))
	...
	values = sparrow._dict()
	values.name = sparrow.db.get_value(doctype, docname, cache=True)

	fields = sparrow.parse_json(fields)
	...
	try:
		...
		sparrow.msgprint(
			_("You need {0} permission to fetch values from {1} {2}").format(
				sparrow.bold(_("Read")), sparrow.bold(doctype), sparrow.bold(docname)
			),
...
def insert_doc(doc) -> "Document":
	...
	doc = sparrow._dict(doc)
	if sparrow.is_table(doc.doctype):
		if not (doc.parenttype and doc.parent and doc.parentfield):
			sparrow.throw(_("Parenttype, Parent and Parentfield are required to insert a child record"))

		# inserting a child record
		parent = sparrow.get_doc(doc.parenttype, doc.parent)
		parent.append(doc.parentfield, doc)
		parent.save()
		return parent

	return sparrow.get_doc(doc).insert()
...
def delete_doc(doctype, name):
	...
	if sparrow.is_table(doctype):
		values = sparrow.db.get_value(doctype, name, ["parenttype", "parent", "parentfield"])
		if not values:
			raise sparrow.DoesNotExistError
		parenttype, parent, parentfield = values
		parent = sparrow.get_doc(parenttype, parent)
