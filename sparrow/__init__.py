...
Sparrow, pronounced fra-pay, is a full stack, batteries-included, web
...
from sparrow.query_builder import (
	get_query,
	get_query_builder,
	patch_query_aggregation,
	patch_query_execute,
)
from sparrow.utils.caching import request_cache
from sparrow.utils.data import cint, cstr, sbool
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from sparrow.translate import get_all_translations
...
def get_lang_dict(fortype: str, name: str | None = None) -> dict[str, str]:
	...
	from sparrow.translate import get_dict

	return get_dict(fortype, name)


def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `sparrow.local.lang`"""
	from sparrow.translate import get_user_lang
...
if TYPE_CHECKING:
	from sparrow.database.mariadb.database import MariaDBDatabase
	from sparrow.database.postgres.database import PostgresDatabase
	from sparrow.model.document import Document
	from sparrow.query_builder.builder import MariaDB, Postgres
	from sparrow.utils.redis_wrapper import RedisWrapper
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force=False) -> None:
	"""Initialize sparrow for the current site. Reset thread locals `sparrow.local`"""
...
def connect(
	...
	:param site: If site is given, calls `sparrow.init`.
	:param db_name: Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from sparrow.database import get_db
...
def get_traceback(with_context: bool = False) -> str:
	...
	from sparrow.utils import get_traceback
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `sparrow.ValidationError`"""
	msgprint(
		msg,
		raise_exception=exc,
		title=title,
...
def create_folder(path, with_init=False):
	...
	from sparrow.utils import touch_file
...
def get_roles(username=None) -> list[str]:
	...
	return sparrow.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from sparrow.utils import md_to_html
...
def read_only():
	def innfn(fn):
		def wrapper_fn(*args, **kwargs):
			...
			# sparrow.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `sparrow.PermissionError` if the user does not have any of the permitted roles.
...
def clear_cache(user: str | None = None, doctype: str | None = None):
	...
	import sparrow.utils.caching

	if doctype:
		sparrow.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		sparrow.cache_manager.clear_user_cache(user)
	else:  # everything
		from sparrow import translate

		sparrow.cache_manager.clear_user_cache()
		sparrow.cache_manager.clear_domain_cache()
	...
	sparrow.utils.caching._SITE_CACHE.clear()
	local.role_permissions = {}
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	permissions = sparrow.permissions.get_role_permissions(doctype, user=user)
...
def has_permission(
	...
	*,
	...
	Raises `sparrow.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = sparrow.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `sparrow.PermissionError` if not permitted.
...
def get_precision(
	...
	from sparrow.model.meta import get_field_precision
...
def new_doc(
	...
	from sparrow.model.create_new import get_new_doc

	return get_new_doc(doctype, parent_doc, parentfield, as_dict=as_dict)


def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `sparrow.client.set_value`"""
	import sparrow.client

	return sparrow.client.set_value(doctype, docname, fieldname, value)
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `sparrow.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = sparrow.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = sparrow.get_doc("ToDo", "TD0001")

	"""
	import sparrow.model.document

	doc = sparrow.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `sparrow.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `sparrow.model.meta.Meta` instance of given doctype name."""
	import sparrow.model.meta

	return sparrow.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import sparrow.modules

	return sparrow.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `sparrow.model.delete_doc.delete_doc`.
	...
	import sparrow.model.delete_doc

	return sparrow.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return sparrow.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `sparrow.model.rename_doc.rename_doc`
	"""
...
def get_module_path(module, *joins):
	...
	from sparrow.modules.utils import get_module_app
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = sparrow.utils.strip(content)
...
def import_doc(path):
	...
	from sparrow.core.doctype.data_import.data_import import import_doc
...
def compare(val1, condition, val2):
	"""Compare two values using `sparrow.utils.compare`
	...
	return sparrow.utils.compare(val1, condition, val2)
...
def redirect(url):
	...
	from sparrow.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        sparrow.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import sparrow.desk.reportview

	return sparrow.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `sparrow.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        sparrow.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        sparrow.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])

	        # filter as a list of dicts
	        sparrow.get_list("ToDo", fields="*", filters = {"description": ("like", "test%")})
	"""
	import sparrow.model.db_query

	return sparrow.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `sparrow.model.db_query`. Will **not** check for permissions.
	Parameters are same as `sparrow.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        sparrow.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        sparrow.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])

	        # filter as a list of dicts
	        sparrow.get_all("ToDo", fields=["*"], filters = {"description": ("like", "test%")})
	"""
...
def get_value(*args, **kwargs):
	...
	Alias for `sparrow.db.get_value`
...
def as_json(obj: dict | list, indent=1, separators=None) -> str:
	from sparrow.utils.response import json_handler
...
def get_test_records(doctype):
	...
	from sparrow.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import sparrow.utils.formatters

	return sparrow.utils.formatters.format_value(*args, **kwargs)
...
def attach_print(
	...
	from sparrow.utils import scrub_urls
	from sparrow.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import sparrow.realtime

	return sparrow.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return sparrow.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: sparrow.local.cache[namespace]
	:param key: sparrow.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import sparrow.utils.background_jobs

	return sparrow.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return sparrow.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(
	...
	from sparrow.utils.logger import get_logger
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from sparrow.utils.safe_exec import safe_eval
...
def get_active_domains():
	from sparrow.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> sparrow.get_version('User', 'foobar@gmail.com')
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from sparrow.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from sparrow.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from sparrow.utils import squashify
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from sparrow.desk.search import sanitize_searchfield
...
if _tune_gc:
	...
	# This number is quite small for sparrow where a single query can potentially create 700+
	# objects easily.
