function setup_list_filters() {
	...
	sparrow.call('sparrow.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = sparrow.utils.get_query_params()[df.fieldname];
			const f = sparrow.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(sparrow.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = sparrow.utils.make_query_string(query_params);
...
sparrow.ready(setup_list_filters);
