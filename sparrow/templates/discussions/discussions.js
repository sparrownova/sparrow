...
const setup_socket_io = () => {
	sparrow.socketio.init(window.socketio_port || "9000");
	sparrow.socketio.socket.on("publish_message", (data) => {
		publish_message(data);
	});
	sparrow.socketio.socket.on("update_message", (data) => {
		update_message(data);
	});
	sparrow.socketio.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const publish_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == sparrow.session.user) {
	...
	} else if (topic.owner == sparrow.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (reply) {
		...
		sparrow.call({
			method: "sparrow.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = sparrow.get_palette(element.attr("title"));
...
const perform_action = (e) => {
	...
	if (action === "edit") {
		...
		sparrow.call({
			method: "sparrow.website.doctype.discussion_reply.discussion_reply.delete_message",
			args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	sparrow.session.user != owner && $template.find(".dropdown").addClass("hide");
	return $template.prop("outerHTML");
};
