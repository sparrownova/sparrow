...
class DbManager:
	...
	def restore_database(target, source, user, password):
		...
		from sparrow.utils import make_esc
		...
		command = (
			...
			+ ("-P{port}" if sparrow.db.port else "")
			+ " {target} {source}"
		)
		command = command.format(
			...
			host=esc(sparrow.db.host),
			target=esc(target),
			source=source,
			port=sparrow.db.port,
		)

		os.system(command)
		sparrow.cache().delete_keys("")  # Delete all keys associated with this site.
