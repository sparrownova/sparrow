...
from sparrow.database.mariadb.schema import MariaDBTable
from sparrow.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if self.user not in (sparrow.flags.root_login, "root"):
			conn_settings["database"] = self.user

		if self.port:
			conn_settings["port"] = int(self.port)

		if sparrow.conf.local_infile:
			conn_settings["local_infile"] = sparrow.conf.local_infile

		if sparrow.conf.db_ssl_ca and sparrow.conf.db_ssl_cert and sparrow.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": sparrow.conf.db_ssl_ca,
				"cert": sparrow.conf.db_ssl_cert,
				"key": sparrow.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via sparrow.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = sparrow.as_unicode(escape_string(sparrow.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = sparrow.cache().get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			sparrow.cache().set_value("db_tables", tables)
