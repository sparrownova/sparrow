...
from sparrow.database.schema import DBTable, get_definition
from sparrow.model import log_types
from sparrow.utils import cint, flt


class PostgresTable(DBTable):
	def create(self):
		varchar_len = sparrow.db.VARCHAR_LEN
		...
		if (
			...
			sparrow.db.create_sequence(self.doctype, check_not_exists=True, cache=sparrow.db.SEQUENCE_CACHE)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			sparrow.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{sparrow.db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				sparrow.db.sql(final_alter_query)
			if create_contraint_query:
				# nosemgrep
				sparrow.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				sparrow.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if sparrow.db.is_duplicate_fieldname(e):
				sparrow.throw(str(e))
			elif sparrow.db.is_duplicate_entry(e):
