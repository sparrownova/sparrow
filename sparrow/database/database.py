...
from sparrow.database.utils import (
	DefaultOrderBy,
	EmptyQueryValues,
	FallBackDateTimeStr,
	LazyMogrify,
...
from sparrow.exceptions import DoesNotExistError, ImplicitCommitError
from sparrow.model.utils.link_count import flush_local_link_count
from sparrow.query_builder.functions import Count
from sparrow.utils import cast as cast_fieldtype
from sparrow.utils import cint, get_datetime, get_table_name, getdate, now, sbool
from sparrow.utils.deprecations import deprecated, deprecation_warning
...
class Database:
	...
	class InvalidColumnName(sparrow.ValidationError):
		pass
	...
	def __init__(
		...
		self.setup_type_map()
		self.host = host or sparrow.conf.db_host or "127.0.0.1"
		self.port = port or sparrow.conf.db_port or ""
		self.user = user or sparrow.conf.db_name
		self.db_name = sparrow.conf.db_name
		self._conn = None

		if ac_name:
			self.user = ac_name or sparrow.conf.db_name

		if use_default:
			self.user = sparrow.conf.db_name

		self.transaction_writes = 0
		self.auto_commit_on_many_writes = 0

		self.password = password or sparrow.conf.db_password
		self.value_cache = {}
		self.logger = sparrow.logger("database")
	...
	def sql(
		...
		Examples:
		        ...
		        sparrow.db.sql("select name from tabCustomer", as_dict=True)

		        # return names beginning with a
		        sparrow.db.sql("select name from tabCustomer where name like %s", "a%")

		        # values as dict
		        sparrow.db.sql("select name from tabCustomer where name like %(name)s and owner=%(owner)s",
		...
		try:
			...
			if self.is_syntax_error(e):
				sparrow.errprint(f"Syntax error in query:\n{query} {values or ''}")

			elif self.is_deadlocked(e):
				raise sparrow.QueryDeadlockError(e) from e

			elif self.is_timedout(e):
				raise sparrow.QueryTimeoutError(e) from e
				...
				sparrow.throw(
					...
					exc=sparrow.InReadOnlyMode,
				)

			# TODO: added temporarily
			elif self.db_type == "postgres":
				traceback.print_stack()
				sparrow.errprint(f"Error in query:\n{e}")
				raise

			elif isinstance(e, self.ProgrammingError):
				if sparrow.conf.developer_mode:
					traceback.print_stack()
					sparrow.errprint(f"Error in query:\n{query, values}")
				raise
		...
		if debug:
			time_end = time()
			sparrow.errprint(f"Execution time: {time_end - time_start:.2f} sec")
	...
	def _log_query(
		...
		if sparrow.conf.allow_tests and sparrow.cache().get_value("flag_print_sql"):
		...
		if debug:
			...
			sparrow.errprint(_query)

		if sparrow.conf.logging == 2:
			_query = _query or str(mogrified_query)
			sparrow.log(f"#### query\n{_query}\n####")
	...
	def mogrify(self, query: Query, values: QueryValues):
		...
		try:
			...
			if isinstance(values, dict):
				return query % {k: sparrow.db.escape(v) if isinstance(v, str) else v for k, v in values.items()}
			elif isinstance(values, (list, tuple)):
				return query % tuple(sparrow.db.escape(v) if isinstance(v, str) else v for v in values)
			return query, values
	...
	def explain_query(self, query, values=None):
		...
		sparrow.errprint("--- query explain ---")
		try:
			self._cursor.execute(f"EXPLAIN {query}", values)
		except Exception as e:
			sparrow.errprint(f"error in query explain: {e}")
		else:
			sparrow.errprint(json.dumps(self.fetch_as_dict(), indent=1))
			sparrow.errprint("--- query explain end ---")
	...
	def sql_list(self, query, values=(), debug=False, **kwargs):
		...
		Example:
		        ...
		        doctypes = sparrow.db.sql_list("select name from DocType")
		"""
		...
		if query[:6].lower() in ("update", "insert", "delete"):
			...
			if self.transaction_writes > self.MAX_WRITES_PER_TRANSACTION:
				if self.auto_commit_on_many_writes:
					...
					raise sparrow.TooManyWritesError(msg)
	...
	def fetch_as_dict(self, formatted=0, as_utf8=0) -> list[sparrow._dict]:
		...
		if not as_utf8:
			return [sparrow._dict(zip(keys, row)) for row in result]
		...
		for r in result:
			...
			ret.append(sparrow._dict(zip(keys, values)))
		return ret

	@staticmethod
	def clear_db_table_cache(query):
		if query and is_query_type(query, ("drop", "create")):
			sparrow.cache().delete_key("db_tables")
	...
	def get_value(
		...
		*,
		...
		Example:
		        ...
		        sparrow.db.get_value("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        sparrow.db.get_value("User", "test@example.com", "last_login")

		        last_login, last_ip = sparrow.db.get_value("User", "test@example.com",
		                ["last_login", "last_ip"])

		        # returns default date_format
		        sparrow.db.get_value("System Settings", None, "date_format")
		"""
	...
	def get_values(
		...
		*,
		...
		Example:
		        ...
		        # return first customer starting with a
		        customers = sparrow.db.get_values("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        user = sparrow.db.get_values("User", "test@example.com", "*")[0]
		"""
		out = None
		...
		if isinstance(filters, list):
			...
			if (filters is not None) and (filters != doctype or doctype == "DocType"):
				try:
					...
					if ignore and (sparrow.db.is_missing_column(e) or sparrow.db.is_table_missing(e)):
						# table or column not found, return None
						out = None
					elif (not ignore) and sparrow.db.is_table_missing(e):
	...
	def get_singles_dict(self, doctype, debug=False, *, for_update=False, cast=False):
		...
		Example:
		        ...
		        account_settings = sparrow.db.get_singles_dict("Accounts Settings")
		"""
		queried_result = sparrow.qb.get_query(
			"Singles",
		...
		if not cast:
			return sparrow._dict(queried_result)
	...
	def get_all(*args, **kwargs):
		return sparrow.get_all(*args, **kwargs)

	@staticmethod
	def get_list(*args, **kwargs):
		return sparrow.get_list(*args, **kwargs)
	...
	def set_single_value(
		...
		**kwargs,
		...
		Example:
		        ...
		        company = sparrow.db.set_single_value("System Settings", "deny_multiple_sessions", True)
		"""
	...
	def get_single_value(self, doctype, fieldname, cache=True):
		...
		Example:
		        ...
		        company = sparrow.db.get_single_value('Global Defaults', 'default_company')
		"""
		...
		df = sparrow.get_meta(doctype).get_field(fieldname)

		if not df:
			sparrow.throw(
				_("Invalid field name: {0}").format(sparrow.bold(fieldname)), self.InvalidColumnName
			)
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		if update_modified:
			...
			modified_by = modified_by or sparrow.session.user
		...
		if is_single_doctype:
			...
			query = (
				sparrow.qb.into("Singles").columns("doctype", "field", "value").insert(*singles_data)
			).run(debug=debug)
			sparrow.clear_document_cache(dt, dt)
			...
			if isinstance(dn, str):
				sparrow.clear_document_cache(dt, dn)
			else:
				# TODO: Fix this; doesn't work rn - gavin@sparrow.io
				# sparrow.cache().hdel_keys(dt, "document_cache")
				# Workaround: clear all document caches
				sparrow.cache().delete_value("document_cache")
	...
	def touch(self, doctype, docname):
		...
		sparrow.qb.update(DocType).set(DocType.modified, modified).where(DocType.name == docname).run()
		return modified
	...
	def set_temp(value):
		...
		sparrow.cache().hset("temp", key, value)
		return key

	@staticmethod
	def get_temp(key):
		"""Return the temperory value and delete it."""
		return sparrow.cache().hget("temp", key)
	...
	def set_default(key, val, parent="__default", parenttype=None):
		...
		sparrow.defaults.set_default(key, val, parent, parenttype)
	...
	def add_default(key, val, parent="__default", parenttype=None):
		...
		sparrow.defaults.add_default(key, val, parent, parenttype)

	@staticmethod
	def get_defaults(key=None, parent="__default"):
		"""Get all defaults"""
		defaults = sparrow.defaults.get_defaults_for(parent)
		if not key:
			return defaults
		...
		return defaults.get(sparrow.scrub(key))

	def begin(self, *, read_only=False):
		read_only = read_only or sparrow.flags.read_only
		mode = "READ ONLY" if read_only else ""
		self.sql(f"START TRANSACTION {mode}")

	def commit(self):
		"""Commit current transaction. Calls SQL `COMMIT`."""
		for method in sparrow.local.before_commit:
			sparrow.call(method[0], *(method[1] or []), **(method[2] or {}))
	...
	def add_before_commit(self, method, args=None, kwargs=None):
		sparrow.local.before_commit.append([method, args, kwargs])

	@staticmethod
	def flush_realtime_log():
		for args in sparrow.local.realtime_log:
			sparrow.realtime.emit_via_redis(*args)
	...
	def savepoint(self, save_point):
		...
		Changes can be undone to a save point by doing sparrow.db.rollback(save_point)
	...
	def rollback(self, *, save_point=None):
		...
		if save_point:
			...
			self.sql("rollback")
			self.begin()
			for obj in dict.fromkeys(sparrow.local.rollback_observers):
				if hasattr(obj, "on_rollback"):
					obj.on_rollback()
			sparrow.local.rollback_observers = []

			sparrow.local.realtime_log = []
			sparrow.flags.enqueue_after_commit = []
	...
	def a_row_exists(self, doctype):
		...
		return sparrow.get_all(doctype, limit=1, order_by=None, as_list=True)
	...
	def count(self, dt, filters=None, debug=False, cache=False, distinct: bool = True):
		...
		if cache and not filters:
			cache_count = sparrow.cache().get_value(f"doctype:count:{dt}")
			if cache_count is not None:
				return cache_count
		count = sparrow.qb.get_query(
			table=dt,
			filters=filters,
			fields=Count("*"),
			distinct=distinct,
		...
		if not filters and cache:
			sparrow.cache().set_value(f"doctype:count:{dt}", count, expires_in_sec=86400)
		return count
	...
	def get_creation_count(self, doctype, minutes):
		...
		from sparrow.utils import now_datetime

		Table = sparrow.qb.DocType(doctype)
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = sparrow.cache().hget("table_columns", table)
		if columns is None:
			information_schema = sparrow.qb.Schema("information_schema")

			columns = (
				sparrow.qb.from_(information_schema.columns)
			...
			if columns:
				sparrow.cache().hset("table_columns", table, columns)
	...
	def get_system_setting(self, key):
		return sparrow.get_system_settings(key)
	...
	def is_column_missing(e):
		return sparrow.db.is_missing_column(e)

	def get_descendants(self, doctype, name):
		"""Return descendants of the group node in tree"""
		from sparrow.utils.nestedset import get_descendants_of
	...
	def log_touched_tables(self, query):
		if is_query_type(query, ("insert", "delete", "update", "alter", "drop", "rename")):
			...
			if sparrow.flags.touched_tables is None:
				sparrow.flags.touched_tables = set()
			sparrow.flags.touched_tables.update(tables)
	...
	def create_sequence(self, *args, **kwargs):
		from sparrow.database.sequence import create_sequence

		return create_sequence(*args, **kwargs)

	def set_next_sequence_val(self, *args, **kwargs):
		from sparrow.database.sequence import set_next_val

		set_next_val(*args, **kwargs)

	def get_next_sequence_val(self, *args, **kwargs):
		from sparrow.database.sequence import get_next_val
...
def enqueue_jobs_after_commit():
	...
	if sparrow.flags.enqueue_after_commit and len(sparrow.flags.enqueue_after_commit) > 0:
		for job in sparrow.flags.enqueue_after_commit:
			q = get_queue(job.get("queue"), is_async=job.get("is_async"))
			q.enqueue_call(
				execute_job,
				timeout=job.get("timeout"),
				kwargs=job.get("queue_args"),
				failure_ttl=sparrow.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
				result_ttl=sparrow.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
...
def get_query_execution_timeout() -> int:
	...
	if not sparrow.conf.get("enable_db_statement_timeout"):
		return 0

	# Zero means no timeout, which is the default value in db.
	timeout = 0
	with suppress(Exception):
		if getattr(sparrow.local, "request", None):
			timeout = sparrow.conf.http_timeout or 300
