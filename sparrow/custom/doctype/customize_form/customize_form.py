...
from sparrow.custom.doctype.property_setter.property_setter import delete_property_setter
from sparrow.model import core_doctypes_list, no_value_fields
...
class CustomizeForm(Document):
	def on_update(self):
		sparrow.db.delete("Singles", {"doctype": "Customize Form"})
		sparrow.db.delete("Customize Form Field")
	...
	def validate_doctype(self, meta):
		...
		if self.doc_type in core_doctypes_list:
			sparrow.throw(_("Core DocTypes cannot be customized."))

		if meta.issingle:
			sparrow.throw(_("Single DocTypes cannot be customized."))

		if meta.custom:
			sparrow.throw(_("Only standard DocTypes are allowed to be customized from Customize Form."))
	...
	def get_name_translation(self):
		...
		return sparrow.get_value(
			"Translation",
			{"source_text": self.doc_type, "language": sparrow.local.lang or "en"},
	...
	def set_name_translation(self):
		...
		if not self.label:
			if current:
				# clear translation
				sparrow.delete_doc("Translation", current.name)
			return
			...
			sparrow.get_doc(
				{
					...
					"language_code": sparrow.local.lang or "en",
				}
			).insert()
			return

		if self.label != current.translated_text:
			sparrow.db.set_value("Translation", current.name, "translated_text", self.label)
	...
	def save_customization(self):
		...
		if self.flags.update_db:
			sparrow.db.updatedb(self.doc_type)

		if not hasattr(self, "hide_success") or not self.hide_success:
			sparrow.msgprint(_("{0} updated").format(_(self.doc_type)), alert=True)
	...
	def allow_property_change(self, prop, meta_df, df):
		if prop == "fieldtype":
			...
			sparrow.msgprint(_("You can't set 'Options' for field {0}").format(df.label))
			return False

		elif prop == "translatable" and not supports_translation(df.get("fieldtype")):
			sparrow.msgprint(_("You can't set 'Translatable' for field {0}").format(df.label))
			return False
	...
	def set_property_setters_for_actions_and_links(self, meta):
		...
		for doctype, fieldname, field_map in (
			...
			for i, d in enumerate(self.get(fieldname) or []):
				d.idx = i
				if sparrow.db.exists(doctype, d.name) and not d.custom:
					# check property and apply property setter
					original = sparrow.get_doc(doctype, d.name)
	...
	def update_order_property_setter(self, has_custom, fieldname):
		...
		if has_custom:
			...
			sparrow.db.delete("Property Setter", dict(property=property_name, doc_type=self.doc_type))
	...
	def clear_removed_items(self, doctype, items):
		...
		"""
		if items:
			sparrow.db.delete(doctype, dict(parent=self.doc_type, custom=1, name=("not in", items)))
		else:
			sparrow.db.delete(doctype, dict(parent=self.doc_type, custom=1))
	...
	def update_custom_fields(self):
		for i, df in enumerate(self.get("fields")):
			...
			if not sparrow.db.exists("Custom Field", {"dt": self.doc_type, "fieldname": df.fieldname}):
	...
	def add_custom_field(self, df, i):
		d = sparrow.new_doc("Custom Field")
	...
	def update_in_custom_field(self, df, i):
		...
		custom_field = sparrow.get_doc("Custom Field", meta_df[0].name)
		changed = False
		for prop in docfield_properties:
	...
	def delete_custom_fields(self):
		...
		for fieldname in fields_to_remove:
			...
			if not is_standard_or_system_generated_field(df):
				sparrow.delete_doc("Custom Field", df.name)
	...
	def get_existing_property_value(self, property_name, fieldname=None):
		...
		if fieldname:
			...
			if sparrow.db.has_column("DocType", property_name):
				property_value = sparrow.db.get_value("DocType", self.doc_type, property_name)
			else:
				property_value = None
	...
	def validate_fieldtype_change(self, df, old_value, new_value):
		...
		if allowed:
			old_value_length = cint(sparrow.db.type_map.get(old_value)[1])
			new_value_length = cint(sparrow.db.type_map.get(new_value)[1])
	...
	def validate_fieldtype_length(self):
		for field in self.check_length_for_fieldtypes:
			df = field.get("df")
			max_length = cint(sparrow.db.type_map.get(df.fieldtype)[1])
			...
			label = df.label
			for doc in docs:
				links.append(sparrow.utils.get_link_to_form(self.doc_type, doc.name))
			...
			if docs:
				sparrow.throw(
					...
					).format(sparrow.bold(label), links_str, sparrow.bold(max_length)),
...
def reset_customization(doctype):
	...
	for setter in setters:
		sparrow.delete_doc("Property Setter", setter)
	...
	for field in custom_fields:
		sparrow.delete_doc("Custom Field", field)
