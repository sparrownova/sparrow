...
from sparrow.model import core_doctypes_list
from sparrow.model.docfield import supports_translation
from sparrow.model.document import Document
from sparrow.query_builder.functions import IfNull
from sparrow.utils import cstr, random_string
...
class CustomField(Document):
		...
		if not self.fieldname:
			...
			if not label:
				if self.fieldtype in ["Section Break", "Column Break", "Tab Break"]:
					...
					sparrow.throw(_("Label is mandatory"))
	...
	def validate(self):
		...
		from sparrow.core.doctype.doctype.doctype import check_fieldname_conflicts
		...
		if not self.fieldname:
			sparrow.throw(_("Fieldname not set for Custom Field"))
	...
	def on_update(self):
		...
		if not sparrow.flags.in_create_custom_fields:
			sparrow.clear_cache(doctype=self.dt)
			sparrow.db.updatedb(self.dt)

	def on_trash(self):
		# check if Admin owned field
		if self.owner == "Administrator" and sparrow.session.user != "Administrator":
			sparrow.throw(
				_(
					"Custom Field {0} is created by the Administrator and can only be deleted through the Administrator account."
				).format(sparrow.bold(self.label))
			)

		# delete property setter entries
		sparrow.db.delete("Property Setter", {"doc_type": self.dt, "field_name": self.fieldname})
	...
	def validate_insert_after(self, meta):
		if not meta.get_field(self.insert_after):
			sparrow.throw(
				...
				sparrow.DoesNotExistError,
			)

		if self.fieldname == self.insert_after:
			sparrow.throw(_("Insert After cannot be set as {0}").format(meta.get_label(self.insert_after)))
...
def get_fields_label(doctype=None):
	...
	if doctype in core_doctypes_list:
		return sparrow.msgprint(_("Custom Fields cannot be added to core DocTypes."))

	if meta.custom:
		return sparrow.msgprint(_("Custom Fields can only be added to a standard DocType."))

	return [
		{"value": df.fieldname or "", "label": _(df.label or "")}
		for df in sparrow.get_meta(doctype).get("fields")
	]


def create_custom_field_if_values_exist(doctype, df):
	df = sparrow._dict(df)
	if df.fieldname in sparrow.db.get_table_columns(doctype) and sparrow.db.count(
...
def create_custom_field(doctype, df, ignore_validate=False, is_system_generated=True):
	...
	if not df.fieldname and df.label:
		df.fieldname = sparrow.scrub(df.label)
	if not sparrow.db.get_value("Custom Field", {"dt": doctype, "fieldname": df.fieldname}):
...
def create_custom_fields(custom_fields, ignore_validate=False, update=True):
	...
	try:
		sparrow.flags.in_create_custom_fields = True
		doctypes_to_update = set()

		if sparrow.flags.in_setup_wizard:
		...
		for doctypes, fields in custom_fields.items():
			...
			for doctype in doctypes:
				...
				for df in fields:
					field = sparrow.db.get_value("Custom Field", {"dt": doctype, "fieldname": df["fieldname"]})
					if not field:
						try:
							df = df.copy()
						...
						except sparrow.exceptions.DuplicateEntryError:
							pass
		...
		sparrow.flags.in_create_custom_fields = False
...
def rename_fieldname(custom_field: str, fieldname: str):
	...
	field: CustomField = sparrow.get_doc("Custom Field", custom_field)
	...
	if field.is_system_generated:
		sparrow.throw(_("System Generated Fields can not be renamed"))
	if sparrow.db.has_column(parent_doctype, fieldname):
		sparrow.throw(_("Can not rename as fieldname {0} is already present on DocType."))
	if old_fieldname == new_fieldname:
		sparrow.msgprint(_("Old and new fieldnames are same."), alert=True)
		return

	sparrow.db.rename_column(parent_doctype, old_fieldname, new_fieldname)
