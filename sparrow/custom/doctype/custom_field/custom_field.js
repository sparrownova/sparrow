...
sparrow.ui.form.on("Custom Field", {
	setup: function (frm) {
	...
	dt: function (frm) {
		...
		return sparrow.call({
			method: "sparrow.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && sparrow.utils.has_special_chars(frm.doc.label)) {
	...
	add_rename_field(frm) {
		frm.add_custom_button(__("Rename Fieldname"), () => {
			sparrow.prompt(
				...
				function (data) {
					sparrow.call({
						method: "sparrow.custom.doctype.custom_field.custom_field.rename_fieldname",
						args: {
...
sparrow.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";
