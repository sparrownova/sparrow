...
from sparrow import _, msgprint
...
def check_admin_or_system_manager(user=None):
	from sparrow.utils.commands import warn

	warn(
		"The function check_admin_or_system_manager will be deprecated in version 15."
		'Please use sparrow.only_for("System Manager") instead.',
	...
	if ("System Manager" not in sparrow.get_roles(user)) and (user != "Administrator"):
		sparrow.throw(_("Not permitted"), sparrow.PermissionError)


def print_has_permission_check_logs(func):
	def inner(*args, **kwargs):
		sparrow.flags["has_permission_check_logs"] = []
		result = func(*args, **kwargs)
		self_perm_check = True if not kwargs.get("user") else kwargs.get("user") == sparrow.session.user
		...
		if not result and self_perm_check and raise_exception:
			msgprint(("<br>").join(sparrow.flags.get("has_permission_check_logs", [])))
		sparrow.flags.pop("has_permission_check_logs", None)
		return result
...
def has_permission(
	...
	*,
	...
	:param raise_exception:
	        ...
	        If not False, will display a message using sparrow.msgprint
	...
	if ptype == "share" and sparrow.get_system_settings("disable_document_sharing"):
		return False
	...
	if doc:
		if isinstance(doc, str):
			doc = sparrow.get_doc(meta.name, doc)
		perm = get_doc_permissions(doc, user=user, ptype=ptype).get(ptype)
		if not perm:
			push_perm_check_log(
				_("User {0} does not have access to this document").format(sparrow.bold(user))
			)
	else:
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					sparrow.bold(user), sparrow.bold(doctype)
				)
			)
	...
	def false_if_not_shared():
		if ptype in ("read", "write", "share", "submit", "email", "print"):
			...
			if doc:
				...
				shared = sparrow.share.get_shared(
					doctype,
					user,
					rights=rights,
			...
			elif sparrow.share.get_shared(doctype, user, rights=rights, limit=1):
...
def get_doc_permissions(doc, user=None, ptype=None):
	...
	if sparrow.is_table(doc.doctype):
		return {"read": 1, "write": 1}

	meta = sparrow.get_meta(doc.doctype)
...
def get_role_permissions(doctype_meta, user=None, is_owner=None):
	...
	if isinstance(doctype_meta, str):
		doctype_meta = sparrow.get_meta(doctype_meta)  # assuming doctype name was passed
	...
	if not sparrow.local.role_permissions.get(cache_key):
		perms = sparrow._dict(if_owner={})
		...
		sparrow.local.role_permissions[cache_key] = perms

	return sparrow.local.role_permissions[cache_key]


def get_user_permissions(user):
	from sparrow.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None):
	...
	apply_strict_user_permissions = sparrow.get_system_settings("apply_strict_user_permissions")
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = sparrow.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None):
	...
	methods = sparrow.get_hooks("has_permission").get(doc.doctype, [])

	if not methods:
		return None

	for method in reversed(methods):
		controller_permission = sparrow.call(sparrow.get_attr(method), doc=doc, ptype=ptype, user=user)
...
def get_all_perms(role):
	...
	perms = sparrow.get_all("DocPerm", fields="*", filters=dict(role=role))
	custom_perms = sparrow.get_all("Custom DocPerm", fields="*", filters=dict(role=role))
	doctypes_with_custom_perms = sparrow.get_all("Custom DocPerm", pluck="parent", distinct=True)
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return sparrow.get_all("Role", pluck="name")  # return all available roles
		else:
	...
	roles = sparrow.cache().hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype="DocPerm"):
	...
	return sparrow.get_all(perm_doctype, fields=["*"], filters=filters)


def get_doctypes_with_custom_docperms():
	"""Returns all the doctypes with Custom Docperms"""

	doctypes = sparrow.get_all("Custom DocPerm", fields=["parent"], distinct=1)
...
def set_user_permission_if_allowed(doctype, name, user, with_message=False):
	if get_role_permissions(sparrow.get_meta(doctype), user).set_user_permissions != 1:
...
def add_user_permission(
	...
	"""Add user permission"""
	from sparrow.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not sparrow.db.exists(doctype, name):
			sparrow.throw(_("{0} {1} not found").format(_(doctype), name), sparrow.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = sparrow.db.get_value(
		"User Permission", dict(user=user, allow=doctype, for_value=name)
	)
	sparrow.delete_doc("User Permission", user_permission_name)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	if user:
		filters["user"] = user
	user_permissions_for_doctype = sparrow.get_all("User Permission", filters=filters)
	for d in user_permissions_for_doctype:
		sparrow.delete_doc("User Permission", d.name)


def can_import(doctype, raise_exception=False):
	if not ("System Manager" in sparrow.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise sparrow.PermissionError(f"You are not allowed to import: {doctype}")
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False):
	if "System Manager" in sparrow.get_roles():
		...
		role_permissions = sparrow.permissions.get_role_permissions(doctype)
		has_access = role_permissions.get("export") or role_permissions.get("if_owner").get("export")
		if not has_access and raise_exception:
			raise sparrow.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access
...
def update_permission_property(
	...
	from sparrow.core.doctype.doctype.doctype import validate_permissions_for_doctype

	out = setup_custom_perms(doctype)

	name = sparrow.db.get_value(
		"Custom DocPerm", dict(parent=doctype, role=role, permlevel=permlevel, if_owner=if_owner)
	)
	table = DocType("Custom DocPerm")
	sparrow.qb.update(table).set(ptype, value).where(table.name == name).run()
...
def setup_custom_perms(parent):
	...
	if not sparrow.db.exists("Custom DocPerm", dict(parent=parent)):
		copy_perms(parent)
		return True
...
def add_permission(doctype, role, permlevel=0, ptype=None):
	...
	custom_docperm = sparrow.get_doc(
		{
...
def copy_perms(parent):
	...
	for d in sparrow.get_all("DocPerm", fields="*", filters=dict(parent=parent)):
		custom_perm = sparrow.new_doc("Custom DocPerm")
...
def reset_perms(doctype):
	...
	sparrow.db.delete("Custom DocPerm", {"parent": doctype})


def get_linked_doctypes(dt: str) -> list:
	meta = sparrow.get_meta(dt)
	linked_doctypes = [dt] + [
		d.options
		for d in meta.get(
			"fields",
...
def push_perm_check_log(log):
	if sparrow.flags.get("has_permission_check_logs") is None:
		return

	sparrow.flags.get("has_permission_check_logs").append(log)
...
def has_child_permission(
	...
	if isinstance(child_doc, str):
		child_doc = sparrow.db.get_value(
			child_doctype,
			child_doc,
	...
	if not parent_doctype:
		push_perm_check_log(
			_("Please specify a valid parent DocType for {0}").format(sparrow.bold(child_doctype))
		)
		return False

	parent_meta = sparrow.get_meta(parent_doctype)
	...
	if parent_meta.istable or not (
		...
		push_perm_check_log(
			_("{0} is not a valid parent DocType for {1}").format(
				sparrow.bold(parent_doctype), sparrow.bold(child_doctype)
			)
		)
		return False
	...
	if child_doc:
		...
		if not parentfield:
			push_perm_check_log(
				_("Parentfield not specified in {0}: {1}").format(
					sparrow.bold(child_doctype), sparrow.bold(child_doc.name)
				)
			)
			return False
		...
		if parentfield not in valid_parentfields:
			push_perm_check_log(
				_("{0} is not a valid parentfield for {1}").format(
					sparrow.bold(parentfield), sparrow.bold(child_doctype)
				)
			)
			return False
		...
		if permlevel > 0 and permlevel not in parent_meta.get_permlevel_access(ptype, user=user):
			push_perm_check_log(
				_("Insufficient Permission Level for {0}").format(sparrow.bold(parent_doctype))
			)
			return False
