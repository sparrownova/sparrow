...
class GoogleContacts(Document):
	def validate(self):
		if not sparrow.db.get_single_value("Google Settings", "enable"):
			sparrow.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = sparrow.bold(_("Allow Google Contacts Access"))
...
@sparrow.whitelist(methods=["POST"])
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	oauth_code = (
		sparrow.db.get_value("Google Contacts", g_contact, "authorization_code") if not code else code
	)
	oauth_obj = GoogleOAuth("contacts")
...
def get_google_contacts_object(g_contact):
	...
	"""
	account = sparrow.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = sparrow.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	sparrow.db.set_value("Google Contacts", account.name, "last_sync_on", sparrow.utils.now_datetime())

	for idx, connection in enumerate(results):
		sparrow.publish_realtime(
			"import_google_contacts", dict(progress=idx + 1, total=len(results)), user=sparrow.session.user
		)
		# Work-around to fix
		# https://github.com/sparrownova/sparrow/issues/22648
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not sparrow.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		sparrow.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		sparrow.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		sparrow.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		sparrow.msgprint(
