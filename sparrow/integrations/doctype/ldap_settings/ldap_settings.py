...
import sparrow
from sparrow import _, safe_encode
from sparrow.model.document import Document
from sparrow.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			sparrow.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			sparrow.throw(_("Invalid username or password"))
		except Exception as ex:
			sparrow.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = sparrow.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "sparrow.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def sync_roles(self, user: "User", additional_groups: list = None):
		...
		all_mapped_roles = {r.shopper_role for r in self.ldap_groups}
		matched_roles = {
			r.shopper_role for r in self.ldap_groups if r.ldap_group.lower() in lower_groups
		}
	...
	def create_or_update_user(self, user_data: dict, groups: list = None):
		...
		if sparrow.db.exists("User", user_data["email"]):
			user = sparrow.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = sparrow.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			sparrow.throw(_("LDAP is not enabled."))
		...
		try:
			...
			sparrow.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					...
					sparrow.msgprint(_("Password changed successfully."))
				else:
					sparrow.throw(_("Failed to change password."))
			else:
				sparrow.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			sparrow.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	# LDAP LOGIN LOGIC
	args = sparrow.form_dict
	ldap: LDAPSettings = sparrow.get_doc("LDAP Settings")

	user = ldap.authenticate(sparrow.as_unicode(args.usr), sparrow.as_unicode(args.pwd))

	sparrow.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(sparrow.local.login_manager):
			return False

	sparrow.form_dict.pop("pwd", None)
	sparrow.local.login_manager.post_login()
