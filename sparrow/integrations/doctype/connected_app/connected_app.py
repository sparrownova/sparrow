...
if any((os.getenv("CI"), sparrow.conf.developer_mode, sparrow.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	def validate(self):
		base_url = sparrow.utils.get_url()
		callback_path = (
			"/api/method/sparrow.integrations.doctype.connected_app.connected_app.callback/" + self.name
		)
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = sparrow.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		sparrow.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if sparrow.db.exists("Token Cache", token_cache_name):
			token_cache = sparrow.get_doc("Token Cache", token_cache_name)
...
@sparrow.whitelist(methods=["GET"], allow_guest=True)
def callback(code=None, state=None):
	...
	if sparrow.session.user == "Guest":
		sparrow.local.response["type"] = "redirect"
		sparrow.local.response["location"] = "/login?" + urlencode({"redirect-to": sparrow.request.url})
		return

	path = sparrow.request.path[1:].split("/")
	if len(path) != 4 or not path[3]:
		sparrow.throw(_("Invalid Parameters."))

	connected_app = sparrow.get_doc("Connected App", path[3])
	token_cache = sparrow.get_doc("Token Cache", connected_app.name + "-" + sparrow.session.user)

	if state != token_cache.state:
		sparrow.throw(_("Invalid state."))
	...
	sparrow.local.response["type"] = "redirect"
	sparrow.local.response["location"] = token_cache.get("success_uri") or connected_app.get_url()


@sparrow.whitelist()
def has_token(connected_app, connected_user=None):
	app = sparrow.get_doc("Connected App", connected_app)
	token_cache = app.get_token_cache(connected_user or sparrow.session.user)
