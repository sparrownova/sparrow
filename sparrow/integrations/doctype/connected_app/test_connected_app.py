...
def get_connected_app():
	...
	connected_app = sparrow.new_doc(doctype)
	connected_app.provider_name = "sparrow"
...
def get_oauth_client():
	oauth_client = sparrow.new_doc("OAuth Client")
...
class TestConnectedApp(SparrowTestCase):
	def setUp(self):
		...
		Sparrow comes with it's own oAuth2 provider that we can test against. The
		...
		Sparrow's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/sparrow.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		if getattr(self, "oauth_client", None):
			tokens = sparrow.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = sparrow.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = sparrow.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = sparrow.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
