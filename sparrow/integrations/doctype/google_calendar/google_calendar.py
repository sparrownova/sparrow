...
from sparrow.integrations.google_oauth import GoogleOAuth
...
from sparrow.utils.password import set_encrypted_password
...
class GoogleCalendar(Document):
	def validate(self):
		...
		if not google_settings.enable:
			sparrow.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			sparrow.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = sparrow.bold(_("Allow Google Calendar Access"))
			raise sparrow.ValidationError(_("Click on {0} to generate Refresh Token.").format(button_label))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	"""
	google_settings = sparrow.get_doc("Google Settings")
	google_calendar = sparrow.get_doc("Google Calendar", g_calendar)

	redirect_uri = (
		get_request_site_address(True)
		+ "?cmd=sparrow.integrations.doctype.google_calendar.google_calendar.google_callback"
	)

	if not google_calendar.authorization_code or reauthorize:
		sparrow.cache().hset("google_calendar", "google_calendar", google_calendar.name)
		...
		try:
			...
			sparrow.local.response["location"] = "/app/Form/{}/{}".format(
				quote("Google Calendar"), quote(google_calendar.name)
			)

			sparrow.msgprint(_("Google Calendar has been configured."))
...
def google_callback(code=None):
	...
	"""
	google_calendar = sparrow.cache().hget("google_calendar", "google_calendar")
	sparrow.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	sparrow.db.commit()
...
def sync(g_calendar=None):
	...
	google_calendars = sparrow.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	"""
	google_settings = sparrow.get_doc("Google Settings")
	account = sparrow.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": sparrow.db.get_single_value("System Settings", "time_zone"),
			}
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				sparrow.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				sparrow.throw(msg)
	...
	for idx, event in enumerate(results):
		sparrow.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=sparrow.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not sparrow.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			sparrow.get_doc(
				{
					...
					"reference_name": sparrow.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in Sparrow Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	sparrow.get_doc(calendar_event).insert(ignore_permissions=True)


def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in Sparrow Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = sparrow.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	"""
	if (
		not sparrow.db.exists("Google Calendar", {"name": doc.google_calendar})
	...
	try:
		...
		sparrow.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		sparrow.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in Sparrow Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if Sparrow Event is deleted.
	"""

	if not sparrow.db.exists("Google Calendar", {"name": doc.google_calendar}):
		return
