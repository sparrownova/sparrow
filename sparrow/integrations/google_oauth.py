...
import sparrow
from sparrow.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/sparrow.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "sparrow.email.oauth.authorize_google_access",
	"contacts": "sparrow.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"drive": "sparrow.integrations.doctype.google_drive.google_drive.authorize_access",
	"indexing": "sparrow.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = sparrow.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			sparrow.throw(sparrow._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			sparrow.throw(sparrow._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		sparrow.log_error(
			sparrow._(error_title), sparrow._(response.get("error_description", error_message))
		)

		if raise_err:
			sparrow.throw(sparrow._(error_title), GoogleAuthenticationError, sparrow._(error_message))
...
@sparrow.whitelist(methods=["GET"])
def callback(state: str, code: str = None, error: str = None) -> None:
	"""Common callback for google integrations.
	Invokes functions using `sparrow.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to sparrow site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			sparrow.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
