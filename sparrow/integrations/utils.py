...
from sparrow.utils import get_request_session
...
def make_request(method, url, auth=None, headers=None, data=None):
	...
	try:
		...
		sparrow.flags.integration_request = s.request(method, url, data=data, auth=auth, headers=headers)
		sparrow.flags.integration_request.raise_for_status()

		if sparrow.flags.integration_request.headers.get("content-type") == "text/plain; charset=utf-8":
			return parse_qs(sparrow.flags.integration_request.text)

		return sparrow.flags.integration_request.json()
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = sparrow.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else sparrow.as_json(obj, indent=1)
