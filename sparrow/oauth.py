...
from sparrow.utils.data import get_system_timezone
...
class OAuthWebRequestValidator(RequestValidator):
	...
	def validate_client_id(self, client_id, request, *args, **kwargs):
		...
		cli_id = sparrow.db.get_value("OAuth Client", {"name": client_id})
		if cli_id:
			request.client = sparrow.get_doc("OAuth Client", client_id).as_dict()
			return True
		else:
			return False
	...
	def validate_redirect_uri(self, client_id, redirect_uri, request, *args, **kwargs):
		...
		redirect_uris = sparrow.db.get_value("OAuth Client", client_id, "redirect_uris").split(
			get_url_delimiter()
		)
	...
	def get_default_redirect_uri(self, client_id, request, *args, **kwargs):
		...
		redirect_uri = sparrow.db.get_value("OAuth Client", client_id, "default_redirect_uri")
		return redirect_uri
	...
	def save_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		oac = sparrow.new_doc("OAuth Authorization Code")
	...
	def authenticate_client(self, request, *args, **kwargs):
		...
		if request.client_id:
			oc = sparrow.get_doc("OAuth Client", request.client_id)
		else:
			# Extract token, instantiate OAuth Bearer Token and use clientid from there.
			if "refresh_token" in sparrow.form_dict:
				oc = sparrow.get_doc(
					"OAuth Client",
					sparrow.db.get_value(
						"OAuth Bearer Token",
						{"refresh_token": sparrow.form_dict["refresh_token"]},
						"client",
					),
				)
			elif "token" in sparrow.form_dict:
				oc = sparrow.get_doc(
					"OAuth Client",
					sparrow.db.get_value("OAuth Bearer Token", sparrow.form_dict["token"], "client"),
				)
			else:
				oc = sparrow.get_doc(
					"OAuth Client",
					sparrow.db.get_value(
						"OAuth Bearer Token",
						sparrow.get_request_header("Authorization").split(" ")[1],
						"client",
					),
				)
		try:
		...
		return sparrow.session.user == user_id

	def authenticate_client_id(self, client_id, request, *args, **kwargs):
		cli_id = sparrow.db.get_value("OAuth Client", client_id, "name")
		if not cli_id:
			# Don't allow public (non-authenticated) clients
			return False
		else:
			request["client"] = sparrow.get_doc("OAuth Client", cli_id)
			return True
	...
	def validate_code(self, client_id, code, client, request, *args, **kwargs):
		...
		if code in checkcodes:
			request.scopes = sparrow.db.get_value("OAuth Authorization Code", code, "scopes").split(
				get_url_delimiter()
			)
			request.user = sparrow.db.get_value("OAuth Authorization Code", code, "user")
			code_challenge_method = sparrow.db.get_value(
				"OAuth Authorization Code", code, "code_challenge_method"
			)
			code_challenge = sparrow.db.get_value("OAuth Authorization Code", code, "code_challenge")

			if code_challenge and not request.code_verifier:
				if sparrow.db.exists("OAuth Authorization Code", code):
					sparrow.delete_doc("OAuth Authorization Code", code, ignore_permissions=True)
					sparrow.db.commit()
				return False
	...
	def confirm_redirect_uri(self, client_id, code, redirect_uri, client, *args, **kwargs):
		saved_redirect_uri = sparrow.db.get_value("OAuth Client", client_id, "default_redirect_uri")

		redirect_uris = sparrow.db.get_value("OAuth Client", client_id, "redirect_uris")
	...
	def save_bearer_token(self, token, request, *args, **kwargs):
		...
		otoken = sparrow.new_doc("OAuth Bearer Token")
		otoken.client = request.client["name"]
		try:
			otoken.user = (
				request.user
				if request.user
				else sparrow.db.get_value(
					"OAuth Bearer Token",
			...
			otoken.user = sparrow.session.user
		...
		default_redirect_uri = sparrow.db.get_value(
	...
	def invalidate_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		sparrow.db.set_value("OAuth Authorization Code", code, "validity", "Invalid")
		sparrow.db.commit()
	...
	def validate_bearer_token(self, token, scopes, request):
		...
		otoken = sparrow.get_doc("OAuth Bearer Token", token)
		...
		is_token_valid = (
                                 sparrow.utils.datetime.datetime.utcnow().replace(tzinfo=pytz.utc) < token_expiration_utc
		) and otoken.status != "Revoked"
		client_scopes = sparrow.db.get_value("OAuth Client", otoken.client, "scopes").split(
			get_url_delimiter()
		)
	...
	def get_original_scopes(self, refresh_token, request, *args, **kwargs):
		...
		# request.
		obearer_token = sparrow.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token})
	...
	def revoke_token(self, token, token_type_hint, request, *args, **kwargs):
		...
		if token_type_hint == "access_token":
			sparrow.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		elif token_type_hint == "refresh_token":
			sparrow.db.set_value("OAuth Bearer Token", {"refresh_token": token}, "status", "Revoked")
		else:
			sparrow.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		sparrow.db.commit()
	...
	def finalize_id_token(self, id_token, token, token_handler, request):
		# Check whether sparrow server URL is set
		id_token_header = {"typ": "jwt", "alg": "HS256"}

		user = sparrow.get_doc("User", request.user)
		...
		return sparrow.safe_decode(id_token_encoded)

	def get_authorization_code_nonce(self, client_id, code, redirect_uri, request):
		if sparrow.get_value("OAuth Authorization Code", code, "validity") == "Valid":
			return sparrow.get_value("OAuth Authorization Code", code, "nonce")

		return None

	def get_authorization_code_scopes(self, client_id, code, redirect_uri, request):
		scope = sparrow.get_value("OAuth Client", client_id, "scopes")
		if not scope:
			scope = []
		else:
	...
	def validate_id_token(self, token, scopes, request):
		try:
			id_token = sparrow.get_doc("OAuth Bearer Token", token)
	...
	def validate_jwt_bearer_token(self, token, scopes, request):
		try:
			jwt = sparrow.get_doc("OAuth Bearer Token", token)
	...
	def validate_silent_login(self, request):
		...
		- OpenIDConnectHybrid
		"""
		if sparrow.session.user == "Guest" or request.prompt.lower() == "login":
			return False
		else:
			return True
	...
	def validate_user_match(self, id_token_hint, scopes, claims, request):
		...
		if id_token_hint:
			try:
				...
				client_id, client_secret = sparrow.get_value(
					"OAuth Client",
					payload.get("aud"),
				...
				if payload.get("sub") and client_id and client_secret:
					user = sparrow.db.get_value(
						"User Social Login",
						{"userid": payload.get("sub"), "provider": "sparrow"},
						"parent",
					)
					...
					if verified_payload:
						return user.name == sparrow.session.user

			except Exception:
				return False

		elif sparrow.session.user != "Guest":
			return True
...
def delete_oauth2_data():
	...
	code_list = sparrow.get_all("OAuth Authorization Code", filters={"validity": "Invalid"})
	token_list = sparrow.get_all("OAuth Bearer Token", filters={"status": "Revoked"})
	if len(code_list) > 0:
		commit_code = True
	if len(token_list) > 0:
		commit_token = True
	for code in code_list:
		sparrow.delete_doc("OAuth Authorization Code", code["name"])
	for token in token_list:
		sparrow.delete_doc("OAuth Bearer Token", token["name"])
	if commit_code or commit_token:
		sparrow.db.commit()


def get_client_scopes(client_id):
	scopes_string = sparrow.db.get_value("OAuth Client", client_id, "scopes")
...
def get_userinfo(user):
	...
	userinfo = sparrow._dict(
		{
			"sub": sparrow.db.get_value(
				"User Social Login",
				{"parent": user.name, "provider": "sparrow"},
				"userid",
			),
...
def generate_json_error_response(e):
	...
	sparrow.local.response["http_status_code"] = getattr(e, "status_code", 500)
	return


def get_server_url():
	request_url = urlparse(sparrow.request.url)
	request_url = f"{request_url.scheme}://{request_url.netloc}"
	return sparrow.get_value("Social Login Key", "sparrow", "base_url") or request_url
