...
class EventConsumer(Document):
	def validate(self):
		...
		# sparrow.flags.in_test won't work here as tests are running on the consumer site
	...
	def on_update(self):
		if not self.incoming_change:
			...
			sparrow.db.set_value(self.doctype, self.name, "incoming_change", 0)

	def clear_cache(self):
		from sparrow.event_streaming.doctype.event_update_log.event_update_log import (
			ENABLED_DOCTYPES_CACHE_KEY,
		)

		sparrow.cache().delete_value(ENABLED_DOCTYPES_CACHE_KEY)
		return super().clear_cache()

	def on_trash(self):
		for i in sparrow.get_all("Event Update Log Consumer", {"consumer": self.name}):
			sparrow.delete_doc("Event Update Log Consumer", i.name)
	...
	def update_consumer_status(self):
		...
		event_producer = sparrow._dict(event_producer)
		...
		for entry in config:
			...
			entry["status"] = sparrow.db.get_value(
...
def register_consumer(data):
	...
	if sparrow.db.exists("Event Consumer", data["event_consumer"]):
		return None

	user = data["user"]
	if not sparrow.db.exists("User", user):
		sparrow.throw(_("User {0} not found on the producer site").format(user))

	if "System Manager" not in sparrow.get_roles(user):
		sparrow.throw(_("Event Subscriber has to be a System Manager."))

	consumer = sparrow.new_doc("Event Consumer")
...
def notify(consumer):
	...
	if consumer_status == "online":
		try:
			...
			client.post_request(
				{
					"cmd": "sparrow.event_streaming.doctype.event_producer.event_producer.new_event_notification",
	...
	if not consumer.flags.notified:
		enqueued_method = "sparrow.event_streaming.doctype.event_consumer.event_consumer.notify"
		jobs = get_jobs()
		if not jobs or enqueued_method not in jobs[sparrow.local.site] and not consumer.flags.notifed:
			sparrow.enqueue(
...
def has_consumer_access(consumer, update_log):
	...
	if isinstance(consumer, str):
		consumer = sparrow.get_doc("Event Consumer", consumer)

	if not sparrow.db.exists(update_log.ref_doctype, update_log.docname):
		# Delete Log
		# Check if the last Update Log of this document was read by this consumer
		last_update_log = sparrow.get_all(
			"Event Update Log",
			filters={
		...
		last_update_log = sparrow.get_doc("Event Update Log", last_update_log[0].name)
		return len([x for x in last_update_log.consumers if x.consumer == consumer.name])

	doc = sparrow.get_doc(update_log.ref_doctype, update_log.docname)
	try:
		for dt_entry in consumer.consumer_doctypes:
			...
			if condition.startswith("cmd:"):
				...
				return sparrow.call(cmd, **args)
			else:
				return sparrow.safe_eval(condition, sparrow._dict(doc=doc))
