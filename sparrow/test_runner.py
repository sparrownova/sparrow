...
from sparrow.modules import get_module_name, load_doctype_module
...
def main(
	...
	if doctype_list_path:
		...
		with open(sparrow.get_app_path(app, doctype_list_path)) as f:
			doctype = f.read().strip().splitlines()

	if ui_tests:
		print(
			"Selenium testing has been deprecated\nUse snova --site {site_name} run-ui-tests for Cypress tests"
		)
	...
	try:
		sparrow.flags.print_messages = verbose
		...
		sparrow.clear_cache()
		scheduler_disabled_by_user = sparrow.utils.scheduler.is_scheduler_disabled()
		if not scheduler_disabled_by_user:
			sparrow.utils.scheduler.disable_scheduler()

		set_test_email_config()

		if not sparrow.flags.skip_before_tests:
			if verbose:
				print('Running "before_tests" hooks')
			for fn in sparrow.get_hooks("before_tests", app_name=app):
				sparrow.get_attr(fn)()
...
def run_all_tests(
	...
	for app in apps:
		for path, folders, files in os.walk(sparrow.get_pymodule_path(app)):
...
def run_tests_for_doctype(
	...
	for doctype in doctypes:
		module = sparrow.db.get_value("DocType", doctype, "module")
		if not module:
		...
		if force:
			for name in sparrow.db.sql_list("select name from `tab%s`" % doctype):
				sparrow.delete_doc(doctype, name, force=True)
...
def _run_unittest(
	...
	sparrow.flags.tests_verbose = verbose
...
def _add_test(app, path, filename, verbose, test_suite=None, ui_tests=False):
	...
	app_path = sparrow.get_pymodule_path(app)
...
def make_test_records(doctype, verbose=0, force=False, commit=False):
	...
	if sparrow.flags.skip_test_records:
		return
	...
	for options in get_dependencies(doctype):
		...
		if options not in sparrow.local.test_objects:
			sparrow.local.test_objects[options] = []
...
def get_dependencies(doctype):
	...
	for df in meta.get_table_fields():
		link_fields.extend(sparrow.get_meta(df.options).get_link_fields())
...
def make_test_records_for_doctype(doctype, verbose=0, force=False, commit=False):
	...
	if hasattr(test_module, "_make_test_records"):
		sparrow.local.test_objects[doctype] += test_module._make_test_records(verbose)

	elif hasattr(test_module, "test_records"):
		if doctype in sparrow.local.test_objects:
			sparrow.local.test_objects[doctype] += make_test_objects(
		...
		test_records = sparrow.get_test_records(doctype)
		if test_records:
			sparrow.local.test_objects[doctype] += make_test_objects(
...
def make_test_objects(doctype, test_records=None, verbose=None, reset=False, commit=False):
	...
	for doc in test_records:
		...
		if sparrow.db.exists(d.doctype, d.name) and not reset:
			sparrow.db.rollback()
...
def add_to_test_record_log(doctype):
	...
	if doctype not in test_record_log:
		sparrow.flags.test_record_log.append(doctype)
		with open(sparrow.get_site_path(".test_log"), "w") as f:
			f.write("\n".join(filter(None, sparrow.flags.test_record_log)))


def get_test_record_log():
	"""Return the list of doctypes for which test records have been created"""
	if "test_record_log" not in sparrow.flags:
		if os.path.exists(sparrow.get_site_path(".test_log")):
			with open(sparrow.get_site_path(".test_log")) as f:
				sparrow.flags.test_record_log = f.read().splitlines()
		else:
			sparrow.flags.test_record_log = []

	return sparrow.flags.test_record_log
