...
def run_all(skip_failing: bool = False, patch_type: PatchType | None = None) -> None:
	...
	executed = set(sparrow.get_all("Patch Log", fields="patch", pluck="patch"))
	...
	for patch in sparrow.flags.final_patches:
...
def get_all_patches(patch_type: PatchType | None = None) -> list[str]:
	...
	if patch_type and not isinstance(patch_type, PatchType):
		sparrow.throw(f"Unsupported patch type specified: {patch_type}")
...
def get_patches_from_app(app: str, patch_type: PatchType | None = None) -> list[str]:
	...
	patches_txt = sparrow.get_pymodule_path(app, "patches.txt")
	...
	try:
		...
		if patch_type.value in parser.sections():
			...
			sparrow.throw(sparrow._("Patch type {} not found in patches.txt").format(patch_type))
		...
		if not patch_type or patch_type == PatchType.pre_model_sync:
			return sparrow.get_file_items(patches_txt)
...
def reload_doc(args):
	...
	run_single(method=sparrow.modules.reload_doc, methodargs=args)
...
def execute_patch(patchmodule: str, method=None, methodargs=None):
	...
	print(
		f"Executing {patchmodule or methodargs} in {sparrow.local.site} ({sparrow.db.cur_db_name}){docstring}"
	)
	...
	try:
		if patchmodule:
			if patchmodule.startswith("finally:"):
				...
				sparrow.flags.final_patches.append(patchmodule)
			else:
				if has_patch_file:
					_patch()
				else:
...
def update_patch_log(patchmodule):
	...
	sparrow.get_doc({"doctype": "Patch Log", "patch": patchmodule}).insert(ignore_permissions=True)
...
def executed(patchmodule):
	...
	return sparrow.db.get_value("Patch Log", {"patch": patchmodule})


def _patch_mode(enable):
	"""stop/start execution till patch is run"""
	sparrow.local.flags.in_patch = enable
	sparrow.db.commit()


def check_session_stopped():
	"""This function is deprecated. Use maintenance_mode in site config instead."""
	if sparrow.db.get_global("__session_status") == "stop":
		sparrow.msgprint(sparrow.db.get_global("__session_status_message"))
