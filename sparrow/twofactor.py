...
from sparrow.utils import cint, get_datetime, get_url, time_diff_in_seconds
from sparrow.utils.background_jobs import enqueue
from sparrow.utils.password import decrypt, encrypt

PARENT_FOR_DEFAULTS = "__2fa"


def get_default(key):
	return sparrow.db.get_default(key, parent=PARENT_FOR_DEFAULTS)


def set_default(key, value):
	sparrow.db.set_default(key, value, parent=PARENT_FOR_DEFAULTS)


def clear_default(key):
	sparrow.defaults.clear_default(key, parent=PARENT_FOR_DEFAULTS)
...
def toggle_two_factor_auth(state, roles=None):
	...
	for role in roles or []:
		role = sparrow.get_doc("Role", {"role_name": role})
...
def two_factor_is_enabled(user=None):
	...
	if enabled:
		...
		if bypass_two_factor_auth and user:
			...
			if restrict_ip_list and sparrow.local.request_ip:
				for ip in restrict_ip_list:
					if sparrow.local.request_ip.startswith(ip):
						enabled = False
						break
...
def get_cached_user_pass():
	...
	user = pwd = None
	tmp_id = sparrow.form_dict.get("tmp_id")
	if tmp_id:
		user = sparrow.safe_decode(sparrow.cache().get(tmp_id + "_usr"))
		pwd = sparrow.safe_decode(sparrow.cache().get(tmp_id + "_pwd"))
	return (user, pwd)


def authenticate_for_2factor(user):
	"""Authenticate two factor for enabled user before login."""
	if sparrow.form_dict.get("otp"):
		return
	otp_secret = get_otpsecret_for_(user)
	token = int(pyotp.TOTP(otp_secret).now())
	tmp_id = sparrow.generate_hash(length=8)
	cache_2fa_data(user, token, otp_secret, tmp_id)
	verification_obj = get_verification_obj(user, token, otp_secret)
	# Save data in local
	sparrow.local.response["verification"] = verification_obj
	sparrow.local.response["tmp_id"] = tmp_id


def cache_2fa_data(user, token, otp_secret, tmp_id):
	"""Cache and set expiry for data."""
	pwd = sparrow.form_dict.get("pwd")
	...
	if verification_method in ["SMS", "Email"]:
		expiry_time = sparrow.flags.token_expiry or 300
		sparrow.cache().set(tmp_id + "_token", token)
		sparrow.cache().expire(tmp_id + "_token", expiry_time)
	else:
		expiry_time = sparrow.flags.otp_expiry or 180
	for k, v in {"_usr": user, "_pwd": pwd, "_otp_secret": otp_secret}.items():
		sparrow.cache().set(f"{tmp_id}{k}", v)
		sparrow.cache().expire(f"{tmp_id}{k}", expiry_time)
...
def two_factor_is_enabled_for_(user):
	...
	role_doctype = sparrow.qb.DocType("Role")
	no_of_users = sparrow.db.count(
		role_doctype,
...
def get_verification_method():
	return sparrow.db.get_single_value("System Settings", "two_factor_method")


def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	"""Confirm otp matches."""
	from sparrow.auth import get_login_attempt_tracker

	if not otp:
		otp = sparrow.form_dict.get("otp")
	if not otp:
	...
	if not tmp_id:
		tmp_id = sparrow.form_dict.get("tmp_id")
	hotp_token = sparrow.cache().get(tmp_id + "_token")
	otp_secret = sparrow.cache().get(tmp_id + "_otp_secret")
	if not otp_secret:
	...
	if hotp_token:
		if hotp.verify(otp, int(hotp_token)):
			sparrow.cache().delete(tmp_id + "_token")
...
def get_verification_obj(user, token, otp_secret):
	otp_issuer = sparrow.db.get_single_value("System Settings", "otp_issuer_name")
	verification_method = get_verification_method()
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = sparrow.db.get_value("User", user, ["phone", "mobile_no"], as_dict=1)
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		sparrow.db.get_single_value("System Settings", "otp_issuer_name")
	)
	subject = sparrow.render_template(subject_template, kwargs_dict)
	return subject
...
def get_email_body_for_2fa(kwargs_dict):
	...
	"""
	body = sparrow.render_template(body_template, kwargs_dict)
	return body
...
def get_link_for_qrcode(user, totp_uri):
	...
	key = sparrow.generate_hash(length=20)
	key_user = f"{key}_user"
	key_uri = f"{key}_uri"
	lifespan = int(sparrow.db.get_single_value("System Settings", "lifespan_qrcode_image")) or 240
	sparrow.cache().set_value(key_uri, totp_uri, expires_in_sec=lifespan)
	sparrow.cache().set_value(key_user, user, expires_in_sec=lifespan)
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	try:
		from sparrow.core.doctype.sms_settings.sms_settings import send_request
	except Exception:
		return False
	...
	ss = sparrow.get_doc("SMS Settings", "SMS Settings")
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	user_email = sparrow.db.get_value("User", user, "email")
	if not user_email:
		return False
	hotp = pyotp.HOTP(otp_secret)
...
def qrcode_as_png(user, totp_uri):
	...
	png_file_name = f"{sparrow.generate_hash(length=20)}.png"
	...
	file_path = os.path.join(sparrow.get_site_path("public", "files"), _file.file_name)
	url = qrcreate(totp_uri)
...
def create_barcode_folder():
	...
	folder = sparrow.db.exists("File", {"file_name": folder_name})
	if folder:
		return folder
	folder = sparrow.get_doc(
...
def delete_qrimage(user, check_expiry=False):
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = sparrow.get_doc("File", barcode.name)
		sparrow.delete_doc("File", barcode.name, ignore_permissions=True)


def delete_all_barcodes_for_users():
	"""Task to delete all barcodes for user."""

	users = sparrow.get_all("User", {"enabled": 1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, str):
		barcode = sparrow.get_doc("File", barcode)
	lifespan = sparrow.db.get_single_value("System Settings", "lifespan_qrcode_image") or 240
...
def disable():
	sparrow.db.set_single_value("System Settings", "enable_two_factor_auth", 0)


@sparrow.whitelist()
def reset_otp_secret(user: str):
	if sparrow.session.user != user:
		sparrow.only_for("System Manager", message=True)

	settings = sparrow.get_cached_doc("System Settings")
	...
	otp_issuer = settings.otp_issuer_name or "Sparrow Framework"
	user_email = sparrow.get_cached_value("User", user, "email")
	...
	sparrow.msgprint(_("OTP Secret has been reset. Re-registration will be required on next login."))
