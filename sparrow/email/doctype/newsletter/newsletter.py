...
from sparrow.email.doctype.email_group.email_group import add_subscribers
from sparrow.rate_limiter import rate_limit
from sparrow.utils.safe_exec import is_job_queued
...
class Newsletter(WebsiteGenerator):
	...
	def get_sending_status(self):
		count_by_status = sparrow.get_all(
			"Email Queue",
		...
		emails_queued = is_job_queued(
			job_name=sparrow.utils.get_job_name("send_bulk_emails_for", self.doctype, self.name),
			queue="long",
		)
	...
	def send_test_email(self, email):
		test_emails = sparrow.utils.validate_email_address(email, throw=True)
		self.send_newsletter(emails=test_emails)
		sparrow.msgprint(_("Test email sent to {0}").format(email), alert=True)
	...
	def validate_newsletter_status(self):
		if self.email_sent:
			sparrow.throw(_("Newsletter has already been sent"), exc=NewsletterAlreadySentError)

		if self.get("__islocal"):
			sparrow.throw(_("Please save the Newsletter before sending"), exc=NewsletterNotSavedError)

	def validate_newsletter_recipients(self):
		if not self.newsletter_recipients:
			sparrow.throw(_("Newsletter should have atleast one recipient"), exc=NoRecipientFoundError)
		self.validate_recipient_address()

	def validate_sender_address(self):
		"""Validate self.send_from is a valid email address or not."""
		if self.sender_email:
			sparrow.utils.validate_email_address(self.sender_email, throw=True)
			self.send_from = (
	...
	def validate_recipient_address(self):
		...
		for recipient in self.newsletter_recipients:
			sparrow.utils.validate_email_address(recipient, throw=True)

	def validate_publishing(self):
		if self.send_webview_link and not self.published:
			sparrow.throw(_("Newsletter must be published to send webview link in email"))
	...
	def queue_all(self):
		...
		self.email_sent_at = sparrow.utils.now()
	...
	def send_newsletter(self, emails: list[str]):
		...
		sender = self.send_from or sparrow.utils.get_formatted_email(self.owner)
		args = self.as_dict()
		args["message"] = self.get_message()

		is_auto_commit_set = bool(sparrow.db.auto_commit_on_many_writes)
		sparrow.db.auto_commit_on_many_writes = not sparrow.flags.in_test
		...
		sparrow.db.auto_commit_on_many_writes = is_auto_commit_set

	def get_message(self) -> str:
		message = self.message
		if self.content_type == "Markdown":
			message = sparrow.utils.md_to_html(self.message_md)
		if self.content_type == "HTML":
			message = self.message_html

		return sparrow.render_template(message, {"doc": self.as_dict()})

	def get_recipients(self) -> list[str]:
		"""Get recipients from Email Group"""
		emails = sparrow.get_all(
			"Email Group Member",
	...
	def get_email_groups(self) -> list[str]:
		...
		return [x.email_group for x in self.email_group] or sparrow.get_all(
...
def confirmed_unsubscribe(email, group):
	...
	doc = sparrow.get_doc("Email Group Member", {"email": email, "email_group": group})
...
def subscribe(email, email_group=None):  # noqa
	...
	api_endpoint = sparrow.utils.get_url(
		"/api/method/sparrow.email.doctype.newsletter.newsletter.confirm_subscription"
	)
	...
	email_confirmation_template = sparrow.db.get_value(
	...
	if email_confirmation_template:
		...
		email_template = sparrow.get_doc("Email Template", email_confirmation_template)
		email_subject = email_template.subject
		content = sparrow.render_template(email_template.response, args)
	else:
...
def confirm_subscription(email, email_group=_("Website")):  # noqa
	...
	if not sparrow.db.exists("Email Group", email_group):
		sparrow.get_doc({"doctype": "Email Group", "title": email_group}).insert(ignore_permissions=True)
...
def send_scheduled_email():
	...
	scheduled_newsletter = sparrow.get_all(
		"Newsletter",
		filters={
			"schedule_send": ("<=", sparrow.utils.now_datetime()),
			"email_sent": False,
	...
	for newsletter_name in scheduled_newsletter:
		try:
			newsletter = sparrow.get_doc("Newsletter", newsletter_name)
			...
			sparrow.db.set_value("Newsletter", newsletter_name, "email_sent", 0)
