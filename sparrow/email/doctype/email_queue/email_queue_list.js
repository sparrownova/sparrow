sparrow.listview_settings["Email Queue"] = {
...
function show_toggle_sending_button(list_view) {
	...
	const sending_disabled = cint(sparrow.sys_defaults.suspend_email_queue);
	const label = sending_disabled ? __("Resume Sending") : __("Suspend Sending");

	list_view.page.add_inner_button(label, async () => {
		await sparrow.xcall(
			"sparrow.email.doctype.email_queue.email_queue.toggle_sending",
		...
		sparrow.sys_defaults.suspend_email_queue = sending_disabled ? 0 : 1;
