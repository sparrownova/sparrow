...
import sparrow
from sparrow import _, safe_encode, task
from sparrow.core.utils import html2text
from sparrow.email.doctype.email_account.email_account import EmailAccount
from sparrow.email.email_body import add_attachment, get_email, get_formatted_html
from sparrow.email.queue import get_unsubcribed_url, get_unsubscribe_message
...
class EmailQueue(Document):
	...
	def prevent_email_queue_delete(self):
		if sparrow.session.user != "Administrator":
			sparrow.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = sparrow.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False):
		...
		doc = sparrow.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = sparrow.get_doc("Communication", self.communication)
	...
	def get_email_account(self):
		if self.email_account:
			return sparrow.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			sparrow.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(sparrow.db.get_default("suspend_email_queue")) == 1
		):
			return False
	...
	def send(self, is_background_task: bool = False, smtp_server_instance: SMTPServer = None):
		...
		with SendMailContext(self, is_background_task, smtp_server_instance) as ctx:
			...
			if sparrow.flags.in_test:
				sparrow.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = sparrow.qb.DocType("Email Queue")
		email_recipient = sparrow.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			sparrow.qb.from_(email_queue)
			.delete()
		...
		(
			sparrow.qb.from_(email_recipient)
			.delete()
...
class SendMailContext:
	...
	def get_tracker_str(self) -> str:
		if sparrow.conf.use_ssl and self.email_account_doc.track_email_status:
			tracker_url_html = f'<img src="{get_url()}/api/method/sparrow.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"/>'
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = sparrow.get_doc("File", file_filters)
				...
				print_format_file = sparrow.attach_print(**attachment)
...
def retry_sending(name):
	doc = sparrow.get_doc("Email Queue", name)
...
def toggle_sending(enable):
	...
	sparrow.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)
...
def on_doctype_update():
	...
	sparrow.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(sparrow.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/sparrow.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/sparrow.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				sparrow.qb.from_(EmailUnsubscribe)
	...
	def process(self, send_now=False):
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in sparrow.utils.create_batch(final_recipients, 1000):
				sparrow.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=sparrow.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=sparrow.flags.in_test or send_now,
					queue="long",
				)
	...
	def as_dict(self, include_recipients=True):
		...
		except sparrow.InvalidEmailAddressError:
