...
from sparrow.email.receive import Timed_IMAP4, Timed_IMAP4_SSL, Timed_POP3, Timed_POP3_SSL
...
class EmailDomain(Document):
	def validate(self):
		...
		if sparrow.local.flags.in_patch or sparrow.local.flags.in_test or sparrow.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in sparrow.get_all("Email Account", filters={"domain": self.name}):
			try:
