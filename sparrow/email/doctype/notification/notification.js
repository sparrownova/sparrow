...
sparrow.notification = {
	setup_fieldname_select: function (frm) {
		...
		sparrow.model.with_doctype(frm.doc.document_type, function () {
			...
			let fields = sparrow.get_doc("DocType", frm.doc.document_type).fields;
			let options = $.map(fields, function (d) {
				return in_list(sparrow.model.no_value_type, d.fieldtype)
					? null
					: get_select_options(d);
			});
			...
			if (frm.doc.channel === "Email") {
				receiver_fields = $.map(fields, function (d) {
					...
					if (sparrow.model.table_fields.includes(d.fieldtype)) {
						let child_fields = sparrow.get_doc("DocType", d.options).fields;
...
sparrow.ui.form.on("Notification", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		sparrow.notification.setup_fieldname_select(frm);
		sparrow.notification.setup_example_message(frm);
	...
	view_properties: function (frm) {
		sparrow.route_options = { doc_type: frm.doc.document_type };
	...
	event: function (frm) {
		if (in_list(["Days Before", "Days After"], frm.doc.event)) {
			frm.add_custom_button(__("Get Alerts for Today"), function () {
				sparrow.call({
					method: "sparrow.email.doctype.notification.notification.get_documents_for_today",
					args: {
						notification: frm.doc.name,
					},
					callback: function (r) {
						if (r.message && r.message.length > 0) {
							sparrow.msgprint(r.message.toString());
						} else {
							sparrow.msgprint(__("No alerts for today"));
						}
					},
				});
			});
