...
from sparrow.desk.query_report import build_xlsx_data
...
from sparrow.utils.csvutils import to_csv
...
class AutoEmailReport(Document):
	def autoname(self):
		...
		if sparrow.db.exists("Auto Email Report", self.name):
	...
	def validate_report_count(self):
		...
		if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
			...
			sparrow.throw(msg, title=_("Report limit reached"))
		...
		if self.format not in valid_report_formats:
			sparrow.throw(
				_("{0} is not a valid report format. Report format should one of the following {1}").format(
					sparrow.bold(self.format), sparrow.bold(", ".join(valid_report_formats))
				)
			)

	def validate_mandatory_fields(self):
		# Check if all Mandatory Report Filters are filled by the User
		filters = sparrow.parse_json(self.filters) if self.filters else {}
		filter_meta = sparrow.parse_json(self.filter_meta) if self.filter_meta else {}
		throw_list = []
	...
	def get_report_content(self):
		...
		report = sparrow.get_doc("Report", self.report)

		self.filters = sparrow.parse_json(self.filters) if self.filters else {}
		...
		# add serial numbers
		columns.insert(0, sparrow._dict(fieldname="idx", label="", width="30px"))
		...
		if self.format == "HTML":
			...
			sparrow.throw(_("Invalid Output Format"))

	def get_html_table(self, columns=None, data=None):

		date_time = global_date_format(now()) + " " + format_time(now())
		report_doctype = sparrow.db.get_value("Report", self.report, "ref_doctype")

		return sparrow.render_template(
			"sparrow/templates/emails/auto_email_report.html",
			{
				"title": self.name,
	...
	def prepare_dynamic_filters(self):
		self.filters = sparrow.parse_json(self.filters)
	...
	def send(self):
		if self.filter_meta and not self.filters:
			sparrow.throw(_("Please set filters value in Report Filter table."))
...
def download(name):
	...
	auto_email_report = sparrow.get_doc("Auto Email Report", name)
	auto_email_report.check_permission()
	data = auto_email_report.get_report_content()

	if not data:
		sparrow.msgprint(_("No Data"))
		return

	sparrow.local.response.filecontent = data
	sparrow.local.response.type = "download"
	sparrow.local.response.filename = auto_email_report.get_file_name()
...
def send_daily():
	...
	enabled_reports = sparrow.get_all(
		"Auto Email Report", filters={"enabled": 1, "frequency": ("in", ("Daily", "Weekdays", "Weekly"))}
	)

	for report in enabled_reports:
		auto_email_report = sparrow.get_doc("Auto Email Report", report.name)
...
def send_monthly():
	...
	for report in sparrow.get_all("Auto Email Report", {"enabled": 1, "frequency": "Monthly"}):
		sparrow.get_doc("Auto Email Report", report.name).send()
	...
	for row in data:
		...
		for col in columns:
			...
			if col.fieldtype == "Link":
				...
				doc = sparrow.get_doc(col.parent, doc_name) if doc_name and col.get("parent") else None
				# Pass the Document to get the currency based on docfield option
				row[col.fieldname] = sparrow.format_value(row[col.fieldname], col, doc=doc)
	return columns, data
