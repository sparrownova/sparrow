...
import sparrow
from sparrow import _, are_emails_muted, safe_encode
from sparrow.desk.form import assign_to
from sparrow.email.doctype.email_domain.email_domain import EMAIL_DOMAIN_FIELDS
from sparrow.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from sparrow.email.smtp import SMTPServer
from sparrow.email.utils import get_port
from sparrow.model.document import Document
from sparrow.utils import cint, comma_or, cstr, parse_addr, validate_email_address
from sparrow.utils.background_jobs import enqueue, get_jobs
from sparrow.utils.jinja import render_template
...
def cache_email_account(cache_name):
	def decorator_cache_email_account(func):
		...
		def wrapper_cache_email_account(*args, **kwargs):
			if not hasattr(sparrow.local, cache_name):
				setattr(sparrow.local, cache_name, {})

			cached_accounts = getattr(sparrow.local, cache_name)
...
class EmailAccount(Document):
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				sparrow.throw(_("Login Id is required"))
		else:
			self.login_id = None

		# validate the imap settings
		if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
			sparrow.throw(_("You need to set one IMAP folder for {0}").format(sparrow.bold(self.email_id)))

		if sparrow.local.flags.in_patch or sparrow.local.flags.in_test:
			return
		...
		if not sparrow.local.flags.in_install and not self.awaiting_password:
			if validate_oauth or self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					if not use_oauth:
						sparrow.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				sparrow.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
			...
			for folder in self.imap_folder:
				if folder.append_to:
					...
					if folder.append_to not in valid_doctypes:
						sparrow.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))

	def validate_smtp_conn(self):
		if not self.smtp_server:
			sparrow.throw(_("SMTP Server is required"))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					sparrow.bold(_("Default Incoming")), sparrow.bold(_("Enable Incoming"))
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					sparrow.bold(_("Default Outgoing")), sparrow.bold(_("Enable Outgoing"))
				)
			)
		if messages:
			if len(messages) == 1:
				(as_list, messages) = (0, messages[0])
			sparrow.msgprint(messages, as_list=as_list, indicator="orange", title=_("Defaults Updated"))
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in sparrow.get_all("Email Account", filters={field: 1}):
				if email_account.name == self.name:
					continue

				email_account = sparrow.get_doc("Email Account", email_account.name)
				email_account.set(field, 0)
				email_account.save()

	@sparrow.whitelist()
	def get_domain_values(self, domain: str):
		return sparrow.db.get_value("Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True)

	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		"""Returns logged in POP3/IMAP connection object."""
		if sparrow.cache().get_value("workers:no-internet") == True:
			return None
		...
		if not args.get("host"):
			sparrow.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(sparrow._dict(args))
	...
	def from_record(cls, record):
		email_account = sparrow.new_doc(cls.DOCTYPE)
		email_account.update(record)
		return email_account

	@classmethod
	def find(cls, name):
		return sparrow.get_doc(cls.DOCTYPE, name)

	@classmethod
	def find_one_by_filters(cls, **kwargs):
		name = sparrow.db.get_value(cls.DOCTYPE, kwargs)
	...
	def get_account_details_from_site_config(cls):
		if not sparrow.conf.get("mail_server"):
			return {}
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "Sparrow"},
		...
		for doc_field_name, d in field_to_conf_name_map.items():
			...
			value = [sparrow.conf.get(k) for k in conf_names if sparrow.conf.get(k)]
	...
	def set_failed_attempts_count(self, value):
		sparrow.cache().set(f"{self.name}:email-account-failed-attempts", value)

	def get_failed_attempts_count(self):
		return cint(sparrow.cache().get(f"{self.name}:email-account-failed-attempts"))
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				if self.use_imap:
					self.handle_bad_emails(mail.uid, mail.raw_message, sparrow.get_traceback())
				exceptions.append(sparrow.get_traceback())
			else:
				sparrow.db.commit()

		if exceptions:
			raise Exception(sparrow.as_json(exceptions))
		...
		def process_mail(messages, append_to=None):
			for index, message in enumerate(messages.get("latest_messages", [])):
				...
				if self.email_sync_option != "UNSEEN" or seen_status != "SEEN":
					...
					mails.append(InboundMail(message, self, sparrow.safe_decode(uid), seen_status, append_to))
	...
	def handle_bad_emails(self, uid, raw, reason):
		if cint(self.use_imap):
			...
			unhandled_email = sparrow.get_doc(
				{
					"raw": raw,
					"uid": uid,
					"reason": reason,
	...
	def send_auto_reply(self, communication, email):
		...
		from sparrow.core.doctype.communication.email import set_incoming_outgoing_accounts
	...
	def on_trash(self):
		...
		Communication = sparrow.qb.DocType("Communication")
		sparrow.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def after_rename(self, old, new, merge=False):
		sparrow.db.set_value("Email Account", new, "email_account_name", new)
	...
	def mark_emails_as_read_unread(self, email_server=None, folder_name="INBOX"):
		...
		EmailFlagQ = sparrow.qb.DocType("Email Flag Queue")
		flags = (
			sparrow.qb.from_(EmailFlagQ)
			.select(EmailFlagQ.name, EmailFlagQ.communication, EmailFlagQ.uid, EmailFlagQ.action)
			.where(EmailFlagQ.is_completed == 0)
			.where(EmailFlagQ.email_account == sparrow.db.escape(self.name))
		).run(as_dict=True)
		...
		if flags and uid_list:
			...
			EmailFlagQueue = sparrow.qb.DocType("Email Flag Queue")
			sparrow.qb.update(EmailFlagQueue).set(EmailFlagQueue.is_completed, 1).where(
	...
	def set_communication_seen_status(self, docnames, seen=0):
		...
		if not docnames:
			return
		Communication = sparrow.qb.from_("Communication")
		sparrow.qb.update(Communication).set(Communication.seen == seen).where(
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				sparrow.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if sparrow.db.exists(
				"Email Account", {"enable_automatic_linking": 1, "name": ("!=", self.name)}
			):
				sparrow.throw(_("Automatic Linking can be activated only for one Email Account."))
	...
	def get_oauth_token(self):
		if self.auth_method == "OAuth":
			connected_app = sparrow.get_doc("Connected App", self.connected_app)
...
def get_append_to(
	...
	for dt in sparrow.get_all("DocType", filters=filters, fields=["name", "email_append_to"]):
		email_append_to_list.append(dt.name)

	# Set Email Append To DocTypes set via Customize Form
	for dt in sparrow.get_list(
...
def notify_unreplied():
	...
	for email_account in sparrow.get_all(
		...
		if append_to:
			...
			for comm in sparrow.get_all(
				...
				comm = sparrow.get_doc("Communication", comm.name)

				if sparrow.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	from sparrow.integrations.doctype.connected_app.connected_app import has_token

	if sparrow.cache().get_value("workers:no-internet") == True:
		if test_internet():
			sparrow.cache().set_value("workers:no-internet", False)
		return

	doctype = sparrow.qb.DocType("Email Account")
	email_accounts = (
	...
	for email_account in email_accounts:
		...
		if now:
			...
			queued_jobs = get_jobs(site=sparrow.local.site, key="job_name")[sparrow.local.site]
...
def pull_from_email_account(email_account):
	...
	email_account = sparrow.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(
	...
	from sparrow.core.doctype.user.user import ask_pass_update
	...
	user_names = sparrow.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		UserEmail = sparrow.qb.DocType("User Email")
		sparrow.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
		...
		users = " and ".join([sparrow.bold(user.get("name")) for user in user_names])
		sparrow.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def set_email_password(email_account, password):
	account = sparrow.get_doc("Email Account", email_account)
