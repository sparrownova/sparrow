...
from sparrow.email.doctype.email_account.email_account import notify_unreplied
...
class TestEmailAccount(FrappeTestCase):
	...
	def setUp(self):
		...
		sparrow.flags.sent_mail = None
		sparrow.db.delete("Email Queue")
		sparrow.db.delete("Unhandled Email")
	...
	def test_incoming(self):
		...
		comm = sparrow.get_doc("Communication", {"sender": "test_sender@example.com"})
		self.assertTrue("test_receiver@example.com" in comm.recipients)
		# check if todo is created
		self.assertTrue(sparrow.db.get_value(comm.reference_doctype, comm.reference_name, "name"))
	...
	def test_incoming_with_attach(self):
		...
		existing_file = sparrow.get_doc({"doctype": "File", "file_name": "shopper-conf-14.png"})
		sparrow.delete_doc("File", existing_file.name)
		...
		self.assertTrue("shopper-conf-14.png" in [f.file_name for f in attachments])
	...
	def test_outgoing(self):
		...
		mail = email.message_from_string(sparrow.get_last_doc("Email Queue").message)
	...
	def test_sendmail(self):
		...
		sent_mail = email.message_from_string(sparrow.safe_decode(sparrow.flags.sent_mail))
	...
	def test_print_format(self):
		...
		sent_mail = email.message_from_string(sparrow.get_last_doc("Email Queue").message)
	...
	def test_threading(self):
		...
		sent_name = make(
			...
			doctype="ToDo",
			name=sparrow.get_last_doc("ToDo").name,
			send_email=True,
		)["name"]
		...
		sent = sparrow.get_doc("Communication", sent_name)
	...
	def test_threading_by_message_id(self):
		...
		event = sparrow.get_doc(dict(doctype="Event", subject="test-message")).insert()
		...
		last_mail = sparrow.get_doc("Email Queue", dict(reference_name=event.name))
	...
	def test_handle_bad_emails(self):
		...
		self.assertTrue(sparrow.db.get_value("Unhandled Email", {"message_id": message_id}))
	...
	def test_append_to(self):
		...
		self.assertTrue(sparrow.db.exists(communication.reference_doctype, communication.reference_name))
	...
	def test_append_to_with_imap_folders(self):
		...
		for mail in mails:
			...
			if mail.append_to == "ToDo":
				...
				self.assertTrue(communication.reference_name)
				self.assertTrue(
					sparrow.db.exists(communication.reference_doctype, communication.reference_name)
				)
			else:
				test_folder_mails += 1
	...
	@patch("sparrow.email.receive.EmailServer.select_imap_folder", return_value=True)
	@patch("sparrow.email.receive.EmailServer.logout", side_effect=lambda: None)
	def mocked_get_inbound_mails(
		email_account, messages={}, mocked_logout=None, mocked_select_imap_folder=None
	):
		from sparrow.email.receive import EmailServer
...
class TestInboundMail(FrappeTestCase):
	...
	def test_find_parent_communication_from_header(self):
		...
		communication = self.new_communication()
		mail_content = self.get_test_mail(fname="reply-4.raw").replace(
			"{{ message_id }}", f"<{communication.name}@{sparrow.local.site}>"
		)
...
def cleanup(sender=None):
	...
	names = sparrow.get_list("Communication", filters=filters, fields=["name"])
	for name in names:
		sparrow.delete_doc_if_exists("Communication", name.name)
		sparrow.delete_doc_if_exists("Communication Link", {"parent": name.name})
