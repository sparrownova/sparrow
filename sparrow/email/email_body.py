...
class EMail:
	...
	def attach_file(self, n):
		...
		_file = sparrow.get_doc("File", {"file_name": n})
	...
	def validate(self):
		...
		self.recipients = [strip(r) for r in self.recipients if r not in sparrow.STANDARD_USERS]
		self.cc = [strip(r) for r in self.cc if r not in sparrow.STANDARD_USERS]
		self.bcc = [strip(r) for r in self.bcc if r not in sparrow.STANDARD_USERS]
	...
	def make(self):
		...
		for hook in sparrow.get_hooks("make_email_body_message"):
			sparrow.get_attr(hook)(self)
...
def get_formatted_html(
	...
	email_account=None,
	header=None,
	unsubscribe_link: sparrow._dict | None = None,
	sender=None,
	...
	rendered_email = sparrow.get_template("templates/emails/standard.html").render(
		{
...
def get_email_html(template, args, subject, header=None, with_container=False):
	...
	email = sparrow.utils.jinja.get_email_from_template(template, args)
...
def inline_style_in_html(html):
	...
	css_files = sparrow.get_hooks("email_css")
...
def get_message_id():
	...
	return email.utils.make_msgid(domain=sparrow.local.site)
...
def get_footer(email_account, footer=None):
	...
	sender_address = sparrow.db.get_default("email_footer_address")

	if sender_address:
		args.update({"sender_address": sender_address})

	if not cint(sparrow.db.get_default("disable_standard_email_footer")):
		args.update({"default_mail_footer": sparrow.get_hooks("default_mail_footer")})

	footer += sparrow.utils.jinja.get_email_from_template("email_footer", args)[0]

	return footer


def replace_filename_with_cid(message):
	"""Replaces <img embed="assets/sparrow/images/filename.jpg" ...> with
...
def get_filecontent_from_path(path):
	...
	if path.startswith("assets/"):
		...
		# public file
		full_path = sparrow.get_site_path("public", path)
	elif path.startswith("private/files/"):
		# private file
		full_path = sparrow.get_site_path(path)
	else:
		full_path = path
...
def get_header(header=None):
	...
	from sparrow.utils.jinja import get_email_from_template
	...
	if not title:
		title = sparrow.get_hooks("app_title")[-1]
