...
from sparrow.database.utils import DefaultOrderBy, FallBackDateTimeStr, NestedSetHierarchy
from sparrow.model import get_permitted_fields, optional_fields
from sparrow.model.meta import get_table_columns
from sparrow.model.utils import is_virtual_doctype
from sparrow.model.utils.user_settings import get_user_settings, update_user_settings
from sparrow.query_builder.utils import Column
...
class DatabaseQuery:
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or sparrow.session.user
	...
	def doctype_meta(self):
		if not hasattr(self, "_doctype_meta"):
			self._doctype_meta = sparrow.get_meta(self.doctype)
	...
	def build_and_run(self):
		...
		if sparrow.db.db_type == "postgres" and args.order_by and args.group_by:
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {sparrow.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			sparrow.throw(_("Use of sub-query or function is restricted"), sparrow.DataError)
		...
		for field in self.fields:
			...
			if SUB_QUERY_PATTERN.match(field):
				...
				if function in blacklisted_functions:
					sparrow.throw(
						_("Use of function {0} in field is restricted").format(function), exc=sparrow.DataError
					)
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					sparrow.throw(_("Illegal SQL Query"))
	...
	def append_link_table(self, doctype, fieldname):
		...
		self.link_tables.append(
			sparrow._dict(doctype=doctype, fieldname=fieldname, table_name=f"`tab{doctype}`")
		)
	...
	def _set_permission_map(self, doctype: str, parent_doctype: str | None = None):
		ptype = "select" if sparrow.only_has_select_perm(doctype) else "read"
		val = sparrow.has_permission(
			doctype,
			ptype=ptype,
			parent_doctype=parent_doctype or self.doctype,
		)
		if not val:
			sparrow.flags.error_message = _("Insufficient Permission for {0}").format(sparrow.bold(doctype))
			raise sparrow.PermissionError(doctype)
	...
	def get_table_columns(self):
		...
		except sparrow.db.TableMissingError:
			if self.ignore_ddl:
				return None
			else:
				raise
	...
	def apply_fieldlevel_read_permissions(self):
		...
		Note: Does not apply to `sparrow.model.core_doctype_list`
		...
		Example:
		        ...
		        - Query: fields=["*"]
		        - Result: fields=["title", ...] // will also include Sparrow's meta field like `name`, `owner`, etc.
		"""
		if self.flags.ignore_permissions or not sparrow.get_system_settings(
		...
		for i, field in enumerate(self.fields):
			...
			# labels / pseudo columns or sparrow internals
			...
			# handle child / joined table fields
				...
				if wrap_grave_quotes(table) in self.query_tables:
					...
					raise sparrow.PermissionError(ch_doctype)
	...
	def prepare_filter_condition(self, f):
		...
		from sparrow.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in NestedSetHierarchy:
			...
			lft, rgt = "", ""
			if f.value:
				lft, rgt = sparrow.db.get_value(ref_doctype, f.value, ["lft", "rgt"])
			...
			fallback = "''"
			value = [sparrow.db.escape((cstr(v.name) or "").strip(), percent=False) for v in result]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
			...
			fallback = "''"
			value = [sparrow.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
			...
			if f.operator in (">", "<", ">=", "<=") and (f.fieldname in ("creation", "modified")):
				...
				value = sparrow.db.format_date(f.value)
				fallback = "'0001-01-01'"

			elif (df and df.fieldtype == "Datetime") or isinstance(f.value, datetime):
				value = sparrow.db.format_datetime(f.value)
				fallback = f"'{FallBackDateTimeStr}'"
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if sparrow.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif escape and isinstance(value, str):
				value = f"{sparrow.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and sparrow.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True) -> str | list:
		...
		if not self.user:
			self.user = sparrow.session.user

		if not self.tables:
			self.extract_tables()

		role_permissions = sparrow.permissions.get_role_permissions(self.doctype_meta, user=self.user)
		self.shared = sparrow.share.get_shared(self.doctype, self.user)
		...
		if (
			...
			only_if_shared = True
			if not self.shared:
				sparrow.throw(_("No permission to read {0}").format(_(self.doctype)), sparrow.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {sparrow.db.escape(self.user, percent=False)}"
				)

			# add user permission only if role has read perm
			elif role_permissions.get("read") or role_permissions.get("select"):
				# get user permissions
				user_permissions = sparrow.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(sparrow.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if sparrow.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(sparrow.db.escape(doc, percent=False) for doc in docs)
	...
	def get_permission_query_conditions(self):
		conditions = []
		condition_methods = sparrow.get_hooks("permission_query_conditions", {}).get(self.doctype, [])
		if condition_methods:
			for method in condition_methods:
				c = sparrow.call(sparrow.get_attr(method), self.user)
				if c:
					conditions.append(c)

		permision_script_name = get_server_script_map().get("permission_query", {}).get(self.doctype)
		if permision_script_name:
			script = sparrow.get_doc("Server Script", permision_script_name)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if "select" in _lower and "from" in _lower:
			sparrow.throw(_("Cannot use sub-query in order by"))
		...
		for field in parameters.split(","):
			...
			if full_field_name:
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					sparrow.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))

			if function in blacklisted_sql_functions:
				sparrow.throw(_("Cannot use {0} in order/group by").format(field))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if sparrow.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = sparrow.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	"""
	from_date = sparrow.utils.nowdate()
	to_date = sparrow.utils.nowdate()
	...
	if df and df.fieldtype == "Datetime":
		data = "'{}' AND '{}'".format(
			sparrow.db.format_datetime(from_date),
			sparrow.db.format_datetime(to_date),
		)
	else:
		data = f"'{sparrow.db.format_date(from_date)}' AND '{sparrow.db.format_date(to_date)}'"
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = sparrow._dict(sparrow.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = sparrow._dict(option)
