...
if TYPE_CHECKING:
	...
	from sparrow.workflow.doctype.workflow.workflow import Workflow


class WorkflowStateError(sparrow.ValidationError):
	pass


class WorkflowTransitionError(sparrow.ValidationError):
	pass


class WorkflowPermissionError(sparrow.ValidationError):
	pass


def get_workflow_name(doctype):
	workflow_name = sparrow.cache().hget("workflow", doctype)
	if workflow_name is None:
		workflow_name = sparrow.db.get_value(
			"Workflow", {"document_type": doctype, "is_active": 1}, "name"
		)
		sparrow.cache().hset("workflow", doctype, workflow_name or "")
...
def get_transitions(
	...
	if not isinstance(doc, Document):
		doc = sparrow.get_doc(sparrow.parse_json(doc))
		doc.load_from_db()
	...
	if not current_state:
		if raise_exception:
			...
			sparrow.throw(_("Workflow State not set"), WorkflowStateError)
...
def get_workflow_safe_globals():
	# access to sparrow.db.get_value, sparrow.db.get_list, and date time utils.
	return dict(
		sparrow=sparrow._dict(
			db=sparrow._dict(get_value=sparrow.db.get_value, get_list=sparrow.db.get_list),
			session=sparrow.session,
			utils=sparrow._dict(
				now_datetime=sparrow.utils.now_datetime,
				add_to_date=sparrow.utils.add_to_date,
				get_datetime=sparrow.utils.get_datetime,
...
def apply_workflow(doc, action):
	...
	if not transition:
		sparrow.throw(_("Not a valid Workflow Action"), WorkflowTransitionError)

	if not has_approval_access(user, doc, transition):
		sparrow.throw(_("Self approval is not allowed"))
	...
	if doc.docstatus.is_draft() and new_docstatus == DocStatus.draft():
		...
		sparrow.throw(_("Illegal Document Status for {0}").format(next_state.state))
...
def validate_workflow(doc):
	...
	if current_state != next_state:
		bold_current = sparrow.bold(current_state)
		bold_next = sparrow.bold(next_state)
...
def get_workflow(doctype) -> "Workflow":
	return sparrow.get_cached_doc("Workflow", get_workflow_name(doctype))
...
def get_workflow_field_value(workflow_name, field):
	return sparrow.get_cached_value("Workflow", workflow_name, field)
...
def bulk_workflow_approval(docnames, doctype, action):
	...
	print("Clearing sparrow.message_log...")
	...
	for (idx, docname) in enumerate(docnames, 1):
		...
		try:
			...
			apply_workflow(sparrow.get_doc(doctype, docname), action)
			sparrow.db.commit()
		except Exception as e:
			if not sparrow.message_log:
			...
			if not message_dict:
				if sparrow.message_log:
					messages = sparrow.get_message_log()
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		for doc in messages.keys():
			if len(messages[doc]):
				html = f"<details><summary>{sparrow.utils.get_link_to_form(doctype, doc)}</summary>"
		...
		sparrow.msgprint(msg, title=_("Workflow Status"), indicator=indicator, is_minimizable=True)
	...
	try:
		for (i, doc) in enumerate(docs, 1):
			...
			actions = [
				...
				if has_approval_access(sparrow.session.user, doc, t)
			]
			if not actions:
				return []
...
def set_workflow_state_on_action(doc, workflow_name, action):
	workflow = sparrow.get_doc("Workflow", workflow_name)
