...
import sparrow
from sparrow import _, bold
from sparrow.model.document import Document
from sparrow.model.dynamic_links import get_dynamic_link_map
from sparrow.model.naming import validate_name
from sparrow.model.utils.user_settings import sync_user_settings, update_user_settings_data
from sparrow.query_builder import Field
from sparrow.utils.data import sbool
from sparrow.utils.password import rename_password
from sparrow.utils.scheduler import is_scheduler_inactive
...
def update_document_title(
	...
	**kwargs,
	...
	# TODO: omit this after runtime type checking (ref: https://github.com/sparrownova/sparrow/pull/14927)
	for obj in [docname, updated_title, updated_name]:
		if not isinstance(obj, (str, NoneType)):
			sparrow.throw(f"{obj=} must be of type str or None")
	...
	if title_updated:
		try:
			...
			doc.save()
			sparrow.msgprint(_("Saved"), alert=True, indicator="green")
		except Exception as e:
			if sparrow.db.is_duplicate_entry(e):
				sparrow.throw(
					_("{0} {1} already exists").format(doctype, sparrow.bold(docname)),
					title=_("Duplicate Name"),
					exc=sparrow.DuplicateEntryError,
				)
			raise
...
def rename_doc(
	...
	if not (new_usage_style or old_usage_style):
		raise TypeError(
			"{doctype, old, new} or {doc, new} are required arguments for sparrow.model.rename_doc"
		)
	...
	if validate:
		old_doc = doc or sparrow.get_doc(doctype, old)
	...
	# call after_rename
	new_doc = sparrow.get_doc(doctype, new)
	...
	if merge:
		new_doc.add_comment("Edit", _("merged {0} into {1}").format(sparrow.bold(old), sparrow.bold(new)))
	else:
		new_doc.add_comment(
			"Edit", _("renamed from {0} to {1}").format(sparrow.bold(old), sparrow.bold(new))
		)

	if merge:
		sparrow.delete_doc(doctype, old)

	new_doc.clear_cache()
	sparrow.clear_cache()
	if rebuild_search:
		sparrow.enqueue("sparrow.utils.global_search.rebuild_for_doctype", doctype=doctype)
...
def update_assignments(old: str, new: str, doctype: str) -> None:
	old_assignments = sparrow.parse_json(sparrow.db.get_value(doctype, old, "_assign")) or []
	new_assignments = sparrow.parse_json(sparrow.db.get_value(doctype, new, "_assign")) or []
	...
	for user in common_assignments:
		...
		for todo in todos:
			sparrow.delete_doc("ToDo", todo.name)

	unique_assignments = list(set(old_assignments + new_assignments))
	sparrow.db.set_value(doctype, new, "_assign", sparrow.as_json(unique_assignments, indent=0))
...
def update_user_settings(old: str, new: str, link_fields: list[dict]) -> None:
	...
	UserSettings = sparrow.qb.Table("__UserSettings")

	user_settings_details = (
		sparrow.qb.from_(UserSettings)
...
def update_customizations(old: str, new: str) -> None:
	sparrow.db.set_value("Custom DocPerm", {"parent": old}, "parent", new, update_modified=False)


def update_attachments(doctype: str, old: str, new: str) -> None:
	if doctype != "DocType":
		File = sparrow.qb.DocType("File")

		sparrow.qb.update(File).set(File.attached_to_name, new).where(
			(File.attached_to_name == old) & (File.attached_to_doctype == doctype)
		).run()


def rename_versions(doctype: str, old: str, new: str) -> None:
	Version = sparrow.qb.DocType("Version")

	sparrow.qb.update(Version).set(Version.docname, new).where(
		(Version.docname == old) & (Version.ref_doctype == doctype)
	).run()


def rename_eps_records(doctype: str, old: str, new: str) -> None:
	EPL = sparrow.qb.DocType("Energy Point Log")

	sparrow.qb.update(EPL).set(EPL.reference_name, new).where(
...
def rename_parent_and_child(doctype: str, old: str, new: str, meta: "Meta") -> None:
	sparrow.qb.update(doctype).set("name", new).where(Field("name") == old).run()
...
def update_autoname_field(doctype: str, new: str, meta: "Meta") -> None:
	...
	if meta.get("autoname"):
		...
		if field and field[0] == "field":
			sparrow.qb.update(doctype).set(field[1], new).where(Field("name") == new).run()
...
def validate_rename(
	...
	if save_point:
		_SAVE_POINT = f"validate_rename_{sparrow.generate_hash(length=8)}"
		sparrow.db.savepoint(_SAVE_POINT)

	exists = (
		sparrow.qb.from_(doctype).where(Field("name") == new).for_update().select("name").run(pluck=True)
	)
	exists = exists[0] if exists else None

	if not sparrow.db.exists(doctype, old):
		sparrow.throw(_("Can't rename {0} to {1} because {0} doesn't exist.").format(old, new))

	if old == new:
		sparrow.throw(_("No changes made because old and new name are the same.").format(old, new))
	...
	if merge and not exists:
		sparrow.throw(_("{0} {1} does not exist, select a new target to merge").format(doctype, new))
	...
	if not (force or ignore_permissions) and not meta.allow_rename:
		sparrow.throw(_("{0} not allowed to be renamed").format(_(doctype)))

	# validate naming like it's done in doc.py
	new = validate_name(doctype, new)

	if save_point:
		sparrow.db.rollback(save_point=_SAVE_POINT)
...
def rename_doctype(doctype: str, old: str, new: str) -> None:
	...
	fields_with_options = ("Link",) + sparrow.model.table_fields
...
def update_link_field_values(link_fields: list[dict], old: str, new: str, doctype: str) -> None:
	for field in link_fields:
		if field["issingle"]:
			try:
				single_doc = sparrow.get_doc(field["parent"])
			...
			sparrow.db.set_value(parent, {docfield: old}, docfield, new, update_modified=False)
...
def get_link_fields(doctype: str) -> list[dict]:
	...
	if not sparrow.flags.link_fields:
		sparrow.flags.link_fields = {}

	if doctype not in sparrow.flags.link_fields:
		dt = sparrow.qb.DocType("DocType")
		df = sparrow.qb.DocType("DocField")
		cf = sparrow.qb.DocType("Custom Field")
		ps = sparrow.qb.DocType("Property Setter")

		st_issingle = sparrow.qb.from_(dt).select(dt.issingle).where(dt.name == df.parent).as_("issingle")
		standard_fields = (
			sparrow.qb.from_(df)
		...
		cf_issingle = sparrow.qb.from_(dt).select(dt.issingle).where(dt.name == cf.dt).as_("issingle")
		custom_fields = (
			sparrow.qb.from_(cf)
		...
		ps_issingle = (
			sparrow.qb.from_(dt).select(dt.issingle).where(dt.name == ps.doc_type).as_("issingle")
		)
		...
		sparrow.flags.link_fields[doctype] = standard_fields + custom_fields + property_setter_fields

	return sparrow.flags.link_fields[doctype]


def update_options_for_fieldtype(fieldtype: str, old: str, new: str) -> None:
	CustomField = sparrow.qb.DocType("Custom Field")
	PropertySetter = sparrow.qb.DocType("Property Setter")

	if sparrow.conf.developer_mode:
		for name in sparrow.get_all("DocField", filters={"options": old}, pluck="parent"):
			if name in (old, new):
				continue

			doctype = sparrow.get_doc("DocType", name)
			save = False
	...
	DocField = sparrow.qb.DocType("DocField")
	sparrow.qb.update(DocField).set(DocField.options, new).where(
		(DocField.fieldtype == fieldtype) & (DocField.options == old)
	).run()

	sparrow.qb.update(CustomField).set(CustomField.options, new).where(
		(CustomField.fieldtype == fieldtype) & (CustomField.options == old)
	).run()

	sparrow.qb.update(PropertySetter).set(PropertySetter.value, new).where(
...
def update_select_field_values(old: str, new: str):
	from sparrow.query_builder.functions import Replace

	DocField = sparrow.qb.DocType("DocField")
	CustomField = sparrow.qb.DocType("Custom Field")
	PropertySetter = sparrow.qb.DocType("Property Setter")

	sparrow.qb.update(DocField).set(DocField.options, Replace(DocField.options, old, new)).where(
		(DocField.fieldtype == "Select")
		& (DocField.parent != new)
		& (DocField.options.like(f"%\n{old}%") | DocField.options.like(f"%{old}\n%"))
	).run()

	sparrow.qb.update(CustomField).set(
	...
	sparrow.qb.update(PropertySetter).set(
...
def update_parenttype_values(old: str, new: str):
	child_doctypes = sparrow.get_all(
		"DocField",
		fields=["options", "fieldname"],
		filters={"parent": new, "fieldtype": ["in", sparrow.model.table_fields]},
	)

	custom_child_doctypes = sparrow.get_all(
		"Custom Field",
		fields=["options", "fieldname"],
		filters={"dt": new, "fieldtype": ["in", sparrow.model.table_fields]},
	)

	child_doctypes += custom_child_doctypes
	fields = [d["fieldname"] for d in child_doctypes]

	property_setter_child_doctypes = sparrow.get_all(
		"Property Setter",
	...
	for doctype in child_doctypes:
		...
		sparrow.qb.update(table).set(table.parenttype, new).where(table.parenttype == old).run()


def rename_dynamic_links(doctype: str, old: str, new: str):
	Singles = sparrow.qb.DocType("Singles")
	for df in get_dynamic_link_map().get(doctype, []):
		# dynamic link in single, just one value to check
		if sparrow.get_meta(df.parent).issingle:
			refdoc = sparrow.db.get_singles_dict(df.parent)
			if refdoc.get(df.options) == doctype and refdoc.get(df.fieldname) == old:
				sparrow.qb.update(Singles).set(Singles.value, new).where(
			...
			sparrow.qb.update(parent).set(df.fieldname, new).where(
...
def bulk_rename(
	...
	if not rows:
		sparrow.throw(_("Please select a valid csv file with data"))

	if not via_console:
		max_rows = 500
		if len(rows) > max_rows:
			sparrow.throw(_("Maximum {0} rows allowed").format(max_rows))
...
def update_linked_doctypes(
	...
	from sparrow.model.utils.rename_doc import update_linked_doctypes
...
def get_fetch_fields(
	...
	from sparrow.model.utils.rename_doc import get_fetch_fields
...
def show_deprecation_warning(funct: str) -> None:
	...
	message = (
		f"Function sparrow.model.rename_doc.{funct} has been deprecated and "
		"moved to the sparrow.model.utils.rename_doc"
	)
