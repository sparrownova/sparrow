...
from sparrow.desk.doctype.tag.tag import delete_tags_for_document
from sparrow.model.docstatus import DocStatus
from sparrow.model.dynamic_links import get_dynamic_link_map
from sparrow.model.naming import revert_series_if_last
from sparrow.model.utils import is_virtual_doctype
from sparrow.utils.file_manager import remove_all
from sparrow.utils.global_search import delete_for_document
from sparrow.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = sparrow.form_dict.get("dt")
		name = sparrow.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			sparrow.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				sparrow.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", sparrow.model.table_fields)}
				)
				sparrow.db.delete("__global_search", {"doctype": name})
			...
			if not for_reload:
				...
				sparrow.enqueue(
					"sparrow.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and sparrow.flags.in_install != "sparrow":
		sparrow.get_doc(
			dict(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		sparrow.db.delete("Singles", {"doctype": name})
	else:
		sparrow.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if sparrow.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = sparrow.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", sparrow.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		sparrow.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		and sparrow.session.user != "Administrator"
		...
		sparrow.msgprint(
			...
			raise_exception=sparrow.PermissionError,
		)
	...
	if doc.docstatus.is_submitted():
		sparrow.msgprint(
			_("{0} {1}: Submitted Record cannot be deleted. You must {2} Cancel {3} it first.").format(
				_(doc.doctype),
				doc.name,
				"<a href='https://docs.shopper.com//docs/user/manual/en/setting-up/articles/delete-submitted-document' target='_blank'>",
				"</a>",
			),
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from sparrow.model.rename_doc import get_link_fields
	...
	for lf in link_fields:
		...
		if not issingle:
			...
			if sparrow.get_meta(link_dt).istable:
				fields.extend(["parent", "parenttype"])

			for item in sparrow.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
				# available only in child table cases
				item_parent = getattr(item, "parent", None)
				linked_doctype = item.parenttype if item_parent else link_dt

				if linked_doctype in sparrow.get_hooks("ignore_links_on_delete") or (
			...
			if sparrow.db.get_value(link_dt, None, link_field) == doc.name:
				raise_link_exists_exception(doc, link_dt, link_dt)


def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `sparrow.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):

		ignore_linked_doctypes = doc.get("ignore_linked_doctypes") or []

		if df.parent in sparrow.get_hooks("ignore_links_on_delete") or (
...
def clear_timeline_references(link_doctype, link_name):
	sparrow.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", sparrow.scrub(doctype))
