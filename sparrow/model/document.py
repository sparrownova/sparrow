...
import sparrow
from sparrow import _, is_whitelisted, msgprint
from sparrow.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from sparrow.desk.form.document_follow import follow_document
from sparrow.integrations.doctype.webhook import run_webhooks
from sparrow.model import optional_fields, table_fields
from sparrow.model.base_document import BaseDocument, get_controller
from sparrow.model.docstatus import DocStatus
from sparrow.model.naming import set_new_name, validate_name
from sparrow.model.utils import is_virtual_doctype
from sparrow.model.workflow import set_workflow_state_on_action, validate_workflow
from sparrow.utils import cstr, date_diff, file_lock, flt, get_datetime_str, now
from sparrow.utils.data import get_absolute_url
from sparrow.utils.global_search import update_global_search


def get_doc(*args, **kwargs):
	"""returns a sparrow.model.Document object.
...
class Document(BaseDocument):
	...
	def load_from_db(self):
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = sparrow.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = sparrow.new_doc(self.doctype, as_dict=True)
			...
			d = sparrow.db.get_value(
				self.doctype, self.name, "*", as_dict=1, for_update=self.flags.for_update
			)
			if not d:
				sparrow.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), sparrow.DoesNotExistError
				)
		...
		for df in self._get_table_fields():
			...
			# During sparrow is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to sparrow.db.get_value() as it would crash otherwise.
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `sparrow.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permtype)

	def has_permission(self, permtype="read", verbose=False) -> bool:
		"""
		Call `sparrow.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return sparrow.permissions.has_permission(self.doctype, permtype, self)

	def raise_no_permission_to(self, perm_type):
		"""Raise `sparrow.PermissionError`."""
		sparrow.flags.error_message = (
			_("Insufficient Permission for {0}").format(self.doctype) + f" ({sparrow.bold(_(perm_type))})"
		)
		raise sparrow.PermissionError
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Document":
		...
		self.flags.ignore_version = sparrow.flags.in_test if ignore_version is None else ignore_version
	...
	def update_child_table(self, fieldname, df=None):
		...
		if (
			...
			or sparrow.get_meta(df.options).is_virtual == 1
		):
		...
		if rows:
			...
			if len(deleted_rows) > 0:
				...
				sparrow.db.delete(df.options, {"name": ("in", tuple(row[0] for row in deleted_rows))})
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `sparrow.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		sparrow.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in sparrow.db.value_cache:
			del sparrow.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = sparrow.session.user
		...
		sparrow.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					sparrow.bold(_(self.doctype)),
					_("Row"),
					self.idx,
					_("Value cannot be negative for"),
					sparrow.bold(_(df.label)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), sparrow.bold(_(df.label))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				sparrow.throw(msg, sparrow.NonNegativeError, title=_("Negative Value"))
				...
				if fail:
					sparrow.throw(
						_("Value cannot be changed for {0}").format(
							sparrow.bold(self.meta.get_label(field.fieldname))
						),
						exc=sparrow.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += sparrow.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in sparrow.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or sparrow.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = sparrow.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
				for d in self.get(df.fieldname):
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = sparrow.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		new_doc = sparrow.new_doc(self.doctype, as_dict=True)
		self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = sparrow.new_doc(df.options, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			sparrow.msgprint(
				...
				raise_exception=sparrow.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise sparrow.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise sparrow.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise sparrow.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if sparrow.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise sparrow.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			sparrow.throw(_("Could not find {0}").format(msg), sparrow.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			sparrow.throw(_("Cannot link cancelled document: {0}").format(msg), sparrow.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(sparrow.flags.in_import and sparrow.flags.mute_emails)
		...
		from sparrow.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = sparrow.cache().hget(
	...
	def _rename(
		...
		"""Rename the document. Triggers sparrow.rename_doc, then reloads."""
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = sparrow.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in sparrow.flags.currently_saving:
			sparrow.flags.currently_saving.remove((self.doctype, self.name))
	...
	def reset_seen(self):
		...
		if (
			...
			sparrow.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([sparrow.session.user]), update_modified=False
			)

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		if sparrow.flags.in_patch:
			return
		...
		if (
			...
			data = {"doctype": self.doctype, "name": self.name, "user": sparrow.session.user}
			sparrow.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run sparrow.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in sparrow.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", sparrow.session.user)
	...
	def db_get(self, fieldname):
		...
		return sparrow.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from sparrow.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def save_version(self):
		...
		if (
			...
			or sparrow.flags.in_install
			or (not self._doc_before_save and sparrow.flags.in_patch)
		):
			return

		doc_to_compare = self._doc_before_save
		if not doc_to_compare and (amended_from := self.get("amended_from")):
			doc_to_compare = sparrow.get_doc(self.doctype, amended_from)
	...
	def validate_value(self, fieldname, condition, val2, doc=None, raise_exception=None):
		...
		if not sparrow.compare(val1, condition, val2):
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			sparrow.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or sparrow.EmptyTableError
			)
		...
		out = sparrow.get_doc(
			{
				...
				"comment_email": comment_email or sparrow.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not sparrow.flags.read_only:
			_seen = self.get("_seen") or []
			_seen = sparrow.parse_json(_seen)

			if user not in _seen:
				_seen.append(user)
				sparrow.db.set_value(self.doctype, self.name, "_seen", json.dumps(_seen), update_modified=False)
	...
	def add_viewed(self, user=None):
		...
		if hasattr(self.meta, "track_views") and self.meta.track_views:
			view_log = sparrow.get_doc(
				{
					"doctype": "View Log",
					"viewed_by": sparrow.session.user,
					"reference_doctype": self.doctype,
					"reference_name": self.name,
				}
			)
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		existing_key = sparrow.db.exists(
			"Document Share Key",
			{
				"reference_doctype": self.doctype,
		...
		if existing_key:
			doc = sparrow.get_doc("Document Share Key", existing_key)
		else:
			doc = sparrow.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", sparrow._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", sparrow._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		try:
			self.lock()
		except sparrow.DocumentLockedError:
			sparrow.throw(
		...
		return enqueue(
			"sparrow.model.document.execute_action",
			...
			if timeout:
				for i in range(timeout):
					...
					if not file_lock.lock_exists(signature):
						...
						break
			if lock_exists:
				raise sparrow.DocumentLockedError
		file_lock.create_lock(signature)
		sparrow.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in sparrow.local.locked_documents:
			sparrow.local.locked_documents.remove(self)
		...
		if date_diff(self.get(to_date_field), self.get(from_date_field)) < 0:
			sparrow.throw(
				_("{0} must be after {1}").format(
					sparrow.bold(self.meta.get_label(to_date_field)),
					sparrow.bold(self.meta.get_label(from_date_field)),
				),
				sparrow.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = sparrow.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from sparrow.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from sparrow.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = sparrow.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		if sparrow.local.message_log:
			msg = json.loads(sparrow.local.message_log[-1]).get("message")
		else:
			msg = "<pre><code>" + sparrow.get_traceback() + "</pre></code>"
