...
from sparrow.utils import cint, cstr, now_datetime
from sparrow.utils.caching import redis_cache
...
class InvalidNamingSeriesError(sparrow.ValidationError):
	pass
...
class NamingSeries:
	...
	def validate(self):
		if "." not in self.series:
			sparrow.throw(
				_("Invalid naming series {}: dot (.) missing").format(sparrow.bold(self.series)),
		...
		if not NAMING_SERIES_PATTERN.match(self.series):
			sparrow.throw(
				...
				).format(sparrow.bold(self.series)),
				exc=InvalidNamingSeriesError,
			)
	...
	def get_prefix(self) -> str:
		...
		if prefix is None:
			sparrow.throw(_("Invalid Naming Series: {}").format(self.series))
	...
	def update_counter(self, new_count: int) -> None:
		...
		if sparrow.db.get_value("Series", prefix, "name", order_by="name") is None:
			sparrow.qb.into(Series).insert(prefix, 0).columns("name", "current").run()

		(
			sparrow.qb.update(Series).set(Series.current, cint(new_count)).where(Series.name == prefix)
		).run()

	def get_current_value(self) -> int:
		prefix = self.get_prefix()
		return cint(sparrow.db.get_value("Series", prefix, "current", order_by="name"))
...
def set_new_name(doc):
	...
	if autoname.lower() != "prompt" and not sparrow.flags.in_import:
		doc.name = None

	if is_autoincremented(doc.doctype, meta):
		doc.name = sparrow.db.get_next_sequence_val(doc.doctype)
		return
...
def _implicitly_auto_incremented(doctype) -> bool:
	...
	if sparrow.db.db_type == "mariadb":
		...
		values = (sparrow.db.db_name,)
	return sparrow.db.sql(query, values)[0][0] == "bigint"
...
def set_name_from_naming_options(autoname, doc):
	...
	if _autoname.startswith("field:"):
		...
		if not doc.name:
			...
			sparrow.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
...
def set_naming_from_document_naming_rule(doc):
	...
	"""
	from sparrow.model.base_document import DOCTYPES_FOR_DOCTYPE
	...
	document_naming_rules = sparrow.cache_manager.get_doctype_map(
	...
	for d in document_naming_rules:
		sparrow.get_cached_doc("Document Naming Rule", d.name).apply(doc)
		if doc.name:
			break
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		sparrow.throw(sparrow._("Naming Series mandatory"))
...
def make_autoname(key="", doctype="", doc="", *, ignore_validate=False):
	...
	*Example:*
	...
	"""
	if key == "hash":
		return sparrow.generate_hash(length=10)
...
def parse_naming_series(
	...
	for e in parts:
		...
		if e.startswith("#"):
			...
			part = sparrow.get_attr(method[0])(doc, e)
		else:
			part = e
...
def has_custom_parser(e):
	...
	return sparrow.get_hooks("naming_series_variables", {}).get(e)
...
def getseries(key, digits):
	# series created ?
	# Using sparrow.qb as sparrow.get_values does not allow order_by=None
	series = DocType("Series")
	current = (sparrow.qb.from_(series).where(series.name == key).for_update().select("current")).run()

	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		sparrow.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		sparrow.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name, doc=None):
	...
	*Example:*
	...
	series = DocType("Series")
	current = (
		sparrow.qb.from_(series).where(series.name == prefix).for_update().select("current")
	).run()

	if current and current[0][0] == count:
		sparrow.db.sql("UPDATE `tabSeries` SET `current` = `current` - 1 WHERE `name`=%s", prefix)


def get_default_naming_series(doctype: str) -> str | None:
	"""get default value for `naming_series` property"""
	naming_series_options = sparrow.get_meta(doctype).get_naming_series_options()
...
def validate_name(doctype: str, name: int | str, case: str | None = None):
	...
	if not name:
		sparrow.throw(_("No Name Specified for {0}").format(doctype))
	...
	if isinstance(name, int):
		if is_autoincremented(doctype):
			...
			sparrow.db.set_next_sequence_val(doctype, name, is_val_used=True)
			return name

		sparrow.throw(_("Invalid name type (integer) for varchar name column"), sparrow.NameError)

	if name.startswith("New " + doctype):
		sparrow.throw(
			_("There were some errors setting the name, please contact the administrator"), sparrow.NameError
		)
	if case == "Title Case":
		name = name.title()
	if case == "UPPER CASE":
	...
	if not sparrow.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		sparrow.throw(_("Name of {0} cannot be {1}").format(doctype, name), sparrow.NameError)

	special_characters = "<>"
	if re.findall(f"[{special_characters}]+", name):
		message = ", ".join(f"'{c}'" for c in special_characters)
		sparrow.throw(
			_("Name cannot contain special characters like {0}").format(message), sparrow.NameError
		)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = sparrow.db.exists(doctype, filters)
	...
	if exists:
		last = sparrow.db.sql(
			...
			`{fieldname}` DESC LIMIT 1""".format(
				doctype=doctype, fieldname=fieldname, regex_character=sparrow.db.REGEX_CHARACTER
			),
			regex,
		)
...
def _set_amended_name(doc):
	...
	if sparrow.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		sparrow.throw(_("Please set the document name"))
