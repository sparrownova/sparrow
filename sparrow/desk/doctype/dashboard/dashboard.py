...
class Dashboard(Document):
	def on_update(self):
		if self.is_default:
			...
			sparrow.qb.update(DashBoard).set(DashBoard.is_default, 0).where(
	...
	def validate(self):
		if not sparrow.conf.developer_mode and self.is_standard:
			sparrow.throw(_("Cannot edit Standard Dashboards"))
		...
		if self.is_standard:
			...
			if non_standard_docs_map["Dashboard Chart"] or non_standard_docs_map["Number Card"]:
				...
				sparrow.throw(message, title=_("Standard Not Set"), is_minimizable=True)
...
def get_permitted_charts(dashboard_name):
	permitted_charts = []
	dashboard = sparrow.get_doc("Dashboard", dashboard_name)
	for chart in dashboard.charts:
		if sparrow.has_permission("Dashboard Chart", doc=chart.chart):
			chart_dict = sparrow._dict()
...
def get_permitted_cards(dashboard_name):
	...
	for card in dashboard.cards:
		if sparrow.has_permission("Number Card", doc=card.card):
			permitted_cards.append(card)
	return permitted_cards


def get_non_standard_charts_in_dashboard(dashboard):
	non_standard_charts = [doc.name for doc in sparrow.get_list("Dashboard Chart", {"is_standard": 0})]
	return [
...
def get_non_standard_cards_in_dashboard(dashboard):
	non_standard_cards = [doc.name for doc in sparrow.get_list("Number Card", {"is_standard": 0})]
...
def get_non_standard_warning_message(non_standard_docs_map):
	...
	def get_html(docs, doctype):
		html = f"<p>{sparrow.bold(doctype)}</p>"
		for doc in docs:
