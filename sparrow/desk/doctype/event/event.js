...
sparrow.ui.form.on("Event", {
	...
	refresh: function (frm) {
		if (frm.doc.event_participants) {
			frm.doc.event_participants.forEach((value) => {
				frm.add_custom_button(
					...
					function () {
						sparrow.set_route("Form", value.reference_doctype, value.reference_docname);
					},
					__("Participants")
				);
			});
		...
		frm.add_custom_button(
			__("Add Contacts"),
			function () {
				new sparrow.desk.eventParticipants(frm, "Contact");
			},
			__("Add Participants")
		);
		...
		if (frm.doc.google_meet_link && sparrow.datetime.now_date() <= ends_on_date) {
...
sparrow.ui.form.on("Event Participants", {
	event_participants_remove: function (frm, cdt, cdn) {
		if (cdt && !cdn.includes("New Event Participants")) {
			sparrow.call({
				type: "POST",
				method: "sparrow.desk.doctype.event.event.delete_communication",
				args: {
					event: frm.doc,
...
sparrow.desk.eventParticipants = class eventParticipants {
