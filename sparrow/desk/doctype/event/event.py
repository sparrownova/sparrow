...
from sparrow.contacts.doctype.contact.contact import get_default_contact
...
class Event(Document):
	def validate(self):
		...
		if (
			...
			sparrow.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			sparrow.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		if communications:
			for communication in communications:
				sparrow.delete_doc_if_exists("Communication", communication.name)
	...
	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				...
				comms = sparrow.get_all("Communication", filters=filters, fields=["name"])

				if comms:
					for comm in comms:
						communication = sparrow.get_doc("Communication", comm.name)
						self.update_communication(participant, communication)
				else:
					meta = sparrow.get_meta(participant.reference_doctype)
	...
	def update_communication(self, participant, communication):
		...
		communication.sender_full_name = sparrow.utils.get_fullname(self.owner)
		communication.reference_doctype = self.doctype
		communication.reference_name = self.name
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				sparrow.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)


@sparrow.whitelist()
def delete_communication(event, reference_doctype, reference_docname):
	deleted_participant = sparrow.get_doc(reference_doctype, reference_docname)
	...
	comms = sparrow.get_list("Communication", filters=filters, fields=["name"])

	if comms:
		deletion = []
		for comm in comms:
			delete = sparrow.get_doc("Communication", comm.name).delete()
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			sparrow.set_user_lang(user.name, user.language)
			...
			sparrow.sendmail(
				...
				subject=sparrow._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[sparrow._("Events in Today's Calendar"), "blue"],
			)
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = sparrow.get_all(
		"Event Participants",
	...
	if participations:
		for participation in participations:
			if delete_event:
				sparrow.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = sparrow.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					sparrow.db.delete("Event", {"name": participation.parent})
					sparrow.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = sparrow.get_list(
	...
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			sparrow.db.set_value("Event", event.name, "status", "Closed")
