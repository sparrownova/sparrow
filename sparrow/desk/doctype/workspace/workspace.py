...
from sparrow.desk.desktop import save_new_widget
from sparrow.desk.utils import validate_route_conflict
from sparrow.model.document import Document
from sparrow.model.rename_doc import rename_doc
from sparrow.modules.export_file import delete_folder, export_to_files


class Workspace(Document):
	def validate(self):
		if self.public and not is_workspace_manager() and not disable_saving_as_public():
			sparrow.throw(_("You need to be Workspace Manager to edit this document"))
		...
		try:
			...
			sparrow.throw(_("Content data shoud be a list"))

	def clear_cache(self):
		super().clear_cache()
		if self.for_user:
			sparrow.cache().hdel("bootinfo", self.for_user)
		else:
	...
	def on_update(self):
		...
		if sparrow.conf.developer_mode and self.public:
			if self.module:
...
def get_report_type(report):
	report_type = sparrow.get_value("Report", report, "report_type")
...
def save_page(title, public, new_widgets, blocks):
	public = sparrow.parse_json(public)

	filters = {"public": public, "label": title}

	if not public:
		filters = {"for_user": sparrow.session.user, "label": title + "-" + sparrow.session.user}
	pages = sparrow.get_all("Workspace", filters=filters)
	if pages:
		doc = sparrow.get_doc("Workspace", pages[0])
...
def update_page(name, title, icon, parent, public):
	public = sparrow.parse_json(public)
	doc = sparrow.get_doc("Workspace", name)
	...
	if doc:
		...
		if doc.public != public:
			doc.sequence_id = sparrow.db.count("Workspace", {"public": public}, cache=True)
			doc.public = public
		doc.for_user = "" if public else doc.for_user or sparrow.session.user
		...
		child_docs = sparrow.get_all(
			"Workspace", filters={"parent_page": doc.title, "public": doc.public}
		)
		if child_docs:
			for child in child_docs:
				child_doc = sparrow.get_doc("Workspace", child.name)
				child_doc.parent_page = doc.title
				if child_doc.public != public:
					child_doc.public = public
				child_doc.for_user = "" if public else child_doc.for_user or sparrow.session.user
...
def hide_unhide_page(page_name: str, is_hidden: bool):
	page = sparrow.get_doc("Workspace", page_name)

	if page.get("public") and not is_workspace_manager():
		sparrow.throw(
			_("Need Workspace Manager role to hide/unhide public workspaces"), sparrow.PermissionError
		)

	if not page.get("public") and page.get("for_user") != sparrow.session.user:
		sparrow.throw(_("Cannot update private workspace of other users"), sparrow.PermissionError)
...
def duplicate_page(page_name, new_page):
	...
	old_doc = sparrow.get_doc("Workspace", page_name)
	doc = sparrow.copy_doc(old_doc)
	...
	doc.module = ""
	if not doc.public:
		doc.for_user = doc.for_user or sparrow.session.user
...
def delete_page(page):
	...
	if sparrow.db.exists("Workspace", page.get("name")):
		sparrow.get_doc("Workspace", page.get("name")).delete(ignore_permissions=True)
...
def sort_pages(sb_public_items, sb_private_items):
	...
	workspace_private_pages = get_page_list(["name", "title"], {"for_user": sparrow.session.user})
	...
	for seq, d in enumerate(pages):
		for page in workspace_pages:
			if page.title == d.get("title"):
				doc = sparrow.get_doc("Workspace", page.name)
...
def get_page_list(fields, filters):
	return sparrow.get_all("Workspace", fields=fields, filters=filters, order_by="sequence_id asc")


def is_workspace_manager():
	return "Workspace Manager" in sparrow.get_roles()
