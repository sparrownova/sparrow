...
from sparrow.utils import unique
...
def check_user_tags(dt):
	...
	try:
		doctype = DocType(dt)
		sparrow.qb.from_(doctype).select(doctype._user_tags).limit(1).run()
	except Exception as e:
		if sparrow.db.is_column_missing(e):
			DocTags(dt).setup()
...
def get_tagged_docs(doctype, tag):
	sparrow.has_permission(doctype, throw=True)
	doctype = DocType(doctype)
	return (sparrow.qb.from_(doctype).where(doctype._user_tags.like(tag)).select(doctype.name)).run()


@sparrow.whitelist()
def get_tags(doctype, txt):
	tag = sparrow.get_list("Tag", filters=[["name", "like", f"%{txt}%"]])
...
class DocTags:
	...
	def get_tag_fields(self):
		...
		return sparrow.db.get_value("DocType", self.dt, "tag_fields")

	def get_tags(self, dn):
		"""returns tag for a particular item"""
		return (sparrow.db.get_value(self.dt, dn, "_user_tags", ignore=1) or "").strip()
	...
	def add(self, dn, tag):
		...
		if not tag in tl:
			tl.append(tag)
			if not sparrow.db.exists("Tag", tag):
				sparrow.get_doc({"doctype": "Tag", "name": tag}).insert(ignore_permissions=True)
			self.update(dn, tl)
	...
	def update(self, dn, tl):
		...
		try:
			...
			doc = sparrow.get_doc(self.dt, dn)
...
def delete_tags_for_document(doc):
	...
	"""
	if not sparrow.db.table_exists("Tag Link"):
		return

	sparrow.db.delete("Tag Link", {"document_type": doc.doctype, "document_name": doc.name})
...
def get_tags_list_for_awesomebar():
	return sparrow.get_list("Tag", pluck="name", order_by=None)
