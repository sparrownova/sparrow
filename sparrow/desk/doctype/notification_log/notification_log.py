...
class NotificationLog(Document):
	def after_insert(self):
		sparrow.publish_realtime("notification", after_commit=True, user=self.for_user)
	...
	def clear_old_logs(days=180):
		...
		table = sparrow.qb.DocType("Notification Log")
		sparrow.db.delete(table, filters=(table.modified < (Now() - Interval(days=days))))


def get_permission_query_conditions(for_user):
	if not for_user:
		for_user = sparrow.session.user

	if for_user == "Administrator":
		return

	return f"""(`tabNotification Log`.for_user = {sparrow.db.escape(for_user)})"""


def get_title(doctype, docname, title_field=None):
	if not title_field:
		title_field = sparrow.get_meta(doctype).get_title_field()
	title = docname if title_field == "name" else sparrow.db.get_value(doctype, docname, title_field)
	return title
...
def enqueue_create_notification(users: list[str] | str, doc: dict):
	...
	sparrow.enqueue(
		"sparrow.desk.doctype.notification_log.notification_log.make_notification_logs",
		doc=doc,
		users=users,
...
def make_notification_logs(doc, users):
	for user in _get_user_ids(users):
		notification = sparrow.new_doc("Notification Log")
...
def _get_user_ids(user_emails):
	user_names = sparrow.db.get_values(
...
def send_notification_email(doc):
	...
	from sparrow.utils import get_url_to_form, strip_html

	email = sparrow.db.get_value("User", doc.for_user, "email")
	if not email:
		return
...
def get_notification_logs(limit=20):
	notification_logs = sparrow.db.get_list(
...
def mark_all_as_read():
	unread_docs_list = sparrow.get_all(
		"Notification Log", filters={"read": 0, "for_user": sparrow.session.user}
	)
	unread_docnames = [doc.name for doc in unread_docs_list]
	if unread_docnames:
		filters = {"name": ["in", unread_docnames]}
		sparrow.db.set_value("Notification Log", filters, "read", 1, update_modified=False)


@sparrow.whitelist()
def mark_as_read(docname):
	if docname:
		sparrow.db.set_value("Notification Log", docname, "read", 1, update_modified=False)


@sparrow.whitelist()
def trigger_indicator_hide():
	sparrow.publish_realtime("indicator_hide", user=sparrow.session.user)


def set_notifications_as_unseen(user):
	try:
		sparrow.db.set_value("Notification Settings", user, "seen", 0, update_modified=False)
