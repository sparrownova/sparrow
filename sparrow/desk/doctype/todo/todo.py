...
from sparrow.utils import get_fullname, parse_addr
...
class ToDo(Document):
	...
	def validate(self):
		...
		if self.is_new():
			...
			if self.assigned_by == self.allocated_to:
				assignment_message = sparrow._("{0} self assigned this task: {1}").format(
					get_fullname(self.assigned_by), self.description
				)
			else:
				assignment_message = sparrow._("{0} assigned {1}: {2}").format(
			...
			if self.get_db_value("status") != self.status:
				if self.allocated_to == sparrow.session.user:
					removal_message = sparrow._("{0} removed their assignment.").format(
						get_fullname(sparrow.session.user)
					)
				else:
					removal_message = sparrow._("Assignment of {0} removed by {1}").format(
						get_fullname(self.allocated_to), get_fullname(sparrow.session.user)
					)
	...
	def add_assign_comment(self, text, comment_type):
		...
		sparrow.get_doc(self.reference_type, self.reference_name).add_comment(comment_type, text)

	def delete_communication_links(self):
		# unlink todo from linked comments
		return sparrow.db.delete(
	...
	def update_in_reference(self):
		...
		try:
			...
			if sparrow.db.is_table_missing(e) and sparrow.flags.in_install:
				# no table
				return

			elif sparrow.db.is_column_missing(e):
	...
	def get_owners(cls, filters=None):
		...
		rows = sparrow.get_all(cls.DocType, filters=filters or {}, fields=["allocated_to"])
...
def on_doctype_update():
	sparrow.db.add_index("ToDo", ["reference_type", "reference_name"])
...
def get_permission_query_conditions(user):
	...
	todo_roles = sparrow.permissions.get_doctype_roles("ToDo")
	if "All" in todo_roles:
		todo_roles.remove("All")

	if any(check in todo_roles for check in sparrow.get_roles(user)):
		return None
	else:
...
def has_permission(doc, ptype="read", user=None):
	...
	todo_roles = sparrow.permissions.get_doctype_roles("ToDo", ptype)
	if "All" in todo_roles:
		todo_roles.remove("All")
