...
def after_doctype_insert():
	sparrow.db.add_unique("Desktop Icon", ("module_name", "owner", "standard"))
...
def get_desktop_icons(user=None):
	...
	user_icons = sparrow.cache().hget("desktop_icons", user)
	...
	if not user_icons:
		...
		blocked_doctypes = sparrow.get_all(
			"DocType",
		...
		standard_icons = sparrow.get_all("Desktop Icon", fields=fields, filters={"standard": 1})
		...
		user_blocked_modules = sparrow.get_doc("User", user).get_blocked_modules()
		for icon in user_icons:
		...
		sparrow.cache().hset("desktop_icons", user, user_icons)
...
def add_user_icon(_doctype, _report=None, label=None, link=None, type="link", standard=0):
	...
	icon_name = sparrow.db.exists(
		"Desktop Icon", {"standard": standard, "link": link, "owner": sparrow.session.user}
	)

	if icon_name:
		if sparrow.db.get_value("Desktop Icon", icon_name, "hidden"):
			# if it is hidden, unhide it
			sparrow.db.set_value("Desktop Icon", icon_name, "hidden", 0)
			clear_desktop_icons_cache()
		...
		if not sparrow.db.get_value("Report", _report):
			_report = None
			userdefined_icon = sparrow.db.get_value(
		...
		except sparrow.UniqueValidationError as e:
			sparrow.throw(_("Desktop Icon already exists"))
	...
	for i, module_name in enumerate(new_order):
		if module_name not in ("Explore",):
			if user:
				...
				name = sparrow.db.get_value("Desktop Icon", {"standard": 1, "module_name": module_name})
				if name:
					icon = sparrow.get_doc("Desktop Icon", name)
				else:
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
	...
	if not int(sparrow.defaults.get_defaults().setup_complete or 0):
		sparrow.db.delete("Desktop Icon", {"standard": 0})

	# set standard as blocked and hidden if setting first active domain
	if not sparrow.flags.keep_desktop_icons:
		sparrow.db.sql("update `tabDesktop Icon` set blocked=0, hidden=1 where standard=1")

	# set as visible if present, or add icon
	for module_name in visible_list:
		name = sparrow.db.get_value("Desktop Icon", {"module_name": module_name})
		if name:
			sparrow.db.set_value("Desktop Icon", name, "hidden", 0)
		else:
			if sparrow.db.exists("DocType", module_name):
				try:
...
def set_hidden(module_name, user=None, hidden=1):
	...
	if user:
		...
		if hidden and icon.custom:
			sparrow.delete_doc(icon.doctype, icon.name, ignore_permissions=True)
			return

		# hidden by user
		icon.db_set("hidden", hidden)
	else:
		icon = sparrow.get_doc("Desktop Icon", {"standard": 1, "module_name": module_name})
...
def get_all_icons():
	return [
		d.module_name
		for d in sparrow.get_all("Desktop Icon", filters={"standard": 1}, fields=["module_name"])
	]


def clear_desktop_icons_cache(user=None):
	sparrow.cache().hdel("desktop_icons", user or sparrow.session.user)
	sparrow.cache().hdel("bootinfo", user or sparrow.session.user)
...
def get_user_copy(module_name, user=None):
	...
	desktop_icon_name = sparrow.db.get_value(
		"Desktop Icon", {"module_name": module_name, "owner": user, "standard": 0}
	)

	if desktop_icon_name:
		return sparrow.get_doc("Desktop Icon", desktop_icon_name)
	else:
...
def make_user_copy(module_name, user):
	...
	standard_name = sparrow.db.get_value("Desktop Icon", {"module_name": module_name, "standard": 1})

	if not standard_name:
		sparrow.throw(_("{0} not found").format(module_name), sparrow.DoesNotExistError)

	original = sparrow.get_doc("Desktop Icon", standard_name)

	desktop_icon = sparrow.get_doc(
...
def sync_from_app(app):
	...
	try:
		modules = sparrow.get_attr(app + ".config.desktop.get_data")() or {}
	except ImportError:
		return []
	...
	for i, m in enumerate(modules_list):
		...
		if desktop_icon_name:
			desktop_icon = sparrow.get_doc("Desktop Icon", desktop_icon_name)
		else:
			# new icon
			desktop_icon = sparrow.get_doc(
		...
		try:
			desktop_icon.save()
		except sparrow.exceptions.UniqueValidationError:
			pass
...
def update_icons(hidden_list, user=None):
	...
	sparrow.msgprint(sparrow._("Updated"), indicator="green", title=_("Success"), alert=True)


def get_context(context):
	context.icons = get_user_icons(sparrow.session.user)
	context.user = sparrow.session.user

	if "System Manager" in sparrow.get_roles():
		context.users = sparrow.get_all(
			"User",
...
def get_module_icons(user=None):
	...
	if not user:
		icons = sparrow.get_all("Desktop Icon", fields="*", filters={"standard": 1}, order_by="idx")
	else:
		sparrow.cache().hdel("desktop_icons", user)
		icons = get_user_icons(user)

	for icon in icons:
		icon.value = sparrow.db.escape(_(icon.label or icon.module_name))
...
def get_user_icons(user):
	...
	from sparrow.boot import get_allowed_pages
