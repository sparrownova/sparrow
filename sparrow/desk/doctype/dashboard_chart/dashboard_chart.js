...
sparrow.ui.form.on("Dashboard Chart", {
	setup: function (frm) {
	...
	before_save: function (frm) {
		...
		static_filters = sparrow.dashboard_utils.remove_common_static_filter_values(
			static_filters,
			dynamic_filters
		);
	...
	refresh: function (frm) {
		...
		frm.is_disabled = !sparrow.boot.developer_mode && frm.doc.is_standard;
		...
		frm.add_custom_button("Add Chart to Dashboard", () => {
			const dialog = sparrow.dashboard_utils.get_add_to_dashboard_dialog(
				frm.doc.name,
				"Dashboard Chart",
				"sparrow.desk.doctype.dashboard_chart.dashboard_chart.add_chart_to_dashboard"
			);

			if (!frm.doc.chart_name) {
				sparrow.msgprint(__("Please create chart first"));
			} else {
				dialog.show();
			}
		});
	...
	is_standard: function (frm) {
		if (sparrow.boot.developer_mode && frm.doc.is_standard) {
	...
	set_heatmap_year_options: function (frm) {
		if (frm.doc.type == "Heatmap") {
			sparrow.db.get_doc("System Settings").then((doc) => {
				const creation_date = doc.creation;
				frm.set_df_property(
					"heatmap_year",
					"options",
					sparrow.dashboard_utils.get_years_since_creation(creation_date)
				);
			});
		}
	},
	...
	set_chart_report_filters: function (frm) {
		...
		if (report_name) {
			if (frm.doc.filters_json.length > 2) {
				...
				sparrow.report_utils.get_report_filters(report_name).then((filters) => {
					if (filters) {
						frm.chart_filters = filters;
						let filter_values = sparrow.report_utils.get_filter_values(filters);
	...
	set_chart_field_options: function (frm) {
		...
		if (frm.doc.dynamic_filters_json && frm.doc.dynamic_filters_json.length > 2) {
			filters = sparrow.dashboard_utils.get_all_filters(frm.doc);
		}
		sparrow
			.xcall("sparrow.desk.query_report.run", {
			...
			.then((data) => {
				...
				if (!frm.doc.use_report_chart) {
					if (data.result.length) {
						frm.field_options = sparrow.report_utils.get_field_options_from_report(
							data.columns,
							data
						);
						frm.set_df_property(
						...
						if (!frm.field_options.numeric_fields.length) {
							...
							let y_field_df = sparrow.meta.get_docfield(
	...
	update_options: function (frm) {
		...
		if (doctype) {
			sparrow.model.with_doctype(doctype, () => {
				...
				sparrow.get_meta(doctype).fields.map((df) => {
	...
	show_filters: function (frm) {
		frm.chart_filters = [];
		sparrow.dashboard_utils.get_filters_for_chart_type(frm.doc).then((filters) => {
			if (filters) {
				frm.chart_filters = filters;
			}
	...
	render_filters_table: function (frm) {
		...
		table.on("click", () => {
			frm.is_disabled && sparrow.throw(__("Cannot edit filters for standard charts"));
			...
			sparrow.dashboards.filters_dialog = dialog;

			if (is_document_type) {
				frm.filter_group = new sparrow.ui.FilterGroup({
			...
			if (frm.doc.chart_type == "Report") {
				...
				sparrow.query_reports[frm.doc.report_name] &&
					sparrow.query_reports[frm.doc.report_name].onload &&
					sparrow.query_reports[frm.doc.report_name].onload(sparrow.query_report);
			}
	...
	render_dynamic_filters_table(frm) {
		...
		let fields = sparrow.dashboard_utils.get_fields_for_dynamic_filter_dialog(
			is_document_type,
			filters,
			frm.dynamic_filters
		);
	...
	set_parent_document_type: async function (frm) {
		...
		let doc_is_table =
			document_type &&
			(await sparrow.db.get_value("DocType", document_type, "istable")).message.istable;

		frm.set_df_property("parent_document_type", "hidden", !doc_is_table);

		if (document_type && doc_is_table) {
			let parents = await sparrow.xcall(
				"sparrow.desk.doctype.dashboard_chart.dashboard_chart.get_parent_doctypes",
