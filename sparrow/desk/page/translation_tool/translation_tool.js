sparrow.pages["translation-tool"].on_page_load = function (wrapper) {
	...
	sparrow.translation_tool = new TranslationTool(page);
};
...
class TranslationTool {
	constructor(page) {
		...
		this.wrapper.append(sparrow.render_template("translation_tool"));
	...
	setup_language_filter() {
		let languages = Object.keys(sparrow.boot.lang_dict).map((language_label) => {
			let value = sparrow.boot.lang_dict[language_label];
			return {
		...
		if (translation_language || sparrow.boot.lang !== "en") {
			language_selector.set_value(translation_language || sparrow.boot.lang);
		} else {
			sparrow.prompt(
				{
	...
	fetch_messages() {
		sparrow.dom.freeze(__("Fetching..."));
		return sparrow
			.xcall("sparrow.translate.get_messages", {
			...
			<div
				...
				<div class="bold ellipsis">
					<span class="indicator ${this.get_indicator_color(message)}">
						<span>${sparrow.utils.escape_html(message.source_text)}</span>
					</span>
				</div>
			</div>
		`;
	...
	get_additional_info(source_id) {
		sparrow.dom.freeze("Fetching...");
		return sparrow
			.xcall("sparrow.translate.get_source_additional_info", {
				source: source_id,
				language: this.page.fields_dict["language"].get_value(),
			})
			.finally(sparrow.dom.unfreeze);
	}

	make_edit_form(translation, { contributions, positions }) {
		if (!this.form) {
			this.form = new sparrow.ui.FieldGroup({
				fields: [
					{
						fieldtype: "HTML",
						fieldname: "header",
	...
	show_confirmation_dialog() {
		this.confirmation_dialog = new sparrow.ui.Dialog({
			fields: [
				{
					label: __("Language"),
					fieldname: "language",
	...
	create_translations() {
		sparrow.dom.freeze(__("Submitting..."));
		return sparrow
			.xcall("sparrow.core.doctype.translation.translation.create_translations", {
	...
	setup_local_contributions() {
		// TODO: Refactor
		sparrow
			.xcall("sparrow.translate.get_contributions", {
				language: this.language,
			})
			.then((messages) => {
				let template = (message) => `
	...
	show_translation_status_modal(e, $el) {
		...
		sparrow.xcall("sparrow.translate.get_contribution_status", { message_id }).then((doc) => {
	...
	get_code_url(path, line_no, app) {
		...
		return `https://github.com/sparrownova/${app}/blob/develop/${code_path}#L${line_no}`;
	}
}
