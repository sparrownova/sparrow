...
from sparrow.utils import cint, get_site_path, get_url
from sparrow.utils.data import convert_utc_to_system_timezone
...
def get_scheduled_backup_limit():
	backup_limit = sparrow.db.get_singles_value("System Settings", "backup_limit")
...
def schedule_files_backup(user_email):
	...
	queued_jobs = get_jobs(site=sparrow.local.site, queue="long")
	method = "sparrow.desk.page.backups.backups.backup_files_and_notify_user"

	if method not in queued_jobs[sparrow.local.site]:
		enqueue(
			"sparrow.desk.page.backups.backups.backup_files_and_notify_user",
			queue="long",
			user_email=user_email,
		)
		sparrow.msgprint(_("Queued for backup. You will receive an email with the download link"))
	else:
		sparrow.msgprint(
...
def backup_files_and_notify_user(user_email=None):
	from sparrow.utils.backups import backup
