...
from sparrow.model import child_table_fields, default_fields, get_permitted_fields, optional_fields
...
from sparrow.utils import add_user_info, cstr, format_duration
...
def get_list():
	...
	if is_virtual_doctype(args.doctype):
		...
		# uncompressed (refactored from sparrow.model.db_query.get_list)
		data = execute(**args)
...
def get_form_params():
	...
	data = sparrow._dict(sparrow.local.form_dict)
	clean_params(data)
	validate_args(data)
	return data
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = sparrow.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			sparrow.throw(_("Invalid aggregate function"))

		if sparrow.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
			data.fields.append(
...
def raise_invalid_field(fieldname):
	sparrow.throw(_("Field not permitted in query") + f": {fieldname}", sparrow.DataError)
...
def get_meta_and_docfield(fieldname, data):
	...
	meta = sparrow.get_meta(parenttype)
...
def update_wildcard_field_param(data):
	if (isinstance(data.fields, str) and data.fields == "*") or (
		...
		if sparrow.get_system_settings("apply_perm_level_on_api_calls"):
			...
			data.fields = sparrow.db.get_table_columns(data.doctype)
		return True
...
def get_parenttype_and_fieldname(field, data):
	if "." in field:
		...
		if parenttype.startswith("`tab"):
			...
			parenttype = sparrow.get_meta(data.doctype).get_field(parenttype).options
	else:
...
def compress(data, args=None):
	...
	from sparrow.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = sparrow.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if sparrow.db.exists("Report", name):
		report = sparrow.get_doc("Report", name)
		if report.is_standard == "Yes":
			sparrow.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			sparrow.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != sparrow.session.user and not report.has_permission("write"):
			sparrow.throw(_("Insufficient Permissions for editing Report"), sparrow.PermissionError)
	else:
		report = sparrow.new_doc("Report")
	...
	sparrow.msgprint(
		_("Report {0} saved").format(sparrow.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
def delete_report(name):
	...
	report = sparrow.get_doc("Report", name)
	if report.is_standard == "Yes":
		sparrow.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		sparrow.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != sparrow.session.user and not report.has_permission("delete"):
		sparrow.throw(_("Insufficient Permissions for deleting Report"), sparrow.PermissionError)

	report.delete(ignore_permissions=True)
	sparrow.msgprint(
		_("Report {0} deleted").format(sparrow.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	title = sparrow.form_dict.title
	sparrow.form_dict.pop("title", None)
	...
	sparrow.permissions.can_export(doctype, raise_exception=True)

	if "selected_items" in form_params:
		si = json.loads(sparrow.form_dict.get("selected_items"))
	...
	if file_format_type == "CSV":
		...
		for r in data:
			...
			writer.writerow([handle_html(sparrow.as_unicode(v)) if isinstance(v, str) else v for v in r])

		f.seek(0)
		sparrow.response["result"] = cstr(f.read())
		sparrow.response["type"] = "csv"
		sparrow.response["doctype"] = title
		...
		sparrow.response["filename"] = _(title) + ".xlsx"
		sparrow.response["filecontent"] = xlsx_file.getvalue()
		sparrow.response["type"] = "binary"
...
def get_labels(fields, doctype):
	...
	for key in fields:
		...
		if parenttype == doctype and fieldname == "name":
			...
			df = sparrow.get_meta(parenttype).get_field(fieldname)
...
def delete_items():
	...
	items = sorted(json.loads(sparrow.form_dict.get("items")), reverse=True)
	doctype = sparrow.form_dict.get("doctype")

	if len(items) > 10:
		sparrow.enqueue("sparrow.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
...
def get_stats(stats, doctype, filters=None):
	...
	try:
		db_columns = sparrow.db.get_table_columns(doctype)
	except (sparrow.db.InternalError, sparrow.db.ProgrammingError):
	...
	for column in columns:
		...
		except sparrow.db.SQLError:
			pass
		except sparrow.db.InternalError as e:
			# raised when _user_tags column is added on the fly
			pass
