...
from sparrow import _, is_whitelisted, validate_and_sanitize_search_inputs
from sparrow.database.schema import SPECIAL_CHAR_PATTERN
from sparrow.permissions import has_permission
from sparrow.utils import cint, cstr, unique
...
def sanitize_searchfield(searchfield):
	...
	if SPECIAL_CHAR_PATTERN.search(searchfield):
		sparrow.throw(_("Invalid Search Field {0}").format(searchfield), sparrow.DataError)
...
def search_link(
	...
	sparrow.response["results"] = build_for_autosuggest(sparrow.response["values"], doctype=doctype)
	del sparrow.response["values"]
...
def search_widget(
	...
	standard_queries = sparrow.get_hooks().standard_queries or {}

	if query and query.split(maxsplit=1)[0].lower() != "select":
		# by method
		try:
			is_whitelisted(sparrow.get_attr(query))
			sparrow.response["values"] = sparrow.call(
				query,
				doctype,
				txt,
				searchfield,
		...
		except sparrow.exceptions.PermissionError as e:
			if sparrow.local.conf.developer_mode:
				raise e
			else:
		...
		if query:
			sparrow.throw(_("This query style is discontinued"))
			# custom query
			# sparrow.response["values"] = sparrow.db.sql(scrub_custom_query(query, searchfield, txt))
		else:
			...
			from sparrow.model.db_query import get_order_by
			...
			if not meta.translated_doctype:
				formatted_fields.append(
					"""locate({_txt}, `tab{doctype}`.`name`) as `_relevance`""".format(
						_txt=sparrow.db.escape((txt or "").replace("%", "").replace("@", "")),
						doctype=doctype,
					)
				)
			...
			sparrow.response["values"] = values
...
def get_names_for_mentions(search_term):
	users_for_mentions = sparrow.cache().get_value("users_for_mentions", get_users_for_mentions)
	user_groups = sparrow.cache().get_value("user_groups", get_user_groups)
	...
	for mention_data in users_for_mentions + user_groups:
		...
		mention_data["link"] = sparrow.utils.get_url_to_form(
...
def get_link_title(doctype, docname):
	...
	if meta.show_title_field_in_link:
		return sparrow.db.get_value(doctype, docname, meta.title_field)
