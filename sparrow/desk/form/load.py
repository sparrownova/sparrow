...
from sparrow.desk.form.document_follow import is_document_followed
from sparrow.model.utils import is_virtual_doctype
from sparrow.model.utils.user_settings import get_user_settings
from sparrow.permissions import get_doc_permissions
...
def getdoc(doctype, name, user=None):
	...
	if not is_virtual_doctype(doctype) and not sparrow.db.exists(doctype, name):
		return []
	...
	if not doc.has_permission("read"):
		sparrow.flags.error_message = _("Insufficient Permission for {0}").format(
			sparrow.bold(doctype + " " + name)
		)
		raise sparrow.PermissionError(("read", doctype, name))
	...
	doc.add_seen()
	set_link_titles(doc)
	if sparrow.response.docs is None:
		sparrow.local.response = _dict({"docs": []})
...
def getdoctype(doctype, with_parent=False, cached_timestamp=None):
	...
	if with_parent and (parent_dt := sparrow.model.meta.get_parent_dt(doctype)):
		docs = get_meta_bundle(parent_dt)
		sparrow.response["parent_dt"] = parent_dt

	if not docs:
		docs = get_meta_bundle(doctype)

	sparrow.response["user_settings"] = get_user_settings(parent_dt or doctype)

	if cached_timestamp and docs[0].modified == cached_timestamp:
		return "use_cache"

	sparrow.response.docs.extend(docs)


def get_meta_bundle(doctype):
	bundle = [sparrow.desk.form.meta.get_meta(doctype)]
	for df in bundle[0].fields:
		if df.fieldtype in sparrow.model.table_fields:
			bundle.append(sparrow.desk.form.meta.get_meta(df.options, not sparrow.conf.developer_mode))
	return bundle


@sparrow.whitelist()
def get_docinfo(doc=None, doctype=None, name=None):
	from sparrow.share import _get_users as get_docshares
	...
	docinfo = sparrow._dict(user_info={})
	...
	docinfo.update(
		{
			...
			"is_document_followed": is_document_followed(doc.doctype, doc.name, sparrow.session.user),
	...
	sparrow.response["docinfo"] = docinfo
...
def add_comments(doc, docinfo):
	...
	for c in comments:
		if c.comment_type == "Comment":
			c.content = sparrow.utils.markdown(c.content)
		...
		sparrow.utils.add_user_info(c.owner, docinfo.user_info)
...
def get_communication_data(
	...
	""".format(
		...
		dict(doctype=doctype, name=name, start=sparrow.utils.cint(start), limit=limit),
		as_dict=as_dict,
	)
...
def run_onload(doc):
	doc.set("__onload", sparrow._dict())
...
def get_view_logs(doctype, docname):
	...
	logs = []
	if hasattr(sparrow.get_meta(doctype), "track_views") and sparrow.get_meta(doctype).track_views:
		view_logs = sparrow.get_all(
			"View Log",
			filters={
				"reference_doctype": doctype,
...
def get_additional_timeline_content(doctype, docname):
	contents = []
	hooks = sparrow.get_hooks().get("additional_timeline_content", {})
	...
	for method in methods_for_all_doctype + methods_for_current_doctype:
		contents.extend(sparrow.get_attr(method)(doctype, docname) or [])
...
def get_title_values_for_link_and_dynamic_link_fields(doc, link_fields=None):
	...
	for field in link_fields:
		...
		link_title = sparrow.db.get_value(
...
def send_link_titles(link_titles):
	"""Append link titles dict in `sparrow.local.response`."""
	if "_link_titles" not in sparrow.local.response:
		sparrow.local.response["_link_titles"] = {}

	sparrow.local.response["_link_titles"].update(link_titles)


def update_user_info(docinfo):
	for d in docinfo.communications:
		sparrow.utils.add_user_info(d.sender, docinfo.user_info)

	for d in docinfo.shared:
		sparrow.utils.add_user_info(d.user, docinfo.user_info)

	for d in docinfo.assignments:
		sparrow.utils.add_user_info(d.owner, docinfo.user_info)
...
def get_user_info_for_viewers(users):
	...
	for user in json.loads(users):
		sparrow.utils.add_user_info(user, user_info)
