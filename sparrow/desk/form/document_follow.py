...
from sparrow.utils import get_url_to_form


@sparrow.whitelist()
def update_follow(doctype, doc_name, following):
	if following:
		return follow_document(doctype, doc_name, sparrow.session.user)
	else:
		return unfollow_document(doctype, doc_name, sparrow.session.user)
...
def follow_document(doctype, doc_name, user):
	...
	if (not sparrow.get_meta(doctype).track_changes) or user == "Administrator":
		return

	if not sparrow.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		return

	if not is_document_followed(doctype, doc_name, user):
		doc = sparrow.new_doc("Document Follow")
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		sparrow.delete_doc("Document Follow", doc[0].name)
		return 1
	return 0
...
def get_user_list(frequency):
	...
	User = DocType("User")
	return (
		sparrow.qb.from_(DocumentFollow)
		.join(User)
...
def get_version(doctype, doc_name, frequency, user):
	...
	if version:
		for v in version:
			change = sparrow.parse_json(v.data)
			time = sparrow.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = sparrow.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append(
			{
...
def _get_filters(frequency, user):
	...
	if frequency == "Weekly":
		filters += [
			["modified", ">", sparrow.utils.add_days(sparrow.utils.nowdate(), -7)],
			["modified", "<", sparrow.utils.nowdate()],
		]

	elif frequency == "Daily":
		filters += [
			["modified", ">", sparrow.utils.add_days(sparrow.utils.nowdate(), -1)],
			["modified", "<", sparrow.utils.nowdate()],
		]

	elif frequency == "Hourly":
		filters += [
			["modified", ">", sparrow.utils.add_to_date(sparrow.utils.now_datetime(), hours=-1)],
			["modified", "<", sparrow.utils.now_datetime()],
		]
