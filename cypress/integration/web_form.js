context("Web Form", () => {
	before(() => {
		...
		return cy
			...
			.its("sparrow")
			.then((sparrow) => {
				return sparrow.xcall("sparrow.tests.ui_test_helpers.clear_notes");
			});
	});
	...
	it("Read Only", () => {
		...
		cy.get('.sparrow-control[data-fieldname="title"] .control-input').should(
			"have.css",
			"display",
			"none"
		);
	...
	it("Navigate and Submit a MultiStep WebForm", () => {
		cy.call("sparrow.tests.ui_test_helpers.update_webform_to_multistep").then(() => {
