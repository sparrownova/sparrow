context("Workspace Blocks", () => {
	before(() => {
		...
		return cy
			...
			.its("sparrow")
			.then((sparrow) => {
				return sparrow.xcall("sparrow.tests.ui_test_helpers.setup_workflow");
			});
	});
	...
	it.skip("Quick List Block", () => {
		...
		cy.intercept({
			method: "GET",
			url: "api/method/sparrow.desk.form.load.getdoctype?**",
		}).as("get_doctype");
		...
		cy.intercept({
			method: "POST",
			url: "api/method/sparrow.desk.reportview.get",
		}).as("refresh-list");
