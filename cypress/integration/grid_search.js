...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.its("sparrow")
			.then((sparrow) => {
				return sparrow.xcall(
					"sparrow.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("sparrow")
			.then((sparrow) => {
				sparrow.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.sparrow-control[data-fieldname="child_table_1"]').as("table");
